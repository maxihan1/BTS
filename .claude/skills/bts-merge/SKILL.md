...
**★★「체크 0건」은 초록이 아니다.** 워크플로우가 아직 큐에 있거나, 파일이 지워졌거나,
이벤트가 안 맞아 아예 안 떴을 때 이 명령은 아무것도 못 낸다. 그것을 「빨간불이 아니니 통과」로
읽으면 **검증 없는 머지**가 된다.
...
| 빨강 | **머지하지 않는다.** 고치고 다시 |
| **체크 0건** | **통과가 아니다.** 아래 판정으로 넘어간다. 그것도 판정 불가면 머지하지 않는다 |

### ★★검증 판정 — 2026-09-11 부터 GHA 가 1차 근거다

`.husky/pre-push` 에서 백엔드·프론트 테스트를 걷어냈다(P5). 그래서 **로컬 훅은 더 이상
백엔드를 검증하지 않는다** — 「로컬에서 돌렸으니 됐다」가 성립하지 않는다. 푸시가 막히지
않으므로 **깨진 커밋이 원격 브랜치에 올라갈 수 있고**, 그것을 잡는 자리가 여기다.

검증은 GitHub Actions `verify.yml` 이 돈다(2026-09-11 이관). 배포는 젠킨스가 계속 소유한다.
**두 출처를 각각 읽는다 — 한쪽 초록을 다른 쪽 초록으로 읽지 않는다.**

```bash
node --experimental-strip-types scripts/workflow/gha-verify-status.ts
echo "EXIT=$?"
```

| 종료 코드 | 판정 |
|---|---|
| `0` 초록 | 통과. 이 브랜치 최신 커밋의 검증 체크가 전부 성공했다 |
| `1` 빨강 | **머지하지 않는다.** 고치고 다시 |
| `2` **판정 불가** | **통과가 아니다.** 체크 0건 · 도는 중 · **전부 skipped** · 모르는 결론값 · 조회 실패가 전부 여기다 |

**「체크 0건」이 종료 코드 2 인 것이 이 스크립트의 존재 이유다.** 빨간불이 아니라고 통과로
읽으면 검증 없는 머지가 된다.

★**`skipped` 는 빨강이 아니다.** `verify.yml` 은 route 판정에 따라 잡을 의도적으로 건너뛴다 —
문서만 고친 PR 이면 backend·frontend·e2e 가 전부 skipped 다. 다만 **전부 skipped 면 판정 불가**다.
`discriminants` 는 조건 없이 항상 돌므로 정상이면 최소 1건이 success 이고, 전부 skipped 라는 것은
그 무조건성이 깨졌다는 신호다.

### 배포 상태 — 젠킨스

배포를 확인해야 하는 경우에만 본다. 검증 초록을 이것으로 대신하지 않는다.
...
종료 코드 규약은 위와 같다(0 초록 · 1 빨강 · 2 판정 불가). 특히 **다른 브랜치의 초록을 이
브랜치의 초록으로 읽는 것**이 가장 나쁜 오독이라 스크립트가 그것을 2 로 분리한다.
