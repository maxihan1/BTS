...
**범위**. 146 FR / 9 BC. **2026-05-20 ~ 2026-08-06**, PR 335건, 커밋 693건
...
초안/발행) 신설로 139 → 143. **그 시점에 신설 4건은 등재만 했고 D 단계는 전량 미착수**였다. 워크플로우가
...
> **2026-09-05 갱신.** **FR-MN-03 신설·완주**(PR #456). 그 시점 정본 145 전량이 D 단계를 마쳤다 — 멘션 알림 확장(댓글·이슈 생성)
...
> **2026-09-08 갱신.** **FR-WF-08 신설**(프로젝트 소유 워크플로우·스킴)로 145 → 146. **그 1건은 등재만 했고
> D 단계는 미착수**다. 바로 위 「정본 145 전량이 D 단계를 마쳤다」는 2026-09-05 시점의 기록이며 지금은 참이 아니다 —
> 숫자만 올리면 시점 기록이 거짓이 되므로 원문은 두고 갱신을 덧붙인다. 프로젝트 관리자가 자기 프로젝트 전용
> 워크플로우·스킴을 만들고 편집한다. Jira Cloud 와 **같은 불변식** — 공유되지 않은 워크플로우만 프로젝트
> 관리자가 편집하고 공유된 것은 전역 관리자 몫이다.
> 아래 2026-08-26 문단의 「정본 144 중 142」는 그 시점의 기록이다.

> **2026-08-26 재갱신 (PR #407).** **FR-WF-06 이 완주했다** — D6(전환 규칙 편집 UI)·D7(E2E)이 닫히면서
> 그 시점 정본 144 중 **142** 의 D 단계가 완료됐다. 남은 미착수는 **FR-WF-07 · FR-BD-04 둘**이다
...
문구도 함께 바뀐다(`더 진행할 전환 없음` 등). 손대지 않은 곳 3종 — Flyway 로 동결된
마이그레이션 21곳(체크섬 검증) · 자동 생성물 `progress.html`(재생성으로 반영) ·
`classify-task` 의 구 표기 키워드(이력·닫힌 브랜치 라우팅용).
...
편집으로 열었다. 둘 다 **백엔드 0줄 · 마이그레이션 0 · 신규 의존성 0**(기존 이슈 PATCH·전환 API 소비).
