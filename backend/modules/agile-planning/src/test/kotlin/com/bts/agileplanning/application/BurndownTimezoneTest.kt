 ...
 *   「무조건 +9 하는」 구현(M2)에 대한 **두 번째** 방어선이다 — 첫 방어선은 아래 UTC− arm 이고,
 *   대조군만으로 M2 를 잡는다고 적으면 실측과 어긋난다(뮤테이션 표 참조).
 ...
 * | M2 | 무조건 +9(zone 무시) | 심야·두-worklog 의 **앞** arm + 새벽의 **뒤**(뉴욕) arm | 새벽의 앞 arm · 서울 arm · 낮 시각 · 근무일 2건 |
 ...
 * M1 은 `Asia/Seoul(+9) 에서 …` 쪽, M2 는 `타임존 미설정 = UTC …` 쪽이다.
 *
 * ★**M2 를 잡는 첫 방어선은 뉴욕 arm 이지 미설정 대조군이 아니다**(실측 · 2026-09-06 정정).
 * 미설정 arm 을 전부 지워도 M2 는 살아남지 못한다 — `02:00Z + 9 = 7/2` 라서 `7/1` 을 기대하는
 * 뉴욕 arm 이 red 다. 그래서 새벽 테스트에서는 **앞(미설정) arm 이 오히려 초록**이고
 * 뒤(뉴욕) arm 만 깨진다. 미설정 대조군은 M2 의 **두 번째** 방어선이다.
 * 대조군의 값은 「M2 를 유일하게 잡는다」가 아니라 **UTC+ 한 방향만 재는 설계를 애초에 못 하게
 * 막는다**는 데 있다 — 뉴욕 arm 이 없는 판본에서는 대조군이 유일한 방어선이 된다.
 *
 ...
 *
 * ## R6(미설정 = 달력일 전부)의 판별력은 **여기 대조군**에 있다 — 통합테스트가 아니다
 *
 * `SprintBurndownIntegrationTest` 의 S1 이 실 DB 에서 `working_days`·`board_timezone` 이 NULL 인
 * 보드를 타는 것은 사실이다. 그러나 그 스프린트는 **2020-01-01~01-03(수·목·금 — 주말이 0일)**이고
 * 단언은 `points.length() == 3` 이라, **미설정을 월~금으로 채우는 구현(M5)도 그대로 초록**이다.
 *
 * 그러므로 통합테스트가 주는 증거는 「실 DB·미설정 경로에서 죽지 않고 **UTC 칸이 유지된다**」까지다.
 * 「미설정 경로 무변경」을 재는 것은 이 파일의 **단위 대조군 14 vs 10** 이다 —
 * 주말이 실제로 걸치는 기간이라야 두 구현이 갈린다.
 *
