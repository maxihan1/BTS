...
class SprintBurndownService(
     ...
     *    ★**단, 차트 자체는 이미 라이브다**(재리뷰 C3 정정). `SprintBurndownService` ·
     *    `BurndownCalculator` · 번다운 화면 · 그 e2e 는 전부 origin/main 에 있다. 이 근거가
     *    성립시키는 것은 「아무도 **설정 화면**을 본 적 없다」이지 「아무도 **차트**를 본 적 없다」가
     *    아니다. 즉 이 릴리스에서 **기존 보드의 세로축이 시간 → 개수로 바뀐다** — 조용한 변경이
     *    아니라 선언된 변경이고, 릴리스 노트에 실려야 한다. 되돌리기는 보드당 라디오 1회이고
     *    일괄은 `UPDATE boards SET time_tracking = 'REMAINING_AND_SPENT'` 한 줄이다.
