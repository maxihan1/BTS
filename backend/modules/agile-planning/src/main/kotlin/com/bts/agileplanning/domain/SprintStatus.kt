// 스프린트 상태 열거형 — 허용 전이 규칙을 보유하는 FSM 노드
 ...
 * 스프린트 상태 열거형.
 *
 * 각 상태가 allowedTransitions 프로퍼티로 전이 가능한 다음 상태 집합을 선언한다.
 * 전이 검증은 Sprint 애그리게이트가 이 규칙을 참조해 수행한다.
 *
 * ### 전이 규칙
 * - PLANNED: ACTIVE 로만 전이 가능.
 * - ACTIVE: COMPLETED 로만 전이 가능.
 * - COMPLETED: 전이 불허. 종료 상태.
 *
 * @property allowedTransitions 이 상태에서 전이 가능한 다음 상태 집합.
 */
enum class SprintStatus {
    ...
    /**
     * 대상 상태로의 전이가 허용되는지 반환한다.
     *
     * @param target 전이를 시도할 목표 상태.
