...
--   V506:66-68 의 「마이그레이션이 기존 데이터를 조용히 바꾸지 않는다」에 대한 **명시적 예외**이고,
...
-- ★ ② 가 내린 status 는 **사후에 집을 수 있다.** now() 는 트랜잭션 타임스탬프라 강등된 전 행이
--   정확히 같은 updated_at 을 공유한다 — `WHERE status='PLANNED' AND updated_at = <그 시각>` 이
--   강등 집합이다. 자동 원복 스크립트는 없지만 「방법이 없다」는 아니다. 되돌릴 수 없는 것은
--   ③ 의 board_id 뿐이다(원래 칸반 id 를 어디에도 안 남긴다).
--   ※ 초안 주석은 status 도 복원 불가라 적었다 — 게이트 2 ceo 렌즈가 실물로 정정했다.
--     사고 시 「방법이 없다」로 판단을 좁히는 것이 이 저장소의 부채 3413 과 부호만 반대인 같은 양식이다.
