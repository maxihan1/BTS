...
-- 알림 정책 Aggregate Root. project_id=NULL 이면 전역(시스템 기본) 정책이다.
-- project_id 가 있으면 해당 프로젝트 전용 정책으로 전역 기본을 덮어쓴다.
CREATE TABLE notification_policies (
    ...
    -- NULLS NOT DISTINCT: project_id=NULL 인 전역 정책도 (event_type, recipient_role, channel) 기준 중복을 막는다.
...
COMMENT ON COLUMN notification_policies.project_id     IS 'NULL=전역 기본 정책, non-NULL=프로젝트 전용 정책 (전역을 덮어씀)';
