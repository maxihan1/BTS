 ...
 *
 * Testcontainers PostgreSQL 1 컨테이너 위에서 Flyway V001 + YAML seed 적재 후
 * WorkflowEngine → WorkflowApplicationService 전체 스택을 검증한다.
 ...
 * - S3. required field fail — RequiredFieldValidator: 필수 필드 누락 → ValidationFailedException
 * - S4. SpEL pass — CustomExpressionValidator: SpEL 표현식 true → 전이 성공
 * - S5. PostAction emitEvents — Notify PostAction 이 DomainEvent 발행
 * - S6. 동시 전이 낙관락 — 동일 요청 두 번 실행, version 기록 후 version 불일치 충돌 시뮬
...
class WorkflowIntegrationTest {
    ...
    companion object {
        ...
        /** 통합 테스트 전용 픽스처 빌더 — software-default 기반 표준 전이 요청 */
        fun softwareDefaultTransitionRequest(
        ...
        fun setup() {
            ...
            // TransactionTemplate — Spring 트랜잭션을 선언적(annotation) 대신 프로그래밍 방식으로 관리
            // WorkflowEngine.plan()은 Propagation.MANDATORY이므로 반드시 활성 트랜잭션 안에서 호출해야 한다
            val txManager = DataSourceTransactionManager(dataSource)
            txTemplate = TransactionTemplate(txManager)

            // seed — 표준 4 YAML 워크플로우 적재 (YamlSeedService 패턴 재현)
            seedStandardWorkflows()
        }

        /**
         * 표준 4 워크플로우를 직접 DB에 삽입한다.
         *
         * YamlSeedService는 Spring Bean이므로 Spring Context 없이는 사용하기 어렵다.
         * 대신 software-default 워크플로우를 JDBC로 직접 삽입해 통합 테스트 seed로 활용한다.
         * (YamlSeedServiceTest에서 검증된 YAML 정합성을 신뢰)
         */
        private fun seedStandardWorkflows() {
            DriverManager.getConnection(postgres.jdbcUrl, postgres.username, postgres.password).use { conn ->
                conn.autoCommit = false

                // software-default 워크플로우 (5 states + 6 transitions)
                val wfId = conn.prepareStatement(
                    "INSERT INTO workflows (key, name) VALUES ('software-default', '소프트웨어 개발 기본 워크플로우') RETURNING id",
                ).use { stmt -> stmt.executeQuery().use { rs -> rs.next(); rs.getObject(1) as java.util.UUID } }
                ...
                insertTransitionWithIds(conn, wfId, openId, inProgressId, "Start Work")
                insertTransitionWithIds(conn, wfId, inProgressId, inReviewId, "Submit for Review")
                insertTransitionWithIds(conn, wfId, inReviewId, doneId, "Approve")
                insertTransitionWithIds(conn, wfId, inReviewId, inProgressId, "Request Changes")
                insertTransitionWithIds(conn, wfId, doneId, closedId, "Close")
                insertTransitionWithIds(conn, wfId, openId, closedId, "Cancel")
        ...
        private fun insertTransitionWithIds(
            conn: java.sql.Connection,
            wfId: java.util.UUID,
         ...
         * factory stub은 호출자가 원하는 Validator/PostAction 인스턴스를 직접 반환하게 설정한다.
         */
        fun buildEngine(
    ...
    fun `S1 - happy path - open 에서 in_progress 로 정상 전이된다`() {
        ...
        val engine = buildEngine(definitionRepo, validatorFactory, postActionFactory)

        val request = softwareDefaultTransitionRequest()
    ...
    /**
     * Given  Permission validator 가 Fail 을 반환하도록 설정
     * When   전이 요청
     * Then   WorkflowValidatorFailureException 발생, validatorType 포함
     */
    @Test
    @Order(2)
    fun `S2 - permission fail - 권한 부족 시 WorkflowValidatorFailureException 이 발생한다`() {
        ...
        val request = softwareDefaultTransitionRequest(actorRoles = setOf("VIEWER"))
    ...
    fun `S3 - required field fail - 필수 필드 누락 시 WorkflowValidatorFailureException 이 발생한다`() {
        // Given — RequiredFieldValidator 를 실제 구현체로 사용 (mock 없이 실제 로직 검증)
        ...
        // resolution 필드 없이 요청
        val request = softwareDefaultTransitionRequest(
            issueFields = mapOf("priority" to "HIGH"),  // resolution 없음
        )
     ...
     * When   전이 요청
     * Then   Validator pass → 전이 성공 (TransitionPlan 반환)
     */
    @Test
    @Order(4)
    ...
    // ── S6. 동시 전이 낙관락 시뮬 — 동일 전이 두 번 시도 ─────────────────────────

    /**
     * Given  version=1 의 첫 번째 전이는 성공
     * When   동일 요청을 version=1 로 다시 시도 (낙관락 충돌 시뮬)
     * Then   WorkflowEngine.plan() 자체는 두 번 모두 TransitionPlan 을 반환한다
     *        (version 충돌 감지는 호출자 BC 책임 — plan() 은 READ-ONLY 결정만 수행)
     *        → 두 결과의 toStateKey 가 동일하게 "in_progress" 임을 검증
     *
     * NOTE. WorkflowEngine.plan() 은 상태를 변경하지 않는다. 낙관락 충돌 처리(version 비교 후
     * OptimisticLockException 등)는 이슈 트래킹 BC 가 plan 결과를 적용할 때 수행한다.
     * 이 시나리오는 "동시 호출 두 건이 모두 동일한 결과를 계산한다"는 멱등성을 검증한다.
     */
    @Test
    @Order(6)
    fun `S6 - 동시 전이 낙관락 - 동일 전이 두 번 호출 시 동일 결과를 반환한다`() {
        ...
        val request1 = softwareDefaultTransitionRequest(version = 1L)
        val request2 = softwareDefaultTransitionRequest(version = 1L)  // 동일 version — 충돌 시나리오
    ...
    // ── S7. cache invalidate — invalidate 후 다음 findByKey 가 DB 재조회 ─────────

    /**
     * Given  cache 에 software-default 가 이미 적재된 상태
     * When   workflowCache.invalidate("software-default") 호출
     * Then   다음 findByKey("software-default") 가 cache miss → DB 재조회 성공
     *        (WorkflowCache.findByKey 가 동일 참조가 아닌 새 객체를 반환)
     */
    @Test
    @Order(7)
    fun `S7 - cache invalidate - invalidate 후 다음 조회가 DB 에서 재적재된다`() {
        ...
        // Then — 다음 조회는 DB 재조회 (miss 경로) — 새 객체 반환
        ...
        // invalidate 전 객체와는 다른 인스턴스 (cache miss 경로로 새로 생성됨)
