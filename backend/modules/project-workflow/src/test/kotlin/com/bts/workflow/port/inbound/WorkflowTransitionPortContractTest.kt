 ...
 * 2. `plan(req: TransitionRequest): TransitionPlan` 메서드를 가진다.
...
class WorkflowTransitionPortContractTest {
    ...
    @Test
    fun `plan 반환 타입은 TransitionPlan 이다`() {
        val planFn = portClass.memberFunctions.find { it.name == "plan" }
        assertThat(planFn).isNotNull()
        assertThat(planFn!!.returnType.classifier).isEqualTo(TransitionPlan::class)
    }
