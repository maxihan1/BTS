 ...
 *
 * 전환 identity = (fromStateKey, toStateKey) — 한 워크플로우 내에서 같은 (from, to) 쌍은 중복 정의 불가.
 ...
 *
 * @property fromStateKey 전환 출발 상태의 키. [Workflow.states] 집합에 포함돼야 한다.
 * @property toStateKey 전환 도착 상태의 키. [Workflow.states] 집합에 포함돼야 한다.
 * @property name 사람 친화 표시 라벨 (예. "Start Work", "Resolve"). 매칭에 사용 안 함.
 * @property key 전환 고유 식별 키. [fromStateKey]__[toStateKey] 합성 — DB 저장 없이 계산된 값. 라우팅/API 호출용 1급.
 */
