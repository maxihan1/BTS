...
/**
 * 워크플로우 전환(Transition) 후 이슈에 워처(Watcher — 이슈 변경 알림을 받는 구독자)를
 * 추가하도록 요청하는 PostAction 구현체.
 *
 * ## 동작 방식
 * - `watcher` 값이 `${actor}` 플레이스홀더이면 전환을 실행한 액터의 userId 로 치환한다.
...
class AddWatcherPostAction(private val watcher: String) : WorkflowPostAction {
    ...
    /**
     * 전환 컨텍스트를 바탕으로 워처 추가 이벤트 계획을 계산한다.
     *
     * @param ctx 전환 시점의 컨텍스트 (이슈 상태, 액터 등)
