...
/**
 * 성능 스모크 상한(ms) — **제품 NFR 그 값**이다.
 *
 * ## 왜 5,000 에서 1,000 으로 내렸나 (2026-09-09)
 *
 * 종전 값은 「NFR 1s 대비 3~5배 여유를 둔 소프트 상한」이었다. 그 여유가 **감시하려는 값보다
 * 커서** 가드가 사실상 사라져 있었다 —
 *
 * | | |
 * |---|---|
 * | 제품 NFR (`product §NFR` 「규칙 충돌 정적 분석 1s」) | 1,000ms |
 * | `#268` 충족 판정 실측 | 876ms |
 * | 종전 상한 | **5,000ms** |
 *
 * 876ms 짜리가 4,999ms 가 돼도 초록이다. **5.7배 후퇴를 감시 못 하는 가드**였고, 실제로
 * 2코어 리눅스에서 12,583ms 가 될 때까지 아무도 몰랐다(젠킨스 빌드 #18).
 *
 * ## 지금 값이 실측으로 뒷받침된다
 *
 * N+1 을 배치로 바꾼 뒤(`AutomationRuleService.hydrateRules`) **2코어 리눅스에서 202ms** 다.
 * 상한 1,000ms 는 그 대비 **4.9배 여유**이므로 정상 변동으로 깨지지 않는다.
 *
 * ★**측정 기계를 함께 적는다.** 종전 주석의 「3~5배 여유」는 어느 기계 기준인지 없어서
 *   환경이 바뀌는 순간 의미가 증발했다. 절대 시간 상한은 기계를 함께 기록해야 계약이 된다.
 *   기준 기계 — NCP VM `101.79.19.193` · Rocky 8.8 x86_64 · **2코어** 16GB ·
 *   젠킨스 컨테이너 `cpus 2.0` · Testcontainers postgres 동거.
 *   개발 맥(ARM64 다코어)은 이보다 빠르므로 이 상한을 자연히 통과한다.
 *
 * ## 이 상한이 유일한 성능 가드는 아니다
 *
 * 절대 시간은 기계를 탄다 — 그래서 [com.bts.automation.adapter.HydrationQueryCountTest] 가
 * **쿼리 왕복 수**(기계 무관)로 N+1 회귀를 따로 막는다. 둘이 보는 것이 달라 함께 있어야 한다.
 * 이 상한은 「쿼리 하나가 느려지는」 회귀를, 쿼리 수 가드는 「왕복이 늘어나는」 회귀를 잡는다.
 */
private const val PERFORMANCE_TIMEOUT_MS = 1_000L
