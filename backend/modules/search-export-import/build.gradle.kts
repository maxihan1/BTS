...
dependencies {
    ...
    // JDBC + Flyway (런타임 DB 마이그레이션 및 Repository 접근, FR-SR-03 saved_filters)
    ...
    // Flyway: 컨테이너 기동 후 V600 SQL 적용 (TC_INITSCRIPT hook 사용)
    ...
    // Testcontainers (테스트용 DB를 도커로 자동 실행하는 라이브러리) — V600 마이그레이션 / Repository 통합 테스트
...
// V600 SQL을 적용한 뒤 information_schema introspection으로 정확한 Kotlin 소스를 생성한다.
jooq {
    configurations {
        create("main") {
            ...
            jooqConfiguration.apply {
                ...
                // Testcontainers PostgreSQL JDBC URL — 자동 컨테이너 기동 + TC_INITSCRIPT로 V600 적용
                ...
                generator.apply {
                    ...
                    target.apply {
                        packageName = "com.bts.search.jooq"
