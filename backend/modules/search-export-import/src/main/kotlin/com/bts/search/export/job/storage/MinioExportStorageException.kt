// Export 결과 MinIO 오브젝트 스토리지 오류 — put/openStream/remove 실패 시 래핑

package com.bts.search.export.job.storage

/**
 * [MinioExportStorageAdapter] 가 MinIO SDK 예외를 래핑해 던지는 도메인 예외.
 *
 ...
 * ## BC 격리
 *
 * issue-tracking 의 `MinioStorageException` 과 별개 클래스로 정의한다.
 * search-export-import BC 는 issue-tracking 내부 클래스를 import 하지 않는다 (NEVER-1).
 *
 * @param message 오류 메시지. 대상 오브젝트 키를 포함한다.
 * @param cause 원인 예외. MinIO SDK 가 던진 원본 예외.
 */
class MinioExportStorageException(
    message: String,
    cause: Throwable,
