// Import 작업 pgmq consumer — q_import_jobs 큐 폴링 + CAS 클레임 + 처리 위임 (FR-IM-01 PR1 Task 10)
...
/**
 * pgmq `q_import_jobs` 큐를 폴링하여 비동기 Import 작업을 처리하는 스케줄 워커.
 *
 * `export` BC [com.bts.search.export.job.worker.ExportJobWorker] 를 1:1 미러하되
 * importJobId 필드명 + [ImportJobProcessor] 위임으로 조정했다.
 *
 * ## 처리 흐름
 * 1. [Scheduled] 폴링 → `pgmq.read(queue, vt, qty)` 로 메시지 읽기 (read_ct 포함)
 * 2. 메시지의 importJobId 에 대해 작업레벨 CAS [ImportJobRepository.claimForRun]
 *    - 1 row(true) → [ImportJobRepository.findById] 로 전체 작업 로드 → [ImportJobProcessor.process]
 *    - 0 row(false) → [ImportJobRepository.findStatus] 로 현재 상태 조회
 ...
 * [ImportJobProcessor.process] 는 최대 10만 행을 스트리밍 파싱하며 행마다 별도 트랜잭션을 여는
 * **분 단위** I/O 집약 작업이므로 @Transactional 없이 설계됐다.
 ...
 * - 처리 예산: 10만행 스트리밍 파싱 + 행별 IssueImportPort 위임 ≈ 최대 2~3분(export 미러).
 * - [VISIBILITY_TIMEOUT_SECONDS] = **300초** (처리 예산 초과, BulkOperation 60초 미러 금지).
 * - [ImportJobRepository.STALE_RUNNING_THRESHOLD_SECONDS] = **600초** (VT 2배 + 안전마진).
 ...
 * @param importRepo [ImportJobRepository]. CAS claim + 상태 조회 + 전체 작업 로드 담당.
 * @param processor [ImportJobProcessor]. Import 작업 실제 처리 담당.
 */
@Component
class ImportJobWorker(
    private val dsl: DSLContext,
    private val importRepo: ImportJobRepository,
    private val processor: ImportJobProcessor,
) {
    private val log = LoggerFactory.getLogger(javaClass)

    /**
     * pgmq 큐를 폴링하여 대기 중인 Import 작업을 처리한다.
     *
     * 메시지가 없으면 즉시 반환한다.
     * **@Transactional 없음 — 의도적 설계** (클래스 KDoc 참조).
     */
    @Scheduled(fixedDelayString = "\${bts.import.worker.poll-interval-ms:1000}")
    fun pollAndProcess() {
        val messages =
            dsl.fetch(
     ...
     * @param messageJson pgmq 메시지 JSON. `{"importJobId":"<UUID>"}` 형식.
    ...
    private fun processMessage(
        ...
        if (jobId == null) {
            log.error("import_worker_invalid_message msgId={} readCt={} message={}", msgId, readCt, messageJson)
            handlePoison(msgId, readCt, jobId = null)
            return
        }

        log.info("import_worker_received msgId={} importJobId={} readCt={}", msgId, jobId.value, readCt)

        try {
            val claimed = importRepo.claimForRun(jobId)
            if (!claimed) {
            ...
            val job =
                importRepo.findById(jobId)
                    ?: error(
                        "import_worker: job not found after successful claim msgId=$msgId importJobId=${jobId.value}",
                    )
            ...
            log.error(
                "import_worker_processing_failed msgId={} importJobId={} message={}",
                msgId,
                jobId.value,
                e.message,
                e,
    ...
    private fun handleClaimFalse(
        ...
        val currentStatus = importRepo.findStatus(jobId)
        when {
            currentStatus == ImportJobStatus.COMPLETED || currentStatus == ImportJobStatus.FAILED -> {
                log.info(
                    "import_worker_claim_skipped_terminal msgId={} importJobId={} status={}",
                    msgId,
                    jobId.value,
                    currentStatus,
                )
            ...
            currentStatus == ImportJobStatus.RUNNING -> {
                log.info(
                    "import_worker_claim_skipped_active msgId={} importJobId={} reason=other_worker_running",
                    msgId,
                    jobId.value,
                )
            }
                ...
                log.warn(
                    "import_worker_claim_skipped_unknown msgId={} importJobId={} status={} readCt={}",
                    msgId,
                    jobId.value,
                    currentStatus,
                    readCt,
    ...
    private fun handlePoison(
        ...
        if (readCt > MAX_RECEIVE_COUNT) {
            log.error(
                "import_worker_dead_letter msgId={} importJobId={} readCt={} action=archive",
                msgId,
                jobId?.value,
                readCt,
            )
            ...
            log.warn(
                "import_worker_poison_retry msgId={} importJobId={} readCt={} maxReceiveCount={}",
                msgId,
                jobId?.value,
                readCt,
                MAX_RECEIVE_COUNT,
    ...
    private fun deleteMessage(msgId: Long) {
        ...
        log.debug("import_worker_deleted msgId={}", msgId)
    }

    /**
     * 메시지 JSON 에서 [ImportJobId] 를 파싱한다. 파싱 실패 시 null 반환 (`!!` 금지).
     */
    private fun parseJobId(messageJson: String): ImportJobId? =
        try {
            val uuidStr = UUID_PATTERN.find(messageJson)?.groupValues?.get(1) ?: return null
            ImportJobId(UUID.fromString(uuidStr))
        } catch (e: IllegalArgumentException) {
            log.warn("import_worker_parse_failed message={} error={}", messageJson, e.message)
            null
        }

    companion object {
        /**
         * pgmq 작업 큐 이름 — [ImportJobEnqueuePublisher.QUEUE_NAME] 과 일치해야 한다.
         * V604 마이그레이션에서 생성된 큐.
         */
         ...
         *
         * **VT ↔ stale 정합 근거** (BLOCKER — stale > VT 필수).
         * - 처리 예산: 10만행 스트리밍 파싱 + 행별 이슈 생성 ≈ 최대 2~3분(export 미러).
         * - VT = **300초** (처리 예산 초과, BulkOperation 60초 미러 금지).
         * - stale([ImportJobRepository.STALE_RUNNING_THRESHOLD_SECONDS]) = **600초** (VT 2배 + 안전마진).
        ...
        /** importJobId UUID 추출을 위한 JSON 패턴. */
        private val UUID_PATTERN = Regex(""""importJobId"\s*:\s*"([^"]+)"""")
    }
}
