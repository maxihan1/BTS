...
class AqlParser(private val tokens: List<AqlToken>) {
    ...
    private fun parseComparison(): AqlNode {
        ...
        return when (opToken.type) {
            ...
            AqlTokenType.EQ, AqlTokenType.NEQ, AqlTokenType.TILDE -> parseBinaryComparison(fieldName, fieldToken.position)
            else -> throw AqlSyntaxException(
    ...
    private fun parseNotIn(
        ...
        advance() // NOT 소비
        val inToken = peek()
            ?: throw AqlSyntaxException("'NOT' 뒤에 'IN' 이 예상됩니다.", fieldPos)
        if (inToken.type != AqlTokenType.KW_IN) {
            throw AqlSyntaxException(
                "'NOT' 뒤에 'IN' 이 예상되었지만 '${inToken.lexeme}' 를 만났습니다.",
                inToken.position,
            )
        }
        advance() // IN 소비
    ...
    private fun parseValueList(fieldPos: Int): List<AqlValue> {
        val lparenToken = peek()
            ?: throw AqlSyntaxException("'(' 가 예상되었지만 입력이 끝났습니다.", fieldPos)
        if (lparenToken.type != AqlTokenType.LPAREN) {
            throw AqlSyntaxException(
                "'(' 가 예상되었지만 '${lparenToken.lexeme}' 를 만났습니다.",
                lparenToken.position,
            )
        }
        advance() // '(' 소비
        ...
        val values = mutableListOf(parseValue())
     ...
     */
    private fun parseValue(): AqlValue {
        val token = peek()
            ?: throw AqlSyntaxException("값이 예상되었지만 입력이 끝났습니다.", currentPosition())
        return when (token.type) {
            AqlTokenType.QUOTED_STRING -> {
                advance()
                AqlValue.Str(token.lexeme)
            }
            AqlTokenType.IDENT, AqlTokenType.BARE_WORD -> {
                advance()
                AqlValue.Str(token.lexeme)
            }
            AqlTokenType.NUMBER -> {
    ...
    private fun parseOptionalOrderBy(): List<AqlSort> {
        ...
        if (token.type != AqlTokenType.KW_ORDER) return emptyList()

        advance() // ORDER 소비
        val byToken = peek()
            ?: throw AqlSyntaxException("'ORDER' 뒤에 'BY' 가 예상됩니다.", token.position)
        if (byToken.type != AqlTokenType.KW_BY) {
            throw AqlSyntaxException(
                "'ORDER' 뒤에 'BY' 가 예상되었지만 '${byToken.lexeme}' 를 만났습니다.",
                byToken.position,
            )
        }
        advance() // BY 소비

        val sorts = parseSortList(token.position)

        // 두 번째 ORDER BY 금지
        if (peek()?.type == AqlTokenType.KW_ORDER) {
            val dupToken = peek()!!
    ...
    private fun parseSortItem(): AqlSort {
        ...
        val direction = when (peek()?.type) {
            AqlTokenType.KW_ASC -> { advance(); SortDirection.ASC }
            AqlTokenType.KW_DESC -> { advance(); SortDirection.DESC }
