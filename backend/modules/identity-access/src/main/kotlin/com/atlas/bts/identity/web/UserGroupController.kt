...
class UserGroupController(
    ...
    /**
     * 핸들러 본문을 실행하고 도메인 예외를 HTTP 응답으로 매핑한다.
     *
     * 명세에 없는 예외는 [mapServiceException] 이 재발생시킨다(silently swallow 금지).
     */
    ...
    /**
     * 도메인/검증 예외를 snake_case `error` 코드 + HTTP 상태로 매핑한다.
     *
     * 알 수 없는 예외는 재발생시켜 전역 처리에 위임한다(은폐 금지).
     */
    private fun mapServiceException(ex: RuntimeException): ResponseEntity<Map<String, String>> =
        when (ex) {
            ...
            is IllegalArgumentException -> errorResponse(HttpStatus.BAD_REQUEST, "group_name_invalid")
            else -> throw ex
        }
