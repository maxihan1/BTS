// identity-access non-prod IssuePermissionResolver fallback — dev/test/staging 항상 허용, @Profile("!prod")로 운영 차단.
...
/**
 * identity-access BC non-prod [IssuePermissionResolver] fallback.
 *
 * ## 존재 이유 (BC 격리)
 * identity-access 의 `MyIssuePermissionController` 가 shared-kernel 포트
 * [IssuePermissionResolver] 를 주입받는다. 운영(prod)에서는
 * [IdentityAccessIssuePermissionResolver] (`@Profile("prod")`) 가 그 포트를 채우지만,
 * non-prod 에서는 issue-tracking BC 의 `AlwaysAllowIssuePermissionResolver`
 * (`@Profile("!prod")`) 만 그 포트를 채운다. 그런데 identity-access 컨텍스트
 * ([com.atlas.bts.identity] component-scan)는 issue-tracking 모듈을 스캔하지 않으므로,
 * non-prod identity-access 컨텍스트에는 [IssuePermissionResolver] Bean 이 하나도 없다.
 ...
 *
 * prod profile 에서는 [IdentityAccessIssuePermissionResolver] (`@Profile("prod")`) 만
 * 활성화되어 멤버 게이트 + role_permissions 매트릭스로 실제 판정한다.
 * 두 Bean 은 profile 로 상호 배타적(mutually exclusive)이다.
 * - [DevAllowIssuePermissionResolver] — `@Profile("!prod")` (개발/테스트/스테이징)
 ...
 * issue-tracking 컨텍스트엔 자기 `AlwaysAllowIssuePermissionResolver` 만 로드되어 충돌하지 않는다.
 *
...
@Component
@Profile("!prod")
class DevAllowIssuePermissionResolver : IssuePermissionResolver {
    ...
    override fun hasPermission(
        ...
        log.warn(
            "DevAllowIssuePermissionResolver: granting {} on {} to actor {} — non-prod fallback (운영 미활성)",
            permission,
            scope,
            actorId,
        )
