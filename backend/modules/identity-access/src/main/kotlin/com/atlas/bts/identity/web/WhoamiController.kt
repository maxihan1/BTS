 ...
 *   [SystemPermissionResolver.isSystemAdmin] 판정을 반영한다. 프로젝트 생성 권한(`canCreateProject`, FR-PJ-01/
 *   FR-PM-10)은 [SystemPermissionResolver.hasGlobalPermission] `(userId, "CREATE_PROJECT")` 위임값을 반영한다 —
 *   판정식이 `grant OR isSystemAdmin` 이라 개별 부여자와 SYSTEM_ADMIN 이 모두 통과하며(hasGrant 직접호출은 SYSTEM_ADMIN
 *   을 탈락시키므로 금지), 미부여·판정 불가는 false 로 수렴한다(fail-closed). whoami 는 self 전용이라 이 값은 호출자
 *   본인의 권한만 노출하고 타인 권한을 노출하지 않으며, 프론트의 "새 프로젝트" 버튼 게이팅용 UX 힌트일 뿐 최종 방어는
 *   백엔드 `POST /projects` 가 담당한다(canCreateProject 조작이 생성 우회로 이어지지 않는다). MFA 강제 등록 필요 여부(`mfaEnrollmentRequired`)는
 ...
 *   강제 변경·시스템 관리자·프로젝트 생성 권한(`canCreateProject`)·MFA 강제 등록 플래그는 봇 컨텍스트(PAT)와
 *   무관하므로 모두 조회 없이 false 로 고정한다(봇은 프로젝트 생성 UI 를 쓰지 않는다).
...
class WhoamiController(
     ...
     * @return 사용자 식별 정보 + authMethod + mustChangePassword + isSystemAdmin + canCreateProject + mfaEnrollmentRequired
     */
