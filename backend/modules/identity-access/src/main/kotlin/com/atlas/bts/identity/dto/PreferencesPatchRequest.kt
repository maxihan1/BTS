 ...
 * theme/locale/dateFormat/startPage 는 값이 있으면 반드시 고정 열거값 중 하나이고 "명시적 삭제(null)"
 * 개념이 없다(profile 의 `department` 3-state 와 대비). 따라서 각 필드를 nullable 로 선언해
 * `null`(부재)=미변경, 값=변경 의도의 2-state 만 표현한다 —
 ...
 * @property startPage 부재=미변경, 명시=검증 후 반영(허용값 밖이면 서비스가 400, FR-PF-02).
 */
