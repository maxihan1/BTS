// 버전 권한 prod 판정기 — 멤버 게이트 + role_permissions 매트릭스. FR-PM-03 stub 교체.
...
/**
 * [VersionPermissionResolver] prod 구현체 (FR-PM-03).
 *
 * ## 판정 알고리즘
 * 1. 멤버 게이트 — [ProjectMembershipRepository.findByProjectAndUser] 결과가 null(비멤버)이면 false.
 * 2. 매트릭스 판정 — [PermissionSchemeRepository.roleHasPermission]로 역할이 MANAGE_VERSIONS를 보유하는지 확인.
 *
 ...
 * 동형이나 포트 [VersionPermissionResolver.hasPermission]가 projectId를 직접 받으므로
 * IssueScope → projectId 해석 단계(ProjectDirectory 주입)가 없다. 생성자는 멤버십·스킴 리포 둘뿐.
 *
 * ## @Profile 배타성
 * `@Profile("prod")` — `AlwaysAllowVersionPermissionResolver`는 `@Profile("!prod")`이므로
...
@Component
@Profile("prod")
class IdentityAccessVersionPermissionResolver(
...
/**
 * [VersionPermission] → permission_code 매핑 (SDD 12.3, Jira식 단일 관리 권한).
 *
 * | VersionPermission         | permission_code   |
 * |---------------------------|-------------------|
 * | CREATE / UPDATE / DELETE  | MANAGE_VERSIONS   |
 *
 * 세 값이 같은 단일 관리 권한으로 묶이므로 non-null 반환. `when` else 없이 3종 전부 명시 —
 * enum 값 추가/리네임 시 컴파일 에러로 drift 차단.
 */
private fun VersionPermission.toPermissionCode(): String =
    when (this) {
        VersionPermission.CREATE,
        VersionPermission.UPDATE,
        VersionPermission.DELETE,
        -> "MANAGE_VERSIONS"
    }
