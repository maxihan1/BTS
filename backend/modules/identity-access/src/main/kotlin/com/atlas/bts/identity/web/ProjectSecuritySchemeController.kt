 ...
 * 3. **PROJECT_ADMIN 판정은 서비스([ProjectSecuritySchemeService]) 책임**이다. 컨트롤러는 actor 를
 *    전달만 하고, 거부는 서비스의 [ProjectSchemeAccessDeniedException] → 403 으로 매핑한다.
 *    `@PreAuthorize hasRole` 을 쓰지 않는 이유 — 프로젝트별 역할은 멤버십 DB 진실원천에서
 *    판정해야 하고, JWT claim/PAT 경로에 프로젝트 역할 정보가 없기 때문이다.
 *
