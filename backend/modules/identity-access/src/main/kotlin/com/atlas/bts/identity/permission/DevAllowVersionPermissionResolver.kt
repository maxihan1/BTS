// identity-access non-prod VersionPermissionResolver fallback — dev/test/staging 항상 허용, @Profile("!prod")로 운영 차단.
...
/**
 * identity-access BC non-prod [VersionPermissionResolver] fallback.
 *
 * ## 존재 이유 (BC 격리)
 * identity-access 의 `MyProjectPermissionController` (FR-PM-03 task-2 확장)가
 * shared-kernel 포트 [VersionPermissionResolver] 를 주입받는다. 운영(prod)에서는
 * prod adapter (`@Profile("prod")`) 가 그 포트를 채우지만, non-prod 에서는
 * issue-tracking BC 의 `AlwaysAllowVersionPermissionResolver` (`@Profile("!prod")`)
 * 만 그 포트를 채운다. 그런데 identity-access 컨텍스트
 * ([com.atlas.bts.identity] component-scan)는 issue-tracking 모듈을 스캔하지 않으므로,
 * non-prod identity-access 컨텍스트에는 [VersionPermissionResolver] Bean 이 하나도 없다.
 ...
 * issue-tracking 컨텍스트엔 자기 `AlwaysAllowVersionPermissionResolver` 만 로드되어 충돌하지 않는다.
 *
...
@Component
@Profile("!prod")
class DevAllowVersionPermissionResolver : VersionPermissionResolver {
    ...
    override fun hasPermission(
        ...
        log.warn(
            "DevAllowVersionPermissionResolver: granting {} on project {} to actor {} — non-prod fallback (운영 미활성)",
            permission,
            projectId,
            actorId,
        )
