...
/**
 * 신뢰 디바이스 식별용 불투명 토큰의 생성/해시 값 객체.
 *
 * [rawToken] 은 [TOKEN_BYTES]바이트 CSPRNG(암호학적으로 안전한 난수) 난수를 소문자 hex 로 인코딩한
 * 평문이며, MFA verify 성공 시 `Set-Cookie` 로 클라이언트에 단 한 번 내려간다. [hash] 는
 * `SHA-256(rawToken)` 소문자 hex [HASH_LENGTH]자로, DB(`trusted_devices.token_hash`)에는 이 해시만
 * 저장한다. RefreshToken 선례와 **동일한 hex 인코딩**(base64url 혼용 금지).
 *
 * ## 보안 정책
 * - **[rawToken] 평문은 DB·로그에 절대 저장/기록 금지**(DEVELOPMENT.md §1.1.1 — 비밀값 미저장).
 *   해시는 단방향이라 DB 탈취 시에도 raw token 을 역산할 수 없다(refresh_token·백업 코드 선례).
