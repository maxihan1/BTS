// 버전 권한 prod 판정기 전수 통합테스트 — 실제 DB + V009 + 9케이스 매트릭스 (FR-PM-03 Task 4)
...
/**
 * [IdentityAccessVersionPermissionResolver] 전수 통합테스트 (FR-PM-03 Task 4).
 *
 * ## 목적
 * - prod 프로파일에서 실제 DB(V001~V009 Flyway 적용) 위에 9케이스 권한 매트릭스를 검증한다.
 * - V009 시드(기본 스킴 × PROJECT_ADMIN × MANAGE_VERSIONS) 위에서 거부 경로(MEMBER/비멤버 → false)를
 ...
 * - Bean 배타: prod 프로파일에서 [IdentityAccessVersionPermissionResolver]가
 *   [VersionPermissionResolver]로 주입됨을 확인한다(AlwaysAllow*는 @Profile "!prod"이라 미등록).
 *
 * ## 9케이스 매트릭스 (3 actor × CREATE/UPDATE/DELETE)
 * | Actor          | Permission             | 기대  | 근거                       |
 * |----------------|------------------------|-------|----------------------------|
 * | PROJECT_ADMIN  | CREATE/UPDATE/DELETE   | true  | V009 MANAGE_VERSIONS 부여  |
 * | MEMBER         | CREATE/UPDATE/DELETE   | false | MANAGE_VERSIONS 미부여     |
 ...
 *
 * @see IdentityAccessVersionPermissionResolver
...
@ActiveProfiles("prod")
class IdentityAccessVersionPermissionResolverIntegrationTest {
    companion object {
        /**
    ...
    /** prod 프로파일에서 IdentityAccessVersionPermissionResolver가 주입되어야 한다. */
    @Autowired
    private lateinit var resolver: VersionPermissionResolver
    ...
    /**
     * prod 프로파일에서 [IdentityAccessVersionPermissionResolver]가 [VersionPermissionResolver]로 주입된다.
     * AlwaysAllowVersionPermissionResolver(@Profile "!prod")는 이 컨텍스트에 등록되지 않는다.
     */
    @Test
    fun `prod 프로파일에서 IdentityAccessVersionPermissionResolver가 VersionPermissionResolver로 주입된다`() {
        assertThat(resolver).isInstanceOf(IdentityAccessVersionPermissionResolver::class.java)
    }

    // ── 9케이스 매트릭스 (@TestFactory) ─────────────────────────────────────

    @TestFactory
    fun `버전 권한 매트릭스 — 3 actor × CREATE_UPDATE_DELETE 9케이스 전수 검증`(): List<DynamicTest> {
        data class Case(
            val label: String,
            val actorId: UUID,
            val permission: VersionPermission,
            val expected: Boolean,
        )

        val permissions = VersionPermission.entries
        val actors =
            listOf(
    ...
    private fun seedUsers() {
        listOf(
            Triple(adminId, "itest_ver_admin", "Version Admin"),
            Triple(memberId, "itest_ver_member", "Version Member"),
            Triple(nonMemberId, "itest_ver_nonmember", "Version NonMember"),
