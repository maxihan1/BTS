// 이슈 템플릿 권한 prod 판정기 전수 통합테스트 — 실제 DB + V024 + 9케이스 매트릭스 (FR-TM-01 Task 8)
...
/**
 * [IdentityAccessTemplatePermissionResolver] 전수 통합테스트 (FR-TM-01 Task 8).
 *
 * ## 목적
 * - prod 프로파일에서 실제 DB(V001~V024 Flyway 적용) 위에 9케이스 권한 매트릭스를 검증한다.
 * - V024 시드(기본 스킴 × PROJECT_ADMIN × MANAGE_TEMPLATES) 위에서 거부 경로(MEMBER/비멤버 → false)를
 *   실 DB로 확정한다. 단위 mock(MockK)은 schemeRepo.roleHasPermission 반환을 임의로 stub 하므로
 *   거부 ground-truth가 아니다.
 * - Bean 배타: prod 프로파일에서 [IdentityAccessTemplatePermissionResolver]가
 *   [TemplatePermissionResolver]로 주입됨을 확인한다(DevAllow*는 @Profile "!prod"이라 미등록).
 *
 * ## 9케이스 매트릭스 (3 actor × CREATE/UPDATE/DELETE)
 * | Actor          | Permission             | 기대  | 근거                            |
 * |----------------|------------------------|-------|---------------------------------|
 * | PROJECT_ADMIN  | CREATE/UPDATE/DELETE   | true  | V024 MANAGE_TEMPLATES 부여      |
 * | MEMBER         | CREATE/UPDATE/DELETE   | false | MANAGE_TEMPLATES 미부여         |
 ...
 * 기본 스킴(00000000-...-001) fallback 경로(V008+V024)를 그대로 사용한다.
 *
 * 선례: [IdentityAccessCustomFieldPermissionResolverTest] 100% 동형.
 *
...
@ActiveProfiles("prod")
class IdentityAccessTemplatePermissionResolverTest {
    companion object {
        /**
    ...
    /** prod 프로파일에서 IdentityAccessTemplatePermissionResolver가 주입되어야 한다. */
    @Autowired
    private lateinit var resolver: TemplatePermissionResolver
    ...
    /**
     * prod 프로파일에서 [IdentityAccessTemplatePermissionResolver]가 [TemplatePermissionResolver]로 주입된다.
     * DevAllowTemplatePermissionResolver(@Profile "!prod")는 이 컨텍스트에 등록되지 않는다.
     */
    @Test
    fun `prod 프로파일에서 IdentityAccessTemplatePermissionResolver가 TemplatePermissionResolver로 주입된다`() {
        assertThat(resolver).isInstanceOf(IdentityAccessTemplatePermissionResolver::class.java)
    }

    // ── 9케이스 매트릭스 (@TestFactory) ─────────────────────────────────────

    @TestFactory
    fun `이슈 템플릿 권한 매트릭스 — 3 actor × CREATE_UPDATE_DELETE 9케이스 전수 검증`(): List<DynamicTest> {
        data class Case(
            val label: String,
            val actorId: UUID,
        ...
        val permissions = TemplatePermission.entries
        val actors =
            listOf(
    ...
    private fun seedUsers() {
        listOf(
            Triple(adminId, "itest_tpl_admin", "Template Admin"),
            Triple(memberId, "itest_tpl_member", "Template Member"),
            Triple(nonMemberId, "itest_tpl_nonmember", "Template NonMember"),
