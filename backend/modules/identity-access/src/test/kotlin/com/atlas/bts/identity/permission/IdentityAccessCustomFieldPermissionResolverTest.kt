// 커스텀 필드 권한 prod 판정기 전수 통합테스트 — 실제 DB + V017 + 9케이스 매트릭스 (FR-IS-10 Task 6)
...
/**
 * [IdentityAccessCustomFieldPermissionResolver] 전수 통합테스트 (FR-IS-10 Task 6).
 *
 * ## 목적
 * - prod 프로파일에서 실제 DB(V001~V017 Flyway 적용) 위에 9케이스 권한 매트릭스를 검증한다.
 * - V017 시드(기본 스킴 × PROJECT_ADMIN × MANAGE_CUSTOM_FIELDS) 위에서 거부 경로(MEMBER/비멤버 → false)를
 ...
 * - Bean 배타: prod 프로파일에서 [IdentityAccessCustomFieldPermissionResolver]가
 *   [CustomFieldPermissionResolver]로 주입됨을 확인한다(AlwaysAllow*는 @Profile "!prod"이라 미등록).
 *
 * ## 9케이스 매트릭스 (3 actor × CREATE/UPDATE/DELETE)
 * | Actor          | Permission             | 기대  | 근거                            |
 * |----------------|------------------------|-------|---------------------------------|
 * | PROJECT_ADMIN  | CREATE/UPDATE/DELETE   | true  | V017 MANAGE_CUSTOM_FIELDS 부여  |
 * | MEMBER         | CREATE/UPDATE/DELETE   | false | MANAGE_CUSTOM_FIELDS 미부여     |
 ...
 * 기본 스킴(00000000-...-001) fallback 경로(V008+V017)를 그대로 사용한다.
 *
 * 선례: [IdentityAccessComponentPermissionResolverIntegrationTest] 100% 동형.
 *
...
@ActiveProfiles("prod")
@Testcontainers
class IdentityAccessCustomFieldPermissionResolverTest {
    companion object {
        @Container
        @JvmStatic
    ...
    /** prod 프로파일에서 IdentityAccessCustomFieldPermissionResolver가 주입되어야 한다. */
    @Autowired
    private lateinit var resolver: CustomFieldPermissionResolver
    ...
    /**
     * prod 프로파일에서 [IdentityAccessCustomFieldPermissionResolver]가 [CustomFieldPermissionResolver]로 주입된다.
     * AlwaysAllowCustomFieldPermissionResolver(@Profile "!prod")는 이 컨텍스트에 등록되지 않는다.
     */
    @Test
    fun `prod 프로파일에서 IdentityAccessCustomFieldPermissionResolver가 CustomFieldPermissionResolver로 주입된다`() {
        assertThat(resolver).isInstanceOf(IdentityAccessCustomFieldPermissionResolver::class.java)
    }

    // ── 9케이스 매트릭스 (@TestFactory) ─────────────────────────────────────

    @TestFactory
    fun `커스텀 필드 권한 매트릭스 — 3 actor × CREATE_UPDATE_DELETE 9케이스 전수 검증`(): List<DynamicTest> {
        data class Case(
            val label: String,
            val actorId: UUID,
            val permission: CustomFieldPermission,
            val expected: Boolean,
        )

        val permissions = CustomFieldPermission.entries
        val actors =
            listOf(
    ...
    private fun seedUsers() {
        listOf(
            Triple(adminId, "itest_cf_admin", "CustomField Admin"),
            Triple(memberId, "itest_cf_member", "CustomField Member"),
            Triple(nonMemberId, "itest_cf_nonmember", "CustomField NonMember"),
