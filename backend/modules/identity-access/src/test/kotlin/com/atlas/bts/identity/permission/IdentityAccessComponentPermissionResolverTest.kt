// IdentityAccessComponentPermissionResolver 단위테스트 — 멤버 게이트 + 매트릭스 판정 (FR-PM-03 Task 2)
...
/**
 * [IdentityAccessComponentPermissionResolver] 단위테스트.
 *
 * ## 검증 시나리오 (plan Task 2 RED)
...
class IdentityAccessComponentPermissionResolverTest {
    ...
    fun `비멤버는 CREATE_UPDATE_DELETE 모두 거부 — 멤버 게이트`() {
        ...
        ComponentPermission.entries.forEach { permission ->
    ...
    @Test
    fun `PROJECT_ADMIN 멤버는 CREATE_UPDATE_DELETE 모두 허용 — MANAGE_COMPONENTS 코드로 판정`() {
        every { membershipRepo.findByProjectAndUser(projectId, actor) } returns membership(ProjectRole.PROJECT_ADMIN)
        every {
            schemeRepo.roleHasPermission(projectId, "PROJECT_ADMIN", "MANAGE_COMPONENTS")
        } returns true
        ...
        // CREATE/UPDATE/DELETE 3종 모두 동일 코드("MANAGE_COMPONENTS")로 매핑되어 3회 호출.
        verify(exactly = 3) {
            schemeRepo.roleHasPermission(projectId, "PROJECT_ADMIN", "MANAGE_COMPONENTS")
        }
    }
    ...
    fun `MEMBER 멤버는 CREATE_UPDATE_DELETE 모두 거부 — 매트릭스 미보유`() {
        ...
        every {
            schemeRepo.roleHasPermission(projectId, "MEMBER", "MANAGE_COMPONENTS")
        } returns false
