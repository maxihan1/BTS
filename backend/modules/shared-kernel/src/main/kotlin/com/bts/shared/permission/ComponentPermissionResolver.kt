// 컴포넌트 권한 판정 포트(공용) — prod 실판정은 FR-PM-03 이연.
...
/**
 * 컴포넌트 권한 평가 outbound port — 전 BC 공용.
 *
 * issue-tracking BC 의 ComponentController/ComponentApplicationService 가 이 interface 를 통해
 * 권한 판정을 요청한다. 개발/스테이징 환경에서는 issue-tracking 내부의
 * `AlwaysAllowComponentPermissionResolver` (`@Profile("!prod")`) stub 이 활성화된다.
 *
 ...
 * 컴포넌트는 항상 프로젝트 스코프에 종속된다(Global/Issue 범위가 없음).
 ...
 */
interface ComponentPermissionResolver {
    /**
     ...
     * @param permission 검증할 컴포넌트 권한. [ComponentPermission] 참조.
     * @param projectId 컴포넌트가 속한 프로젝트의 UUID. 컴포넌트 권한은 항상 프로젝트 스코프.
