// 워크플로우 전환 inbound port — issue-tracking / automation BC 호출 (Propagation.MANDATORY)
...
/**
 * 워크플로우 전환 inbound port.
 *
 * issue-tracking BC 와 automation BC 가 전환을 요청할 때 이 인터페이스를 호출한다.
 ...
 *   하나 이상이 전환을 거부했을 때. [TransitionResult.ValidatorFailure.message] 에 사유 포함.
...
interface WorkflowTransitionPort {
     ...
     *
     * @param req 전환 요청 DTO. [TransitionRequest.validate] 를 통과한 상태여야 한다.
     * @return 전환 검증 결과. 4 케이스 반환 계약은 [WorkflowTransitionPort] KDoc 참조.
     */
    @Transactional(propagation = Propagation.MANDATORY)
    fun plan(req: TransitionRequest): TransitionResult

    /**
     * 현재 상태에서 이동 가능한 전환 목록을 조회하고 반환한다.
     *
     * 이 메서드는 **열거와 검증만** 수행한다. 상태 변경·post-action 트리거는 절대 수행하지 않는다.
     * 호출자 BC 는 반환된 목록을 UI 표시 또는 전환 가능 여부 사전 확인에 사용한다.
     *
     * ### 반환 계약
     *
     ...
     *
     * @param req 가용 전환 열거 요청 DTO. [AvailableTransitionsRequest.validate] 를 통과한 상태여야 한다.
     * @return 가용 전환 열거 결과. 2 케이스 반환 계약은 위 KDoc 참조.
     */
