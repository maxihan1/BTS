// 프로젝트 멤버십 조회 포트 — 사용자가 속한 프로젝트 키 집합을 cross-BC 방식으로 반환
 ...
 * 실패하도록 설계된 안전망이다. allow-all default 를 추가하면 프로젝트 멤버십 판정이
 * 우회되어 공유 가시성 누출이 발생하므로 절대 금지한다.
 *
 * ### 새 보안 경로 금지
 * 프로젝트 멤버십 기반 가시성 판정은 이 포트로만 이루어진다. 이 포트 외부에서 별도의
 * 프로젝트 멤버십 조회 경로를 만드는 것을 금지한다.
 *
 * ### BC 경계 규칙
...
interface ProjectMembershipPort {
    /**
     * 주어진 사용자 ID([userId])가 멤버로 속한 프로젝트의 키 집합을 반환한다.
     *
     * 프로젝트 멤버십이 없거나 조회 불가 시 빈 Set 을 반환해야 한다(구현 계약, fail-closed).
     *
     * @param userId 조회 대상 사용자 UUID
     * @return 사용자가 멤버로 속한 프로젝트 키 문자열 집합. 빈 Set 은 "멤버십 0개"를 의미한다.
     */
    fun projectKeysOf(userId: UUID): Set<String>
}
