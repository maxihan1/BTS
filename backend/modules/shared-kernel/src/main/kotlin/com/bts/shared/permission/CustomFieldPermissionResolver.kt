// 커스텀 필드 정의 관리 권한 판정 포트(공용) — prod 실판정은 FR-IS-10 T6(identity-access)에서 채운다.
...
/**
 * 커스텀 필드 정의 관리 권한 평가 outbound port — 전 BC 공용.
 *
 * issue-tracking BC 의 CustomFieldApplicationService 가 이 interface 를 통해 권한 판정을
 * 요청한다. 개발/스테이징 환경에서는 issue-tracking 내부의
 * `AlwaysAllowCustomFieldPermissionResolver` (`@Profile("!prod")`) stub 이 활성화된다.
 *
 * prod 실판정 adapter 는 FR-IS-10 T6 의 `IdentityAccessCustomFieldPermissionResolver`
 ...
 *
 * ## 호출 위치
 * CustomFieldApplicationService 의 각 mutation(CREATE/UPDATE/DELETE) 메서드 진입 직후.
 * `@PreAuthorize` SpEL 표현식 대신 명시적 메서드 호출로 권한을 검증한다(ComponentPermissionResolver 동형).
 *
 * ## ArchUnit 강제
 ...
 */
fun interface CustomFieldPermissionResolver {
    /**
     ...
     * @param permission 검증할 커스텀 필드 권한. [CustomFieldPermission] 참조.
     * @param projectId 커스텀 필드 정의가 속한 프로젝트의 UUID. 커스텀 필드 권한은 항상 프로젝트 스코프.
