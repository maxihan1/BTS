 ...
 * **조립 계층의 문제**다 — 각 BC 를 단독 부팅할 때는 그 BC 의 `data-dev.sql` 이 시드를 하고, 두 모듈이
 * 같은 리소스 이름(`classpath:data-dev.sql`)을 써서 **조립에서만** 충돌해 비활성화돼 있다
 * (`application.yml` 의 `spring.sql.init.mode: never` 주석). #321 이 확립한 A안 —
 * *"조립 계층의 결함은 조립 모듈에서 봉합한다"* — 을 그대로 따른다.
 *
