...
tasks.withType<Test> {
    ...
    //   configuration cache 를 저장하지 못했다(9개 모듈에서 같은 문제 8건).
    //   providers.exec 는 값을 요구받을 때까지 실행을 미루므로 doFirst 에서 소비하면
    //   실행 시점 호출이 되고 configuration cache 가 열린다. 동작(주입 값)은 그대로다.
    val dockerHost =
        providers.exec {
