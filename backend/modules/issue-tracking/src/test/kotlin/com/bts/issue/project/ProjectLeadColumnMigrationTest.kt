// projects.lead_user_id 컬럼 추가 마이그레이션(V013) 적용 결과 검증
...
/**
 * Flyway V001~V013 마이그레이션 적용 후 projects.lead_user_id 컬럼을 검증한다.
 * Testcontainers PostgreSQL 을 직접 사용하며 Spring 컨텍스트 없이 실행한다.
 *
 * 검증 범위.
 * - projects.lead_user_id 컬럼 존재
 * - 타입이 uuid
 ...
 *
 * 참조. FR-CM-04 plan Task 1 / DATA.md §3 소프트 삭제.
 */
@Testcontainers
class ProjectLeadColumnMigrationTest {
    companion object {
    ...
    // ── 컬럼 존재·타입·nullable 검증 ─────────────────────────────────────────

    @Test
    fun `V013 projects lead_user_id 컬럼이 존재한다`() {
        val info = columnInfo("projects", "lead_user_id")
        assertThat(info).isNotNull()
    }

    @Test
    fun `V013 projects lead_user_id 타입은 uuid 다`() {
        val (dataType, _) = columnInfo("projects", "lead_user_id")!!
        assertThat(dataType).isEqualTo("uuid")
    }

    @Test
    fun `V013 projects lead_user_id 는 nullable 이다`() {
        val (_, isNullable) = columnInfo("projects", "lead_user_id")!!
