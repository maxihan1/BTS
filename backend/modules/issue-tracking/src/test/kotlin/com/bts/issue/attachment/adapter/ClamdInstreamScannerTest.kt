 ...
 *
 * ## 가짜 clamd 서버 동작
 * 실 clamd 는 `zINSTREAM\x00` 수신 후 청크 스트림을 소비하고, 응답을 NUL(`\x00`) 로 종결한다.
 ...
 * - INSTREAM 프레이밍 단언: `zINSTREAM\x00` + 청크 + `\x00\x00\x00\x00` 수신 확인
 */
class ClamdInstreamScannerTest : DescribeSpec({
     ...
     */
    fun responseBytes(text: String, trailingNul: Boolean = false, trailingNewline: Boolean = false): ByteArray {
        val suffix = when {
    ...
    describe("CLEAN 판정") {
        it("stream: OK + trailing NUL → CLEAN (실 clamd 응답 형식)") {
            ...
            withFakeClam(response) { port ->
                val scanner: VirusScanPort = ClamdInstreamScanner(
                    host = "127.0.0.1",
                    port = port,
    ...
    describe("fail-closed — AttachmentScanUnavailableException") {
        ...
        it("연결 거부(닫힌 포트) → 예외") {
            // 서버 없이 닫힌 포트로 접속 시도
            val closedPort = ServerSocket(0).use { it.localPort } // 즉시 닫아 사용 불가 포트 확보
            val scanner: VirusScanPort = ClamdInstreamScanner(
                host = "127.0.0.1",
                port = closedPort,
                connectTimeoutMs = 2000,
    ...
    describe("INSTREAM 프레이밍 검증") {
        it("zINSTREAM\\x00 헤더 + 청크 + 0-길이 종결자를 올바르게 송신해야 한다") {
            val response = responseBytes("stream: OK", trailingNul = true)
            ...
            // 헤더: "zINSTREAM\x00" (10 바이트)
            val expectedHeader = "zINSTREAM ".toByteArray(Charsets.UTF_8)
            captured.size shouldBe (expectedHeader.size + 4 + "프레이밍 검증용 데이터".toByteArray(Charsets.UTF_8).size + 4)
