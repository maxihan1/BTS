...
class BoardIssueLookupAdapterTest : IssueTestcontainersBase() {
    ...
    // ── FR-UX-14 B2. 카드 밀도 3필드 — 대조군 없이는 아무것도 못 잰다 ────────
    //
    // 축마다 서로 다른 값 2건 + 빈 값 대조군 1건을 넣고 **키 단위로** 대조한다.
    // 값이 전부 같으면 매핑이 뒤바뀌거나 조인이 틀린 행을 물어와도 초록이다 (스펙 §9.3 GAP-1).
    // 증인은 팩토리 헬퍼로 만든 VO 가 아니라 여기 실제로 넣은 행이다 (GAP-2).
    ...
    fun `D2 - type JOIN 이 붙어도 카드 수가 이슈 수와 같고 중복이 없다`() {
        ...
        val page = adapterWith(unrestricted()).listVisibleIssuesByProject("TPRJ", viewer)
