// IssueRepository CFD 원천 조회 Testcontainers 통합 테스트 — 활성·가시 이슈만 반환 검증 (FR-RP-03 Task 2)
...
/**
 * [IssueRepository.fetchActiveVisibleIssuesForCfd] 통합 테스트 (FR-RP-03 Task 2).
 *
 * [IssueTestcontainersBase] JVM singleton PostgreSQL + Flyway 마이그레이션을 재사용한다.
 * 정본 보안 술어 `buildActiveSecureWhere` 재사용 여부를 [filterVisibleIssueKeys] 형제 메서드와
 * 동일한 시나리오 구성(soft-delete/보안등급/타 프로젝트)으로 검증한다(복제 금지, CONCERN-1).
 *
 * ## 검증 시나리오
 * - CFD-1. 프로젝트 이슈 메타(id·typeId·currentStateKey·createdAt)를 정확히 반환한다.
 * - CFD-2. 소프트 삭제된 이슈는 결과에서 제외된다.
 * - CFD-3. viewer 가 접근 불가한 보안 등급 이슈는 결과에서 제외된다.
 * - CFD-4. 타 프로젝트 이슈는 결과에서 제외된다.
 */
class IssueRepositoryCfdSourceTest : IssueTestcontainersBase() {
    /** V003 seed 의 task 타입 id — value class 는 lateinit 불가, nullable var 사용. */
    private var taskTypeId: IssueTypeId? = null

    /** CFD 를 조회하는 viewer UUID — 가시성 판단 기준. */
     ...
     * When   fetchActiveVisibleIssuesForCfd(unrestricted) 호출.
     * Then   반환된 [CfdIssueSourceRow] 의 id·typeId·currentStateKey·createdAt 이 삽입값과 일치한다.
     */
    @Test
    fun `CFD-1 - 프로젝트 이슈의 id typeId currentStateKey createdAt 을 반환한다`() {
        val issue = insertIssue(seq = 1L)

        val rows = repository.fetchActiveVisibleIssuesForCfd("TPRJ", viewerId, unrestrictedAccess)
     ...
     */
    @Test
    fun `CFD-2 - 소프트 삭제된 이슈는 결과에서 제외된다`() {
    ...
    // ── CFD-3. 보안 등급 차단 이슈 제외 ─────────────────────────────────────

    /**
     * Given  공개 이슈 1건 + viewer 가 접근 불가한 보안등급 이슈 1건.
     * When   viewer 가 restrictedLevel 미소속(NULL 등급만 접근)인 access 로 조회.
     * Then   기밀 이슈는 결과에서 제외되고 공개 이슈만 반환된다(기밀 누출 0).
     */
    @Test
    fun `CFD-3 - viewer 가 접근 불가한 보안 등급 이슈는 결과에서 제외된다`() {
        ...
        val rows = repository.fetchActiveVisibleIssuesForCfd("TPRJ", viewerId, restrictedAccess())
     ...
     * When   projectKey="TPRJ" 로 fetchActiveVisibleIssuesForCfd 조회.
     * Then   TPRJ2 이슈는 결과에서 제외되고 TPRJ 이슈만 반환된다.
     */
    @Test
    fun `CFD-4 - 타 프로젝트 이슈는 결과에서 제외된다`() {
