// 비prod issue-tracking 컨텍스트에 VersionPermissionResolver 빈이 정확히 1개(AlwaysAllow) 주입됨을 고정하는 부팅 회귀 가드 (FR-VR-01 task-5).
...
/**
 * 비prod issue-tracking 컨텍스트 [VersionPermissionResolver] 부팅 회귀 가드 (FR-VR-01 task-5).
 *
 * ## 회귀 배경 (ComponentPermissionResolverBootTest 동형)
 * VersionController/VersionApplicationService 가 shared-kernel 포트 [VersionPermissionResolver] 를
 * 주입받는데, 비prod 컨텍스트에 그 포트를 채우는 Bean 이 사라지면 컨텍스트 부팅 자체가
 * `UnsatisfiedDependency` / `No qualifying bean` 으로 실패한다.
 * FR-PM-02 에서 동일 유형의 회귀가 66개 통합테스트를 깨뜨린 선례가 있다.
 *
 * ## 구조 (ComponentPermissionResolver 동형)
 * 소비자(VersionController)와 비prod 구현([AlwaysAllowVersionPermissionResolver]) 모두
 * issue-tracking 同모듈이라 cross-BC 스캔 누락은 구조적으로 발생하지 않는다.
 ...
 * [VersionPermissionResolver] 구현이 없다. 따라서 이 부팅테스트는 비prod(`test`)로 기동한다.
 *
...
@ActiveProfiles("test")
class VersionPermissionResolverBootTest {
    @Autowired
    ...
    companion object {
        ...
        val postgres: PostgreSQLContainer<*> =
            PostgreSQLContainer(
                ...
                .withDatabaseName("bts_version_perm_test")
                .withUsername("bts")
    ...
    @Test
    fun `비prod 컨텍스트에 VersionPermissionResolver 빈이 정확히 1개 존재한다`() {
        val beans = context.getBeansOfType(VersionPermissionResolver::class.java)

        assertThat(beans).hasSize(1)
        assertThat(beans.values.single()).isInstanceOf(AlwaysAllowVersionPermissionResolver::class.java)
    }
}
