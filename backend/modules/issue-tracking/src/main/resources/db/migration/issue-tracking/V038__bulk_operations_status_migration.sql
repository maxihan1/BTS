...
--
-- ── 롤백 절차 ────────────────────────────────────────────────────────────────
-- ★순서가 먼저다. 되돌림 마이그레이션(허용값을 다시 좁히는 것)을 적용하기 전에
--   STATUS_MIGRATION 생산자 코드(WorkflowStatusMigrationAdapter 를 호출하는 발행 경로)를 먼저 내린다.
--   생산자가 살아 있는 채로 좁히면 다음 큐잉이 CHECK 위반으로 터지고, PR 7b 결선 뒤에는 그 큐잉이
--   발행 트랜잭션 안에 있으므로 워크플로우 발행 자체가 통째로 중단된다. 아래 개수를 세는 것도
--   생산자가 멈춘 뒤여야 한다 — 멈추기 전에 센 0 은 다음 순간 0 이 아니다.
