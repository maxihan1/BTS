...
-- V029 미러 (FR-BL-01): (project_id, rank) 복합 인덱스 — 백로그 정렬(WHERE project_id=? ORDER BY rank) 인덱스 스캔용.
...
-- ═══════════════════════════════════════════════════════════════════════════
-- V029: issues.rank 컬럼 + (project_id, rank) 인덱스 (FR-BL-01 백로그 LexoRank 정렬)
-- 원본: db/migration/issue-tracking/V029__issue_rank.sql
-- jOOQ: Issues.RANK(non-null String) + idx_issues_project_rank 생성 대상.
--       위 issues CREATE TABLE 인라인(rank VARCHAR(50) NOT NULL)에 미러됨 — 여기서는 추적 주석만.
--       이 미러가 빠지면 RANK 상수 미생성 → IssueRepository(rank 영속/조회) 컴파일 불가 (jooq-init-codegen-mirror).
--       백필 PL/pgSQL DO 블록은 codegen 스키마와 무관(빈 테이블) → 미러 생략, 컬럼/인덱스 DDL 만 미러.
-- ═══════════════════════════════════════════════════════════════════════════
