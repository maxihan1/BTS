// CFD status 전이 이력 배치 조회 jOOQ 리포지토리 — issue_change_group/item JOIN (FR-RP-03 Task 3)
...
/**
 * status 전이 이력을 여러 이슈에 대해 배치 조회하는 jOOQ 리포지토리 (FR-RP-03 Task 3).
 *
 * `issue_change_group`(변경 그룹) 과 `issue_change_item`(개별 필드 변경) 을 조인해
 * [FIELD_STATUS] 인 항목만 추출한다. CFD(Cumulative Flow Diagram, 누적 흐름 다이어그램) 는
 * 이슈의 상태 전이 이력만 필요하므로 assignee/summary 등 다른 필드 변경은 대상에서 제외한다.
 *
 * **jOOQ 화이트리스트 (ArchUnit 룰 2).** `com.bts.issue.jooq..` 생성 코드는 `..repository..`
 * 패키지에서만 접촉 가능하므로, 이 클래스는 `com.bts.issue.cfd.repository` 패키지에 위치한다.
 *
 ...
 */
@Repository
class CfdStatusHistoryRepository(
    private val dsl: DSLContext,
) {
    /**
     * 여러 이슈의 status 전이 이력을 한 번에 조회한다.
     *
     ...
     * @return status 전이 이력 행 목록. 이력이 없으면 빈 리스트.
     */
    @Transactional(readOnly = true)
    fun fetchStatusChanges(issueIds: Set<UUID>): List<CfdStatusChangeRow> {
