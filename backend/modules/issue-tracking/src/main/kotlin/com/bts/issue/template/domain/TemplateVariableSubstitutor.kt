 ...
 *
 * [MentionParser]와 동형으로 `object`(싱글턴)로 설계되어 빈 주입 없이 직접 호출 가능하다.
 *
 * ## 설계 원칙
...
object TemplateVariableSubstitutor {
     ...
     */
    fun substitute(content: String, bindings: Map<TemplateVariable, String>): String {
        return TOKEN_PATTERN.replace(content) { matchResult ->
            val variableName = matchResult.groupValues[1]
            val variable = TemplateVariable.entries.find { it.token == "{{$variableName}}" }
            if (variable != null) {
