 ...
 *
 * 서비스 계층이 상태 이력(DB)에서 첫 IN_PROGRESS 전이 시각과 마지막 DONE 전이 시각만 축약해
 * 전달한다. 나머지 전이 상세는 이 계산기의 관심사가 아니다.
 *
 * @property issueKey 이슈 키.
 * @property createdAt 이슈 생성 시각.
 * @property firstInProgressAt 첫 IN_PROGRESS 전이 시각. 한 번도 IN_PROGRESS를 거치지 않았으면 `null`.
 * @property lastDoneAt 마지막 DONE 전이 시각. 아직 완료되지 않았으면 `null`(= 미완료).
 */
