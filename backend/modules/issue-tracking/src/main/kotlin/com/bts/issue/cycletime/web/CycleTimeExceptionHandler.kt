// Cycle/Lead Time 컨트롤러 전용 예외 핸들러 — CycleTimeController 스코프 한정 (FR-RP-04 Task 6)
...
/**
 * Cycle/Lead Time 컨트롤러 예외 → HTTP 응답 변환 핸들러.
 *
 * `basePackages` 를 `com.bts.issue.cycletime.web` 으로 한정한다.
 *
 ...
 * (FR-WT-01 교훈, [com.bts.issue.cfd.web.CfdExceptionHandler] 선례)를 반복하지 않기 위해
 * 구체 핸들러만 선언한다. 분류되지 않은 예외는 Spring 기본 처리에 위임한다.
 *
 * ### 매핑 규칙
 * - [ResponseStatusException] → 상태 코드 보존 (401/400 등 catch-all 삼킴 차단)
 * - [IssueAccessDeniedException] → 403 Forbidden (detail 에 내부 정보 비노출)
 */
@RestControllerAdvice(basePackages = ["com.bts.issue.cycletime.web"])
class CycleTimeExceptionHandler {
    ...
    fun handleResponseStatus(ex: ResponseStatusException): ProblemDetail {
        ...
        log.info("CYCLE_TIME_{} response_status reason='{}'", status.value(), ex.reason)
        val (errorCode, detail) =
            when (status) {
                HttpStatus.UNAUTHORIZED -> "UNAUTHENTICATED" to "인증이 필요합니다. 세션이 만료되었을 수 있습니다."
                HttpStatus.BAD_REQUEST -> CYCLE_TIME_VALIDATION_FAILED to (ex.reason ?: "요청 파라미터가 올바르지 않습니다.")
    ...
    fun handleAccessDenied(ex: IssueAccessDeniedException): ProblemDetail {
        log.info("CYCLE_TIME_403 access_denied message='{}'", ex.message)
        return problem(
    ...
    companion object {
        /** Cycle/Lead Time 파라미터 검증 실패 에러 코드. */
        const val CYCLE_TIME_VALIDATION_FAILED = "ISSUE_CYCLE_TIME_VALIDATION_FAILED"
    }
}
