...
package com.bts.issue.component.adapter
...
/**
 * issue-tracking BC 컴포넌트 권한 stub resolver.
 *
 * ## 보안 모델
 ...
 *
 * prod profile 에서 [ComponentPermissionResolver] Bean 이 미해소 상태로 남으면
 * Spring 이 `BeanCreationException` 을 던져 부팅 자체를 차단한다.
 * 즉, stub 없이 운영 배포 시 반드시 실제 adapter 가 존재해야 부팅이 성공한다.
 *
 * ## prod 실판정은 FR-PM-03 이연
 * FR-PM-03 에서 prod 프로파일용 컴포넌트 권한 adapter 가 도입되면 두 resolver 는
 * profile 로 상호 배타적(mutually exclusive)으로 동작한다.
 * - `AlwaysAllowComponentPermissionResolver` — `@Profile("!prod")` (개발/테스트/스테이징)
 * - prod adapter (FR-PM-03) — `@Profile("prod")` (운영)
 *
 * ## ArchUnit 강제
 * 소비자(ComponentApplicationService/ComponentController)는 [ComponentPermissionResolver]
...
@Component
@Profile("!prod")
class AlwaysAllowComponentPermissionResolver : ComponentPermissionResolver {
    private val log = LoggerFactory.getLogger(javaClass)

    /**
     * 항상 `true` 를 반환한다.
     *
     * 개발/테스트 환경에서 권한 검사 없이 컴포넌트 CRUD 를 테스트할 수 있도록
    ...
    override fun hasPermission(
        ...
        log.warn(
            "AlwaysAllowComponentPermissionResolver: granting {} on project {} to actor {} — stub (FR-PM-03 미도입)",
            permission,
            projectId,
            actorId,
        )
