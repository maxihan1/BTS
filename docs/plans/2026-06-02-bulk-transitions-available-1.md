# FR-IS-05 후속 — 일괄 전환 가용 전환 교집합 서버 계산 엔드포인트 (Jira 방식)
...
일괄 전환 가용 전환 교집합을 서버가 한 번에 계산하는 백엔드 엔드포인트 신설(Jira 방식,
...
- 신규 엔드포인트: POST /api/v1/issues/bulk-transitions/available { issueKeys[] } → 공통 가용 전환 transitions[]
- 워크플로우 FSM 기반 교집합(가용 전환 조회는 기존 GET /issues/:key/transitions와 동일 시맨틱, 권한은 실행 시점 best-effort)
...
- **BC 확정: issue-tracking** (classify의 project-workflow는 키워드 오판). 근거 — bulk 작업은 `backend/modules/issue-tracking/src/main/kotlin/com/bts/issue/bulk/web/BulkOperationController.kt`, 가용 전환 조회는 `IssueController.availableTransitions` + `IssueApplicationService.availableTransitions(actor, issueKey)`. 둘 다 issue-tracking.
...
- **기존 결정**: 충돌 없음. "서버가 정답지" 원칙([[2026-06-02-issue-permission-query-api]]) 일관, 전환 동일성=(from,to)쌍([[2026-05-28-workflow-transition-identity-policy]]) 준수.
- **신규 ADR**: docs/adr/2026-06-02-bulk-available-transitions-server-side.md (생성됨) — 교집합 서버 이전 결정 + 대안(자체 limiter/p-limit) 기각 사유.
- **BC 격리**: 가용 전환 조회는 이미 issue-tracking이 shared-kernel AvailableTransitionView로 workflow 결과를 받는 기존 경계 사용 → 새 cross-BC 호출 0.
...
서버가 일괄 전환의 공통 가용 전환을 한 번에 계산하는 엔드포인트를 신설하고, 프론트 BulkTransitionDialog를 fan-out(최대 1000건 동시 요청) 대신 **1회 호출**로 전환한다. 교집합 계산의 단일 출처를 서버로 이전한다.
...
- Response 200: `{ "data": { "transitions": TransitionItem[], "unresolvedIssueKeys": string[] } }`
  ...
  - `transitions` = toStateKey 기준 교집합. 해석 성공한 **첫 이슈**의 항목(name/key) 보존(프론트 `intersectTransitions`와 동일 시맨틱). 한 이슈라도 가용 전환이 비면 교집합은 빈 배열.
...
- 기존 E2E `issue-bulk-operations.spec.ts` S2(전환 happy)/S5(교집합 0)는 단일 호출 기반으로 여전히 통과해야 함 → MSW 새 핸들러로 충족(회귀 0).

### 엣지 케이스

- 단일 이슈 선택 → 그 이슈 가용 전환 전체 반환(교집합=자기 자신).
...
- **회귀 경계** — 기존 issue-bulk-operations E2E S2/S5가 새 단일 엔드포인트로 동작하도록 MSW 핸들러 교체. 기존 per-issue transitions 핸들러(상세 화면 전환용)는 유지.
...
### Task 2. 일괄 가용 전환 서비스 (best-effort per-key + 교집합)
...
**내용**: 기존 `e2e/issue-bulk-operations.spec.ts` S2(전환 happy)/S5(교집합 0)가 새 단일 엔드포인트 + MSW 핸들러로 통과하는지 확인. 전체 단위(`pnpm --filter @bts/web test`) + 전체 E2E(`test:e2e`) + typecheck/lint. 백엔드 `:modules:issue-tracking:test` + ktlint/detekt. 회귀 0 확인. 5173 orphan 정리(메모리 e2e-orphan-vite-after-worktree-remove).
**검증**: 위 전부 그린.
