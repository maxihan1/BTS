...
1. **젠킨스 검증 stage 철거를 이번 범위에 넣는가.** 안 넣으면 영구 이중 실행이고
   운영 VM 의 `mem_limit 10g` · `cpus 2.0` 회수가 안 된다.
2. 저장소 공개 시점 — PR ② 실측이 그 뒤라야 의미가 있다 (private + 결제 차단이면 steps=0).
