...
2. 스크럼 보드에서 시간 추적 방식을 고르면 번다운의 **세로축**이 그것을 따른다 —
   `잔여 추정 + 소요 시간` 이면 시간(초), `없음` 이면 **이슈 개수**다. 칸반에서는 잠긴다(갭 C).
   ★이 줄은 task-35 **전까지 red 였다**(저장만 되고 차트에 닿지 않았다 — 리뷰 머지 BLOCKER).
   기본값 `NONE` 이 개수 축이라 배포 시점에 기존 보드의 차트가 바뀐다(스펙 S2 가 그 사실을 진다).
