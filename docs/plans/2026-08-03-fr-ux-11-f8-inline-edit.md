...
### 게이트 2 (2026-08-03) — Maxi 확정 「M-5 를 고치고 머지」

**controller 의 직전 확정을 뒤집은 결정이다.** 나는 *"`취소` 버튼 비대칭은 수용, 손대지 마라"* 로
확정했었다. 담당 에이전트가 그 지시와 리뷰 지적의 **충돌을 정확히 짚고 이전 확정을 유지**한 것은
옳은 처신이었고, 상위 결정이 바뀌어 뒤집혔다.

**★ 내 기각 근거가 틀렸던 지점.** 나는 *"고치면 `issue-body-meta` 가 깨진다"* 고 판단했는데,
그것은 **위쪽 `취소` 에 확인 절차를 붙이는 해법을 전제**했기 때문이다. 그 스펙은 저장 후
`취소` 로 읽기 모드에 나가므로 확인이 붙으면 재조회 타이밍에 따라 패널이 떠 깨진다.
**해법을 바꾸면 그 문제가 없다** — 패널이 뜬 동안만 상단 버튼을 **잠그면** 된다.
그 스펙은 **패널이 안 뜬 상태**에서 `취소` 를 누르므로 영향 0 이고, 확인 중에 뒤의 결정 버튼이
잠기는 것은 다이얼로그의 자연스러운 관례다. **「고칠 수 없다」가 아니라 「내가 본 해법이 하나뿐이었다」였다.**

**봉합 `fix: d946e31a1` · `test: 2d78e7ea4`.** `actionsLocked = isSaving || showDiscardConfirm`
으로 두 사유를 모으되 주석으로 분리했다. `disabled` 로만 잠그고 **DOM 에서 제거하지 않았다** —
제거하면 `getByRole('button', {name:'저장'})` 이 요소 부재로 깨질 수 있고 레이아웃도 튄다.
`title` 은 붙이지 않았다(프리미티브가 `disabled:pointer-events-none` 이라 툴팁이 뜨지 않는다).

**뮤테이션 2종.** `showDiscardConfirm` 제거 → 정확히 1건 빨강 · 패널 닫기 경로 파손 → 4건 빨강
(**잠금 해제 증인이 그중에 포함** — 「잠그고 안 풀리는」 회귀를 잡는다).
**E2E 실제 14건 실행**(13 통과 / 1 선재 skip) — 필터 무력화 없음을 건수로 확인. 최대 위험면
`issue-body-meta` E1~E5 전부 통과로 **예상이 실측으로 확인**됐다. 잠금이 본문 편집기 두 버튼에만
걸리고 같은 `aria-label='저장'` 을 쓰는 환경·라벨 버튼 2개는 영향 0.

### ★ 새 발견 — 비활성 버튼이 시각적으로 잠긴 티가 안 난다 (선재·앱 전역, 범위 밖)

```
disabled: true · pointerEvents: "none" · hasOpacityClass: true · opacity: "1"
```

`button.tsx:9` 의 `disabled:opacity-50` 이 **클래스 문자열에 분명히 있는데 계산값이 1** 이다.
같은 문자열의 `disabled:pointer-events-none` 은 정상 적용된다(계산값 `none`). controller 가
재확인 — 클래스 실재하고 `index.css` 에 opacity 재정의 **없음**.

이 클래스는 `components/ui/*.tsx` **14개 프리미티브가 공유**한다 — 즉 **앱의 모든 비활성 버튼이
같은 상태**이고 이번 변경이 만든 것이 아니다. **기능적 잠금은 완전**하고(클릭 `blocked`, 상태
불변) 바로 아래 빨간 확인 패널이 맥락을 설명하므로 이 PR 의 M-5 봉합 목적은 달성됐다.

**고치지 않았다** — `components/ui/button.tsx` 또는 전역 CSS 를 손대야 하는데 범위 밖이고,
이 PR 파일에만 인라인 투명도를 넣으면 **앱의 다른 비활성 버튼과 어긋나는 디자인 시스템 우회**가
된다. **프리미티브 차원의 별도 task 로 제안한다**(고치면 모든 비활성 버튼이 함께 낫는다).
