...
⚠️ **커밋 `08576578f` 제목의 `(red)` 는 사실과 다르다 (게이트 2 리뷰 정정).** 그 커밋이 얹은
판정(배포 게이트 **동일성**·**순서**·**비가드**)의 대상인 배포 스크럽 줄은 이미
`7ab3ffab4`(Task 4 green)에 들어와 있었다. `08576578f` 이 손댄 것은
`discriminant-hook-wiring.test.ts` 와 `hook-source.ts` 뿐이고 **대상 셸 파일은 안 건드렸다.**
즉 그 시점에 **RED 는 나지 않았고**, plan Task 8 도 「★RED 가 안 난다」로 이미 그렇게 명시했다.
이 저장소는 `test:` → `feat:` 커밋 순서를 기계 대조 증거로 쓰므로, 사실과 다른 `(red)` 는 그
증거를 흐린다. 그 판정의 비-공허는 red-first 가 아니라 **사후 뮤테이션**
(`MC1` · `MC2a` · `MC2b` · `MC3` · `MC4` · `MC4b`)으로 세워졌다 — 아래 표의 그 행들이다.
이력은 이미 푸시돼 **고치지 않는다.** 고치는 대신 여기 적어 다음 사람이 커밋 제목을 증거로
잘못 인용하지 않게 한다.

가드를 고치는 작업이라 **표면을 없애면 판별자도 함께 사라진다.** 그래서 각 판정마다 대상을 일부러
끊어 red 를 한 번씩 봤다. 아래 판정 이름은 전부 실행 로그의 `not ok` 줄에서 뽑은 것이고, 실패
메시지도 로그 원문이다. 이 표가 증명하는 것은 「판정이 있다」가 아니라 **그 판정이 무엇을 볼 수
있고 무엇을 못 보는가**다. 특히 두 자리가 층위의 존재 이유를 값으로 보여준다.
**① `MB1`·`M6`** — 헬퍼를 이미 임포트한 파일에서 호출 하나만 `env:` 를 잃으면 **파일 단위 양방향
대조는 그대로 초록**이고 red 를 내는 것은 호출부 판정뿐이다. 파일 집합은 「헬퍼를 아예 안 쓰는
파일」만 보고 「쓰는데 빠뜨린 호출」은 못 본다. 서로 다른 두 파일(`git-fixture-isolation.test.ts` ·
`tier-floor.test.ts`)에서 같은 성질이 재현됐으므로 특정 파일의 사정이 아니라 층위의 성질이다.
**② `MC2a`** — 배포 게이트의 스크럽 줄만 `unset GIT_DIR` 로 좁히면 **훅 쪽 존재·비가드·실효 판정이
전부 초록**이다. 훅 판정은 훅 파일만 읽기 때문이다. 배포 쪽 사본이 훅 판정에 무임승차하려면 두 줄이
문자열로 같아야 하고 그 동일성을 기계가 재야 한다 — 동일성 판정이 그 자리에 있는 이유다.
`M7` 이 반대 방향에서 같은 것을 보여준다. 훅 줄을 좁히면 **실효 실측과 동일성**이 red 가 되는데
존재·순서·비가드는 초록이다. 배선이 문법으로는 남아 있기 때문이고, 그것이 R8 을 따로 세운 이유다.

**기준선.** `node --experimental-strip-types --test 'scripts/**/*.test.ts' 'scripts/**/*.test.mjs'`
→ `tests 374 · pass 374 · fail 0 · skipped 0 · EXIT=0`.
**절차.** 뮤테이션은 전부 **GREEN 선커밋 뒤**에 돌렸다. 하나마다 「끊기 → 전량 실행 → 로그에서 red
이름 뽑기 → `git checkout -- <경로>` 원복 → `git status --porcelain` 확인」 한 바퀴다. 미커밋 상태로
끊었다 되돌리면 그 되돌림이 소실이라서다.

| # | 끊은 것 | red 가 된 판정 이름 전량 | 실패 메시지 첫 줄 | 출처 | 원복 확인 |
|---|---|---|---|---|---|
| **M1** | `.husky/pre-push:22` 의 스크럽 줄 **삭제** (전량 `tests 374 · pass 370 · fail 4 · EXIT=1`) | `★판별식을 부르기 전에 GIT_* 를 지운다` · `★그 스크럽이 조건에 안 매달린다 (깊이 0 · 가드 연산자 없음)`(훅) · `★★훅에서 읽어낸 그 줄을 실행하면 GIT_* 가 0개 남는다 (실효 실측)` · `★★배포 게이트의 스크럽 줄이 훅의 그 줄과 문자열로 같다` | `.husky/pre-push 에 GIT_* 스크럽이 없다.` / `.husky/pre-push 의 GIT_* 스크럽이 셸 블록 깊이 null 에 있다.` / `.husky/pre-push 에 GIT_* 스크럽이 없어 실효를 잴 대상이 없다.` / `.husky/pre-push 에서 GIT_* 스크럽을 못 찾았다 — 비교할 원본이 없다.` | Task 5 직접 관측 | `git status --porcelain` 비었음 |
| **M2** | `.husky/pre-push` 의 스크럽 줄을 판별식 호출 **뒤로 이동** (전량 `tests 374 · pass 373 · fail 1 · EXIT=1`) | `★판별식을 부르기 전에 GIT_* 를 지운다` | `.husky/pre-push 의 GIT_* 스크럽이 판별식 호출보다 뒤에 있다.` | Task 5 직접 관측 | `git status --porcelain` 비었음 |
| **M4** | `git-fixture-env` 의 삭제 로직을 no-op 으로 | `GIT_DIR 가 걸려 있어도 픽스처가 진짜 저장소를 안 바꾼다` (Task 2 보고 표기. 파일의 현재 이름은 `GIT_DIR 가 걸려 있어도 헬퍼로 만든 픽스처가 victim 저장소를 안 바꾼다`) | Task 2 보고에 원문 미수록 — 달라진 축만 인용한다. `["commitCount 1 → 2","head 46713c1… → f584a35…"]` | Task 2 관측 | Task 2 가 확인 |
| **M5** | `scripts/workflow/transition-term-guard.test.ts` 에서 헬퍼 임포트와 그 파일의 `env: gitFixtureEnv()` 를 함께 제거 (전량 `tests 374 · pass 372 · fail 2 · EXIT=1`) | `★★git 을 spawn 하는 파일 집합과 헬퍼 임포트 집합이 양방향으로 같다` · `★★옵션 객체에 env 키가 없는 호출부가 하나도 없다` | `git 을 spawn 하는 파일과 스크럽 헬퍼를 거치는 파일이 어긋난다.` / `git 을 부르면서 env 를 스스로 정하지 않은 호출부가 있다 — 훅 안에서 GIT_DIR 를 상속해` | Task 5 직접 관측 | `git status --porcelain` 비었음 |
| **M6** | `scripts/workflow/tier-floor.test.ts:161` 의 `git log` 호출에서 **`env:` 만** 제거. 임포트와 나머지 호출의 `env:` 는 그대로 (전량 `tests 374 · pass 373 · fail 1 · EXIT=1`) | `★★옵션 객체에 env 키가 없는 호출부가 하나도 없다` | `git 을 부르면서 env 를 스스로 정하지 않은 호출부가 있다 — 훅 안에서 GIT_DIR 를 상속해` (자리 전수 `scripts/workflow/tier-floor.test.ts:161 spawnSync`) | Task 5 직접 관측 | `git status --porcelain` 비었음 |
| **M7** | `.husky/pre-push:22` 의 스크럽을 `unset GIT_DIR` 하나만 지우도록 **좁힘** (전량 `tests 374 · pass 372 · fail 2 · EXIT=1`) | `★★훅에서 읽어낸 그 줄을 실행하면 GIT_* 가 0개 남는다 (실효 실측)` · `★★배포 게이트의 스크럽 줄이 훅의 그 줄과 문자열로 같다` | `.husky/pre-push 의 스크럽 줄이 **문법적으로는 있는데 실제로는 GIT_* 를 안 지운다.**` (잔존 `4 !== 0`) / `infra/deploy/bts-deploy.sh 의 스크럽 줄이 .husky/pre-push 의 그 줄과 다르다.` | Task 5 직접 관측 | `git status --porcelain` 비었음 |
| **M8** | 파일 단위 파생 집합 정규식 무력화 | `★★파생 집합이 비어 있지 않고 알려진 픽스처 생성자를 실제로 문다 (비-공허 짝)` · 이어서 `★★git 을 spawn 하는 파일 집합과 헬퍼 임포트 집합이 양방향으로 같다` | Task 3 보고에 원문 미수록 | Task 3 관측 | Task 3 이 확인 |
| **MB1** | 배선된 호출부 하나에서 `env:` 삭제 (`git-fixture-isolation.test.ts`) | `★★옵션 객체에 env 키가 없는 호출부가 하나도 없다` | Task 7 보고에 원문 미수록 | Task 7 관측 | Task 7 이 확인 |
| **MB2** | 호출부 파생 집합 정규식 무력화 | `★★파생 집합이 비어 있지 않고 알려진 픽스처 생성자를 실제로 문다 (비-공허 짝)` · `★★호출부 파생 집합이 비어 있지 않고 여러 줄로 쓴 호출까지 문다 (비-공허 짝)` · `★★git 을 spawn 하는 파일 집합과 헬퍼 임포트 집합이 양방향으로 같다` | Task 7 보고에 원문 미수록 | Task 7 관측 | Task 7 이 확인 |
| **MB2b** | 호출부 스캐너를 줄 단위로 좁힘 | `★★호출부 파생 집합이 비어 있지 않고 여러 줄로 쓴 호출까지 문다 (비-공허 짝)` | Task 7 보고에 원문 미수록 | Task 7 관측 | Task 7 이 확인 |
| **MC1** | `infra/deploy/bts-deploy.sh` 의 스크럽 줄 삭제 | `★★배포 게이트의 스크럽 줄이 훅의 그 줄과 문자열로 같다` · `★배포 스크럽이 전량 검증 게이트보다 앞에 있다` · `★그 스크럽이 조건에 안 매달린다 (깊이 0 · 가드 연산자 없음)`(배포) | Task 8 보고에 원문 미수록 | Task 8 관측 | Task 8 이 확인 |
| **MC2a** | 배포 줄**만** `unset GIT_DIR` 로 좁힘 | `★★배포 게이트의 스크럽 줄이 훅의 그 줄과 문자열로 같다` | Task 8 보고에 원문 미수록 | Task 8 관측 | Task 8 이 확인 |
| **MC2b** | 배포 줄**만** `sed` 를 `awk` 로 바꿈 | `★★배포 게이트의 스크럽 줄이 훅의 그 줄과 문자열로 같다` | Task 8 보고에 원문 미수록 | Task 8 관측 | Task 8 이 확인 |
| **MC3** | 배포 스크럽을 전량 검증 게이트 뒤로 이동 | `★배포 스크럽이 전량 검증 게이트보다 앞에 있다` | Task 8 보고에 원문 미수록 | Task 8 관측 | Task 8 이 확인 |
| **MC4** | 배포 줄을 `&&` 로 앞 명령에 매닮 | `★★배포 게이트의 스크럽 줄이 훅의 그 줄과 문자열로 같다` · `★그 스크럽이 조건에 안 매달린다 (깊이 0 · 가드 연산자 없음)`(배포) | Task 8 보고에 원문 미수록 | Task 8 관측 | Task 8 이 확인 |
| **MC4b** | **두 파일을 같은 형태로** `&&` 로 매닮 | `★그 스크럽이 조건에 안 매달린다 (깊이 0 · 가드 연산자 없음)`(훅) · `★그 스크럽이 조건에 안 매달린다 (깊이 0 · 가드 연산자 없음)`(배포) | Task 8 보고에 원문 미수록 | Task 8 관측 | Task 8 이 확인 |

**M3 은 따로 안 돌렸다.** 계획표의 M3(훅 스크럽을 `&&` 로 앞 명령에 매닮)은 `MC4b` 가 **두 파일을
같은 형태로** 매달면서 이미 훅 쪽 비가드 판정을 red 로 만들었다. 같은 자리를 다시 끊는 것이라
재관측을 생략했고, 생략했다는 사실을 여기 적어 둔다.

### 초록으로 남은 자리 — 층위가 갈리는 지점

red 만 적으면 판별식이 실제보다 넓어 보인다. **안 죽은 것**이 각 판정의 사정거리를 말해 준다.

| 뮤테이션 | 초록으로 남은 판정 | 그래서 무엇이 필요했나 |
|---|---|---|
| `M6` · `MB1` | `★★git 을 spawn 하는 파일 집합과 헬퍼 임포트 집합이 양방향으로 같다` | 파일 단위 대조는 「헬퍼를 안 쓰는 파일」만 본다. 「쓰는데 빠뜨린 호출」을 보는 것은 호출부 판정 하나다 (Task 7 이 생긴 이유) |
| `M7` | `★판별식을 부르기 전에 GIT_* 를 지운다` · `★그 스크럽이 조건에 안 매달린다 (깊이 0 · 가드 연산자 없음)`(훅) | 스크럽 줄이 문법으로 남아 있으면 배선 판정은 만족한다. 좁아진 네임스페이스를 보는 것은 실효 실측(R8)뿐이다 |
| `M1` | `★배포 스크럽이 전량 검증 게이트보다 앞에 있다` · `★그 스크럽이 조건에 안 매달린다 (깊이 0 · 가드 연산자 없음)`(배포) | 배포 쪽 자리 판정은 배포 파일만 읽으므로 훅을 지워도 안 죽는다. 두 파일을 잇는 것은 동일성 판정이다 |
| `MC2a` · `MC2b` | `★판별식을 부르기 전에 GIT_* 를 지운다` · `★그 스크럽이 조건에 안 매달린다 (깊이 0 · 가드 연산자 없음)`(훅) · `★★훅에서 읽어낸 그 줄을 실행하면 GIT_* 가 0개 남는다 (실효 실측)` | 훅 판정은 훅 파일만 읽는다. 배포 줄이 훅 판정에 무임승차하는 근거는 오직 문자열 동일성이다 (Task 8 이 생긴 이유) |
| `MC4b` | `★★배포 게이트의 스크럽 줄이 훅의 그 줄과 문자열로 같다` | 두 줄을 **같은 형태로** 함께 망가뜨리면 동일성은 만족된다. 그 조합을 잡는 것은 배포 쪽 비가드 판정이므로 동일성만으로 자리를 대신할 수 없다 |

**원복 확인.** 뮤테이션마다 `git checkout -- <경로>` 직후 `git status --porcelain` 이 비었음을
확인했고, 마지막 원복 뒤 전량을 다시 돌려 `tests 374 · pass 374 · fail 0 · skipped 0 · EXIT=0`
으로 기준선에 복귀했다.

### ★판정 이름·메시지 드리프트 (게이트 2 라운드 1 이후)

**위 두 표의 인용은 게이트 2 수정 전 로그 원문이다.** 라운드 1 수정으로 판정 이름과 실패 메시지가
바뀌었으므로, 지금 전량을 돌리면 아래 이름이 나온다. **옛 인용을 새 이름으로 덮어쓰지 않는다** —
관측은 그 시점의 이름으로 이뤄졌고, 덮어쓰면 「그 이름으로 red 를 봤다」는 거짓 기록이 된다.
그래서 대응만 여기 남긴다.

| 표의 옛 이름 (수정 전 관측) | 현재 이름 (수정 후) |
|---|---|
| `★그 스크럽이 조건에 안 매달린다 (깊이 0 · 가드 연산자 없음)`(훅) | `★푸시 훅의 스크럽이 조건에 안 매달린다` |
| `★그 스크럽이 조건에 안 매달린다 (깊이 0 · 가드 연산자 없음)`(배포) | `★배포 게이트의 스크럽이 조건에 안 매달린다` |
| `★★훅에서 읽어낸 그 줄을 실행하면 GIT_* 가 0개 남는다 (실효 실측)` | `★★훅에서 읽어낸 그 줄을 실행하면 GIT_* 가 하나도 안 남는다 (실효 실측)` |

앞의 두 줄은 **한 이름이 훅·배포 두 자리에 같이 쓰이던 것**이 자리별 이름으로 갈라진 결과다.
표에서 `(훅)`·`(배포)` 를 괄호로 덧붙여 구분하던 것이 이제 이름 자체에 들어 있다.

**실패 메시지도 바뀌었다.**

| 뮤테이션 | 표에 적힌 메시지 (수정 전) | 현재 메시지 (수정 후) |
|---|---|---|
| `M1` | `… 의 GIT_* 스크럽이 셸 블록 깊이 null 에 있다.` | `… 에 GIT_* 스크럽이 아예 없다 — 어떤 깊이에도 있지 않다.` |
| `M7` | 잔존 `4 !== 0` | `남은 것: GIT_EDITOR · GIT_PREFIX · GIT_SSH_COMMAND · GIT_EXEC_PATH` |

둘 다 **부재·잔존을 값으로 말하게** 바꾼 결과다. `M1` 쪽은 「없는 것을 깊이로 말하지 않는다」가
판정으로 따로 섰고(`부재를 「어떤 깊이에 있다」로 말하지 않는다`), `M7` 쪽은 개수가 아니라 **이름 집합**을
말한다(`실효 실측이 남은 변수를 이름으로 말한다 (개수가 아니라 집합)`).

**`M1` 은 red 판정이 하나 늘었다** — `★손으로 적은 스크럽 사본이 훅·배포의 그 줄과 문자로 같다`.
게이트 2 라운드 1 이 그 판정을 새로 얹으면서 훅 파일의 그 줄을 읽는 자리가 하나 더 생겼기 때문이다.
표의 `M1` 행은 **그 판정이 있기 전의 관측**이라 그 이름이 없다.
...
### ★게이트 2 라운드 1 이후의 실행 시간

전량이 **6.74s → 8.32s** 가 됐다. 증가분은 전부 「원본 픽스처 생성자를 자식 프로세스로 **실제로
실행해** 값으로 판정하는」 자리다 — `select` 1.32s · `todos` 0.78s · 합성 자식 ≈0.3s ·
victim 저장소 추가 ≈0.45s.

**결정 — 병렬화하지 않는다.** 근거 둘이다.
① **8초는 사람이 `--no-verify` 를 쓰기 시작하는 선이 아니다.** 훅이 성가셔지는 지점을 넘지 않는 한,
   시간을 깎는 것보다 판정을 그대로 두는 쪽이 이득이다.
② **자식 둘을 병렬로 돌리면 0.8s 를 아끼는 대신 victim 저장소를 공유하게 된다.** 그 순간 가드 자신에
   flaky 가 들어오고, 간헐적으로 빨개지는 가드는 곧 「무시하고 넘기는 가드」가 된다 — 이 PR 이
   막으려는 상태가 정확히 그것이다. 0.8s 와 맞바꿀 것이 아니다.
...
### 게이트 2 라운드 1 (2026-08-21) — 렌즈 전종 · A안 채택

**렌즈.** `code-reviewer` · `security` · `maintainability` · `adversarial` 이 돌았고 전부 **실측**으로
지적을 냈다. 판정은 **BLOCKER `A1`~`A5`** 와 **등재 의무 `A6`** 다.

**Maxi 판정 — A안(전부 수정 후 재리뷰).** 일부만 닫고 나머지를 부채로 미루는 쪽을 안 골랐다.
`A1`~`A5` 는 Task 10·11 이 닫았고, `A6` 는 이 커밋이 닫는다.

**★번호↔내용 대응은 저장소에 남아 있지 않다.** 라운드 1 리뷰 세션에만 있다. 그래서 아래는 번호가 아니라
**라운드 1 이후 쌓인 red→green 커밋 짝 전수**로 적는다 — 다음 라운드가 대조할 대상이 이 목록이다.
어느 자리가 BLOCKER 등급이고 어느 자리가 주의였는지는 기록이 없으므로 **여기서 단정하지 않는다.**

| 닫은 자리 | 어느 판정이 닫았나 | 커밋 (red → green) |
|---|---|---|
| 비-공허 짝이 자기 환경의 `GIT_*` 를 함께 상속해 제3 저장소를 물들일 수 있었다 | `★★비-공허 짝이 주변 GIT_* 를 함께 상속하지 않는다 — 제3 저장소 무손상` | `3cda525c5` → `be032272e` |
| 주석 걷기가 정규식 리터럴 뒤에서 부패해 훑기가 어긋났다 | `★★정규식 리터럴 뒤의 주석이 지워지고 그 뒤 코드는 남는다 (비-공허 짝)` · `★★scripts 전량에서 살아남은 줄 주석이 하나도 없다` | `5ee102384` → `3e7b3412f` |
| 무조건성 판정이 자기 완결형 한 줄 조건을 못 보고 부재를 「깊이 null」로 말했다 | `★자기 완결형 한 줄에 담긴 조건을 조건부로 읽는다` · `부재를 「어떤 깊이에 있다」로 말하지 않는다` | `538463706` → `b2aae1366` |
| 호출부 판정이 `env` **키**만 보고 값을 안 봐서 빈 객체도 통과했다 | `★★키만이 아니라 값을 본다 — 통과해야 하는 형태도 함께 잰다 (비-공허 짝)` | `07eeeb716` → `750e35957` |
| 실효 실측이 잔존을 개수로 뭉개고, 빈 출력을 「안 남았다」로 읽고, `GIT_DIR` 없는 환경을 안 쟀다 | `실효 실측이 남은 변수를 이름으로 말한다 (개수가 아니라 집합)` · `실효 실측이 열거 지점 도달을 확인한다 (빈 출력을 「안 남았다」로 읽지 않는다)` · `★한 줄 조건부 스크럽을 실효 실측이 잡아낸다` | `dbd1c67b7` → `093033212` |
| 픽스처 생성자를 **원본 그대로 자식으로 돌려** 오염을 값으로 재지 않았다 | `★★원본을 그 파일만 단독 실행해도 victim 이 그대로다` · `★★같은 하네스에 스크럽 없는 픽스처를 물리면 victim 이 실제로 바뀐다 (비-공허 짝)` · `★자식으로 못 도는 픽스처 생성자는 자식으로 도는 파일이 임포트한다` | `ee75a9cb5` → `eda2ca87e` · `2dfc3aafe` |

**판정을 새로 세우지 않고 닫은 자리도 적는다.** 구조·문구 수정이라 red→green 짝이 없다 —
`3273a6cf4`(훅·배포 무조건성 판정을 한 헬퍼로 모음) · `a738ac6bb`(훑는 확장자를 러너 글롭에서 뽑음) ·
`0e3750752`(`hook-source` 헤더의 「한 벌만 둔다」를 사실로 정정 — 부채 `82` 가 그 자리다) ·
`6cda786fa`(무조건성 판정을 축별 함수로 분리) · `ccba93df7`(러너 명령을 한 자리에서 읽음).

**`A6` — 등재 의무.** 「고치지 않고 남긴 것을 전부 장부에 올려라」다. `TODOS.md` 와 마스터
§전수 매핑에 `81`~`87` 로 등재했다(`docs/plans/2026-08-12-debt24-master.md`). 그 등재가 실재하는지는
`debt-ledger-mapping.test.ts` 가 양방향 집합 대조로 강제한다 — 한쪽에만 적으면 즉시 red 다.
번호가 `60` 이 아니라 `81` 에서 시작하는 이유는 마스터 §전수 매핑의 그 각주에 적었다.


### 게이트 2 라운드 2 (2026-08-21) — BLOCKER 3건 · Maxi 판정 ①(라운드 3 수정)

**렌즈.** `code-reviewer` 한 종만 돌았다. BLOCKER 확정으로 나머지 렌즈를 보류한 것이지 생략이 아니다 —
라운드 3 에서 함께 돈다.

**지적이 5 → 3 으로 수렴했다.** 세 건 다 라운드 2 가 **실측으로 재현**했고, 셋 중 하나는
라운드 1 대비 **회귀**다. 이 세션의 지배 패턴이 그대로 한 번 더 나왔다 — 「고치는 과정이 새 결함을 만든다」.

| 닫은 자리 | 어느 판정이 닫았나 | 커밋 (red → green) |
|---|---|---|
| `stripComments` 가 후위 증감 뒤의 `/` 를 정규식 시작으로 오판해 같은 줄 주석을 통째로 살렸다 (라운드 1 대비 회귀) | `★★정규식 리터럴 뒤의 주석이 지워지고 그 뒤 코드는 남는다 (비-공허 짝)` 에 후위 증가·감소 트랩 2종 추가 · `★★scripts 전량에서 살아남은 줄 주석이 하나도 없다` 를 줄머리 제약에서 **파생 술어 만족** 판정으로 교체 | `bf2fdaeb2` → `a56bade98` |
| 자식이 **실제로 돌았다는 보증이 없어** `test.skip` 과 테스트 0개 파일이 「돌았다」로 세어졌다 | `★★아무것도 안 돈 자식을 「돌았다」로 세지 않는다 (비-공허 짝)` | `ac66868d9` → `0d7485163` |
| `DIRECTLY_RUNNABLE_CREATORS` 가 이름 둘짜리 사람 목록이라 `env` 우회 형태에 실측 백스톱이 없었다 | `★★git 을 부르는 전량이 자식으로 돌거나 자식이 임포트한다` · `★★자식 대상 파생이 비어 있지 않고 저 자신을 실제로 빼고 있다 (비-공허 짝)` | `20bb53ece` → `9336586fd` |

**★세 번째 자리의 처방을 「형태를 더 무는 정규식」으로 안 잡았다.** 우회 형태를 정규식에 얹으면 그 목록이
**사람이 유지하는 세 번째 목록**이 되고, 목록에 없는 형태는 조용히 통과한다. 대신 자식 대상을 파생으로
바꿔 **실측 백스톱**을 세웠다 — 우회한 파일은 `GIT_DIR` 아래 자식으로 돌 때 victim 을 실제로 바꾼다.
자식이 2 → 6 으로 늘었다. **「사각지대 2건이 함께 닫혔다」고 적었는데 절반만 참이었다** —
`push-backend-tests.ts` 는 닫혔지만 `changed-paths.ts` 는 자식 대상에 **들어오기만** 했다.
그 파일은 git 을 읽기 전용으로만 부르므로 관측 4축이 안 움직여 오염이 안 보인다(라운드 3 실측).
남은 정적 사각은 부채 `88`, 읽기 전용 사각은 부채 `89` 로 나눠 등재했다.

**저 자신은 이름이 아니라 `import.meta.url` 로 뺀다.** 자식이 이 파일을 다시 돌리면 끝나지 않기 때문이다.
그 뺄셈이 no-op 이 아님을 값으로 확인하는 판정을 함께 세웠다 — 자기 식별이 어긋나면 보이는 것은
「판정이 틀렸다」가 아니라 「하네스가 멈췄다」라 원인에서 멀다.

**CONCERNS 도 닫았다.** `RepoSnapshot` 주석의 「오염되면 이 중 하나 이상이 반드시 변한다」는 거짓이었다
(`git config user.email` · `remote add` 는 네 축이 전부 불변이고, 이 하네스 자신이 `user.email` 을 부른다).
범위를 사실대로 좁혀 적었다(`3458d8d39`). `stepInsideCode` 37줄은 리터럴 열기·주석 걷기를 갈라 규칙 안에 넣었다.

**부채 `87` 의 거짓 서술을 고쳤다.** 「`scripts/` 전량에서 살아남은 줄 주석 0」이라 적었는데 그때 판정은
줄머리만 봤다 — **막아 둔 범위를 실제보다 넓게 적은 것**이다. 지금 범위대로 다시 적었다.

**뮤테이션 4종으로 새 판정의 비-공허를 확인했다** (GREEN 선커밋 뒤).
후위 연산자 판정 제거 · 자식 계수 판정 제거 · 파생을 spawner 만으로 좁힘 · 저 자신 빼기를 no-op 으로 —
전부 red 였고 복원 후 전량 초록이다.


### 게이트 2 라운드 3 (2026-08-21) — 렌즈 2종 · 지적 9건 · 수렴 안 함

**렌즈.** `code-reviewer` 와 **적대적 렌즈**를 병렬로 돌렸다. 라운드 2 가 한 종만 돈 것을 여기서 갚았다.
두 렌즈 모두 워크트리를 한 글자도 안 고쳤다(실험은 전부 복사본).

**★수렴하지 않았다.** 5 → 3 → **9**(BLOCKER 8 · CONCERN 1)다. 늘어난 까닭은 새 결함이 생겨서가 아니라
**적대적 렌즈가 처음 들어와 정적 층위를 실제로 뚫었기** 때문이다. 라운드 2 까지는 「판정이 옳게 적혔나」를
봤고, 라운드 3 은 「판정을 우회할 수 있나」를 봤다.

| 지적 | 처리 | 커밋 (red → green) |
|---|---|---|
| 예약어 뒤의 `/` 를 나눗셈으로 오판 — 주석 생존(green 쪽) · 코드 소실(대칭 실명) | 낱말 전체를 보고 예약어 14종을 가린다. 원소마다 제 형태를 세워 하나가 빠지면 red | `e2209209c` → `756cce5c7` |
| 도달성을 **글자**로 재서 문자열 언급 한 줄과 디렉터리 다른 동명 파일이 간선을 만든다 | `from`·`import()`·`require()` 만 물고 지정자를 importer 기준으로 푼다 | `db66ec245` → `f0f705485` |
| 살아남은 **블록** 주석을 전량 판정이 안 본다 | 주석 여는 자리 둘을 함께 본다. 오탐 0 을 실측하고 넣었다 | `f0f705485` 에 포함 |
| 자식에 건너뛴 판정 하나가 생기면 이 파일이 거짓 red — 고칠 자리가 오해된다 | 「전량 통과」에서 「선 판정이 있는가」로 좁히고 메시지를 자식 쪽으로 돌렸다 | `f7bf727ac` |
| 부채 88 의 「victim 을 바꾸므로 red」가 8종 중 6종에만 참 | 잡히는 경로 둘을 실측대로 적고, 안 잡히는 두 파일을 이름으로 남겼다 | 이 커밋 |
| plan 의 「사각지대 2건이 함께 닫혔다」가 절반만 참 | 실측대로 정정 | 이 커밋 |
| 부채 87 의 「오판을 부르는 소스는 없다」가 문자 그대로 거짓 | 「오판은 3자리, 출력 차분 0」으로 정정 | 이 커밋 |

**코드로 안 막고 등재한 것.**

- **부채 `89`** — 자식 백스톱이 **읽기 전용** git 호출을 못 본다. `changed-paths.ts` 가 스크럽을 잃으면
  **남의 저장소의 diff 를 읽는데** 관측 4축이 안 움직여 전량 초록이다. 훅의 1차 입력이라 가볍지 않다.
- **부채 `90`** — 임포트 도달성 ≠ 실행 도달성. 자식이 그 함수를 안 부르면 오염이 안 관측된다.
- **부채 `84`(기등재)** — 프로그램 이름을 변수로 올리는 등 어휘 우회 6종. 적대적 렌즈가 BLOCKER 로 냈으나
  **이 PR 이 만든 것이 아니고 이미 등재돼 있으며, 그 항목이 「어휘를 넓히는 것은 처방이 아니다」를 이미 적는다.**
  등급을 낮춰 잡은 근거를 여기 남긴다.

**★사정거리를 값으로 못 박았다 (파생 8종 × 스크럽 제거 × 전량 실행).**
6종은 「자식 자신이 `GIT_DIR` 오염으로 판정을 못 세움」으로 잡히고, `changed-paths.ts` ·
`snapshot-baseline-guard.test.ts` 는 **안 잡힌다.** 이 표가 부채 88·89 의 근거다.
