...
**GREEN**.
- `automation-rule-fixtures.ts` 의 기존 `SCENARIO_KEY`(:29-34)에 3키 추가 — `IMPORT_FAILED_INDEX: 'msw:automation-rule:import-failed-index'` · `IMPORT_TOO_LARGE: 'msw:automation-rule:import-too-large'` · `IMPORT_WEBHOOK_TOKENS: 'msw:automation-rule:import-webhook-tokens'`. 네이밍은 기존 `msw:<bc>-<entity>:<scenario>` 관례.
...
const labels = {
  ...
  confirmWarning: '적용하면 기존 룰이 덮어쓰일 수 있습니다. 계속할까요?',
...
- 에러 → `extractAutomationRuleErrorCode` + `extractAutomationImportFailedIndex`. `failedIndex !== null` 이면 `${failedIndex + 1}번째 룰에서 실패했습니다.` 접두 + 서버 `detail`. **항상 `rollbackNote` 병기**.
...
**검증**. `node_modules/.bin/vitest run src/components/automation/AutomationYamlImportDialog.test.tsx` → 신규 11 PASS.

> **리스크**. jsdom의 `File.prototype.text()` 미지원 시 `new File([yaml], 'r.yaml')` 픽스처에서 실패할 수 있다. 실패하면 테스트에서 `Object.defineProperty(file, 'text', { value: () => Promise.resolve(yaml) })` 로 스텁하지 말고 **prod 코드를 `await new Response(file).text()`** 로 바꾼다(브라우저·jsdom 양쪽 지원).
...
it('YAML 내보내기/가져오기 버튼을 렌더하고 클릭 시 콜백을 호출한다', async () => {
  ...
  render(<AutomationRuleList {...baseProps} onExportYaml={onExportYaml} onImportYaml={onImportYaml} isExportingYaml={false} />)
...
it('내보내기 진행 중이면 내보내기 버튼이 disabled 다 (EC6)', () => {
  render(<AutomationRuleList {...baseProps} isExportingYaml />)
  expect(screen.getByTestId('automation-yaml-export-button')).toBeDisabled()
})
```

**GREEN**. `AutomationRuleListProps`(:86-95)에 prop 3종 추가 + `labels`(:22)에 `exportYamlButton: 'YAML 내보내기'` · `importYamlButton: 'YAML 가져오기'` 추가. 헤더 행(:361-367)의 "룰 추가" 버튼을 `<div className="flex items-center gap-2">` 로 감싸 3버튼을 한 줄에 배치(Maxi 확정 시안).
...
<div className="flex items-center justify-between">
  ...
  <div className="flex items-center gap-2">
    ...
    <Button size="sm" variant="outline" data-testid="automation-yaml-export-button"
            disabled={isExportingYaml} onClick={onExportYaml}>{labels.exportYamlButton}</Button>
...
**REFACTOR**. 없음(기존 구조 유지, 버튼만 그룹화).

**검증**. `node_modules/.bin/vitest run src/components/automation/AutomationRuleList.test.tsx` → 신규 2 PASS + 기존 전량 PASS(props 추가로 기존 render 호출이 깨지지 않는지 확인 — **필수 prop 추가라 기존 테스트의 `baseProps` 갱신 필요**).
...
it('Dialog 재오픈 시 이전 상태가 초기화된다 (EC8)', ...)
```
...
```
`<AutomationRuleList onExportYaml={() => exportMutation.mutate()} isExportingYaml={exportMutation.isPending} onImportYaml={() => setYamlImportOpen(true)} ... />` + `<AutomationYamlImportDialog open={yamlImportOpen} onOpenChange={setYamlImportOpen} projectKey={projectKey} />`. EC8은 Dialog가 `open` false→true 전이에서 내부 상태를 리셋(`key={String(yamlImportOpen)}` 대신 `useEffect(() => { if (open) reset() }, [open])` — [[react-usestate-stale-key-prop]] 회피).

**REFACTOR**. 토스트 문구를 페이지 `labels` 상수로(기존 페이지 관례 확인 후 정렬).

**검증**. `node_modules/.bin/vitest run src/routes/__tests__/projects.$projectKey.settings.automation.test.tsx` → 신규 4 PASS + 기존 전량 PASS.
