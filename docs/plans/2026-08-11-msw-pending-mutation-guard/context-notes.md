...
## D10. ★★가드가 초록인 채로 스위트를 60% 느리게 만들었다

첫 전량 GREEN 은 578파일 9443테스트 통과였지만 **226초 → 360초**였다. 초록이라 그냥 넘어가기 쉬운
형태다(메모리 `ci-slowdown-is-runner-memory-not-code` 의 반대 사례 — 그때는 머신이 범인이었고
이번엔 **내 코드**가 범인이다. 그래서 「머신 탓」으로 넘기지 않고 대조 실험을 했다).

**대조 실험** — `src/mocks` 49파일(RTL·react-query 미사용 순수 유닛)에서 `setup.ts` 만 main 으로 원복.
...
**원인.** `setup.ts` 는 **모든 테스트 파일**에서 실행된다. 거기서 `@testing-library/react` 를
static import 하면 **RTL 을 쓰지 않는 순수 유닛 테스트 전량**이 그 로드 비용을 문다.
파일당 +0.38초 × 577 파일이 전량 +134초와 일치했다.
...
**교훈.** `setup.ts` 에 무엇을 import 하는지는 **전 스위트 비용**이다. 전역 훅을 추가할 때는
초록만 보지 말고 **소요 시간도 대조군과 비교**한다. 「초록인 채 느려진다」가 이 부채의 형태다.
