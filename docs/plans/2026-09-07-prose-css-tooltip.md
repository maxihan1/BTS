...
### ❓ 발견 3 (스스로 보강 · **구현 중 사실 정정**) — 시각 회귀 스냅샷

초안은 「`issue-detail-{light,dark}.png` 2장을 갱신한다」였다. **틀렸다.**

구현 중 실측 — `apps/web/e2e/visual/__screenshots__/` 는 **저장소에 커밋된 적이 없다.**
main 워크스페이스에 `?? apps/web/e2e/visual/__screenshots__/` 로 **untracked** 상태로만
존재했고, 그것을 보고 「baseline 이 있다」고 읽은 것이 이 서술의 출처다. 브랜치에는 그
디렉터리 자체가 없으므로 **갱신할 대상이 없다.**

⇒ 스냅샷 갱신 대신 **실제 브라우저 눈확인**(계약 §6)으로 대신했다. 라이트/다크 각 1장을
찍어 번호 마커·중첩 마커·체크박스·인용 좌측선·코드블록·표 테두리·링크·멘션을 확인했다.
**그 눈확인이 결함 1건을 잡았다** — 아래 발견 5.

### ❓ 발견 5 (구현 중 · 눈확인이 잡음) — 다크에서 인라인 코드가 안 보인다

다크 스크린샷에서 `인라인 코드` 의 배경이 본문과 구별되지 않았다. 원인은 토큰 값이다 —
다크 `--accent` 가 `#A1BDD914`(알파 8%)라 어두운 배경 위에서 거의 투명하다. 라이트에서는
같은 토큰이 `#F1F2F4` 라 뚜렷하게 보여, **한쪽 테마만 보면 결코 발견되지 않는다.**

처방은 배경 + **얇은 테두리**(`1px solid var(--border)`)다. Jira 도 인라인 코드에 둘을 함께
준다. 새 색을 만들지 않으므로 NFR N3(ADS 토큰만)을 지킨다.

★이것이 계약 §6 이 「라이트/다크 **양쪽**」을 요구하는 이유의 실물 사례다.
...
#### ⚠ 구현 중 뒤집힘 — 컴파일러 강제의 비용을 실측했다

위 설계를 **실제로 적용해 보고 접었다.** 기본값을 빼자 **테스트 픽스처 174곳·61파일**이
함께 컴파일 에러가 났다(2026-09-07 실측). 전부 「이슈를 하나 저장해 두고 다른 것을
검증한다」는 준비 코드라 본문 HTML 과 무관한데, 174줄을 `, null` 로 채우면 그 노이즈가
이 PR 의 진짜 변경을 diff 에서 가린다.

테스트 소스셋에 확장 함수(`fun IssueRepository.insert(issue) = insert(issue, null)`)를 두어
우회를 시도했다 — 프로덕션에서는 안 보이므로 강제가 유지되는 영리한 해법이었지만,
**import 가 없는 93파일이 그대로 남았다**(Kotlin 확장 함수는 import 가 필요하다).

⇒ **생산 호출자 2곳을 위해 테스트 174곳(또는 import 93곳)을 고치는 거래**라 접었다.
기본값을 두고 강제를 `issue-insert-body-guard.test.ts` 로 옮겼다 — 검사 범위를 `src/main`
으로 좁히면 목록이 **2줄**이라 차집합이 정확하고, 새 생산 호출자가 인자를 빠뜨리면
pre-push 에서 red 가 난다. 뮤테이션 2건으로 실제로 잡는 것을 확인했다.

**교훈** — 「더 강한 처방」이 항상 옳지는 않다. 강제의 **폭발 반경**을 먼저 재야 한다.
