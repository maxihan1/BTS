...
`/code-review high 377` 이 6건을 올렸고 각각 실측 검증 후 전부 반영했다.
**전제가 하나 뒤집혔다** — 리뷰는 「이 저장소는 squash 본문에 PR 설명을 통째로 싣는다」고
했으나 실제 설정은 `squash_message: COMMIT_MESSAGES` 라 PR 설명은 실리지 않는다
(`gh api repos/{owner}/{repo}` 실측). 다만 **브랜치 커밋 메시지 경유** 위험은 그대로 유효하고
(PR #345 전례), 지적의 결론은 바뀌지 않는다.

| # | 지적 | 판정 | 처방 |
|---|---|---|---|
| 1 | 되감기가 「토큰을 가진 모든 커밋」에 걸려 **진짜 낡은 run 을 보호**한다 → PR #366 부채 부활 | **타당 (medium)** | 되감기 조건을 **토큰 AND 생성 커밋 패턴**으로 좁힘. 신규 판별식 1건 |
| 2 | 「Step 3 훅이 HEAD 를 민다」는 인과가 부정확 — `post-merge` 는 로컬 merge/pull 에서만 발화 | **타당 (medium)** | 실제 트리거를 `gh pr merge --delete-branch` 의 checkout+pull 로 특정. 「항상은 아니다」를 명시 |
| 3 | 스크립트 `:79` 에 「현재 HEAD 무접촉」 문구 잔존 | **타당 (low)** | 정정 |
| 4 | `TODOS.md:2817` 정본이 옛 계약을 기술 | **타당 (low)** | §재해소 절 신설 |
| 5 | 토큰 판정이 대소문자를 구분 — `[SKIP CI]` 를 놓치면 결함 정확히 재현 | **타당 (low)** | `nocasematch` 적용. 신규 판별식 1건 |
| 6 | 테스트 파일 안에서 모순되는 주석 (공백 눕히기 ↔ `@json`) | **타당 (low)** | 정정 |

### ★지적 1 이 왜 medium 인가 — 두 방향 모두 확인했다

- **되감기가 너무 넓으면.** 내용 있는 머지가 `[skip ci]` 를 물고 오면 그것을 지나쳐 훨씬 낡은
  커밋을 기준으로 잡고, 거기 붙은 낡은 run 을 보호한다 → PR #366 이 닫은 「1시간 43분 점유」 부활.
- **좁혀도 손해가 없다.** 그 머지 커밋은 GitHub 도 CI 를 건너뛰어 run 이 0건이므로, 되감지 않고
  기준으로 삼아도 **보호할 run 이 없다.** 그보다 낡은 run 은 예정대로 취소된다.

즉 좁히는 쪽이 두 방향에서 모두 안전하다. 대신 생성 커밋 패턴과 `.husky/post-merge` 가
**두 목록**이 되므로([[two-lists-never-check-each-other]]), 판별식이 **훅 파일에서 커밋 메시지를
실제로 읽어** 먹이도록 짝을 채웠다.
...
| 뮤테이션 | 결과 | 잡은 케이스 |
|---|---|---|
| M1. skip-ci 판정 제거 | **RED** (fail 5) | 5 케이스 (판정 자체를 없앴으므로) |
| M2. 보호 집합을 기준 커밋 하나로 축소 | **RED** (fail 1) | `[skip ci]` 커밋 자신의 run 보호 |
| M3. 검사 대상을 제목만으로 | **RED** (fail 1) | HEAD-skip-ci 부모 보호 |
| M4. 기준 미확인 시 fail-open 제거 | **RED** (fail 2) | 전부-skip-ci · HEAD 미확인 |
| M6. 되감기를 「토큰만」으로 되돌림 | **RED** (fail 1) | 내용 있는 머지의 토큰 무시 |
| M7. 대소문자 무시 제거 | **RED** (fail 1) | 대문자 `[SKIP CI]` |
| M5. jq 정형 되돌림 | GREEN | 동작 보존 변형 — 정상 (§M5) |
