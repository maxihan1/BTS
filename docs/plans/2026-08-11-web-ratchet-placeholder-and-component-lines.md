...
⚠️ **PR #364 본문 정정 필요** — 「프로덕션 코드 0줄」은 D1 채택으로 거짓이 됐다 (**18파일 28곳**).
→ **2026-08-11 정정 완료** (코드리뷰 BLOCKER-1). ★수치도 함께 정정 — 「17파일 27줄」은 **리터럴만 센 값**이고
템플릿 리터럴 1건(`ComponentMultiSelect.tsx`)이 빠져 있었다. 정본은 **27 리터럴 + 1 템플릿 = 28건 / 18파일**.
...
- ④ PR #364 본문의 「프로덕션 코드 0줄」을 정정한다 (D1 채택으로 **18파일 28곳**).
...
- [x] ④ **PR #364 본문 정정** — 「프로덕션 코드 0줄」·「27곳을 옮기지 않는다」를 실제 결과
      (**18파일 28곳 이전 · 예외목록 0**)로 교체. 범위가 바뀐 사실(D1)을 본문 상단에 명시하고
      체크박스 7칸을 실제 상태로 갱신했다. **「프로덕션 동작 변경 0」은 참으로 유지** — 값 보존 28/28.

**처방 목록에 없던 추가 이행 3건** (번호를 이어 붙이지 않고 별도로 적는다 — 아래 ★ 참조).

- [x] (추가 ㉠) `TODOS.md` `IssueCreateForm` 227줄 항목 — **닫지 않고** 동결 문구 한 단락만 추가
      (처방 ② 본문의 「닫지 않는다」 지시를 이행한 결과물).
- [x] (추가 ㉡) 신규 부채 등재 — 「apps/web — 같은 문구를 `placeholder` 와 `aria-label` 이 각자 만든다」
      (`ComponentMultiSelect.tsx:81` · `UserMappingStep.tsx:173`). Jira 패리티 계약 §2 확인을 착수 전 필독으로 못박았다.
- [x] (추가 ㉢) **잔여 부채 3종을 `⬜` 열린 섹션으로 승격** + 대시보드 렌더러 결함 등재.
      상세는 ✅ 섹션의 「★남은 것」 표가 정본이고, 승격한 섹션은 **열린 상태를 표시하는 입구**다.

> **★★이 두 목록의 번호가 한 칸 밀려 있었다 (코드리뷰 BLOCKER-2 적발).**
> 이행 기록이 처방에 없던 「IssueCreateForm 동결」을 ③ 으로 잡는 바람에 처방 ③·④ 가 각각
> ④·(없음)으로 밀렸고, **처방 ④(PR 본문 정정)만 조용히 사라져 실제로 미이행 상태였다.**
> **이 PR 이 설치하려던 `[[two-lists-never-check-each-other]]` 가드의 대상 양식이 이 PR 자신의 계획서 안에서 재발한 것이다.**
> ⇒ 처방과 이행의 번호를 **①②③④ 로 고정**하고, 처방에 없던 것은 번호를 잇지 않고 **㉠㉡㉢ 로 분리**했다.
> 번호를 이어 붙이는 순간 다음 사람이 대응 관계를 세지 않게 되는 것이 이 사고의 기전이었다.
...
★**그 preview 의 수치 자체가 과소였다** (코드리뷰 BLOCKER-3) — 리터럴 27곳만 세고 템플릿 리터럴
1건(`ComponentMultiSelect.tsx`)을 빼서 17파일로 적었다. 실제는 **18파일 28곳**이다.
결정(D1 = 전부 이전)은 수치와 무관하게 유효하지만, **선택지에 제시한 숫자가 틀렸었다**는 사실은 남긴다.
