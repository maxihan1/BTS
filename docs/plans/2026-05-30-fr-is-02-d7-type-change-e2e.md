...
- 본보기 패턴 — `apps/web/e2e/issue-transition.spec.ts` (FR-IS-01 전이 E2E, PR #41). `loginAsAlice` → `page.goto('/issues/ATLAS-1')` → `getByRole('combobox', { name })` → `selectOption` → `getByTestId` 단언.
...
- `playwright-getbyrole-exact-strict-mode` — 같은 텍스트 셀렉터 충돌 회피. `getByRole('combobox', { name })` 로 유형/전이 셀렉터 구분(전이 셀렉터도 같은 패널에 있으므로 name 으로 정확 구분 필수). 텍스트 단언은 exact:true.
...
- **S2 셀렉터 옵션** — 유형 셀렉터에 활성 표준 5종(epic/story/task/subtask/bug) 옵션 노출. 전이 셀렉터와 혼동 없이 `name: typeSelectLabel` 로 정확 타깃.
...
**검증**: `cd .worktrees/fr-is-02-d7-type-change-e2e && pnpm --filter web test:e2e -- issue-type-change.spec.ts` (또는 전체 E2E). 셀렉터는 i18nLabels 정본 참조, getByRole name 으로 전이/유형 셀렉터 구분.
