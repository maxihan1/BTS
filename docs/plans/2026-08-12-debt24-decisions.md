...
## ⚠️ 이 문서는 2026-08-12 시점의 **계획 기록**이다 — 아래 두 가지가 폐기됐다

**본문은 고치지 않는다.** 계획 문서를 나중 사실로 덮어쓰면 「그때 무엇을 계획했나」라는
기록 자체가 거짓이 된다. 대신 무엇이 폐기됐는지를 여기 한 번만 적는다.

| 폐기된 것 | 언제 · 왜 | 지금의 정본 |
|---|---|---|
| **`TODOS.md` 줄번호 키** (`Task 1` 표 · `Task 2` 표 · 본문 곳곳) | 2026-08-12 PR #375 — 줄번호는 휘발성이라 **작성 시점부터 22건 중 18건이 어긋나 있었다** | `2026-08-12-debt24-master.md` §전수 매핑의 **제목 키**. `scripts/workflow/debt-ledger-mapping.test.ts` 가 강제 |
| **담당 PR `#368`~`#374`** (`Task 1` 표 · `Task 2` 표 `4` 행 · 본문 `#374 예정` · `#371 담당`) | 2026-08-14 — 변경 0파일 **예약 draft PR 7건을 폐기**했다. 부채는 하나도 안 닫혔고 전부 `미배정` 으로 남았다 | 같은 문서 §전수 매핑·§PR 별 집계의 **미배정**. `⬜` 행에 PR 번호를 쓰는 것은 이제 판별식이 막는다 |

폐기 경위와 근거는 `2026-08-12-debt24-master.md` §PR 별 집계 아래 **★★2026-08-14** 블록.
