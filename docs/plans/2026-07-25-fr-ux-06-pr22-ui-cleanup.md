...
### T6 완료 기록 (2026-07-25 세션 2)

**검증 성적.** typecheck 0 · eslint 0 error(경고 8 사전존재 유지) · 유닛 **513파일 / 7934 green**
(기준선 512/7929 → 신규 가드 1파일 5테스트). 커밋 3개 = `test:` → `feat:` → `refactor:` 순서 준수.

**함정 회피 실증.** 세션 1이 전량 원복했던 일괄 치환 사고를 재발시키지 않았다.
파일별 `variant=` 개수를 기준선과 전수 대조해 **14/14 예상 일치**, diff 기준 `<Button` 추가 23 /
`<button` 제거 23으로 **1:1**, 기존 `<Button>` 오염 **0**을 확인했다.

**신규 가드 `components/__tests__/button-primitive-usage.test.ts` (5테스트).**
남은 원시 `<button>`은 전부 직전 줄에 `PR22 OUT — P4|P5|P6` 사유 주석을 가져야 한다는 계약.
행 번호가 아니라 **`파일::패턴코드`로 키를 잡아** GREEN의 줄 이동에 불변이다. 뮤테이션 3종으로
실효를 입증했다 — ①사유 주석 1건 제거 → 2건 실패 ②`Button` 1건을 원시로 되돌림 → 2건 실패
③주석 본문에 `<button>` 리터럴 삽입 → **오탐 없이 통과**.

**계획 대비 편차 3건 (사유 포함).**
1. **RED을 파일별 렌더 테스트가 아니라 소스 전수 스캔 가드로 구현했다.** plan의 RED 예시가 지목한
   `TopBar`가 **실측상 배치2**였다(교차 오배정 흔적). 14개 테스트 파일의 서로 다른 렌더 헬퍼에
   어서션을 흩뿌리면 *일부만* 덮이는 [[guard-handler-matrix-blindfold]] 위험이 있어, T1·T4 선례
   (`state-tokens.test.ts` · `skeleton-usage.test.ts`)를 따라 **행렬 전수 비교**로 갔다.
2. **REFACTOR 주석을 21개가 아니라 4개만 달았다.** `variant="ghost" size="icon-xs"`는 선언 자체로
   자명하고, 매핑 표가 이미 이 문서에 정본으로 있다. 21개는 `DEVELOPMENT.md §2.3`("주석은 **왜**")를
   거스르는 noise다. 미래 편집이 조용히 깨뜨릴 4곳만 남겼다(유일한 outline · `NEW_ISSUE_CLASS` 공유 ·
   `asChild` 주입 · `min-h` WCAG 근거).
3. **OUT 사유 주석을 T7이 아니라 T6에서 달았다.** plan은 T7 REFACTOR 항목으로 뒀지만, T6의 가드가
   이 주석을 판정자로 쓰고 T8 `overrides`가 1:1 대응해야 하므로 T6 범위가 맞다.

**T7 착수 시 주의 2건.**
- **JSX 주석 위치.** `return (` 직후에는 `{/* … */}`를 쓸 수 없다(객체 리터럴로 파싱돼
  `TS1005`). JSX 자식 위치가 아니면 `//`를 써야 한다 — T6에서 `issues.index.tsx`가 실제로 깨졌다.
