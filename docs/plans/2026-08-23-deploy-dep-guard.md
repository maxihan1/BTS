...
착수 기준선 **393/393** → 최종 **405/405**.

### Task 1 (부채 95) — red 5건

```
not ok - ★★의존성 판정이 `.bin` 셰임 존재로 서지 않는다
not ok - ★모듈 확인 헬퍼가 셰임이 아니라 `package.json` 실체를 본다
not ok - ★부재 메시지가 복구 명령과 그 전제를 함께 준다
not ok - ★헬퍼 정의가 게이트 블록 **밖**에 있다
not ok - ★게이트가 vitest 를, 빌드 폴백이 vite 를 각각 확인하고 부른다
```

### Task 4 (부채 81) — red 2건

```
not ok - ★배포 게이트가 미분류로 떨어지지 않는다
not ok - ★★훅과 배포 게이트의 티어가 같다
```

### 범위 확장분 (F2b) — red 2건

```
not ok - 해소 전환은 소실이 아니라 **해소**로 접힌다
not ok - 음성 대조군 — 해소하면서 본문을 줄이면 접지 않는다 (관대함 차단)
```

## A/B 실측 — 고장을 재현해 옛 판정과 새 판정에 각각 물렸다

셰임만 남고 모듈이 파괴된 트리를 만들었다(`.bin/vitest` 실행 가능 · `vitest/` 부재 ·
`@radix-ui` 빈 스코프 디렉터리 — 2026-08-23 파괴 양식 그대로).

| 판정 | 결과 |
|---|---|
| 옛 — `[ -x apps/web/node_modules/.bin/vitest ]` | **통과** (파괴된 트리를 지나간다) |
| 새 — `require_web_module vitest` | **차단** + 복구 안내 4줄 |

헬퍼 자체도 실행해 봤다 — 실재 모듈 통과 · 부재 모듈 `exit 1`. 소스 문자열 대조만이 아니다.

## 뮤테이션 결과 — 전건이 이름 붙은 red 를 낸다

### 배포 게이트·표면 (M)

| 뮤테이션 | red 판정 |
|---|---|
| M1 게이트를 `-x` 셰임으로 되돌림 | ★★셰임 판정 · ★게이트가 vitest 를 확인 |
| M2 폴백만 `-x` 셰임으로 되돌림 | ★★셰임 판정 · ★폴백이 vite 를 확인 |
| M3 복구 명령 줄 삭제 | ★복구 명령과 전제 |
| M3b 대가 줄만 삭제 | ★전제를 어겼을 때의 대가 |
| M3c 헬퍼를 게이트 블록 안으로 이동 | ★헬퍼 정의가 게이트 밖 · ★★게이트가 pnpm 을 거치지 않는다 외 2 |
| M4 `infra/deploy/**` 제거 | ★미분류 아님 · ★★훅과 티어가 같다 |
| M5 글로브를 실파일 없는 경로로 | 모든 글로브가 실제 파일을 문다 외 2 |

**★뮤테이션이 판정 공백 2건을 잡아냈다.** 1차 실행에서

- **M3b 가 red 0** — 복구 명령 줄 하나가 「명령」과 「전제」를 다 만족시켜, 대가를 적은 줄만
  지워도 초록이었다. 그 줄에만 있는 문자열을 아무도 안 본 것이다.
- **M1 과 M2 가 같은 이름으로 red** — 한 판정에 두 단언을 넣어 어느 자리가 깨졌는지 못 갈랐다.

둘 다 판정을 갈라 고쳤고(커밋 `c6f5ee787`) 재실행에서 위 표가 됐다.

### 장부 무손실 판별식 (N)

| 뮤테이션 | red 판정 |
|---|---|
| N1 본문 보존 검사 무력화 | 음성 대조군 — 본문을 줄이면 접지 않는다 |
| N2 해소를 목록에 안 남김 | 해소 전환은 소실이 아니라 해소로 접힌다 |
| N3 `judgePureMove` 의 `resolved` 조건 제거 | **red 0** — §아래 참조 |
| N4 해소 마커 키 오타 | 해소 전환 · F2b 실파일 판정 |
| N5 접기에 제목 동일까지 요구 | 해소 전환 · F2b 실파일 판정 |

**N3 이 red 0 인 것은 신규 결함이 아니다.** 같은 자리의 **기존** 조건 2건을 같은 잣대로 쟀다.

| 지운 조건 | red |
|---|---|
| `items.renamed.length === 0` | 0 |
| `items.edited.length === 0` | 0 |
| `items.resolved.length === 0` (신규) | 0 |

셋 다 줄 축이 먼저 `clean` 을 깨서 단독으로는 판정되지 않는다. 부채 `98` 로 등재하고
`judgePureMove` 주석에 그 사실을 적었다 — 판정되고 있다고 착각하지 않도록.

## 범위 확장 기록 — 계획 밖 1파일

**막힌 지점.** Task 7(장부 갱신)에서 `todos-reorder-integrity` 의 F2b 가 부채 81 의 ⬜→✅
전환을 **소실**로 읽어 red 를 냈다. 접기가 둘뿐인데(개명 = `(상태, 본문)` 동일 ·
편집 = `(상태, 제목)` 동일) 해소는 **상태가 바뀌는 것**이라 어느 쪽에도 안 걸린다.

**합성 픽스처 4종 실측 — 어떤 형태로도 못 지나간다.**

| 시도 | 결과 |
|---|---|
| 마커만 `⬜`→`✅` (제목·본문 그대로) | lost 1 |
| 마커 + 제목 괄호 | lost 1 |
| 마커 + 괄호 + 해소 주석 (관례 형태) | lost 1 |
| **대조군 — 본문을 통째로 지우고 ✅** | lost 1 |

정당한 해소와 본문 삭제가 **같은 신호**를 낸다. 엄격한 것이 아니라 판정 불능이다.

**왜 5일간 잠복했나.** F2b 가 착지한 `5a1852839`(2026-08-18 · #390) 이후 ✅ 개수가
**76 에서 한 번도 안 움직였다**(⬜ 는 28 → 69). 그 사이 장부를 건드린 커밋 7건이 전부 등재였다.
**이 PR 이 F2b 착지 이후 첫 해소다.**

**Maxi 판정 (2026-08-23) — A안.** 세 번째 접기를 이 PR 에서 더한다. 접는 조건은 본문 전 줄
보존이라 「✅ 로 덮고 본문을 지웠다」는 여전히 `lost` 다. 제목 동일은 요구하지 않는다 —
제목은 이 파일의 책임이 아니라 `debt-ledger-mapping.test.ts` 가 마스터와 대조하는 조인 키이고,
그 분담은 이 파일 머리가 이미 그렇게 적고 있다.

**이 확장이 만든 사고 1건.** GREEN 선커밋 전에 뮤테이션 하네스를 돌렸다. 하네스의
`git restore` 가 **미커밋 구현을 HEAD 로 되돌려 통째로 지웠다.** `CLAUDE.md §함정` 의
「뮤테이션 검증은 GREEN 선커밋 뒤. 미커밋 원복은 소실이다」를 그대로 밟았다. 구현은
재적용해 복구했고, 이후 순서를 지켰다(커밋 → 뮤테이션).

## 신규 등재 — 부채 98

**워크플로우 — 순수 이동 판정의 항목 축 조건 3건을 아무 판정도 안 잡는다.** 위 N3 표가 근거다.
지우지 않고 두되(지우면 「줄이 항목 사이를 이동하는 재배치」가 무방비가 된다) 주석에 사실을 적었다.

## 절차 이탈 — 게이트 2 요약에 그대로 싣는다

1. **sub-agent dispatch 0회.** 이 세션 하네스가 「사용자가 요청하지 않으면 Agent 툴 금지」라
   `bts-impl` 의 wave dispatch 대신 컨트롤러가 인라인 구현했다. TDD red-first · 뮤테이션 관측은
   그대로 지켰다.
2. **worktree 배선이 부재한 상태로 시작했다.** `bts-start` 절차의 심볼릭 3요소가 전부 없었다.
   손으로 걸고 첫 커밋에서 pre-commit 이 실제로 도는 것을 확인한 뒤 진행했다. 부채 `97` 로 등재.
3. **범위 +1파일** (`todos-reorder-integrity.mjs`). Maxi 승인 A안. §범위 확장 기록.

## 게이트 2 라운드 1 (2026-08-23) — 리뷰 2종 · 지적 8건 · 전건 처리

두 렌즈 다 **CONCERNS · BLOCKER 0**. 둘 다 계획의 실측을 믿지 않고 직접 재현했고
**뮤테이션 9건 전건이 계획이 적은 그대로** 나왔다(렌즈 A). 절대 규칙 19 + DATA.md 5원칙 위반 0.

### 반영 4건

| # | 지적 | 처리 |
|---|---|---|
| 1 | **해소 접기에 `l`↔`g` 동일성 요구가 없다.** 빈 멀티셋은 무엇에나 포함되므로 본문 없는 항목이 아무 ✅ 에나 붙고, 짝 없는 ✅ 하나가 무관한 삭제를 흡수한다. 짝이 엇갈리면 **엉뚱한 이름**으로 red 가 난다 | 접기에 **제목 어간 동일** 조건 추가. 조건 하나로 셋이 닫힌다. 어간 규칙은 `stripStatusParen` 으로 `todos-reorder-integrity.mjs` 에 두고 `debt-ledger-mapping.test.ts` 가 **import** 한다 — 사본 0 |
| 2 | **배포 게이트 판정 5건이 전부 소스 문자열 대조다.** `\|\| test -x …` 폴백을 얹어 셰임 수용으로 되돌렸는데 405종 전부 초록이었다 | ① 문자열 판정을 **형태에서 자리로** 바꿨다 — 「헬퍼 밖에서 `.bin` 이 나오는 줄은 실행 두 줄뿐」. `test -x`·`command -v`·`[[ -x ]]`·`[ ! -x ]` 를 전부 덮는다. ② 헬퍼를 임시 트리에서 **실제로 실행**하는 판정 4건 추가 |
| 3 | **`.bin` 실행 가능 확인이 사라졌다.** `CLAUDE.md §함정` 이 「`.bin` 부재부터 의심」으로 이름 붙인 자리다. fail-closed 이나 진단 후퇴 | 헬퍼가 모듈 실체를 본 **뒤** 셰임도 본다. 메시지를 갈랐다 — 모듈 부재는 스토어 파괴, 셰임 부재는 링크 미배선 |
| 4 | **`★★훅과 배포 게이트의 티어가 같다` 가 단독 발화하지 않는다.** 두 글로브를 함께 지우면 「T1 == T1」로 초록 | 절대 단언(`=== 'T2'`)을 먼저 둔다. 상대 비교는 결합 문서화로 남긴다 |

### 등재 2건 (범위 밖)

- **99** 프로덕션 이미지·리버스 프록시가 티어 카탈로그에 없다. 부채 81 의 잔여 —
  `infra/prod/**` · `infra/docker-compose.prod.yml` 전량이 `T1 · UNMAPPED` 다.
- **100** `DEVELOPMENT.md:140` 의 PR 제목 규칙이 관행과 어긋나고 판정 0건.

### 그 밖 2건

- CLI `main()` 이 `resolved` 를 안 찍어 「조용히 접지 않는다」 주석과 어긋났다 → 찍는다.
  `edited` 쪽 같은 구멍은 **선재**였고 함께 닫았다.
- `양성 ⑩` 이름이 「판정된다」고 오해를 준다 → 「**줄 축**이 판정한다」를 이름에 박았다.

### 라운드 1 반영분 뮤테이션 — 전건 이름 붙은 red

| 뮤테이션 | red 판정 |
|---|---|
| R1 게이트를 셰임 수용으로 약화(`\|\| test -x`) — **리뷰가 뚫은 그 형태** | 셰임만 남은 트리를 막는다 · 끊어진 심볼릭 |
| R2 헬퍼의 셰임 2차 확인 제거 | 모듈은 있는데 셰임이 없는 트리도 막는다 |
| R3 헬퍼 밖에 `test -x` 프로브 심기 | ★★헬퍼 밖에서는 `.bin` 을 부를 때만 쓴다 |
| R4 배포·훅 글로브를 **함께** 제거 (S2) | ★배포 게이트가 미분류로 · ★★훅과 티어가 같다 |
| R5 접기의 제목 어간 조건 제거 | 빈 본문 흡수 · 짝 엇갈림 · 어간 동일 |

**R1 이 핵심이다.** 리뷰 전에는 이 뮤테이션이 405/405 초록이었다. 이제 행동 판정이 문다.

### 라운드 1 에서 배운 것

- **판정을 실행에 묶지 않으면 형태만 지킨다.** A/B 실측을 scratchpad 에서만 하고 하네스에
  안 넣은 것이 이 PR 의 실질적 구멍이었다. 리뷰가 그것을 정확히 뚫었다.
- **접기를 하나 늘리면 동일성 증거를 함께 요구해야 한다.** 「무엇을 접는가」만 정하고
  「무엇이 같아야 접는가」를 안 정하면 접기는 곧 구멍이다.
- `transition-term-guard` 는 구 표기를 **부분문자열**로 잡아서 「前」 뜻의 정상 한국어 어절이
  오탐으로 걸린다. 이 계획서를 쓰다 **두 번** 밟았다 — 두 번째는 그 오탐을 **설명하는 문장**
  자신이 걸렸다. 문구를 바꿔 피했다(동결 갱신도 가드가 허용하지만 안 건드리는 쪽을 골랐다).
  doc-index 규칙의 `NFR-SEC-01` 오탐과 같은 계열이다. 커밋 메시지는 스캔 대상이 아니라
  구체 어절은 그쪽에 남겼다.
