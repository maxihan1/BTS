...
## ★ 재리뷰 실측 — 문서 봉합 (2026-08-07 · 코드 로직 0줄)

### C1·C2 닫힘 확인

재리뷰가 코드리뷰 CONCERNS **C1(부분 문자열 영구초록)·C2(`hasLocalDefinition` 이 `const`/
`function` 만 봄)** 를 **닫힘으로 확정**했다. `9cb6c3b0f` 봉합 후 위 M1~M3 에 더해 **추가 우회
5종**을 태워 **전부 red** 를 실측했다(재리뷰 보고 기준 — 우회 목록의 정본은 재리뷰 보고서다).
남은 지적은 **문서 부재 1건**뿐이었고, 그것이 아래 ⑫ 다.

### 남은 1건 = §4.11 후속 ⑫ 로 등재 — 서술되지 않은 강한 불변식

⑦ 봉합의 `toEqual` **정확** 대조와 `tsconfig.app.json:14` `noUnusedLocals: true` 가 맞물려
**「공유 모듈의 모든 export 를 짝 파일 2개가 둘 다 실제로 사용해야 한다」**는 불변식이 생겼다.
「로컬 복사본 금지」보다 훨씬 센 제약인데 **코드에도 정본에도 서술이 0건**이었다.

**그리고 정본이 스스로 예고한 다음 작업(⑩ 처방 후보 ③)이 이 불변식을 위반한다.**
`BacklogFilterBar.test.tsx` 의 `EPIC_CONTROL_ROLE` 사용처는 `:245` **단 한 줄**이라(`:16` 은
import 지정자 · `grep -o` 2건) ③ 을 하면 사용처가 0이 된다. 재리뷰가 탈출 경로 4개를 전부
태웠고 **합법 상태가 없었다** — (a) `TS6133` · (b) 판별식 8종 대조 · (c) 판별식 3건 ·
(d) 판별식 2건 + `TS2459`. 판별식 자체를 손대야(파일별 기대 목록 분리 또는 부분집합 대조 완화)
열린다. **차단 아님** — 양방향 모두 시끄럽게 실패하고 조용한 green 0건 · 프로덕션 영향 0.

### ★ dispatch 수치 2건이 실측으로 뒤집혔다 — 이 FR 에서 **5번째**

| 항목 | dispatch | 실측 |
|---|---|---|
| `noUnusedLocals` 위치 | `tsconfig.app.json:24` | **`:14`** (`grep -n` 확인) |
| 패널의 `EPIC_CONTROL_ROLE` 사용처 | 20곳 | **22곳** (`grep -o` 23줄 중 `:14` 는 import 지정자) |

정정본을 §4.11 ⑫ 본문에 **그대로 적어 두었다.** 앞선 4건은 F5 줄번호 · F15 `:246,248` ·
F16 「슬롯 그대로 쓴다」 · ⑦ 「복사본 2벌」이다.

**⑫ 표의 RED 건수는 조합 의존이라는 단서도 함께 남겼다.** 판별식의 import 대조는
`it.each(PAIRED_TEST_FILES)` 라 **파일당 1건**이므로 (c) 의 3건은 **두 파일이 8종을 그대로
import 한 상태**에서만 나온다 — (b) 를 누적해 필터바 import 까지 뺀 조합이면 필터바 쪽이
7 ⟺ 7 로 통과해 **2건**이 된다. 착수자가 「3건이 아니네」로 혼란하지 않도록 못박았다.

### 이 커밋의 변경 — 문서 3파일 · 코드 로직 0줄

| 파일 | 변경 |
|---|---|
| `docs/plan/product/personalization.md` | ⑫ 신설 · ⑩ 에 역참조 1문단 · 헤더 등재 수 **11 → 12**(완료 1 · 잔여 **11**) · 「실측에 뒤집힌 사례」 **4건 → 5건** |
| `apps/web/src/test/backlog-epic-control-contract.ts` | `EPIC_CONTROL_ROLE` KDoc 에 **⛔ 함정 경고 5줄** 추가. **주석만** — export 표면·값·시그니처 무변경(바뀌면 판별식이 red 다) |
| 이 문서 | 이 절 |
...
| 항목 | 값 |
|---|---|
| 판별식 단독 (`epic-control-contract.test.ts`) | `EXIT=0` · **6 passed (6)** — 봉합 전후 동일 |
| `tsc -p tsconfig.app.json --noEmit` | `EXIT=0` |
| `eslint src` | `EXIT=0` — **0 errors / 8 warnings**(선재, 직전과 동일) |
| `scripts/verify-master-plan.sh` | `EXIT=0` · `PASS` |
| FR 개수 | **139 / 139** 불변 (후속 항목은 FR 이 아니다) |
| `build-doc-index.mjs --check` | `EXIT=0` |

**후속 등재는 11 → 12건**(완료 1 · 잔여 11)이고 **FR 139 는 불변**이다.
