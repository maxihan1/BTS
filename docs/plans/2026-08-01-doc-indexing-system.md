...
## 뮤테이션 확인 결과 (2026-08-01 실측)

하네스. `scratchpad/mutation-probe.sh` — 클린 전제 검사 → 주입 → 측정 → 원복 → 재측정.

**1차 — 6종 중 5종이 공허(장식)였다.**

| 룰 | 주입 | 1차 | 원인 |
|---|---|---|---|
| I | 인덱스 한 줄 수정 | **green** | 룰 I 이 쓰기 모드로 재생성해 검사 대상을 덮어씀 |
| J-추가 | 문서 생성 | red ✅ | — |
| J-삭제 | 인덱스 행 삭제 | **green** | 룰 I 의 재생성이 주입을 지움 (연쇄) |
| K | 깨진 링크 | **green** | 〃 |
| L | SOURCES 추가 | **green** | `'docs/**'` 를 무조건 OR 로 붙여 무관 경로도 커버 판정 |

**교정.** ① 생성기에 `--check`(비교만, 파일 안 씀) 신설 — 판별식이 스스로 증거를 지우지 않는다.
② 룰 L 을 실제 접두 일치만 인정하도록 수정.
...
|---|---|---|---|
| I | 인덱스 한 줄 수정 | ✅ fail 2 | ✅ |
| J-추가 | 인덱스에 없는 문서 생성 | ✅ fail 2 | ✅ |
| J-삭제 | 인덱스에서 한 줄 삭제 | ✅ fail 2 | ✅ |
| K | 없는 파일 링크 추가 | ✅ fail 2 | ✅ |
| L | SOURCES 에 CI 모르는 경로 추가 | ✅ fail 1 | ✅ |

최종 서명 — **클린인데 green** (`git status` 비어 있고 `pass 6 / fail 0`).

**교훈.** 뮤테이션을 돌리지 않았다면 6종 중 5종이 장식인 채로 "판별식 설치 완료"가 됐다.
게다가 룰 I 하나의 부작용이 다른 3종을 **연쇄로** 무력화했다 — 개별 룰만 보면 알 수 없고,
전량을 한 하네스에서 주입해야 드러난다.
