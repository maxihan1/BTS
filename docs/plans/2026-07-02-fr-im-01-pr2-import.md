# FR-IM-01 PR2 — Import 컴포넌트/버전 자동생성 + 소스 상태 전환
...
PR1 코어(완료, #218) → **PR2 컴포넌트/버전 자동생성 + 소스 상태 전환** → PR3 댓글/Worklog → PR4 첨부(zip)/이력.
...
- Jira **소스 이슈 상태** → BTS 워크플로우 상태로 전환 (생성 후 목표 상태로 이동)
...
- **확장 지점 3곳** (모두 기존 파일 확장, 신규 서비스 최소):
  ...
  3. `IssueImportAdapter` (issue-tracking) — 컴포넌트 auto-create, 버전 auto-create+링크, 상태 전환
- **영향 엔티티**: Component, Version, `issue_affects_versions`/`issue_fix_versions`(V017 기존), WorkflowState. **신규 마이그레이션 불필요**(모든 테이블 기존).
- **새 용어**: 없음 (Component/Version/전환 모두 glossary 기존 용어).
...
- 상태 전환 직접 set 선례: `IssueMoveService`(FR-MV-01)가 `workflowPort.plan()` 우회, `IssueRepository.moveIssue(... targetStateKey)`로 `current_state_key` 직접 set. 유효성=대상 워크플로우 상태집합 포함 여부(FSM edge 아님).
...
- 초기 상태: `createIssue`가 항상 워크플로우 시작 상태로 강제(status 파라미터 없음). 소스 상태 반영은 생성 후 별도 전환 필요.
...
- **D-B. 상태 전환 메커니즘**. FSM `transitionIssue`(유효 edge만, Jira 상태 대부분 도달불가) vs **직접 set(IssueMoveService 선례)**. (추천: 직접 set + 대상 상태집합 포함 검증 + IssuePermission.TRANSITION 게이트. 미매칭/미도달 상태 name은 경고+시작 상태 유지)
- **D-C. 트랜잭션 원자성**. 행 1건 = create+컴포넌트/버전 auto-create+링크+전환 한 tx. auto-create 실패의 best-effort 경고화는 tx poison(23505) 회피 위해 사전 find(findByProject+name) 후 없을 때만 insert 필요. 동시 행 race는 실 UNIQUE가 최종 방어.
...
- 권한 3축(생성=admin 경고 / 편집=basic 행실패 / 전환=경고), OCC 버전 6단계 스레딩, dry-run 실경로 미러 확장.
...
**REFACTOR**. KDoc §책임에 자동생성/링크/상태 추가. 권한 3축(생성 admin=사전체크 경고 / 편집 basic=행실패 / 전환=경고) 주석. `@Suppress` 재점검.
