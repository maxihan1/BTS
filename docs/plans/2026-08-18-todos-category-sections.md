...
**Task 7개 전량 이행.** 커밋 15개(`test:` → `feat:` 3쌍 포함). 구현은 **인라인**
(게이트 1 확정 · 선행 #389 Maxi 결정 ① 승계) — `bts-impl` 을 dispatch 모드로 부르지 않았다.
**절차 이탈로 게이트 2 요약에 싣는다.**

| Task | 이행 | 실측 |
|---|---|---|
| T1 `parseTodos` `section` | ✅ | 기존 소비처 3곳 **수정 0줄** 통과 (전량 364/364) |
| T2 무손실 대조 + F2b | ✅ | 양성 6종 · 음성 1종 · F2b 실파일 축 3종 |
| T3 구조 계약 5축 | ✅ | 재배열 **전 5축 red** → 후 축①~④ green → W10 후 축⑤ green |
| T4 W7 재배열 | ✅ | 무손실 EXIT 0 · HTML **529,176 bytes 바이트 동일** |
| T5 W10 접두 | ✅ | 장부 대조 10/10 무영향 |
| T6 등재 서식 | ✅ | 정본 포인터 단언 + 이름 사본 제거 |
| T7 선행 계획서 이월 기록 | ✅ | 3곳(약속 2 + 이행 목록 1) |

### 계획에서 벗어난 것 2건 (둘 다 계획보다 강해졌다)

**① E6 를 사람 판정에서 기계 판정으로 올렸다.** 계획은 「항목 밖 줄은 도구가 열거하고
사람이 판정한다」였다. 구현 중 **줄 멀티셋 대조(`compareAllLines`)**를 넣으면 항목 안팎을
가리지 않고 기계가 전부 덮는다는 것을 알았다 — 새 스캐너도 필요 없다(줄 나누기라 파싱이 아니다).

**그리고 그 축이 곧바로 실제 소실을 잡았다.** 재배열 1패스에서 옛 시간축 H1
`# 2026-08-09 전수 실측에서 새로 드러난 항목` 을 없애자 **그 아래 인용문 3줄이 함께 사라졌다.**
항목 축은 차집합 0 이었다 — 항목은 하나도 안 잃었기 때문이다.
사라진 3줄은 **살아 있는 Maxi 결정 기록**이었다(「원 항목은 좁게 고치고, 새로 드러난 표면은
전부 신규 등재」). 2패스로 원문 그대로 파일 머리에 되살렸다.
**리뷰 발견 6(「사람 판정은 결국 아무도 안 보는 것이 된다」)이 실증됐다** — 사람에게 맡겼다면
5,000줄 diff 에서 이 3줄을 놓쳤다.

**② 무손실 대조에 개명 인식을 넣었다.** F2b 를 실파일에 처음 태우자마자 red 가 났다 —
T5(W10 제목 수정)를 「항목이 사라졌다」로 읽었다. 리뷰가 예고한 리스크(「정당한 변경도 red」)의
실현이다. 처방은 **상태·본문이 같은 lost/gained 쌍을 개명으로 접는 것**이고, 접는 조건이
본문 동일이라 관대하지 않다(양성 ⑥ 이 고정). 개명은 **조용히 접지 않고 출력한다** —
제목은 장부의 조인 키라 바뀐 사실이 기록 없이 지나가면 안 된다.
**이동 커밋 검증(CLI)에서는 개명도 0** 이어야 한다 — 개명은 편집이지 이동이 아니다.

### 뮤테이션 실측 13종 — 전부 설계대로 red

**GREEN 선커밋 뒤**에 돌렸고 매번 `git checkout --` 로 원복했다. 대상은 **판별식이 아니라
검사 대상**(상수·데이터·파서)이다 — 단언을 지우는 뮤테이션은 「지우면 안 잡힌다」는 자명한
사실만 보여 준다.

| # | 끊은 것 | red 가 난 파일 |
|---|---|---|
| M1 | `parseTodos` 의 `section` 대입 | `build-dashboard` 3건 · `structure-contract` 5건 |
| M2 | H1 판정의 펜스 게이트 | `build-dashboard` 3건 |
| M3 | 멀티셋 소비 제거(집합화) | `reorder-integrity` 1건 |
| M4 | `gained` 를 빈 배열로 | `reorder-integrity` 2건 |
| M5 | 지문에서 본문 제외 | `reorder-integrity` 2건 |
| M6 | `CATEGORIES` 에 6번째 추가 | `structure-contract` 1건 |
| **M7** | `AREA_CATEGORIES` 한 값 변경 | **`structure-contract` 1건** · `plain-language` 1건 |
| M8 | 해소 절 제목을 카테고리 절로 | `structure-contract` 2건 |
| M9 | 첫 카테고리 H1 제거 | `structure-contract` 3건 |
| M10 | 항목 1건의 영역 접두 제거 | `structure-contract` 2건 |
| M11 | `parseTodos` 를 빈 배열로 | `structure-contract` 6건 |
| M12 | 머리의 정본 포인터 제거 | `plain-language` 1건 (★아래 참조) |
| M12b | 절 배치 규칙 문단 통째 삭제 | `plain-language` 1건 |

**★M7 은 파일을 구분해 확인했다**(리뷰 발견 3). 기존 `plain-language` 도 동시에 red 라
구분하지 않으면 「축②가 지킨다」가 아니라 「누군가 잡는다」만 보인다. `structure-contract`
자체가 red 임을 따로 확인했으므로 **축②는 동어반복이 아니다** — 파일(actual)과 상수(expected)가
다른 출처다.

**★M12 가 처음에 GREEN 이었다 — 진짜 무보호를 잡았다.**
원인은 머리에 `AREA_CATEGORIES` 가 **두 번** 나온 것이다. 새로 쓴 절 배치 규칙(L36)과
선행 PR 이 쓴 「왜 두 줄인가」 문단(L49). 앞을 지워도 뒤가 단언을 만족시켰다 —
`[[invariant-satisfied-by-helptext-not-logic]]` 그대로다.
게다가 **뒤 문단은 카테고리 이름 5종을 나열까지** 하고 있었다. 같은 PR 에서 내가
「절 이름을 여기 옮겨 적지 않는다」고 선언한 그 사본이다 —
**PR 이 막겠다고 한 것을 PR 자신이 뚫고 있는 상태**였고, #388·#389 가 각각 한 번씩 밟은
양식의 **세 번째 재현**이다. 사본을 지우니 둘이 한 번에 닫혔다.

### 검증 최종

> ★아래는 **코드 리뷰 이전** 시점의 숫자다. 리뷰 반영 후 값은 §코드 리뷰 반영에 따로 적는다.

- 전량 **387/387** (착수 시 364 → 신설 23)
- `build-doc-index --check` **EXIT 0** · `verify-master-plan.sh` **EXIT 0**
- 무손실 대조 CLI **EXIT 0** — 항목 104 → 104 · 차집합 0 · 구조 아닌 줄 차이 0
- `renderTodos` HTML **529,176 bytes 바이트 동일**(재배열 전후) — N6 추론이 실측으로 확인
- `docs/progress.html` 변경 **2줄** — W10 제목 접두뿐, 재배열 기인 0
- 뮤테이션 **13/13 설계대로**
- 커밋 15개 · `test:` → `feat:` **3쌍** · 이동(T4)과 편집(T5) **다른 커밋**(N5)
- 브랜치 커밋에 CI 건너뛰기 대괄호 표시 **0건** (squash 본문 오염 방지)

## 코드 리뷰 반영 (게이트 2)

판정 **CONCERNS** — 지적 7건(C1~C4 + 사소 3). 전량 반영했고 머지 차단 항목은 없었다.
리뷰가 붙인 재현 절차를 그대로 다시 돌려 확인한 뒤 고쳤다.

| # | 지적 | 처방 | 확인 |
|---|---|---|---|
| **C1** | 「순수 이동」 판정이 H1 통삭제를 축복 · 그 판정에 테스트 0개 | `judgePureMove` 추출 · H1 **소실**은 이동 아님 · 승인은 `--allow-h1-loss` 명시 | M14~M16 |
| **C2** | base 미획득이 `t.skip`(exit 0) 으로 조용히 통과 · ref 가 `origin/main` 고정 | skip → **fail**(opt-out `BTS_ALLOW_NO_BASE=1`) · ref 후보 목록 `['origin/main','main']` | M17·M18 |
| **C3** | 축③ 을 단독으로 끊은 적이 없다 | 아래 M19 로 단독 red 확보 | M19 |
| **C4** | 머리 헤딩 스캐너 사본 2벌 · 마커 목록 `[⬜📌✅]` 가 상수 사본 | `ledgerHead()` 하나로 통합 · 마커는 `TODO_STATUSES` 파생 | M20 |
| 사소 ① | `todoFingerprint` 가 단일 사용처인데 export | export 제거 | 전수 grep 0건 |
| 사소 ② | `beforeCount > 10` 매직 넘버 | `> 0` — 막으려는 것은 파서 0건 하나다 | — |
| 사소 ③ | 축⑤ 문구 「매핑에 없어도 괜찮다」가 ⬜·📌 에는 거짓 | 해소에만 참임을 문구에 적고 축② 를 가리킴 | — |

### 리뷰가 지시하지 않았으나 뮤테이션이 잡은 것 2건

**① ref 폴백의 기본값을 아무도 안 봤다.** 후보 목록을 **인자로 넘겨** 확인하는 테스트만
두었더니 기본값을 `['origin/main']` 로 되돌려도 판별식 393종이 전량 초록이었다.
C2 를 고치면서 그 고침의 판별자를 안 만든 셈이다. 원격이 아예 없는 임시 저장소를 만들어
**기본 인자 그대로** 부르는 테스트로 고정했다.

**② 합친 머리 스캐너의 펜스 인식이 무보호였다.** `ledgerHead` 에서 펜스 게이트를 떼어도
14종 전량 초록이다 — 오늘 머리에 펜스 안 `## ` 예시가 **없기 때문**이고, 그 부재는
언제든 뒤집힌다. 합성 장부 양성 대조군으로 끊어 red 1회를 확인했다.

### 뮤테이션 실측 추가 7종 — 전부 설계대로 red

**GREEN 선커밋 뒤**에 돌렸고 매번 `git checkout --` 로 원복했다.
M14 는 리뷰가 실증한 구멍 그 자체다 — 반영 전에는 이 뮤테이션에 387종이 전량 초록이었다.

| # | 끊은 것 | red |
|---|---|---|
| **M14** | `isStructuralLine` → `return true` | **2건** (구조 줄 정의 · 양성 ⑧) — 반영 전 **0건** |
| M15 | `isStructuralLine` → `return false` | 3건 (구조 줄 정의 · 음성 대조군 · 명시 승인) |
| M16 | 판정식에서 H1 소실 항 제거 | 1건 (양성 ⑦) |
| M17 | ref 폴백 기본값을 `['origin/main']` 로 | 1건 (원격 없는 체크아웃) |
| M18 | `readBaseTodos` 가 항상 미획득 | **4건 · skipped 0** — 반영 전에는 skip(exit 0) 이었다 |
| **M19** | ✅ 항목 1건을 카테고리 절로 이동 | **축③ 단독 1건** (전량 393 중) |
| M20 | `ledgerHead` 의 펜스 게이트 제거 | 1건 (펜스 양성 대조군) |

**★M19 가 C3 의 답이다.** 리뷰가 쓴 절차(해소 절 H1 을 첫 ✅ 뒤로 이동)로 먼저 돌렸더니
F2b 도 함께 red 였다 — H1 이 첫 항목의 **본문에 흡수**되어 항목 지문이 바뀌기 때문이다.
그래서 **내용을 한 글자도 안 바꾸는** 이동(✅ 항목 1건을 카테고리 절로)으로 바꿨고,
그때 축③ 단독 red 가 나왔다. 축③ 은 동어반복이 아니고 고유 커버리지가 실재한다.

### 이동 커밋 재검증 — 판정이 바뀌었다

C1 반영으로 `4a5e46aa4`(W7 재배열) 의 CLI 판정이 **EXIT 0 → EXIT 1** 로 바뀐다.
...
**이것이 옳은 판정이다.** 그 H1 삭제는 §계획에서 벗어난 것 ① 이 적은 사고 —
아래 인용문 3줄(살아 있는 Maxi 결정 기록)이 함께 사라졌던 그 지점이다. 옛 도구는 같은
커밋에 「순수 이동이다」라고 답했고, 그 3줄은 **줄 축이 따로 잡아서** 겨우 살아났다.
새 판정은 절 제목을 **이름으로 지목**한다. 절을 정말로 없애는 재배열이면
`--allow-h1-loss` 로 승인하고 그때도 무엇을 승인했는지가 출력에 남는다(EXIT 0 재확인).

### 검증 최종 (리뷰 반영 후)

- 전량 **394/394** · skipped 0 (반영 전 387 → 신설 7)
- 뮤테이션 **20/20 설계대로** (기존 13 + 신규 7)
- `build-doc-index --check` **EXIT 0** · `verify-master-plan.sh` **EXIT 0**
- 이동 커밋 CLI — 기본 **EXIT 1**(H1 소실 1건 지목) · `--allow-h1-loss` **EXIT 0**
- 커밋 5개 · `test:` → `fix:` 순서 유지
...
| Code Review | `/bts-codereview` | 게이트 2 — 푸시된 PR 독립 리뷰 | 1 | concerns → 반영 | 7건(C1~C4 + 사소 3) 전량 반영 · §코드 리뷰 반영 |
...
- **게이트 2 VERDICT:** 지적 7건 전량 반영. 리뷰가 「1·2 를 넣으면 PASS 로 올린다」고 적은
  C1·C2 를 포함해 전부 고쳤고, 반영 과정의 뮤테이션이 리뷰가 지시하지 않은 무보호 2건을
  더 잡았다. 판별식 394/394 · skipped 0 · 뮤테이션 20/20.
