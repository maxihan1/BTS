...
### 🔴 이슈 3 — 「계정이 없다」는 전제가 이 머신에서 거짓이었다 (2026-07-29 실측)

첫 부팅 테스트가 **4/4 통과**했다. 그런데 그건 시더 덕분이 아니었다.

| 테이블 | 내용 | `created_at` | 출처 |
|---|---|---|---|
| `users` | `alice`, id `…0001` | **2026-07-10** | `data-dev.sql` (단독 모듈을 `dev` 로 띄웠을 때) |
| `local_credentials` | Argon2 해시 1행 | **2026-07-10** | 동일 |
| `global_permission_grants` | `CREATE_PROJECT` → alice | **2026-07-28** | 이전 세션 수동 부여 추정 |
| `projects` | 0행 | — | — |

**세 행 전부가 오늘(2026-07-29) 이전에 존재했다.** 볼륨 `infra_bts-postgres-data` 는 2026-07-10 생성된
영속 볼륨이라 컨테이너를 지워도 남는다. ⇒ **공유 dev postgres 로 돌리는 한 이 테스트는 판별력 0** 이다.
시더를 통째로 지워도 초록이다.

**전제 정정.** 스펙 §0 이 *"`users` 비어 있음 — `V001` 시드 INSERT 0건"* 이라 적은 것은
**마이그레이션에 대해서는 참**이지만 **살아 있는 DB 에 대해서는 거짓**이었다.
「마이그레이션이 안 넣는다」에서 「데이터가 없다」를 유도한 것이 오류다.
메모리 `debt-record-itself-is-unreliable-remeasure-all`(착수 전 재실측)을 마이그레이션에만 적용하고
**실제 데이터에는 적용하지 않았다.**

**가치의 재정의 (Maxi 확정 — 이슈 3 = A).** 이 머신에서는 이미 로그인이 된다(실 HTTP 200 실증).
가치가 남는 곳은 **새 머신 · CI · 볼륨 삭제 후**이며, 그것을 증명하려면 테스트가
**매 실행 빈 DB(Testcontainers)** 에서 돌아야 한다. 이미지는 `quay.io/tembo/pg16-pgmq` 필수
(일반 `postgres:16` 은 pgmq 확장 부재로 마이그레이션이 죽는다 — `backend-ci.yml:102-103` 동일 제약).

**부수 자백 — RED 를 관측하지 못했다.** GREEN 2파일을 첫 빌드가 도는 중에 작성했고
`:modules:app:compileKotlin` 이 그것을 집어갔다. 커밋 순서(`test:` → `feat:`)는 지켜졌으나
**「빨간 것을 봤다」는 증거가 없었다.** 이후 M1 뮤테이션(프로파일 표현식을 비매칭 값으로 교체)을
주입해 RED 를 다시 세웠다 — 이 조작은 판별력 증명(M1)을 겸한다.
