...
| `BoardSummaryResponse` 에 `canDelete` | ~~`⋯` 메뉴는 현재 보드에만 붙는다. 목록에 실을 소비처가 없다~~ → **2026-09-04 뒤집음** ([PR ⑧](2026-09-04-board-summary-can-delete.md)) — 아래 참조 |

> **뒤집기 근거 (2026-09-04 · Jira 패리티 캠페인 PR ⑧).**
> 이 행을 **지우지 않고 남긴다** — 지우면 「왜 미뤘는지」와 「왜 되돌렸는지」가 함께 사라져
> 다음에 같은 판단이 필요할 때 근거가 0 이 된다.
>
> **기각 사유였던 전제가 무너졌다.** 「목록에 실을 소비처가 없다」는 맞는 관측이었으나,
> 캠페인 PR ⑨ 가 사이드바 하위 목록을 **보드 목록으로 갈아치우고 항목마다 `⋯` 를 단다**
> (`apps/web/src/components/layout/ProjectTree.tsx` 가 그 교체를 이미 예고해 뒀다).
> 목록이 행별로 무엇을 렌더할지 알려면 목록 응답이 판단 근거를 가져야 한다 — Jira 대조 J4·J5.
>
> **당시 우려였던 성능 부담은 목록 경로에서 성립하지 않는다.** 목록 엔드포인트는 `projectKey` 를
> 요청에서 직접 받으므로 **권한 판정 1회로 N건 전부**를 덮는다. 왕복이 보드 수에 비례하지 않는다
> (`BoardControllerIntegrationTest` 의 LIST-6·LIST-7 이 이 성질을 기계로 고정한다).
>
> **삭제 가능 여부를 가르는 도메인 제약도 0건이었다.** 「마지막 보드는 못 지운다」·「활성 스프린트가
> 있으면 못 지운다」류 규칙을 `agile-planning` 전수와 `docs/specs`·`docs/adr` 에서 찾았고 히트 0.
> 순수 권한 파생값이라 단건 조회와 **같은 술어**를 그대로 쓴다 — 두 응답이 갈리지 않는 것이 요점이다.
