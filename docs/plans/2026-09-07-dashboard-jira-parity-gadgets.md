...
**Maxi 승인 — 구현 시작.** BLOCKER 0 · 실측 티어 T2 = 선언 T2 · UNMAPPED 0.

**M-4 — `bar_chart` 는 recharts `BarChart` 로 새로 그린다** (D-2 해소).
`DistributionWidget` 재사용을 검토했으나 채택하지 않는다. 근거 —
`pie_chart` 는 막대 목록으로 그릴 수 없어 recharts 가 **어차피 필요**하고, pie/bar 를 한
컴포넌트의 `variant` 로 겸하려면 bar 도 recharts 여야 설계가 균일하다. 가젯은 타일 크기가
가변이라 고정 폭 막대 목록보다 차트가 잘 늘어난다.
**대가를 명시한다** — 프로젝트 요약 화면(막대 목록)과 대시보드 가젯(recharts 막대)이 같은
4종 분포를 **다른 모양**으로 보여준다. 시각 일관성을 설계 균일성과 맞바꾼 것이고,
「이미 있는 것을 안 썼다」는 사실을 여기 남긴다. 색은 양쪽 다 같은 차트 색 토큰을 쓰므로
톤은 어긋나지 않는다.
...
## 게이트 2 — 리뷰 2라운드 (렌즈 2종 × 2회)

Maxi 판정 **「수정 후 재리뷰」**. 1차 CONCERNS 6건 반영 → 2차에서 새 지적 5건 → 전부 반영.

### 1차 6건 (반영 완료 · 2차에서 판정 확인)

| # | 지적 | 2차 판정 |
|---|---|---|
| 1 | `parseConfigFields` 가 중첩 디스크립터를 삼킴 (P1 미탐) | 적절 |
| 2 | `DashboardGrid.test.tsx` G-9 가짜 그린 (P1) | 적절 |
| 3 | 내장 프로젝트 select 접근명 부재 (P2) | 적절 |
| 4 | 공유받은 필터를 못 고름 (P2 기능 축소) | 적절(잔여 3건은 2차에서 처리) |
| 5 | `gadgetLabels` JSDoc 고아 (P3) | 적절 |
| 6 | MSW summary 관대함 (P3) | **과잉 — 새 결함** |

### 2차 5건 (전부 반영)

**① MSW 가 존재/부재를 뭉쳤다 (P1) — 내가 만든 회귀다.**
1차 지적을 고치며 「ATLAS·`EMPTY` 만 200」으로 좁혔는데 `EMPTY` 는 프로젝트 시드에 없는 키다.
같은 PR 이 `projectKey` 를 드롭다운으로 바꿔 목록 밖 키를 **타이핑할 수단까지 없앤 뒤**라,
이번엔 반대로 **빈 상태 3종이 UI 로 도달 불가**가 됐다.
→ 백엔드를 그대로 미러한다. 시드 안(ATLAS·MIDDLE·ZETA)이면 200(데이터 없으면 빈 요약),
  시드 **밖**이면 404. E4 도 살고 빈 상태도 도달 가능해진다.
★교훈. 「관대함」을 고칠 때 **반대편으로 넘어가지 않았는지**를 같은 호흡에 확인해야 한다.
  유닛 11,322 이 전부 초록이었다 — 가젯 테스트가 훅을 목하므로 기계가 못 잡는 자리다.

**② 같은 공허 단언 4건 잔존 (P2).** `DashboardTile.test.tsx:148,310,360` · `DashboardGrid.test.tsx:302`.
G-9 를 고친 **같은 커밋이 같은 파일의 G-15 를 지나쳤다.** 전부 접근명 기반으로 교체.

**③ 공유 필터가 조용히 잘리는데 하필 최신이 잘린다 (P2).**
백엔드가 `created_at ASC` 라 page 0 은 **가장 오래된** 목록이다 — 「방금 공유받아 가젯에
얹으려는」 가장 흔한 동선이 정확히 잘리는 자리다. 잘림 안내를 띄우고, 페이지 크기를
`SavedFilterMenu` 와 같은 정본 상수(spec FR-3/EC7 의 50)로 통일했다(종전 100 — 화면마다
잘림점이 달랐다).

**④ 조회 실패가 「공유받은 게 없다」와 구분 안 됨 (P3).** `shared.isError` 안내 추가.

**⑤ 파서가 named 인자 순서에 따라 key 를 오독 (P3 잔여).**
`key =` 를 optional 로 두면 인자 목록의 **아무 문자열 리터럴**이나 key 로 읽는다 —
`type → required → enumValues → key` 순으로 재배열하면 `enumValues` 의 `"status"` 를 key 로
방출하고 개수는 그대로라 **등식도 통과한다.**
→ 「다음 출현까지」 창도 버리고 **괄호 균형**으로 인자 목록만 정확히 자른다.
  창 방식은 `url` 항목이 다음 디스크립터까지 뻗어 그 사이 enum 상수의 `key = "pie_chart"` 를
  먼저 잡는 것을 실측했다(고치는 과정에서 발견).

### 뮤테이션 검증 (전부 GREEN 선커밋 뒤)

| 시나리오 | 결과 |
|---|---|
| `itemSchema` 첫 항목 → `targetBoardId`(UUID) | **EXIT=1** · 「선택기가 없는 스코프성 필드: targetBoardId」 |
| named 순서 뒤집기(`key` 를 `enumValues` 뒤로) | **EXIT=1** (종전 파서에서는 초록이던 시나리오) |
| G-9 을 `canEdit:true, isEditing:true` 로 뒤집기 | **EXIT=1** (종전 20 passed) |
| MSW 픽스처 `pie_chart` → `enabled: false` | **EXIT=1** |

### A9 눈확인 — 최종 코드로 **재측정**

①이 빈 상태 도달성을 바꿨으므로 다시 쟀다. `55b8a8fee` 가 세운 「추출 전에 잰 것은
최종 코드의 것이 아니다」와 같은 규율이다.
빈 상태 3종(「표시할 이슈가 없습니다」·「활성 스프린트가 없습니다」·「아직 활동이 없습니다」)이
ZETA 로 도달 가능하고 라이트/다크 양쪽 정상.

### 최종 검증

| 항목 | 결과 |
|---|---|
| 판별식 전량 | **EXIT=0** · 621/621 |
| vitest (dashboard·search·mocks·i18n) | **EXIT=0** · 2,363 passed |
| E2E 대시보드 2종 **3회 연속** | **EXIT=0 ×3** · 18 passed |
| tsc · eslint | **EXIT=0** |
