...
<!-- 미충전. 통과 문구를 미리 심지 않는다 — 심으면 이후 모든 plan 이 가드를 자동 통과한다
     (jira-research-guard.test.ts:97 은 허용 도메인 URL 또는 「대응 없음 + ADS/준용」을 요구).
     /bts-spec 이 실제 근거로 채울 때까지 이 한 건은 판별식에서 red 로 남는다 — 의도된 상태다. -->
