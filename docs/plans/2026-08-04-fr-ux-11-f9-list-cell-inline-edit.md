...
- **★보드 훅에 같은 drift 가 있다 (이 PR 범위 밖).** `hooks/use-change-card-field.ts:264-268`
  ...
  *"…변경 중 **오류가** 발생했습니다. **잠시 후** 다시 시도해 주세요."* 다.

  **출처가 중요하다.** 리뷰(C5)는 목록 훅이 문구를 *"새로 지었다"* 고 보았지만, 실제로는
  **이 보드 훅을 복제**한 것이다 — 목록 훅은 설계 단계부터 `useChangeCardField` 를 형제
  선례로 삼았고 문구까지 따라왔다. 즉 drift 의 발원지는 보드다.

  목록(F9)만 정본으로 맞췄으므로 **지금은 보드만 어긋나 있다.**
  `board-swimlane-field-change.spec.ts:524` 가 그 문구를 텍스트로 단언하므로 보드 훅을
  고칠 때 **그 e2e 도 같은 PR 에서 함께** 갱신해야 한다.

### E. 목록과 상세의 필드 권한 엄격도 불일치 — 통일 방향 미결 (리뷰 C2 봉합 중 발견)

- **상세 `IssueMetaPanel` 은 `priority` 에 필드 권한을 걸지 않는다.** `isFieldDisabled` 를
  거는 곳은 `environment`(`:288`) · `labels`(`:300`) · `assigneeId`(`:315`) 셋뿐이고
  우선순위 셀렉터(`:268-271` `IssuePrioritySelect`)에는 게이트가 없다. 그런데 `priority` 는
  **유효한 필드 키**다 — `IssueMetaPanel.fieldperm.test.tsx:356` 이 `restrictedFields:
  ['priority']` 로 숨김을 검증한다(숨김만 있고 편집 잠금은 없는 상태).
- **결과.** F9 가 목록 우선순위 셀에 필드 권한을 걸었으므로, 이제 **목록 = 엄격 · 상세 =
  느슨**이다. 관리자가 `priority` 를 편집 불가로 잠그면 목록에서는 막히고 상세에서는 열린다.
- **어느 쪽으로 통일할지는 제품 판단이라 이번에 정하지 않는다.** 후보는 둘 —
  ① 상세도 `priority` 에 `isFieldDisabled` 를 건다(목록에 맞춘다)
  ② 목록에서 `priority` 게이트를 뺀다(상세에 맞춘다).
  ①이 필드 권한 기능의 취지에 맞아 보이나, 상세 화면 변경은 F9 범위 밖이다.

### F. 가드 미배치 — **해소됨** (리뷰 C3 후속에서 controller 가 파일을 열어 봉합)

- ~~`useUsersByIds` 의 `keepPreviousWhileIdsChange` 옵션에 직접 가드가 없다~~ →
