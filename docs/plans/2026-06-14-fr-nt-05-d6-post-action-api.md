# FR-NT-05 D6 선행 PR-A — 워크플로우 전환 post-action CRUD API
...
FR-NT-05 D6(워크플로우 post-action 설정 프론트 UI)의 백엔드 enabler. 현재 post-action(CALL_WEBHOOK의 url/method 등)은 `YamlSeedService` → `workflow_post_actions`(V200) 부팅 시드 전용이고 **런타임 편집 엔드포인트가 없다**. 관리자가 전환별 post-action을 추가/수정/삭제할 수 있는 REST API(GET/POST/PUT/DELETE `.../transitions/{transitionKey}/post-actions`)를 admin 권한으로 신설한다.
...
- **BC: project-workflow** (코드 실측 확정). 신규 용어 없음(post-action·전환은 기존 유비쿼터스 언어).

### Q1 (핵심) — YAML seed가 런타임 편집을 덮어쓰는가? **YES, 소실됨** (에이전트 오판 정정)
`YamlSeedService.isDirty`(L313-323)는 `differsInPostActions`(L384-394) 포함. **YAML 정의 전환별로 DB post-action type 목록 vs YAML 목록 비교**: `dbTypes != dtoTypes`. 런타임 API로 YAML 시드 전환에 post-action을 추가하면 DB=[CALL_WEBHOOK]≠YAML=[] → isDirty=true → `deleteWorkflow`(L495, CASCADE)→reinsert → **런타임 post-action 전멸**(부팅마다). → **seed↔런타임 공존 설계가 이 PR의 핵심 결정**(spec 게이트에서 Maxi 확정).
...
- 전환 자연키=`(workflow_key, from_state_key, to_state_key)`. post-action 식별=id(UUID). jOOQ `WorkflowPostActions.kt` 생성됨, 스키마 변경 시 init_codegen 미러.
...
3줄 요약.
- project-workflow에 전환별 post-action CRUD REST API(`.../transitions/{transitionKey}/post-actions`) 신설, MANAGE_SCHEME+Global Guard.
...
> 패키지 `com.bts.workflow`. 전환 식별=(workflowKey, transitionKey=`from__to`). 권한=WorkflowSchemeController 패턴 복사.
...
**RED** (Testcontainers, V200 마이그레이션 + 전환 시드). findByTransitionId 빈 목록·insert→조회·update→반영·deleteById→삭제·displayOrder ASC 정렬.
...
### Task 2. 전환 해석 + PostActionAdminService (검증·캐시무효화)
...
**GREEN**. transitionKey `from__to` 파싱→workflowKey로 transition_id 조회(기존 repository/조회 재사용). `DefaultWorkflowPostActionFactory.create(type,config)`로 검증(예외→400 매핑). CALL_WEBHOOK url http/https 스킴 추가 체크. 캐시 무효화 불필요(post-action은 전환 실행 시 DB 직접 조회, WorkflowCache 비캐시 대상 — codereview CONCERN-1).
**REFACTOR**. 예외 계층 + KDoc.
