...
계약 §1 은 근거 도메인 5종만 인정한다. 그런데 **이 PR 이 고치는 것은 화면 «배치»이고, Atlassian
문서는 배치를 서술하지 않는다.** 실측 — `manage-and-customize-the-project-navigation`
(조회 2026-09-07)은 탭의 이름 변경·추가·재정렬·제거만 다루고 스페이스 제목의 위치, 탭 목록의
상하 관계, 제목행 우측 액션을 한 줄도 적지 않는다. `navigate-to-your-work`(같은 날 조회)도
사이드바·단축키까지고 스페이스 내부 상단 구조는 없다. `what-is-the-calendar-view` 계열 URL 은
404 다.

그래서 근거를 **둘로 갈라 적는다.** 섞으면 「Jira Cloud 문서가 그렇게 말한다」는 하지 않은 주장이
되고, 계약 §1-4 가 금지하는 바로 그것이다.

| 종류 | 무엇을 지지하는가 | 검증 가능성 |
|---|---|---|
| **문서** (`support.atlassian.com`) | 탭 «집합» — 목록·캘린더가 스페이스 탭이라는 것 | URL 로 재확인 가능 |
| **실물 스크린샷** (Maxi 첨부 2026-09-07 · `Deeps Kanban` 스페이스) | 탭 «배치» — 제목이 위, 탭이 아래 | 재확인 불가. 아래 X-J5-15 참조 |

### 문서 근거 (조회일 2026-09-07 · Cloud)

| # | 항목 | 원문·근거 | 출처 |
|---|---|---|---|
| J5-12 | 목록은 **제거할 수 없는 스페이스 탭**이다 — 뷰가 스페이스 안에 산다 | "Note that the List and Apps tabs can't be removed" | [manage and customize the project navigation](https://support.atlassian.com/jira-software-cloud/docs/manage-and-customize-the-project-navigation/) |
| J5-13 | 착지 탭을 스페이스 단위로 정한다 — 탭 목록의 소유자가 스페이스라는 것 | "the tab that all members in your space will land on" | 〃 |

**승계 (계약 §1-0)** — 아래 3건은 캠페인 PR ⑤ 가 2026-09-03 에 조회해
`apps/web/src/components/project/project-view-tabs.ts` KDoc 에 남긴 것을 **URL·조회일 그대로**
가져온다. 이 PR 은 그 탭 집합을 바꾸지 않으므로 재조회 대상이 아니다.
...
| J5-2 | 백로그는 보드와 형제인 독립 탭이다 | [use your scrum backlog](https://support.atlassian.com/jira-software-cloud/docs/use-your-scrum-backlog/) | 2026-09-03 |
| J5-3 | 백로그를 켜면 스페이스 네비게이션에 탭이 생긴다 | [enable the backlog](https://support.atlassian.com/jira-software-cloud/docs/enable-the-backlog/) | 2026-09-03 |

### 실물 근거 (Maxi 첨부 스크린샷 · 2026-09-07 · Cloud `Deeps Kanban`)

| # | 항목 | 스크린샷이 보여 주는 것 |
|---|---|---|
| J5-8 | 스페이스 이름이 **탭바 위**에 온다 | `스페이스 / BORADEEPS` 브레드크럼 → `Deeps Kanban` 제목행 → 탭바 순서 |
| J5-9 | 제목행 우측이 **액션 영역**이다 | 공유·자동화·가져오기·전체화면 아이콘이 제목과 같은 줄 |
| J5-10 | 탭을 바꿔도 제목행이 **그대로 남는다** | `목록` 탭이 활성인 상태에서도 `Deeps Kanban` 이 위에 있다 |
| J5-11 | 뷰는 자기 제목을 **다시 쓰지 않는다** | `목록` 탭 본문에 「목록」이라는 제목이 없다 — 곧바로 뷰 툴바 |
...
- **X-J5-15 (신설) — J5-8~J5-11 의 근거가 URL 이 아니다.**
  계약 §1 은 근거 도메인 5종을 요구하는데, 그 도메인 어디에도 **화면 배치**를 적은 문서가 없다
  (위 §근거의 두 종류의 실측). 그래서 이 네 항목은 Maxi 가 첨부한 Cloud 실물 스크린샷을
  근거로 삼는다. **재확인이 불가능하다는 것을 여기 적어 둔다** — 배치를 다시 논할 때는 문서가
  아니라 실물을 다시 봐야 한다. 계약 §1 을 고치자는 제안이 아니라, 이 PR 이 그 규칙의
  사각을 밟았다는 기록이다.
