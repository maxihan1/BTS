# FR-IS-01 상태전환 PR 2/2 — 전환 UI(프론트) + Playwright E2E
...
PR #38(백엔드 가용전환, 머지됨)의 프론트 짝. 이슈 상세 화면이 현재 상태를 읽기전용 배지
(`IssueMetaPanel.tsx`, `data-testid="issue-state-badge"`)로만 보여주는데, 여기에 **상태 변경 컨트롤**
(shadcn select/dropdown)을 추가한다.

- `GET /api/v1/issues/{key}/transitions` 로 가용전환(서버 권위, validator/조건/권한 평가됨)를 받아 노출.
- `POST /api/v1/issues/{key}/transition` (body `{toStatusKey, expectedVersion}`) 로 전환 실행.
- 백엔드 계약: `GET .../transitions` → `{data:{transitions:[{fromStateKey,toStateKey,name,key}]}}`,
  key=`${fromStateKey}__${toStateKey}`. 에러 404(전환 없음)/422(워크플로우 미설정).
  전환 실행 에러: 409(expectedVersion 불일치 = 낙관적 잠금 충돌).
...
i18n(409/422 에러 메시지) + Playwright E2E(happy 전환 / 가용전환 필터 / 에러).

분류: classify가 qa로 판정했으나(제목 "E2E" 키워드) 본질은 전환 UI 신규 구현 → Maxi 결정으로 ui 재분류.
...
- **가용전환(available transition)** — 현재 상태에서 실행 가능한(서버가 validator/조건/권한 평가) 전환.
- **상태 키(state key)** — 소문자(`open`/`in_progress`/`closed`). 백엔드 resolved 소문자 그대로 사용(대문자 잔재 없음, PR #28 정렬).
- **전환 동일성 = (fromStateKey, toStateKey)** — ADR 2026-05-28-workflow-transition-identity-policy. `name`은 표시용, 식별 사용 금지.

신규 ADR 없음. 도메인 결정은 PR #38 spec에서 종결(옵션 A 풀버전 = 서버 권위 가용전환).

## 스펙 (기존 docs/specs/2026-05-29-fr-is-01-transition.md 활용)

전체 FR-IS-01 스펙은 기존 문서에 충실히 존재. **이번 PR 2/2 범위 = 프론트 + 프론트 E2E**.
- 포함: **FR-T-F1~F5**(전환 컨트롤 UI / 전환 실행+캐시무효화 / api+Zod / MSW stateful / 에러 i18n) + **FR-T-Q1**(Playwright E2E happy·필터·에러).
...
POST /api/v1/issues/{key}/transition
  ...
  404 / 409(전환거부 transition_not_allowed · 낙관락 버전충돌) / 422
```
**재사용 자원.**
- 프론트 `workflowTransitionViewSchema`(`apps/web/src/api/workflows.ts:24`)가 `{fromStateKey,toStateKey,name,key}` 동일 형태 — 전환 Zod 스키마 정합 기준(메모 `frontend-zod-backend-dto-contract-gap` 충족).
- 이슈 상세는 D6 타입변경 mutation에서 이미 `expectedVersion=issue.version` + `ApiError status===409` + TanStack Query 캐시무효화 패턴 보유(`issues.$key.tsx`) — 전환도 동일 패턴 차용.
- `IssueResponse.currentStateKey`/`version` 존재 → 별도 상태 조회 불필요.

UX 시나리오는 기존 spec S1~S6 그대로(happy / 가용전환만 노출 / 409 거부 / 409 버전충돌 / 422 미설정 / 종료상태 전환0건). NFR: WCAG AA(44px·키보드), 전환 중 disabled(중복클릭 방지), 폴링 금지.
...
### Task 1. api 클라이언트 + Zod 스키마 (전환 조회/실행)
...
**GREEN** (`issues.ts`).
- ⚠️ `issues.ts`는 **기존 파일**(fetch/create/update/delete CRUD 보유). 덮어쓰지 말고 **전환 함수/스키마만 추가**.
...
### Task 2. MSW stateful 핸들러 (전환 조회 + 전환 실행)
...
- `GET /api/v1/issues/:key/transitions` — 현재 상태 기준 가용전환 반환. **⚠️ 전환 맵은 `workflow-fixtures.ts`의 `software-default` 정본을 단일 출처로 차용**(전환 invent 금지 — 리뷰 BLOCKER). 정본:
...
- `POST /api/v1/issues/:key/transition` — `{toStatusKey, expectedVersion}` 수신. **분기순서 백엔드 일치(메모 `e2e-msw-serviceworker-block`)**: 404(미존재 key) → 422(`MOCK_NO_WORKFLOW_TRIGGER` 미설정) → 409(`expectedVersion`≠현재 version = **버전충돌**, body `errorCode` 구분 / `MOCK_CONFLICT_TRIGGER` = **전환거부 transition_not_allowed**, body `errorCode` 구분) → 200(state 갱신 + version+1). 전환 후 후속 `GET /:key`·`/transitions`가 새 상태/새 가용전환 반영(stateful).

**GREEN** (`issue-handlers.ts` + `issue-fixtures.ts`).
- `issue-fixtures.ts`에 상태→가용전환 맵 추가 — **`workflow-fixtures.ts` softwareDefault 전환에서 파생**(중복 정의 금지, helper로 from-state 필터). **S6 검증용 `closed` 이슈 fixture 추가**(현재 fixture는 open/in_progress/done뿐, 종료상태 없음).
- `getTransitionsHandler`, `transitionHandler` 추가 + `issueHandlers` 배열 등록(`...issueHandlers`로 자동 합류, 별도 등록 불필요). 409 응답 body에 `errorCode`(`transition_not_allowed` vs `version_conflict`) 포함 — UI 분기용. `resetIssueState()`가 전환 상태도 초기화.

**REFACTOR**. 가용전환 맵을 fixture helper로(상태→전환 배열), `updateIssueHandler` 409 패턴과 분기 일관.

**검증**. `pnpm test -- src/mocks/__tests__/issue-handlers.test.ts`.

### Task 3. use-issue-transitions 훅 (가용전환 useQuery + 전환 useMutation)
...
- `useIssueTransitions(key)` — `fetchIssueTransitions`로 가용전환 useQuery(`['issue-transitions', key]`).
- `useTransitionIssue(key)` — `transitionIssue` useMutation. onSuccess 시 `['issue', key]` + `['issue-transitions', key]` 캐시 무효화(상태 배지·가용전환 갱신).
...
### Task 4. 전환 컨트롤 UI (IssueMetaPanel 표시 + issues.$key 배선) + i18n 에러
...
- **`issues.$key.tsx`(라우트)가 소유** — `useIssueTransitions(key)`(가용전환 조회) + `useTransitionIssue(key)`(전환 mutation) 호출, `expectedVersion=issue.version` 전달, 성공 시 캐시 무효화, 409/422 `ApiError` 분기 처리. `IssueMetaPanel`에 `transitions`/`onTransition`/`isTransitioning`/`transitionError` props로 전달.
- **`IssueMetaPanel`(표시 컴포넌트)** — 순수 표시. `IssueTypeSelect`(네이티브 `<select>`, IssueMetaPanel.tsx:124+)와 동일 패턴의 `IssueStateTransition` 서브컴포넌트로 가용전환 노출 + 선택 시 `onTransition(toStateKey)` 콜백.

**RED** (`IssueMetaPanel.test.tsx` + `issues.$key.test.tsx`).
- 상태 배지(`issue-state-badge`) 영역에 전환 컨트롤 렌더, 가용전환 노출, 선택 시 `onTransition(toStateKey)` 호출.
- 전환 중 컨트롤 `disabled`(중복클릭, NFR3). 가용전환 0건(종료상태 S6, `closed`) → 컨트롤 비노출/비활성 + "더 진행할 전환 없음" 안내.
- 라우트: 전환 선택 → `transitionIssue({toStatusKey: toStateKey, expectedVersion: issue.version})` 호출 + onSuccess 캐시 무효화.
...
**GREEN**. `IssueStateTransition` 서브컴포넌트(IssueMetaPanel) + 라우트 mutation/에러 배선(issues.$key.tsx) + `ko.ts` 전환 라벨·409(2종)·422 문자열.
...
- **S1 happy**: 이슈 상세 진입(`open`) → 전환 컨트롤에서 "Start Work(→in_progress)" 선택 → 상태 배지 `in_progress` 갱신 확인. (전환 라벨은 정본 영문 — workflow-fixtures 일치.)
- **S2 가용전환 필터**: `open`에서 컨트롤 열면 open 출발 전환만(Start Work/Cancel), `in_review` 직행 선택지 없음 확인.
...
- **[BLOCKER] T2 전환 맵 권위 불일치** → `software-default` 정본(`workflow-fixtures.ts`)으로 교체. `in_progress→closed` 가짜 전환 제거(in_progress 출구는 in_review 1개), 영문 name 통일, S6용 `closed` 이슈 fixture 추가. ✅ 반영.
...
- **[CONCERN] 전환 name 언어** → 정본 영문 통일, E2E 셀렉터 일치. ✅ 반영.
- **[정정] `issues.ts` "신규 파일" 오인** → 기존 파일에 전환 함수 추가로 정정(CRUD 덮어쓰기 방지). ✅ 반영.
- design-review skip 사유: 전환 컨트롤이 D6 `IssueTypeSelect`(네이티브 select) 패턴·기존 DESIGN.md 컨벤션 그대로 차용, 새 비주얼 결정 없음(메모 `bts-review-plan-autoplan-overkill` 정신).
