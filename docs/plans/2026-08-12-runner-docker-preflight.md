...
`/code-review high 378` 이 8건을 올렸고 각각 실측 확인 후 전부 반영했다 (`fix: … 독립 리뷰 8건 반영`).
**세 건은 이 PR 이 없애려던 결함과 같은 종류를 새로 만들고 있었다.**

| # | 지적 | 왜 심각한가 | 처방 |
|---|---|---|---|
| 1 | 장부 헤딩이 `## ⬜ … (신규 · 미착수)` 그대로 | `debt-ledger-mapping.test.ts` 의 `OPEN_MARKER` 와 대시보드가 **헤딩만** 본다 — 해소해도 계속 미해결로 집계된다 | `## ✅ … (해소 · PR #378)` + 마스터 표 행 |
| 2 | 차단 배너가 **엔진 절(§4)** 만 안내 | 데몬만 꺼진 사람이 node·java 를 판다 — **이 PR 이 없애려는 오진을 가드가 재생산** | 실패한 층이 절 번호를 기록하고 배너가 그것을 안내 |
| 3 | `docker info` 에 **타임아웃 없음** | 이 결함의 실제 시나리오가 「기동 중」이고 그때는 소켓이 있어 **붙잡힌다.** 잡 타임아웃(3분)으로 번져 `require_docker: false` 인 워크플로우까지 `needs` 로 스킵 — **「절대 막지 않는다」가 뒤집힌다** | 배경 프로브 + 초 단위 폴링 (`timeout(1)` 은 macOS 기본이 아니다) |
| 4 | 경고 전용 경로가 스텝 로그로만 나감 | 자원 점검 스텝은 `::warning` + 요약을 쓰고 전용 단언까지 있다. 표출 실패는 판정 부재와 같은 결과 | 어노테이션 + `GITHUB_STEP_SUMMARY` 추가 |
| 5 | `usesDocker` 정규식의 `\.\/` 분기가 **죽어 있음** | `\s+` 를 요구해 `./scripts/x.sh` 를 절대 못 잡는다 → 배선 누락이 통과 | 인터프리터형과 `./` 형을 갈라서 |
| 6 | `require_docker` 검사가 주석을 안 걷어냄 | `# require_docker: true` 주석만으로 배선 가드가 통과 | `withoutComments()` 적용 (판정 방향 대칭) |
| 7 | INVARIANT `'코드 문제 아님'` 이 공허 | 두 층의 **기존 엔진 배너**가 이미 그 문구를 가져 Docker 블록을 지워도 통과 | Docker 블록에만 있는 문구로 교체 + 타임아웃 상수 추가 |
| 8 | 로컬 층 차단 분기에 **호출자도 테스트도 0건** | `BTS_DOCKER_BIN` 이음매를 심어 놓고 로컬 층 검증에 쓰지 않아 문자열 단언만 남음 | `verify-runner-health.test.ts` 에 4건 신설 |

### ★지적 7 은 내가 이미 한 번 겪은 양식의 재발이다

Task 4 뮤테이션에서 `'docker info'` 가 **복구 안내 문구**에서 만족되는 것을 잡아 고쳤는데,
같은 커밋에 넣은 `'코드 문제 아님'` 이 **기존 엔진 배너**에서 만족되고 있었다. 한 번 진단하고도
**옆줄에서 같은 실수를 반복**했다. INVARIANTS 에 넣을 문자열은 「그 블록에만 나타나는가」를
매번 확인해야 한다 — 낱말이 흔할수록 위험하다.

### 최종 뮤테이션 8종 — 전부 RED

| 뮤테이션 | 잡은 케이스 |
|---|---|
| M1. nginx 데몬 점검 삭제 | 데몬 부재를 문법 오류로 오진하지 않는다 |
| M2. 종료 코드 `8` → `5` | 〃 |
| M3. `backend-ci` 배선 제거 | Docker 를 쓰는 워크플로우는 넘긴다 |
| M4. `frontend-ci` 에 과잉 추가 | 음성 대조군 |
| M5. 로컬 층 판정 삭제 | 로컬 층 4 케이스 전부 |
| M6. 런북 인용 `§7` → `§5` | 인용 절 실재 · 배너가 §7 을 가리킨다 |
| M7. 프로브 타임아웃 제거 | 응답 없으면 타임아웃으로 끊는다 |
| M8. 배선을 **주석으로만** 남김 | Docker 를 쓰는 워크플로우는 넘긴다 |

**★M3 과 M8 은 다른 결함이다** — 「지웠다」와 「주석 처리했다」. 6번 지적 전에는 M8 이 통과했다.
