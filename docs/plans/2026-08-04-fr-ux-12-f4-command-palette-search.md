...
## 게이트 2 — 독립 코드리뷰 결과와 봉합

**★ 이 리뷰가 이 PR 의 유일한 독립 시각이었다.** plan 단계 outside voices 2종을 codex 미설치로
못 돌렸다. 그리고 **그 유일한 시각이 BLOCKER 를 찾았다** — 안 돌렸으면 그대로 머지될 뻔했다.

### 🛑 BLOCKER — `escapeAqlString` 이 이스케이프가 아니라 **파괴**였다

`AqlLexer.scanQuotedString` 에 **백슬래시 분기가 없어** 여는 `"` 이후 첫 `"` 에서 무조건 닫힌다.
`isIdentStart` 도 `\` 를 안 받아 「인식할 수 없는 문자」로 죽는다.

| 사용자 입력 | 전송 쿼리 | 봉합 전 결과 |
|---|---|---|
| `로그인"버그` | `text ~ "로그인\"버그"` | **400 THROW** |
| `C:\Users\temp` | `text ~ "C:\\Users\\temp"` | 통과하되 검색어가 `C:\\Users\\temp` — **오류 없이 조용히 0건** |

**★★ 이 PR 이 고치려던 결함 양식이 한 칸 옆에서 재발했다.** 3중으로 가려져 있었다 —
①프론트 테스트 이름이 「**유효한 AQL 이 된다**」인데 실제로는 **문자열 모양만** 쟀다
②MSW 목의 `isSyntacticallyValidAql` 이 `text ~` 접두사만 보고 **파싱 가능성은 안 봤다**
③E2E 는 `navigate` 인자만 검증. **인젝션은 불가**(리뷰어가 렉서를 포팅해 실행 증명 — 삽입된 `\` 가
항상 문자열 밖에 떨어져 즉시 LexException).

**처방 = B(백엔드 렉서에 이스케이프 추가), Maxi 확정.** 내 추천은 A(프론트에서 제거)였으나
Maxi 가 정공법을 택했다 — AQL 검색 페이지 손입력까지 함께 살아난다.
**FR-UX-12 의 D4/D5 는 「없음」 유지** — 이건 FR-SR-02/04 소유 BC 의 **PRE_EXISTING hot-fix** 다.

**★ 내 근거가 실측으로 뒤집혔다.** 「`plainto_tsquery` 가 구두점을 버리니 `\X` 는 무관」이라 봤는데,
`IssueRepository.kt:2691` 이 FTS 와 **trigram `LIKE '%…%'` 를 OR 결합**(`:2706-2708`)해 후자에서는
역슬래시가 매칭에 관여한다. 그래서 `\X` 는 **리터럴 보존**으로 확정(삭제하면 사용자가 친 글자가
조용히 사라진다). 하위호환도 지켜진다.

**핵심 산출물은 고침이 아니라 크로스 레이어 계약 테스트다.** `AqlLexerTest` 가 프론트
`escapeAqlString` 출력을 박제해 round-trip 을 단언한다 — **두 층 중 하나만 바뀌어도 red**
([[two-lists-never-check-each-other]] 처방). 프론트 테스트 이름도 정직화했다.

### ⚠️ CONCERNS 3건 — 전부 스펙 위반이었고 같이 고쳤다 (Maxi 확정)

| # | 스펙 | 증상 | 봉합 |
|---|---|---|---|
| C-1 | **E8** | 프로젝트 목록 **조회 중**인데 요청을 한 건도 안 보내고 「결과가 없습니다.」 | 훅에 `isResolvingProject` 신설, 그 구간엔 진행 표시 유지 + 탈출구 미마운트 |
| C-2 | **E9** | 조회 **실패(`error`)**를 「프로젝트를 먼저 선택하세요」로 뭉갬. `retry` 소비처 **0건** | `needsProject` 를 `'empty'` 전용으로 좁히고 `projectError:{retry}` 분리 → 「다시 시도」 항목(소비처 0→1) |
| C-3 | **E12** | 공백만 치면 **팔레트가 통째로 빈 화면** | `showQuickLinks` 가 판별 레이어의 `kind==='empty'` 를 쓰게 교체 |

**C-3 이 가장 아프다** — 그 빈 화면이 바로 **정본 §4.10 이 이 FR 을 만든 이유**(「화면이 비고
Enter 도 무반응」)와 **같은 그림**이다. 한글 입력 중 앞 공백이 들어가는 흔한 실수로 도달한다.
**판별 레이어를 만들어 놓고 렌더가 안 쓴 것**이 본질이었다.

**★ 왜 유닛이 못 잡았나 — 가드가 한 층 아래에만 있었다.** 훅 테스트는 `loading` 을 쟀지만
**컴포넌트 테스트는 `loading` 을 한 번도 mock 하지 않았고 `error` 는 0건**이었다. 그래서 새 가드는
전부 **컴포넌트 층**에 세웠다. 계열 — 「유닛 25/25 초록을 실브라우저가 뒤집음」.

### 검증

렉서 **69** + 모듈 전체 **1140/0 실패**(`--rerun-tasks`, XML 로 실행 수 실측) · `detekt` 가
`LoopWithTooManyJumpStatements` 실결함 적발 → 봉합 · 프론트 유닛 **506** ·
E2E `command-palette` **12/12 2회 연속** + 동반 4파일 **30** · 뮤테이션 red **8건**
(백엔드 round-trip 4 + 프론트 C-1/C-2a/C-2b/C-3 4).

### 남은 관찰 (조치 안 함)

- `ActiveProjectGate.tsx` 의 「프로젝트 목록을 불러올 수 없습니다.」/「다시 시도」 문구가 팔레트에
  복제됐다. 그 파일이 "문구는 여기 한 곳에만"이라 선언하고 있어 **drift 위험**이다. 문자열 export
  통합은 별건 — 팔레트 쪽에 출처 주석만 남겼다.
- `AqlParserTest` 에 round-trip 단언 1건 추가 권고(파서는 lexeme 를 그대로 통과시키는 것을 실측
  확인했으나 증인이 없다).
- `useKeyboardShortcuts` 의 effect cleanup 이 leader 를 지우는 구조적 취약점(Task 7 이 증상만 제거).
