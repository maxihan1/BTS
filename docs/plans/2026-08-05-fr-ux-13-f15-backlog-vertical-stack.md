...
- **task 수**. **16** (원안 10 + 계획 리뷰가 적발한 T11 + 구현 중 발견 T12 + **코드 리뷰 BLOCKER 봉합 T13·T14·T15** + **재검증 발견 T16**)
- **wave 실제**. 8 — `W1{T1,T2,T3,T5,T11}` → `W2{T4,T6,T7,T8}` → `W3{T9}` → `W4{T12}` → `W5{T10}`
  → 🛑게이트 2 → `W6{T13,T14}` → `W7{T15}` → 🔍재검증 → `W8{T16}`
  (T12·T13~T16 은 각각 배선·코드리뷰·재검증 이후에야 관측 가능한 결함이라 계획 시점에 존재할 수 없었다)

### Task 13~16 (게이트 2 이후 · 코드 리뷰 BLOCKER 봉합)

| # | 내용 | 커밋 |
|---|---|---|
| **T13** | Esc 취소 복구(`end: [Space]`) · `end`∩`cancel` 공집합 단언 · E2E **S20** · `onDragCancel` 배선 · `collisionDetection` 캡처 | `9160957c8` → `7733e0851` |
| **T14** | 분류 불가 시 완료 차단(`unavailable` 단일 조건) · **위험한 동작을 못박던 E14 테스트 뒤집기** · 409 입력 보존 · **그 분기에 처음 도달하는 하네스** | `c47392790` → `33d0519be` |
| **T15** | 문구 3종 정합 — `workflowLoadFailed`(차단 사실+처방) · `patchConflict`(입력 보존) · **`staleBlocked` 신설**(완료 다이얼로그가 시작 다이얼로그 문구를 빌려 써 새 거짓말이 될 뻔한 것을 스스로 적발) | `6ac4ff3c6` → `7e2cf0ee2` |
| **T16** | **남의 저장분 덮어쓰기 봉합** — 전송 대상을 「기준값과 다른 필드」→「사용자가 편집한 필드」로. ★**GREEN 설계가 공허해 REFACTOR 가 뒤집었다**(폼 state 를 서버값으로 맞추니 두 집합이 다시 일치해 가드가 장식이 됨) → **사본 대신 파생** | `e0aa0a2dd` → `d6436a232` → `3812ea47d` |
...
## 봉합 재검증 (2026-08-06) — BLOCKER 를 찾아낸 리뷰어가 직접 판정

「고쳤다고 한다」를 믿지 않기 위해, **BLOCKER 3건을 낸 리뷰어에게 재검증을 맡겼다.**
그는 코드·dnd-kit 원본을 직접 읽고 **영향 6파일 유닛 207건과 `tsc` 를 자기 손으로 돌렸다**.

**판정 — BLOCKER 3건 전부 닫힘 · 새 BLOCKER 0건 · MINOR-8 닫힘 · 새 CONCERNS 2건.**

| # | 닫힘 근거 |
|---|---|
| B1 | `end: [Space]` 가 `cancel: [Esc]` 와 **서로소**. dnd-kit `:1196-1204` 에서 Esc 가 `end` 를 통과하지 못하고 `handleCancel` 로 내려가 **mutation 경로가 호출되지 않는다**. 스크린리더 안내가 이제 참이다 |
| B2 | `unavailable` 이 `workflows.data === undefined` 와 **동치**라 로딩·실패를 한 조건이 덮는다. **우회로 없음** — 유일한 다른 제출 통로인 재시도 버튼은 이전 실패를 요구하고, 그 실패는 제출이 성사돼야 생기므로 차단 중 도달 불가 |
| B3 | `setValues(toFormValues(fresh))` 소멸 확인. 하네스가 `setQueryData` 로 **프로덕션에서만 도달하던 분기에 처음 진입**한다 |

### 「봉합이 새 구멍을 여는가」 3문항 — 전부 답변됨

- **`end` 가 Space 단독이 된 것.** 새 구멍 없다. `attach()` 가 keydown 리스너를 `setTimeout` 으로
  한 매크로태스크 뒤에 붙여(`core.cjs.development.js:1164`) **활성화시킨 그 keydown 이
  `handleKeyDown` 에 닿지 않는다** — dnd-kit 기본값이 Space 를 양쪽에 두는 이유다
- **완료 차단이 정상 경로를 막지 않는다.** `data !== undefined` 면 항상 `unavailable: false` 이고,
  TanStack Query v5 는 배경 재조회가 실패해도 **기존 `data` 를 유지**하므로 한 번 로드된 뒤엔 다시 잠기지 않는다
- **`version` 갱신 무사.** 짝 단언 `{ version: 9, goal: '내 목표' }` 가 **한 단언에 두 사실**을 묶어 증명한다

### 신규 테스트 4건 — 전부 공허하지 않음 (판정 근거)

- **교집합 단언** — 실제 교집합을 계산해 `toEqual([])` 하고 **두 목록이 모두 비어있지 않음**을 짝으로
  붙였다. 「한쪽이 비면 교집합이 자동 공집합」 함정을 정확히 막았다. `start`∩`end`(둘 다 Space)를
  싸잡지 않고 `end`∩`cancel` 로만 좁힌 것도 옳다 — dnd-kit 설계상 전자는 겹쳐야 한다
- **「DELETE 0건」** — 그 자체는 약하지만 T-CP-2 가 「활성 버튼을 누르면 `calls` 가 찬다」를 이미
  증명하고, 로딩 테스트가 `release()` 후 **같은 버튼이 열린다**를 이어서 재 「항상 잠김」을 배제한다
- **E2E S20 — 이 배치에서 가장 강한 테스트.** 기록기가 엔드포인트를 **열거하지 않고**
  `GET` 아닌 `/api/v1/` 전량을 담아 새 엔드포인트가 생겨도 「0건」이 조용히 참이 되지 않는다.
  `waitForTimeout(500)` 뒤에 세 「아직 안 나갔을 뿐」과 「영영 안 나간다」를 구분하고,
  **같은 테스트·같은 기록기·같은 카드**로 마지막 키만 Space 로 바꿔 기록기 생존을 증명한다
- **`onDragCancel`** — 하이라이트가 걸린 상태를 먼저 만들고 지워지는 것을 잰다. `over` 가 **채워진**
  이벤트를 넘겨서, 핸들러가 `{...event, over: null}` 로 덮지 않으면 red 가 된다

### 🟡 새 CONCERNS-B — **직전 수정이 문제를 옮겼다** (Maxi 확정 2026-08-06 · Task 16 에서 봉합)

`setBaseline(fresh)` 로 기준값만 갈아끼우면서 **「내가 고친 필드」와 「기준값과 다른 필드」가
더 이상 같은 집합이 아니게 됐는데**, `buildPatchBody` 는 후자로 계산한다.

```
1. 내가 시작일만 입력 → 2. Bob 이 goal+endDate 저장(version 2)
3. 내 제출 → 409 → 4. baseline 이 Bob 값으로 교체
5. 재시도 → 내가 안 건드린 endDate·goal 을 null 로 담음
6. 200. Bob 의 저장분 소멸
```

잃는 것이 **미저장 폼 입력 → 이미 저장된 남의 데이터**로 바뀌어 성격이 더 나쁘다.
`api/backlog.ts` 가 3-state partial 을 도입한 목적(*"바뀌지 않은 필드를 습관적으로 실어 보내면
남의 수정을 덮어쓴다"*)을 **한 층 위에서 재현**한 것이다.

**테스트가 못 잡은 이유도 확정됐다** — 픽스처에서 서버가 바꾼 필드와 사용자가 친 필드가
**같은 `goal` 하나뿐**이라 「남이 바꾼 필드를 내가 건드리지 않은」 조합이 **존재하지 않는다**.

### 🟡 새 CONCERNS-A (후속 등록) — 「분류를 모른다」의 나머지 절반

`backlog-completion.ts:94-95` 의 `if (categories === undefined) return true` 는 이번에 손대지 않았다.
`unavailable`(목록 전체 부재)은 막지만 **목록은 왔는데 그 이슈의 상태 키가 목록에 없는 경우**는
똑같이 「미완료」로 분류되면서 버튼이 열려 있다. 결과는 B2 와 동일하다.
도달성은 훨씬 낮다(워크플로우에서 상태를 지우거나 개명한 뒤 그 키를 든 이슈가 남은 경우).
**「사용 중인 상태를 워크플로우에서 제거할 수 있는가」는 미확인.**

### 기존 지적 상태 변화

| # | 상태 |
|---|---|
| MINOR-8 (취소 후 하이라이트 잔존) | **닫힘** ✅ |
| CONCERNS-6 (재검증이 찾은 truncated 가 버튼 미잠금) | **절반 개선** — 버튼 조건은 여전히 prop 기반이나 `staleBlocked` 신설로 문구가 정확해져 오해 소지가 줄었다 |
| CONCERNS-4 · 5 · MINOR-7 · MINOR-9 / C-2 | **그대로 열림** (해당 코드 무변경) |

### 낮은 위험 메모 1건

새 `workflowLoadFailed` 문구가 **「다시 시도」를 부분 문자열로 포함**한다. 완료 다이얼로그는
부분 실패 시 `다시 시도` **버튼**도 그리므로 공존할 수 있다. 현재 조회는 전부
`getByRole('button', …)` 또는 `exact: true` 라 안전하고 `moveFailedError` 가 이미 같은 패턴이라
새 회귀는 아니다. 다만 이 겹침을 지키는 가드는 **없다**.
