...
### 전체 e2e 스위트 — 실패 분류 (2회 실행 + 인과 대조)

| 실행 | 결과 | 실패 대상 |
|---|---|---|
| 1차 전체 | 590 passed / **18 failed** | 9개 스펙에 산재 |
| 2차 전체 | 605 passed / **3 failed** | `board-swimlane-field-change.spec.ts` **만** |
| 단독 재실행(1차 실패분 3개 스펙) | **18/18 passed** | — |

**분류 1 — 15건은 flaky(동시 실행 간섭).** 실행마다 **실패 대상이 바뀌었고** 단독으로는 전부 통과한다. 메모리 `flaky-determination-needs-repeat-not-single-contrast` 가 요구하는 서명("실패 대상이 바뀌면 그게 flaky")과 일치한다. 1회 대조로 끝냈으면 "선재 실패 18건"으로 잘못 기록할 뻔했다.

**분류 2 — 3건은 결정적이지만 PRE_EXISTING.** `board-swimlane-field-change.spec.ts` S1·S2·S7 은 두 실행 모두 실패하고 **단독으로도 재현**된다. 드래그앤드롭 테스트라 *"상단바에 버튼을 추가해 좌표가 밀렸나"* 를 의심했고, **인과를 추측으로 두지 않고 `TopBar` 에서 `<ProjectSwitcher />` 한 줄만 제거해 재실행**했다 — **같은 3건이 그대로 실패**했다. ⇒ 내 변경과 무관하다. 원복 후 클린 확인 완료.

> **★ 이 대조가 없었으면 판정이 반대로 갔을 수 있다.** "새 UI 를 상단바에 넣었고 드래그 테스트가 깨졌다" 는 인과가 그럴듯해서, 추측으로 갔으면 멀쩡한 배치를 되돌리거나 실재하지 않는 좌표 문제를 쫓았을 것이다. **한 줄 제거 후 재실행**이 1분 안에 결론을 냈다.
