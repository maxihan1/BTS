...
| FR-WF-08 | 프로젝트 소유 워크플로우·스킴 (프로젝트 관리자 편집) | 필수 | project-workflow | §2.8 |
...
| FR-MN-03 | 멘션 알림 확장(댓글·생성) + 멘션 자동 watcher | 높음 | issue-tracking | §4.1.3 |
...
- 2026-09-05. **FR-MN-03 신설**(issue-tracking, +1). 합계 144→145. 멘션 알림 확장(댓글·이슈 생성) + 멘션 자동 watcher. `docs/sdd/02-requirements.md` 가 FR-MN-01 을 「본문/**댓글** @멘션」으로 선언했는데 구현 시점(PR #114)에 댓글 기능이 없어 §4.1.1 D4 가 「댓글은 제외」로 이연했고, FR-MN-02 도 「이슈 생성 폼·댓글 멘션 → 후속 FR」로 이연했다 — 그 사이 FR-CO-01/02 로 댓글이 생겼으므로 이 FR 이 **이연된 SDD 범위를 승계**한다(선례. FR-UX-07←FR-UX-05 §4.3). 자동 watcher 는 SDD 미등재 신규이자 **Jira Cloud 편차**다 — Jira autowatch 는 「본인 생성/본인 댓글」만 대상이고 멘션 자동 watcher 는 JRASERVER-27430 으로 2012년부터 미구현(Automation 룰 우회가 표준, 조회일 2026-09-05). Maxi 가 의도적 이탈로 확정.
