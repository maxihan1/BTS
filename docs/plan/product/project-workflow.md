<!-- project-workflow BC — 워크플로우 FSM 8 FR + 워크플로우 FSM PoC + pgmq 트랜잭션 PoC -->
...
> **D6·D7 완료 — 초안 전환·발행·복원 + 이관 마법사 (로드맵 PR 10a·10b, 2026-09-01 ~ 2026-09-02)**.
> D 마커는 완주 단위라(`docs/plan/fr-index.md:295`) 10a 만으로는 닫히지 않았고, 이관 마법사를 낸
> 10b 가 머지되며 닫힌다.
> - **10a 가 낸 것** — 편집기를 **초안 기반으로 전환**했다. 상태 추가·제거·순서와 전환 CRUD 가
>   더는 정규 테이블에 즉시 쓰이지 않고 로컬 초안에 쌓여 자동저장된다. 발행 다이얼로그(이관이
>   필요 없는 경로) · 기본값 복원 · 초안 폐기 · 낙관적 락 충돌 배너.
> - **10b 가 낸 것** — 상태 이관 마법사(사라지는 상태 → 도착지 매핑 · 무조건부 고지)와 진행률
>   폴링·실패 목록. D6b 선행이던 G4(일괄 작업 actor 인증 주체 결선, #425)를 먼저 닫아 폴링이
>   항상 403 이던 차단을 풀었다.
> - **남는 것** — xyflow 다이어그램(D8)뿐이다.
>
...
> 워크플로우·스킴이 전역 자원이라 CRUD 가 전부 SYSTEM_ADMIN 전용이고, 프로젝트 관리자는 이미 배정된 것을 고르기만 할 수 있다. 1,000명 워크스페이스에서 팀마다 다른 절차를 쓰려면 그 팀의 관리자가 자기 프로젝트 것을 직접 만들고 고쳐야 한다. Jira Cloud 도 같은 불변식을 쓴다 — **공유되지 않은 워크플로우만** 프로젝트 관리자가 편집하고, 공유된 것은 전역 관리자 몫이다. 스펙 `docs/specs/2026-09-08-project-owned-workflows.md`.

- [ ] D1. 도메인 — 소유(`projectId: UUID?`)의 의미 · 전역(NULL)=공유 템플릿 · 표준 스킴 보호 불변 (책임. backend-engineer + Maxi)
- [ ] D2. 명세 — 스코프 결정 규칙(소유 유무 → Global/Project) · 권한 매트릭스(CREATE·UPDATE·PUBLISH 허용, DELETE 불허) · 복제가 소유를 싣는 계약 (책임. backend-engineer)
- [ ] D3. 마이그레이션 — `workflows`·`workflow_schemes` 에 `project_id` + 부분 유니크 재편 (책임. db-engineer)
- [ ] D4. 백엔드 — 스코프 결정 · CRUD 판정 전환 · 목록 필터 · `duplicate` 가 소유를 싣는다 (책임. backend-engineer)
- [ ] D5. 백엔드 테스트 — 자기 프로젝트 200 · 남의 프로젝트 403 · 전역 403 · 복제본 소유 확인 (책임. backend-engineer)
- [ ] D6. 프론트 — 프로젝트 설정에 워크플로우·스킴 편집 (기존 `WorkflowEditorPage` 재사용, 신규 편집기 금지) (책임. frontend-engineer)
- [ ] D7. E2E — 프로젝트 관리자가 만들고 고쳐 발행하고, 비-관리자 멤버는 403 안내를 본다 (책임. qa-engineer)

> **shared-kernel 공용 `WorkflowScope` 를 신설한다** (스펙 D8, Maxi 2026-09-08 확정). `WorkflowSchemeScope` 를 일반화하지 않는다 — 이름에 `Scheme` 이 남으면 `WorkflowDefinitionPermission` KDoc 이 경고한 개념 혼동을 그대로 일으키고, 그 경고 때문에 enum 은 이미 둘로 갈라져 있다. 정의 resolver 전용 스코프를 따로 두지도 않는다(두 목록이 서로를 검사하지 않게 된다). ⇒ **D4 를 담는 PR 이 shared-kernel 을 건드리므로 T3 이다.**

> **가짜 그린 주의.** 표준 스킴 시드 4건은 전부 `project_id IS NULL` 이라 **프로젝트 스코프 경로를 한 번도 타지 않고 기존 테스트가 전부 초록이다.** 프로젝트 소유 픽스처 없이는 이 FR 의 신규 경로가 검증 0 회로 머지된다. 부분 유니크도 「컬럼이 생겼다」로는 안 잡히고 같은 키를 전역 1 + 프로젝트 2 + 삭제 1 로 넣어 봐야 한다.

> **D3 실측 — 계획이 세지 못한 파급은 인덱스가 아니라 `ON CONFLICT` 였다.** 착수 전에는 「V206 소프트삭제 인덱스와 조건이 겹치는지」까지만 봤는데, 실제로 깨진 것은 그 인덱스를 **추론 대상으로 삼는 upsert 호출부**였다. 부분 유니크는 `ON CONFLICT` 의 `WHERE` 술어가 인덱스 술어를 함의해야 추론되므로, 술어가 안 맞으면 `there is no unique or exclusion constraint matching the ON CONFLICT specification` 로 죽는다. 두 BC 의 테스트 픽스처 51곳(36파일)이 대상이었다 — `workflows` 는 옛 술어(`WHERE deleted_at IS NULL`)를, `workflow_schemes` 는 술어 자체를 안 적고 있었다(컬럼 UNIQUE 였으므로).
>
> ★★**그 일괄 치환이 스스로 오류를 만들었고 detekt 가 그것을 드러냈다.** 마이그레이션 테스트 3곳(`V207MigrationTest` · `V203ToV206MigrationTest`)은 **중간 타깃에서 돌기 때문에 술어를 붙이면 안 되는 자리**다 — 그 시점엔 `deleted_at`·`project_id` 컬럼이 아직 없다. `V207MigrationTest:148` 주석이 이미 「부분 유니크 술어를 붙이지 않는다」고 적고 있었는데 문자열 매칭이 그것을 덮었다. **같은 SQL 이라도 마이그레이션 시점에 따라 정답이 다르다** — 스키마 문자열을 일괄 치환할 때는 그 자리가 어느 버전에서 도는지를 함께 봐야 한다.
...
- [ ] §2 (FR-WF 8개) 모두 `[x]` 마킹 — **2026-09-08 재실측: 미완 7건(FR-WF-08 D1~D7)**. FR-WF-08(프로젝트 소유 워크플로우·스킴) 신설로 이 게이트가 **세 번째로 열렸다**. 아래 2026-08-18 기록은 그 시점의 것이다. **2026-08-18 재실측: 미완 27건**. WF-01 D1~D7 · WF-02 D1~D7 · WF-03 D1/D2/D4/D5(D3/D6/D7 비해당)까지 18/18 `[x]` 로 닫혀 있었으나, 워크플로우 편집기 FR 4건(WF-04~07)이 신설되며 D 마커 27개가 새로 열렸다. **이 게이트는 2026-07-27 에 한 번 닫혔다가 범위 확대로 다시 열린 것**이다 — 조용히 닫아 두지 않는다.
