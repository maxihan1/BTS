...
- [ ] D1. 도메인 — `statuses` 전역 카탈로그 · `workflow_statuses` N:M · 키 불변 원칙 · 표준 워크플로우 편집 허용 범위 (책임. backend-engineer + Maxi)
- [ ] D2. 명세 — 읽기 API 응답 형태 불변 계약(`GET /workflows/{key}` 는 states[]/transitions[] 유지) · 권한 모델 (책임. backend-engineer)
- [ ] D3. 마이그레이션 — V203 카탈로그 생성 · V204 백필(키당 name/category 유일성 가드) · V205 workflows 컬럼(version/origin/deleted_at/is_locked) (책임. db-engineer)
- [ ] D4. 백엔드 — 워크플로우·상태 CRUD + `YamlSeedService` 부트스트랩 전환 + `WorkflowCache` 무효화 결선 (책임. backend-engineer)
- [ ] D5. 백엔드 테스트 — TDD red-first · 마이그레이션 체인 검증 · 재기동 후 편집분 생존 · 캐시 무효화 누락 감지 (책임. backend-engineer)
- [ ] D6. 프론트 — `/admin/workflows` 목록 + 목록 모드 편집기(상태 추가·제거·순서) (책임. frontend-engineer)
- [ ] D7. E2E — 이름 수정 · 상태 추가 · 재기동 생존 (책임. qa-engineer)
...
- [ ] D3. 마이그레이션 — V206 UNIQUE 해제 · `from_status_id` NULL 허용 · `kind` 추가 · INITIAL 백필(현행 최소 displayOrder 동작 보존) (책임. db-engineer)
...
> **§2 진척**. FR-WF-01 ✅ / FR-WF-02 D1~D5 ✅ 머지 #18 / D6 ✅ 머지 #31 / D7 ✅ 머지 #35 / FR-WF-03 ✅ 머지 #66 (테스트 토큰 정본화 #69) / **FR-WF-04~07 ⬜ 미착수 (2026-08-18 신설)** — 후속. Wave 6(S1~S8 통합테스트 + ADR/SDD) · C1 detekt 정합 · §NFR deferred trigger 도달 시 측정
