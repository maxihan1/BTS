...
- [x] project-workflow §2 (상태 전환 이벤트) 완료 — 2026-07-27 실측: `project-workflow.md` L38~L80(§2 FR-WF 3개) D단계 `[x]` 18건 / 미완 0건, 전환 이벤트 경로 실재 — `RunAutomationPostAction`(`AutomationRequested` 발행 예약) + `V022__pgmq_queue_transition_events.sql`
...
> **★ Webhook 응답 148ms 측정 조건 (PR-C #278 T15).** 팬아웃 100(이슈키 20 × 룰 5, 곱 상한
> `MAX_FANOUT_PRODUCT`와 동일한 최악값) · 60회 · dev postgres(5433) · warm.
> p50 81ms / **p95 148ms** / max 186ms. cold start 포함 시 max 474ms(첫 요청) — 초기 20회 표본에서는
> p95=313ms 였으나 cold start 오염이었다(표본 부족).
> **여유 26%는 팬아웃이 아니라 경합에 대한 여유다.** 요청당 **DB 왕복 103회**(토큰조회 1 + 배달INSERT 1 +
> 룰조회 1 + `SELECT pgmq.send` **100**)이고 왕복당 ~0.79ms 라 지연을 왕복 수가 지배한다. 곱 상한 100이
> 왕복 수를 103으로 하드바운드하므로 **룰·이슈키가 늘어도 200ms를 넘길 수 없다**(최악이 곧 이 측정값).
> 남는 위험은 prod(9-BC 공유 단일 호스트) 경합으로 왕복당 지연이 ~2.6배가 되면 초과한다는 것.
> 해소책은 `pgmq.send_batch`(100→1 왕복)이며 **후속 트랙** — 전환 시 msg_id 반환이 setof 로 바뀌고
> 요청당 100줄인 `automation_execution_enqueued` 로그가 배치 1줄로 바뀌어 실행 추적 로그 계약이 달라진다.
