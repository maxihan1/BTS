...
> **구현 범위 (#219 백엔드 + #220 프론트, Maxi 확정 2026-07-02)**. 백엔드 D1~D5(#219) + 프론트 D6/D7(#220, recharts 라인 차트·E2E). **모듈=agile-planning**(엔드포인트 `/sprints/{id}/burndown`가 Sprint 애그리거트와 co-located, fr-index 논리 BC 라벨 notification-dashboard 유지 — FR-TT 선례). **지표=잔여 추정 시간(초)**(스토리포인트 미구현), **데이터=on-the-fly in-memory**(스냅샷 테이블·스케줄러 없음, SDD §13.6 deviation 동기화). cross-BC 신규 포트 `SprintBurndownLookupPort`(shared-kernel 정의·issue-tracking 구현·agile-planning 소비). **보안=프로젝트 BROWSE + 이슈별 가시성 필터**(`filterVisibleIssueKeys`가 `buildActiveSecureWhere` 재사용, 게이트2 C1). **프론트=전용 라우트 `/projects/$key/sprints/$id/burndown` + 번다운/번업 토글(URL `?view=`)**, WorklogAggregateChart(recharts) 선례 재사용. ADR `2026-07-02-fr-rp-01-burndown-burnup`.
...
- [x] D6. 프론트 UI — recharts 라인 차트 (책임. designer → frontend-engineer) (완료. PR #220 — 전용 라우트·번다운/번업 토글·순수변환 toBurndownSeries·Legend/점선 이중구분·role=img/tablist 접근성)
- [x] D7. E2E (책임. qa-engineer) (완료. PR #220 — apps/web/e2e/sprint-burndown.spec.ts 2 시나리오, SVG 컨테이너 가시성+토글 URL)

### §4.2 FR-RP-02 — 벨로시티 차트

**우선순위**. 필수 | **선행**. §4.1 | **Plan slug**. `report/velocity`

> **구현 범위 (#222 백엔드 D1~D5, Maxi 확정 2026-07-02)**. **지표=추정 시간(초)**(`original_estimate_seconds`, 스토리포인트 미구현 → FR-RP-01 일관), **막대=계획(Commitment, 스프린트 현재 가시이슈 추정합) vs 완료(Completed, DONE 카테고리 추정합)** 2막대. **완료 판정=on-the-fly 현재 상태**(WorkflowStateCatalog DONE, FR-EP-02 선례·스냅샷 없음). **모듈=agile-planning**(엔드포인트 `/projects/{projectKey}/velocity`가 Sprint와 co-located, FR-RP-01 선례·논리 BC 라벨 notification-dashboard 유지). cross-BC 신규 포트 `SprintVelocityLookupPort`(shared-kernel 정의·issue-tracking 구현). **보안=프로젝트 BROWSE + 이슈별 가시성 필터**(`filterVisibleIssueKeys` 재사용, 기밀 이슈 누출 0). 신규 스키마 0. **부수 성과**: TDD가 `WorkflowStateCatalog.listStates`(MANDATORY) 예외의 공유 트랜잭션 오염 버그를 표면화 → `IsolatedWorkflowStateLookup`(REQUIRES_NEW) 격리로 해결. ArchUnit 룰2가 FR-RP-01 번다운 어댑터의 캐시 마스킹된 jOOQ 위반까지 적발 → 두 어댑터 jOOQ를 repository로 추출. 프론트 D6/D7 완료(#223 — recharts 바 차트·평균 참조선·E2E). ADR `2026-07-02-fr-rp-02-velocity`.
...
- [x] D6. 프론트 UI — recharts 바 차트 (책임. designer → frontend-engineer) (완료. PR #223 — 전용 라우트 `/projects/$projectKey/reports/velocity`·백로그 nav 진입·순수변환 toVelocitySeries·계획/완료 그룹 막대 + 평균 참조선 2개(수치 병기)·role=img 접근성·FR-RP-01 미러)
- [x] D7. E2E (책임. qa-engineer) (완료. PR #223 — apps/web/e2e/project-velocity.spec.ts 2 시나리오, 차트 진입+빈 상태·번다운 회귀 0)
...
- [x] D6. 프론트 UI — recharts 영역 차트 (책임. designer → frontend-engineer) (완료. PR #227 — 전용 라우트 `/projects/$projectKey/reports/cfd`·백로그 nav 진입·순수변환 toCfdSeries·누적 영역 3띠(DONE 바닥→IN_PROGRESS→TODO, 색 triad #6366f1/#f59e0b/#cbd5e1)·isCfdEmpty 빈판정·allowDecimals=false 정수축·role=img 접근성·FR-RP-02 미러)
- [x] D7. E2E (책임. qa-engineer) (완료. PR #227 — apps/web/e2e/project-cfd.spec.ts 2 시나리오, 백로그 nav 진입+빈 상태·번다운/벨로시티 회귀 0)
...
- [x] D6. 프론트 UI — 히스토그램 + 박스플롯 (책임. designer → frontend-engineer) (완료. PR #231 — 전용 라우트 `/projects/$projectKey/reports/cycle-time`·백로그 nav "사이클/리드 타임" 진입·Cycle/Lead 세로 스택 섹션·요약 타일(중앙값 p50 강조)·히스토그램 recharts BarChart(순수 toHistogram binning)·**박스플롯 커스텀 SVG**(상자 p25~p75·중앙선 p50·수염 min~max, boxPlotScale 순수 함수, recharts 박스플롯 미지원 → FR-TL-01 자체 SVG 선례)·formatDuration 초→사람친화 단위·isCycleTimeEmpty 빈판정(둘 다 count=0)·기간 피커 없음(기본 30일 창, 형제 리포트 동형)·role=img 접근성·Zod 백엔드 DTO 1:1·CFD 미러)
- [x] D7. E2E (책임. qa-engineer) (완료. PR #231 — apps/web/e2e/project-cycle-time.spec.ts 4 시나리오: 백로그 nav 진입·정상 렌더(히스토그램·박스플롯 role=img 가시)·빈 상태·403, 섹션 h2 스코핑으로 strict-mode 회피·번다운/벨로시티/CFD 회귀 0)
