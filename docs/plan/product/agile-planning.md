...
> 자기 `?board=` 를 들고 뷰 전환 nav 가 그것을 안 싣는다. **✅ PR ⑥(#432)이 링크 전파까지 해소했다
> (2026-09-03)** — 빈 상태 CTA 와 뷰 전환 nav 가 보드 스코프를 싣는다. 공유 상태
> (FR-UX-07 활성 프로젝트 컨텍스트 관계 정리)는 **여전히 범위 밖**이고, 「마지막으로 본 보드」
> 기억의 부재도 그 뿌리라 `TODOS.md` 164 로 남아 있다.
