...
catch 는 `@Transactional` 경계 **밖**이라 실패분이 롤백된 뒤 로그만 남도록 **설계**했다.
**단 롤백 자체는 테스트로 실증하지 않았다** — 부분 실패를 강제하려면 시더 내부에 주입점이 필요한데
협력자 2종이 final 이라 만들 수 없다. 잔여 위험 6 참조.
...
5. **`@Transactional` 롤백 미실증** — D5 의 "실패분은 롤백된다" 는 설계 의도이며 테스트가 없다.
   부분 실패 시 `users` 만 남는 상태가 이론적으로 가능하나, 그 경우에도 다음 부팅의 멱등 보정이 채운다.
6. **교차모델 리뷰 부재** — `codex` CLI 미설치 + 에이전트 호출 금지 지시로 outside voice 를 돌리지 못했다.
   #308·#309·#310 과 동형의 잔여 위험.

## 코드리뷰 자체 적발 (게이트 2, 2026-07-29)

**`runCatching` 이 `Error` 까지 삼켰다.** Kotlin `runCatching` 은 `Throwable` 을 잡는다 —
`OutOfMemoryError`·`StackOverflowError` 같은 JVM 치명 오류도 삼키고 부팅을 계속시킨다.
dev 편의 기능이 치명 오류를 감추면 "왜 이상하게 도는지" 를 아무도 진단할 수 없다.
⇒ `try/catch (ex: Exception)` 로 교체해 `Error` 는 전파시킨다.
회귀 가드 = `NonProdDevSeedRunnerTest.시드가 Error 를 던지면 전파시킨다`.

detekt `TooGenericExceptionCaught` 는 `@Suppress` + 근거 주석으로 처리했다 — 잡을 예외를 열거하면
새 실패 유형이 생길 때마다 목록이 낡아 조용히 부팅을 깬다. `Error` 를 제외한 경계가 억제의 근거다.
