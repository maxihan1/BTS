...
**1차 처방(스페이서 복원)은 2차 리뷰가 다시 뒤집었다 — 최종안은 `ml-auto` 다.**

- **1차.** 검색 컨테이너 직후에 `<div className="flex-1" />` 복원 → 우측 여백 **1046px → 12px** 해소.
- **★ 2차 리뷰 CONCERNS-1 이 이걸 뒤집었다.** `flex-1` = `flex: 1 1 0%` 라 basis 0 · grow 1 형제가
  둘이면 잔여 공간을 **정확히 반씩** 나눈다. 검색칸이 `max-w-md`(448px)에 **≳1334px 부터만**
  도달하고, 그 아래에서는 빈 스페이서와 **항상 같은 폭**이었다 — 1024px 에서 277px 검색칸 옆에
  똑같이 277px 짜리 빈 공간.
- **최종 처방.** 스페이서를 **제거**하고 `만들기` Button 에 **`ml-auto`** 를 준다. grow 요소가 다시
  하나가 되고, `auto` 마진은 grow 해소가 **끝난 뒤** free space 를 먹으므로 우측 정렬은 유지된다.

**최종 실측** (라이트·다크 동일).

| 뷰포트 | 검색칸 폭 (최종) | 1차 처방 시 | 우측 여백 | grow 요소 수 |
|---|---|---|---|---|
| 1920 | **448** | 448 | 12px | 1 |
| 1440 | **448** | 448 | 12px | 1 |
| 1280 | **448** | 405 | 12px | 1 |
...
`max-w-md` 도달 최소 뷰포트 **965px**(이진 탐색 실측, 964→447 · 965→448). 1차 대비 **369px 개선**.

**★ 이 결함이 두 번 통과한 이유 — 잰 것이 매번 달랐다.**
D-7 실측은 「`searchbox` **개수**」만, B2 1차 실측은 「**우측 여백**」만 쟀다. 정작 이 결함의
증상인 **검색칸 폭**을 잰 기록이 두 라운드 모두 없었다. 2차 리뷰가 그 공백을 지적하고서야 측정됐다.
**교훈. 무엇을 고쳤는지가 아니라 무엇을 재지 않았는지를 물어라.**
