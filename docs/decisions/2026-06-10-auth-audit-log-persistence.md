...
**보존 정책 (§2.3.3 + DATA.md §3)**. 감사 로그는 **append-only 영구 보존**(DATA.md §3 "감사 로그 절대 삭제 금지"). SDD §2.3.3의 "감사 로그 보존 1년"은 **최소 보존 floor**로 해석 — 영구 보존이 이를 충족(∞ ≥ 1년). 삭제/정리(@Scheduled purge) 작업은 도입하지 않는다 (DATA.md §3 헌법 준수, Maxi 결정 2026-06-10). DATA.md §3과 SDD "1년"의 충돌은 이 해석으로 해소.
