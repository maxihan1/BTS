...
FR-NT-05 D6(워크플로우 post-action 설정 프론트 UI)를 만들려면 관리자가 전환의 post-action(특히 `CALL_WEBHOOK`)을 런타임에 편집할 수 있어야 한다. 그러나 현재 post-action은 `YamlSeedService`가 부팅 시 `workflow_post_actions`에 시드하는 전용 경로뿐이고 런타임 편집 API가 없다.

**핵심 충돌(코드 실측).** `YamlSeedService.isDirty`가 `differsInPostActions(existing, dto)`를 포함한다. 이는 YAML 정의 전환별로 DB의 post-action type 목록과 YAML 목록을 비교한다(`dbTypes != dtoTypes`). 따라서 런타임 API로 YAML 시드 전환에 post-action을 추가하면 DB(추가분 포함)가 YAML(0건)과 달라져 `isDirty=true` → `deleteWorkflow`(CASCADE) → reinsert → **런타임 post-action이 다음 부팅 시 전멸**한다.
...
전환 실행 시 `DefaultWorkflowDefinitionRepository.findPostActions`(WorkflowEngine 경유)가 DB를 직접 조회한다. `WorkflowCache`의 `Workflow` aggregate에는 post-action 필드가 없다(states/transitions/validator만 캐싱). 따라서 변이 후 캐시 무효화는 불필요하다(codereview CONCERN-1).
