...
**합치면** 래핑 함수를 `/search` 명령(= `commands.ts`, FR-UX-04 소유)과 자유 텍스트(= 신규 레이어,
FR-UX-12 소유)가 **둘 다** 필요로 한다. 래퍼를 신규 레이어에 두면 `commands.ts → 신규 레이어`
**역방향 의존**이 생겨 D-1 이 그은 경계가 첫날부터 무너진다. 중립 위치가 유일한 해다.
