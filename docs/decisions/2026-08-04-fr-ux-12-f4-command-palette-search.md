...
- **FR-UX-12 의 백엔드 0 — 단, PRE_EXISTING hot-fix 1건이 동승한다 (Maxi 확정 2026-08-05).**
  게이트 2 독립 코드리뷰가 **선재 결함**을 적발했다 — `AqlLexer.scanQuotedString` 에 문자열
  이스케이프가 없어 여는 `"` 이후 **첫 `"` 에서 무조건 닫힌다**(백슬래시 분기 부재). 그래서
  `text ~ "로그인\"버그"` 가 400 이고, `text ~ "a\\b"` 는 통과하되 검색어가 `a\\b` 로 훼손된다.
  **AQL 검색 페이지에 사용자가 직접 따옴표를 쳐도 같은 결함**이라 이 PR 이 만든 게 아니라
  **드러냈다**. Maxi 가 정공법(백엔드 렉서 수정)을 확정해 `search-export-import` BC 를 함께 고쳤다.

  **그럼에도 §4.10 의 D4/D5 는 「없음」으로 유지한다.** 이 변경은 FR-UX-12 의 백엔드가 아니라
  **FR-SR-02/04 소유 BC 의 선재 결함 hot-fix** 이기 때문이다(저장소 선례 「PRE_EXISTING hot-fix
  D5 옵션 C 패턴」). D 마커를 `[x]` 로 바꾸면 "FR-UX-12 가 백엔드를 요구했다"는 **거짓**이 된다.

  **크로스 레이어 계약 테스트가 핵심 산출물이다.** `AqlLexerTest` 가 프론트 `escapeAqlString` 의
  출력 문자열을 박제해 round-trip(사용자 원문 복원)을 단언한다 — **두 층 중 하나만 바뀌어도 red**
  다([[two-lists-never-check-each-other]] 처방). 프론트 `aql-text-query.test.ts` 는 이름을
  「…**모양으로 감싼다**(유효성 증인은 백엔드 AqlLexerTest)」로 정직화했다 — 이전 이름이
  「유효한 AQL 이 된다」였는데 실제로는 문자열 모양만 재는 **공허한 주장**이었다.

  **`\X`(X ∉ {`"`,`\`})는 리터럴 보존**으로 확정했다. 근거가 실측으로 뒤집혔다 — 처음엔
  "`plainto_tsquery` 가 구두점을 버리니 무관"으로 봤으나, `IssueRepository.kt:2691`
  `buildTextSearchCondition` 이 FTS 경로와 **trigram `LIKE '%…%'` 경로를 OR 로 결합**(`:2706-2708`)해
  후자에서는 역슬래시가 매칭에 그대로 관여한다. 삭제하면 사용자가 친 글자가 조용히 사라진다.

- **FR-UX-04 ADR D1(프론트 전용)과 충돌 없음.** `personalization.md §4.10` 의
