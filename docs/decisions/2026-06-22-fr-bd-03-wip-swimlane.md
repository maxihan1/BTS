...
**근거**. product 명세 §2.3 D2 "WIP 초과 시 시각 경고만(이동 차단 옵션)". Jira 기본 동작도 경고 전용이다. 이동 차단은 전이 위임 경로(`IssueTransitionPort`)에 추가 가드가 필요해 범위·복잡도를 키운다. 차단 모드는 후속 FR로 명시 이연(Maxi 확정).
...
- **WIP 초과 시 이동 차단** — 전이 위임 경로 추가 가드 + 범위 확대. 기각(결정 1, 경고 전용).
