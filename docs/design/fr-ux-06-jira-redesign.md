...
> ⚠️ **`--type-subtask`는 PR22 시점에 정의하지 않았다** (소비처 0 → ADR §D7 준수).
> 위 표의 라이트 후보값 `#579DFF`는 **흰 배경 대비 2.74:1로 WCAG 1.4.11(비텍스트 3:1) 미달**이다.
> 미래에 서브태스크 색 배지를 실제로 소비하는 PR은 **이 값을 그대로 쓰면 안 된다** — Blue700
> `#0C66E4`(4.56:1) 계열로 내리거나, 색 외 글리프 구분에 의존해야 한다.
> 근거·계산은 `docs/decisions/2026-07-25-fr-ux-06-pr22-chart-categorical-tokens.md` D22-3 참조.
> `--prio-*` 5종도 같은 이유로 미정의이며, `state-tokens.test.ts`의 `UNCONSUMED_TOKENS`
> 음성 테스트가 **선채움을 차단**한다.
