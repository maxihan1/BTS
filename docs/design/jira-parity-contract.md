...
| 관리 진입점 도달성 ✅ **이관됨 J9** | `grep -rln "관리 메뉴" apps/web/e2e/` · `grep -n "ADMIN_ENTRY_NAME" apps/web/e2e/fixtures/admin-hub.ts` | `관리 메뉴` 는 이제 **상단바 링크**(`TopBar.tsx` → `/admin`)의 이름이다 — 사이드바 `<nav>` 가 아니다. 문자열은 동결이되 조회 role 이 `navigation` → `link` 로 옮겨갔고, 그 전환은 `e2e/fixtures/admin-hub.ts` 한 곳이 흡수한다. ★**`page.goto('/admin')` 으로 바꾸지 마라** — hard navigation 이 MSW Service Worker 를 재초기화해 시드된 store 가 리셋된다(`global-permissions` S1~S5 · `notification-policies` S7 전량 사망). ★상단바 링크에 `max-md:hidden` 금지 — 사이드바에서 없어진 뒤로 **유일한 UI 진입로**라 감추면 관리 화면 8개가 모바일에서 도달 불가가 된다 |
