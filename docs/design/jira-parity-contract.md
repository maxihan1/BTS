...
```bash
# ★검색 범위에 TODOS.md 를 포함한다. 조회 결과가 거기 있는 경우가 실재한다 —
#   부채 177 등재 세션이 J7~J21 을 `TODOS.md` 의 `### 지라 근거` 블록에 남겼고,
#   그 블록이 스스로 「착수 시 계약 §1-0 재사용 대상」이라 적어 뒀다.
# ★헤딩 두 가지를 함께 본다. 장부 쪽 블록은 `## Jira 대조` 가 아니라 `### 지라 근거` 다 —
#   2026-09-05 실측. 헤딩 하나만 보던 종전 명령은 J12~J21 을 **한 건도 못 찾아** 4탭을
#   통째로 재조회하게 만들었고, 그 사이 J18(백로그↔활성 스프린트 별도 설정)을 놓쳐
#   스펙 초안이 틀린 채 작성됐다가 재조회로 뒤집혔다.
grep -rlE "## Jira 대조|### 지라 근거" docs/specs/ docs/plans/ TODOS.md | xargs grep -ln "<표면 키워드>"
```
...
| 보드 헤더 `⋯`·스위처는 **컨테이너 스코프**로만 잡는다 ✅ **구현됨 PR ⑧** | `grep -rn "보드 관리\|보드 선택" apps/web/e2e/` · `grep -rn "board-header" apps/web/` | 조회는 반드시 `boardHeader(page)`(= `getByTestId('board-header')`) 를 거친다. 헬퍼 정본은 `apps/web/e2e/fixtures/board-helpers.ts` **한 곳**이고, 이 헬퍼를 쓰는 spec 이 각자 복붙하던 것을 그리로 모았다. ★**앵커(`^…$`)나 `exact: true` 로 대체하지 마라** — 사이드바 보드 `⋯`(PR ⑨)가 `i18n/board-labels.ts` 의 **같은 `triggerAriaLabel` 헬퍼**를 쓰면 접근성 이름이 바이트 단위로 같아져 앵커로는 못 가른다. ★**정규식은 유지**한다 — 접근성 이름이 `보드 관리, {name}` 이고 `board-manage.spec.ts` S3 가 테스트 중간에 이름을 바꾸므로 `exact` 로 굳히면 rename 단계에서 죽는다. ★**testid 를 지우거나 이름을 바꾸면 스코프된 로케이터가 「실패」가 아니라 `count 0` 이 된다** — `toHaveCount(0)` 류 부재 단언은 그것을 그대로 통과시킨다. 지금은 그 앞에 스코프된 존재 단언(`toContainText`)이 서 있어 먼저 red 가 나지만, **양성 단언이 앞서지 않는 경로가 생기면 컨테이너 실재 단언(`await expect(boardHeader(page)).toBeVisible()`)만 남는다.** 그 줄을 걷어내지 마라 |
