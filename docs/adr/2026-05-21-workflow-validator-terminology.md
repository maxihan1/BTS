...
# ADR — 워크플로우 전환 검증 용어 통일 (Validator 채택)
...
`project-workflow` BC 신규 모듈 구현 시 "전환 가능 여부 검증" 역할의 컴포넌트 명명이 필요했다.
...
- `Maxi_wiki/BTS/domain/project-workflow.md` — "WorkflowGate (조건/검증 규칙)", "전환 게이트" 표현 혼재.
...
| Gate | 전환 게이트 개념에 부합 | CLAUDE.md / SKILL.md에서 "게이트 1/2" (Maxi 검토 게이트) 로 이미 사용 중. 동음이의 충돌로 거부. |
...
3. **책임 명확성** — "전환 가능 여부 검증"이라는 책임을 "Validator"가 가장 직접적으로 표현. Guard는 방어/차단, Gate는 통과 제어의 뉘앙스로 검증 계산 역할과 미묘하게 달라 오해 소지 있음.
...
- `Maxi_wiki/BTS/glossary.md` "게이트" 항목 (일반 개념 정의) — "전환에 걸린 조건의 일반 개념"으로 그대로 유지. 구현 클래스 명은 Validator이지만 일반 개념어로서의 "게이트"는 그대로 공존 가능.
