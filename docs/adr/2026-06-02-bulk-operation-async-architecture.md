...
FR-IS-05는 여러 이슈에 같은 변경(필드 편집 또는 상태 전이)을 한 번에 적용한다. 실행 방식으로 두 갈래가 있었다.
...
- 각 이슈 처리는 기존 `IssueApplicationService.updateIssue`/`transitionIssue` 도메인 로직을 재사용한다(도메인 우회 금지). 일괄 전이는 기존 `WorkflowTransitionPort`(project-workflow 위임)를 이슈별로 호출한다.
...
- 멱등성/재시도, 결과 보존 기간(TTL), 혼합 상태 전이 처리, 청크 크기/건수 상한은 spec(D2)에서 확정한다.
