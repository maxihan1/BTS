...
> `V506:69-71` 의 「마이그레이션이 기존 데이터를 조용히 바꾸지 않는다」 원칙과도 어긋나므로,
...
| **A-3. 이관하되 ACTIVE 유지** | J18·`V506:69-71` 과 정합하지만 `limit(1)` 때문에 결함이 자리만 옮긴다. 이관 후에도 안 보이고 409 로 손도 못 대며 2주 뒤 예고 없이 튀어나온다 |
...
`idx_sprints_board_active`(`V506:66-67`)는 **UNIQUE 로 승격하지 않는다.** `V506:69-71` 이 적은
...
- 이관 대상 중 ACTIVE 였던 것이 `PLANNED` 로 내려가 사용자가 시작 버튼을 한 번 더 눌러야 한다.
  단 그 스프린트는 화면에 한 번도 나온 적이 없어 「진행 중」으로 인식된 적 자체가 없다.
