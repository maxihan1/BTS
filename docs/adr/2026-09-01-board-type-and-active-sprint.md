...
> 날짜. 2026-09-01
> 상태. **초안 (Draft)** — Maxi 인가 대기. 티어 **T3**(마이그레이션 + 기존 화면 의미 변경).
> 관련 FR. FR-BD-01(보드) · FR-BL-01/02(백로그·스프린트) · **신규 FR 1건 필요** (보드 종류)
...
1. **이 ADR 을 채택하는가.** D1~D6 전량.
2. **신규 FR 번호.** 「보드 종류와 활성 스프린트 보드」는 새 기능이라 FR 이 하나 는다
   (총수 143 → 144). `fr-index.md` 전수 동기화 대상.
3. **PR 분할.** 한 PR 로 가면 마이그레이션 2 + 백엔드 + 프론트 + E2E 가 한 덩어리다.
   3개로 나누는 안 — ① 스키마·백엔드 ② 보드 생성 플로우 UI ③ 활성 스프린트 보드 화면.
