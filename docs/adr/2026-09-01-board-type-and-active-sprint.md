...
3. **PR 분할.** 최초 3분할로 채택했고, 2026-09-02 에 **6분할**로 확장했다(아래 「후속 3분할」).

| PR | 범위 | 티어 | 마이그 | D 단계 | 상태 |
|---|---|---|---|---|---|
...
| **⑤** | 보드 종류 불변식 — 스프린트 생성 쓰기 가드 · `start` TOCTOU 락 · DTO 계약 2건 | T2 | 0 | D4·D5 잔여 | ✅ #431 |
| **⑥** | 보드 스코프 유실 — 빈 상태 CTA · 뷰 전환 nav 전파 · E2E | T1 | 0 | D6 잔여 | ✅ #432 |

**분할 근거(①~③).** 마이그레이션이 든 PR 을 작게 유지해 되돌리기 쉽게 한다. ②·③ 은 마이그레이션 0 이라
①이 머지된 뒤 병행 가능하지만, ③ 이 ② 의 종류 선택 없이는 스크럼 보드를 만들 수단이 없으므로
**① → ② → ③ 순서**를 지킨다.

### 후속 3분할 (2026-09-02 추가)

③(#424)이 자기 범위 밖으로 밀어낸 것들이다. **④ 는 원래 「③ 전에 닫는다」로 기한이 박혀 있었고
그 기한을 넘겼다** — `docs/plans/2026-09-01-board-scrum-schema.md:384` 가 그 약속을 적었고,
`docs/plans/2026-09-02-scrum-board-screen.md:300-325` 가 대신 PR ④ 로 분리하며 인계 지침 4건을 남겼다.

**치른 대가.** ③ 의 plan 이 「ADR 3분할 표에 ④ 행 추가를 Task 9(문서 동기화)가 같은 PR 에서 처리한다」
(`:324-325`)고 못박았으나 그 plan 의 Task 9 는 실제로는 E2E 였다(`:583`). **문서 동기화 task 가
존재하지 않아** ④ 는 이 표에도, `TODOS.md` 에도 등재되지 않은 채 plan 산문 2곳에만 남았다 —
저장소의 어떤 목록도 이것을 세지 않는 상태로 2026-09-02 까지 왔다
(memory `two-lists-never-check-each-other` 양식). 이 표의 ④⑤⑥ 행과 `TODOS.md` 등재가 그 복구다.

⚠️ **④ 는 「한 PR = 한 BC」의 명시적 예외다.** shared-kernel + issue-tracking + agile-planning
3모듈을 동시에 바꾼다. 포트 경유가 회피 불가이기 때문이고(`AgilePlanningBcArchTest` 가 직접 import 를 막고,
보드 GET 은 동기 읽기라 pgmq 로 대체 불가), 같은 성격의 예외를 `docs/plans/2026-09-02-scrum-board-screen.md:112`
가 이미 기록했다. 의존 방향은 불변이다 — `agile-planning ──(port)─▶ shared-kernel ◀──(impl)── issue-tracking`.

## 다음 한 걸음

**후속 3분할 ④⑤⑥ 완주** — ④ #430 · ⑤ #431 · ⑥ #432. FR-BD-04 의 코드 부채는 닫혔다.

남은 것은 전부 `TODOS.md` 에 별건으로 있다 — ⑤ 가 남긴 3건(165 `start` 가드 ·
166 격리 수준·락 상한 · 167 판별식 3개)과, ⑥ 이 **일부러 안 건드린** 164(「기본 보드」
규칙이 세 곳에서 서로 다르다). 164 의 뿌리는 「마지막으로 본 보드」 기억의 부재 = X7 이고,
그 공유 상태는 FR-UX-07 소관이다.
