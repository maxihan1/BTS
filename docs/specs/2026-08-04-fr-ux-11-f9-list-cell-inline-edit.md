...
| 상태 변경 | 셀 드롭다운에서 전이 선택 | 이슈를 열어야 함 | **★F9 대상** |
...
담당자는 검색이, 상태는 가용 전이 목록이 필요해 셀 안에 직접 못 넣는다. 우선순위만 `<select>`
로도 되지만 **3종의 진입·이탈 조작을 다르게 만들 이유가 없어** popover 로 통일한다.
`components/ui/popover.tsx` 소비 (1→2).

### D-3. 전이·권한은 **셀을 열 때만** 조회 — N+1 회피가 백엔드 0줄의 근거

행마다 필요한 조회가 둘이다.

```
GET /api/v1/issues/{key}/transitions                     가용 전이 (행마다 다름)
...
기각안. **목록 응답에 전이·권한을 실어 보내기** — 백엔드 변경이라 범위 밖(F8 선례 0줄).
...
When 상태 셀을 클릭한다
Then 그 이슈에서 **지금 갈 수 있는 전이만** 목록에 뜬다
...
| **FR7** | 상태는 **그 이슈의 가용 전이만** 노출한다. `POST /issues/{key}/transition {toStatusKey, expectedVersion}` |
| **FR8** | 가용 전이 0건이면 사유를 구분해 안내한다 — **422(워크플로우 미설정)** vs **200+빈 배열(종료 상태)**. `issues.$key.tsx:55` `resolveTransitionUnavailableReason` 을 공용으로 올려 재사용 (복제 금지) |
| **FR14** | **종료 전이(`toCategory === 'DONE'`)는 해결 결과를 먼저 받는다** — 기존 `ResolutionModal` 을 재사용하고 `resolutionId` 를 실어 전이한다 (**Maxi 확정 2026-08-04**) |
| **FR15** | 전이 실패를 사유별로 안내한다 — 409 `TRANSITION_NOT_ALLOWED` / 409 `VERSION_CONFLICT`(재조회 유도) / 422(워크플로우 미설정). `issues.$key.tsx:359-378` 의 분기를 승계한다 |
...
| **FR12** | 전이·권한 조회는 **셀이 열린 이슈에 대해서만** 발생한다 (D-3) |
...
| **NFR2** | 셀 1개 열기당 요청 ≤2 (전이 + 권한). 같은 행 재진입은 캐시 | `staleTime: 30_000` |
...
```
GET  /api/v1/issues/{key}/transitions                     가용 전이 조회 (셀 열 때)
...
| **E4** | 가용 전이 0건 (워크플로우 미설정) | `no-workflow` 안내 (FR8) |
| **E5** | 가용 전이 0건 (종료 상태) | `terminal` 안내 (FR8) |
...
| **E14** | **종료 전이 선택** | popover 를 닫고 `ResolutionModal` 을 연다. 확인 시 `resolutionId` 를 실어 전이, 취소 시 상태 불변 (FR14) |
| **E15** | 결의안 모달이 열린 채 목록이 refetch | 모달은 선택된 전이 항목에 묶인다. 확인 시 `expectedVersion` 이 낡았으면 409 → FR15 경로 |
| **E16** | 전이 목록 조회가 **422** | 워크플로우 미설정 안내. 200+빈 배열(종료 상태)과 **다른 문구** (FR8) |
