...
| **R3** | 이관 후 **모든 보드에서 `ACTIVE` 스프린트는 최대 1건**이다 |
...
`idx_sprints_board_active`(`V506:66-67`)는 **그대로 둔다** — 일부러 UNIQUE 가 아니고,
그 사유(`V506:69-71` · PR #182 Deviation ⑤)가 여전히 유효하다. 이 PR 은 데이터를 정리하지만
스키마로 유일성을 못박지 않는다.
