# FR-NT-05 Webhook 알림 채널 (PR 1: 전환 post-action 이벤트 pgmq 발행) — 스펙
...
워크플로우 전환 후처리(post-action)는 `PostActionPlan.emitEvents`로 도메인 이벤트를 **계산만** 한다(GAP-2 — 실행은 호출자 BC 책임). 4종 post-action이 이벤트를 emit한다.
...
그러나 실제 전환 실행 경로 `IssueApplicationService.transitionIssue()`는 `plan.emitEvents`를 **전부 버리고** `plan.toStateKey`만 사용한다. → 어느 이벤트도 pgmq에 발행되지 않는 근본 결함. WebhookRequested는 dry-run 미리보기(`POST /workflows/{key}/transitions`)의 HTTP 응답에만 노출된다.
...
- **S1 (정상 발행)**: Given 전환에 CallWebhook post-action이 설정된 워크플로우, When 사용자가 그 전환을 실행(상태 변경 성공), Then `WebhookRequested` 이벤트가 `q_transition_events` 큐에 enqueue된다(상태 변경과 동일 트랜잭션).
- **S2 (롤백 시 미발행)**: Given 동일 설정, When 전환 중 버전 충돌 등으로 트랜잭션 롤백, Then 이벤트도 함께 폐기되어 큐에 남지 않는다(outbox 정합).
...
- **S4 (bulk 전환)**: Given 동일 설정, When 일괄 전환(bulk)로 N개 이슈를 전환, Then 각 이슈 전환마다 emitEvents가 발행된다(bulk는 `transitionIssue()`를 거치므로 자동 커버).
- **S5 (post-action 없음)**: Given post-action 미설정 워크플로우(현 default), When 전환 실행, Then `plan.emitEvents`가 비어 아무 것도 발행되지 않는다(no-op, 회귀 0).
...
- **회귀 0**: post-action 미설정 시 emitEvents 빈 리스트 → no-op. 기존 전환/IssueTransitioned 발행 동작 불변.
- **성능**: 전환당 emitEvents 수만큼 `pgmq.send`(보통 0~1건), 기존 트랜잭션 내 추가 비용 미미.
...
- bulk 전환 부분 실패 → 성공한 이슈만 각자 트랜잭션에서 발행(BulkItemApplier 항목별 처리 모델 따름).

## 측정 가능한 완료 기준

1. 단위: `TransitionEventPublisher`가 `pgmq.send`를 큐명·직렬화 payload로 호출(MANDATORY 트랜잭션 강제 검증).
2. 통합(Testcontainers): CallWebhook post-action 시드 → 전환 실행 → `q_transition_events`에서 `WebhookRequested` 1건 read 확인. 롤백 시나리오 → 큐 0건. dry-run → 큐 0건.
3. 회귀: post-action 미설정 전환 → 큐 0건 + 기존 IssueTransitioned는 q_issue_events에 정상.
4. `verify-master-plan.sh` exit 0 (FR 123). ktlintCheck/detekt --rerun-tasks green.

## 제약 / 범위 밖

- **webhook 설정 UI/API 없음**: 어느 전환에 어떤 webhook을 걸지는 기존 워크플로우 post-action 정의(YAML/DB) 메커니즘으로 구성. FR-NT-05 PR1은 발행 배관만.
- **HTTP POST 디스패처 없음**: 외부 URL 실제 호출은 PR 2(notification BC).
- **재시도/DLQ 정책**: PR 2 디스패처 설계 사항(pgmq read/archive/재청구). PR1은 발행까지만.
- **createIssue 시작 전환 범위 밖**: `createIssue()`는 `resolveStart`로 초기 상태만 설정하고 post-action을 실행하지 않는다(현 모델). 시작(Create) 전환 post-action emitEvents는 FR-NT-05 범위 밖(현 동작 불변).
- **post-action fieldChanges 적용 범위 밖**: `transitionIssue()`는 `plan.fieldChanges`(SetField 등)도 현재 버린다 — 이는 "전환 시 필드 자동 채움"이라는 **별개 기능**이라 FR-NT-05(이벤트 발행)와 분리. 본 PR은 `emitEvents`만 발행한다(fieldChanges는 기존대로 미적용 유지, 별도 추적).
...
- bulk 전환은 `BulkItemApplier`가 `transitionIssue()`를 호출 → 단일 변경점이 단건+bulk 커버.
- createIssue는 post-action 미실행 → 범위 밖 명시.
- q_issue_events는 NotificationWorker가 소비 중 → 전용 큐 `q_transition_events`로 오염 회피.
- 통합테스트 참조: `IssueTransitionValidatorEndToEndIntegrationTest`(워크플로우 시드+실 전환 end-to-end) 패턴 재사용, post-action 시드로 확장.
