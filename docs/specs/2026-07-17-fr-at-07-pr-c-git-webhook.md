...
**확정 — 경로**.
> **★★ 4회차 정정 (T10 impl) — ①②를 뒤집었다. 원래 순서가 새 오라클을 만들었다.**
> `①토큰조회 → ②크기상한`이면 **413 vs 401 자체가 토큰 존재 오라클**이 된다 — 초과 본문 + 미존재 토큰
> → 401 / 초과 본문 + **실재** 토큰 → 413. `Content-Length` 사전검사 때문에 **바이트 하나 보내지 않고**
> 후보 토큰의 실재를 판별할 수 있다. **B3-sec가 404를 포기하면서까지 막은 바로 그 오라클.**
> → **크기상한을 맨 앞으로.** 두 불변식을 **둘 다** 지킨다 — EC5("413은 **서명검증** 이전") ✓ ·
> plan Task 10("토큰조회는 **payload 파싱**보다 먼저") ✓. `readNBytes`가 힙을 256KB로 캡하므로 DoS 델타
> 없음(오히려 미존재 토큰의 DB 왕복이 사라짐). **뮤테이션 실증** — 원래 순서 복원 시
> `Status expected:<413> but was:<401>`로 테스트 사망.
```
GitWebhookController
  ① readNBytes(MAX+1) 크기 상한 + Content-Length 이중검사  ← 413 (토큰 조회보다 먼저 = 오라클 차단)
