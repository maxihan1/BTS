...
| **FR11-b** | **저장값 쓰기가 전 인증 페이지로 넓어진다 — 의도된 확장으로 채택한다.** `useResolvedActiveProject` 는 읽기 전용이 아니라 `:85-91` 에서 해소 출처가 `url`·`first` 일 때 저장한다. **소비 주체는 `ProjectTree` 가 아니라 `ProjectSwitcher` 다**(FR7 정정 이후) — 스위처는 트리거에 "지금 무엇이 활성인가"를 표시해야 하므로 저장값·첫 프로젝트 폴백까지 포함한 **전체 해소**가 필요하고, 그 조합을 자체 구현하면 FR-UX-07 FR3-b 가 경고한 드리프트가 확정된다. 스위처가 `TopBar` 에 있으므로 이 해소는 전 인증 페이지에서 돈다. **결과** — 낡은 저장값(삭제·권한 회수)이 `/issues` 방문 전에도 교정되고(스펙 E3), 첫 방문자가 어느 페이지로 들어와도 활성 프로젝트가 앵커된다(FR-UX-07 FR4 의 근거 그대로). **조용히 넘어가지 않고 `ShellLayout.test.tsx` 에 전용 단언을 둔다** |
...
### 8-A. 디자인 리뷰 확정 (2026-07-31 · `/plan-design-review` · PR-B 한정)

초기 평점 **6/10** → 확정 후 **9/10**. 아래는 **Maxi 확정**이며 구현자가 다시 고를 여지가 없다.
...
대시보드
캘린더
즐겨찾기(FavoritesMenu 트리거 버튼)
──────────────
최근 항목  ← 신규, 최하단 (그룹 헤더 + 하위 링크 N개)
```
근거. "매일 여는 진입점을 위로" — 순서가 곧 중요도 신호다. 지라의 실제 순서(For you → Recent)와도
정합하고, 단일 링크(항상 존재)와 목록(비었다 찼다 함)을 위아래로 갈라 그룹 성격이 섞이지 않는다.

**D-B. 접힘(64px 레일)에서 "최근 항목" 은 섹션 전체를 렌더하지 않는다.**
근거. E10 의 *"아이콘만 노출 · 텍스트 `sr-only`"* 는 **단일 링크** 관례지 5개짜리 목록 관례가 아니다.
`ProjectTree.tsx:217-223` 의 첫 글자 뱃지를 그대로 쓰면 `ATLAS-12`·`ATLAS-13` 이 **둘 다 `A`** 라
시각적으로 구분 불가능하다. 5칸을 먹으면서 정보는 0 이면 사이드바를 접은 목적을 배반한다.
**대가 (의도된 것 — 결함 아님).** 접은 채 쓰는 사용자는 최근 항목에 접근할 수 없다. TODOS.md 등록.
**"내 작업" 은 단일 링크이므로 기존 E10 관례를 그대로 따른다** (아이콘 + `sr-only` 텍스트, DOM 유지).

**D-C. 아이콘·항목 스타일은 「사이드바 선례」를 따른다** (드롭다운 선례 아님).

| 대상 | 확정 |
|---|---|
| "내 작업" 아이콘 | lucide **`UserCheck`** + `NAV_ICON_CLASS`(`size-4 shrink-0`). 기존 12종과 중복 없음 |
| "최근 항목" 그룹 헤더 | `Sidebar.tsx:106-108` 관리 메뉴 헤더와 동일 — `px-2 pt-2 text-xs font-semibold uppercase text-sidebar-foreground/60`. **접힘 시 미렌더** |
| "최근 항목" 하위 링크 | **아이콘 없음.** `ProjectTree.tsx:54` `TREE_SUB_LINK_CLASS` 관례 — `block truncate rounded-md px-2 py-1 text-sm text-sidebar-foreground/70` |

근거. 둘 다 같은 사이드바 안에 있으므로 이웃(프로젝트 트리 하위 항목)과 맞추는 것이 한 시스템으로
읽힌다. 반복되는 동일 아이콘 5개는 구분에 기여하지 않는 장식이다(subtraction default).

**D-D. 선례가 강제한 것 (결정이 아니라 정렬 — 구현자 재량 없음).**

| 축 | 값 | 출처 |
|---|---|---|
| 제목 잘림 | `<span className="truncate">` (컨테이너 `min-w-0`) | `FavoritesMenu.tsx:162` · `ProjectTree.tsx:37` |
| 로딩 상태 | 개별 스켈레톤 없음. `useQueries` **전체 settle 전까지 아무것도 렌더하지 않는다** | `FavoritesMenu.tsx:133,146` — 헤더 플리커 방지가 명시 목적 |
| 조회 옵션 | `retry: false` | `FavoritesMenu.tsx:129` — 403/404 재시도는 낭비 |
| 빈 목록 | 섹션(헤더 포함) 미렌더 | `FavoritesMenu.tsx:146` 동형 · 스펙 E3 |

**철회한 지적 1건.** 리뷰가 *"첫 사용자는 최근 목록이 비어 기능 존재를 모른다"* 를 제기했으나
**ADR §D3 이 이미 해결**했다 — 기록이 방문 시 자동이라 이슈를 **하나만 열어도** 섹션이 나타난다.
명시 선택 방식이었다면 성립했을 지적이고, 자동 기록에서는 성립하지 않는다.
