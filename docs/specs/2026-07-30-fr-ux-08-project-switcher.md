...
**S4 — 접은 트리가 유지된다**
- Given 사용자가 ATLAS 트리를 펼친 뒤 "리포트" 그룹을 접었다
- When `/dashboards` 로 갔다가 돌아온다 (또는 새로고침한다)
- Then ATLAS 는 펼쳐진 채, "리포트" 는 접힌 채 그대로다
...
| E12 | 트리 펼침 키와 프로젝트 키의 네임스페이스 충돌 | 펼침 집합은 프로젝트 키 + 중첩그룹 키(`리포트`·`설정`)를 함께 담는다. 저장 시 **접두사로 구분**하거나 기존 `expandedKeys` 키 형식을 그대로 보존 — 구현 시 현재 형식 실측 후 결정 |
