...
### S5. 불허 전환 거부
- **Given** ARCHIVED 상태의 버전
- **When** 상태를 RELEASED로 직접 전환 요청 (그래프에 없는 전환)
...
- **FR-2**. 전환 그래프(허용 전환만, 나머지 거부):
  ...
  - self-transition(같은 상태로) 및 그 외 모든 전환 거부.
...
- **FR-5**. 전환 API: `PATCH /api/v1/projects/{projectIdOrKey}/versions/{id}/status`. 권한 VersionPermission.UPDATE.
- **FR-6**. 응답 DTO에 status, releasedAt 추가(목록/단건/생성/수정 모든 VersionResponse).
- **FR-7**. UI: 버전 목록 행에 상태 뱃지 + 전환 액션(릴리스/되돌리기/보관/보관해제), ARCHIVED 행은 수정/삭제 버튼 비활성화.

## 비기능 요구사항 (NFR)

- **NFR-1**. 전환 판정은 도메인 순수 메서드(부수효과 없음, 테스트 용이). ApplicationService가 거부를 409로 변환.
...
상태 전환 전용 서브리소스(FR-VR-01 `/dates` 동형).
...
| 상황 | 상태 | errorCode |
|---|---|---|
| 그래프에 없는 전환 / self-transition | 409 | VERSION_TRANSITION_NOT_ALLOWED |
...
- **EC1**. 그래프에 없는 전환(ARCHIVED→RELEASED 등) → 409, 상태 불변.
...
- **EC6**. 동시 전환(두 요청). FR-VR-01과 동일하게 OCC 미적용 — last-write-wins(단순 UPDATE라 무결성 위험 없음). 향후 필요 시 별도 FR.
- **EC7**. 소프트 삭제된(deleted_at != null) 버전은 findActiveVersion에서 이미 404 — 전환 대상 아님.
...
- 도메인 단위 테스트: 허용 전환 5종 각각 + 거부(self/그래프 외) + ARCHIVED mutation 거부 + released_at 규칙(now/null/유지).
- 통합 테스트(Testcontainers): PATCH /status happy(S1~S4) + 거부 409(S5/S6) + 400(EC3) + 404 + 응답 status/releasedAt 포함.
- 마이그레이션 테스트: V016 컬럼/제약/기본값.
- 프론트 단위 + MSW: 상태 뱃지 렌더 + 전환 버튼 + ARCHIVED 비활성화.
