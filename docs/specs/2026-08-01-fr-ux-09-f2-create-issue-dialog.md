...
| **FR-15** | **필드를 3덩어리로 나누고 `ui/separator` 로 구분한다** (design 리뷰 D7). ① 기본 — 프로젝트·유형·제목·본문 ② 배정 — 담당자·우선순위·라벨 ③ 추가 — 컴포넌트·보안등급·커스텀 필드. 필수 커스텀 필드 검증 실패 시 해당 칸으로 **자동 스크롤**한다(③이 접혀 있지 않아도 세로가 길어 화면 밖일 수 있다). |
| **FR-16** | **생성 성공 후 이동은 진입 경로가 정한다** (design 리뷰 D8). 폼은 `onSuccess(key)` 를 부를 뿐이고 **호출자가 결정**한다 — `routes/issues.new`(딥링크) → `/issues/$key` 이동 · `TopBar`(제자리 열기) → 그 화면에 머물고 `toast(<키> 만들었습니다, { action: 보기 })`. `<Toaster />` 는 `main.tsx:38` 에 이미 마운트돼 있고 `toast()` 선례는 `useNotificationStream.ts:70` 이다 — **신규 인프라 0**. |
| **FR-17** | **접근 가능한 프로젝트가 0개면 폼 대신 빈 상태를 낸다** (design 리뷰 D9). `ui/empty-state` 로 *"참여 중인 프로젝트가 없어 이슈를 만들 수 없습니다"* + `whoami.canCreateProject`(`api/schemas.ts:112`)가 참이면 **프로젝트 만들기** 버튼. 선례는 `ProjectListTable.tsx:93` 의 `showCreateCta`. |
...
| **NFR-2** | 필드 10종+N 이라 세로가 길다. 본문 영역만 스크롤하고 **만들기 버튼은 항상 보인다**. | **긴 폼 모달 선례를 그대로 따른다** — `ReleaseNotesDialog` 의 `max-w-2xl max-h-[80vh] flex flex-col` + 본문 래퍼만 `overflow-y-auto`, `DialogFooter` 는 스크롤 밖 |
| **NFR-3** | 레이아웃은 기존 관행을 따른다 — **단일 컬럼**, 폭은 **`max-w-2xl`**. | 기존 Dialog 소비처 32곳이 전부 단일 컬럼이고 폭은 `max-w-sm/md/lg/2xl/4xl` 만 쓴다. **`max-w-xl` 은 선례가 없어 채택하지 않는다** (design 리뷰 Pass 5) |
...
## 상호작용 상태 표 (design 리뷰 Pass 2)

사용자가 **무엇을 보는지**로 적는다 (백엔드 동작이 아니라).

| 영역 | 로딩 | 비어 있음 | 오류 | 성공 | 부분 |
|---|---|---|---|---|---|
| 프로젝트 셀렉터 | `ui/skeleton` 한 줄 | **폼 대신 `EmptyState`** — "참여 중인 프로젝트가 없습니다" + 권한 있으면 [프로젝트 만들기] (FR-17) | 목록 조회 실패 시 셀렉터 비활성 + "프로젝트를 불러오지 못했습니다" | 활성 프로젝트가 선택된 상태 | — |
| 이슈 유형 | 셀렉터 **비활성** (B-3) | 발생 불가 (전역 5종 고정) | 비활성 유지 + 안내 | `task` 기본 선택 | — |
| 담당자 검색 | 입력창 옆 소형 스피너 | "검색 결과가 없습니다" (드롭다운 안) | 422 `ASSIGNEE_NOT_FOUND` → 칸 옆 에러 | 선택된 사용자 이름 표시 | — |
| 라벨 | — | 칩 0개 = 입력창만 | 400 → 칸 옆 에러 | 칩으로 표시 | 20개 도달 시 입력창 비활성 |
| 커스텀 필드 | `ui/skeleton` | 프로젝트에 정의 0개면 ③ 덩어리에서 **해당 영역 자체를 숨김** | 필수 미입력 → 해당 칸으로 **자동 스크롤** + 에러 | 값 반영 | — |
| 모달 전체 | 제출 중 **만들기 버튼 비활성** (E7) | — | 403 → 폼 상단 배너(`role="alert"`, 기존 `issues.new.tsx:210` 관례) | 진입 경로별 분기 (FR-16) | 제출 실패 시 **입력값 보존** (다시 채우게 하지 않는다) |
...
| C6 | 신규 E2E — 모달 열기 → 필드 입력 → 1회 제출 | `POST /issues` **호출 1회** · 후속 `PATCH` **0회** 를 네트워크로 단언. **진입 경로별로 나눠 단언한다** (FR-16 조합 효과) — 딥링크 경로는 `/issues/$key` 상세에 5필드 반영, 상단바 경로는 **화면이 그대로이고 토스트가 뜬다** |
