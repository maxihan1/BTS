...
- **FR3. 연결 모드 성공 핸들러 분기** — SAML/OIDC 성공 핸들러가 콜백 시 HttpSession의 intent를 **먼저 소비**(1회용). intent 있으면 일반 로그인을 **하지 않고** 모드별 처리. intent 없으면 기존 JIT 로그인(무변경).
  - LINK 모드: intent.registrationId == 인증 registrationId 검증 → `AccountLinkService.linkExternalSubject(userId, providerId, externalSubject, groups)` 호출(충돌 규칙 적용) → 설정 페이지 `?link=success|already_linked|conflict|error`. **새 세션/JWT 미발급**.
  - REAUTH 모드: 돌아온 `(providerId, externalSubject)`가 intent.userId에 **이미 연결**됐는지 확인(EC9 동형) → 맞으면 `StepUpService.grant(intent.sid)` → `?reauth=success`. 불일치(타 신원) → grant 안 함, `?reauth=failed`.
- **FR4. SSO 연결 충돌 규칙(1차 D5 계승)** — `linkExternalSubject`는 INSERT 전 `findByProviderIdAndExternalSubject` 분기. 타 user→거부(`?link=conflict`)·현재 user→멱등(`?link=already_linked`)·없음→attach(201, `?link=success`). UPSERT는 충돌검사 통과 후에만(ON CONFLICT user_id 보존이 타계정 선점을 조용히 삼키는 것 차단 — PR #8 learning 계승).
- **FR5. SSO 전용 사용자 해제/목록(회귀)** — `DELETE /links/{id}`·`GET /links`는 코드 변경 없음. step-up을 SSO 재인증으로 얻으면 SSO 전용 사용자도 사용 가능함을 통합테스트로 실증.
...
없음. `user_external_accounts`(V002) 재사용. `LinkingIntent`/`ReauthIntent`는 **HttpSession 인메모리 속성**(영속 안 함, 단명). step-up 윈도우는 기존 `StepUpService` 인메모리.
...
- **EC10** **세션 고정(session fixation) 보호로 intent 소실 금지** — Spring 인증 성공 시 `changeSessionId`(기본, 속성 이관) 사용 확인. 속성 미이관 전략(`newSession`)이면 intent 유실 → 검증 필수.
...
- **EC12** 연결 모드 성공 핸들러는 **새 JWT/세션 미발급 + 현재 세션 무변경**(권한 변경/confused-deputy 차단, 필수 검증).
