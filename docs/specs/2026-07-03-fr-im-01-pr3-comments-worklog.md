...
> FR: FR-IM-01 (에픽 PR3 / 4). 신규 FR ID는 게이트1 확정(댓글 도메인 흡수 vs 신규 FR).
...
- **R4 조회 API**. `GET /api/v1/issues/{key}/comments` — VIEW 게이트, `CommentView[]`(id, authorId, body[raw], bodyHtml[렌더], createdAt, updatedAt) created_at ASC. **페이지네이션 없음(MVP)** — 이슈당 댓글 수 제한적, 후속 cursor 여지(FR-API-01 선례). 작성/수정/삭제 REST는 **없음**(별도 FR).
...
- **R8 파서 확장**. `ParsedImportRow`·`ImportRowParser`.
  ...
  - **CSV**. `comment` 다중 컬럼/다중값 각 셀 `date;author;body`(세미콜론 3파트, 3파트 아니면 전체=body·author=null·created=null 폴백). worklog CSV는 `worklog` 컬럼 `author;timeSpentSeconds;started;comment`(4파트 BTS 규약 — Jira CSV는 worklog 표준 export 없음). 파싱 실패 필드는 best-effort 폴백.
...
- **R10 best-effort 경고 3축**. (a) UPDATE 권한 없음 → 댓글·worklog 전부 스킵+경고(사전 체크, 이슈 생성 지속) / (b) worklog timeSpentSeconds ≤ 0 또는 파싱 실패 → 그 worklog 스킵+경고(DB CHECK 회피) / (c) author 이메일 미매칭 → requester 폴백+경고. 경고는 `Success.warnings` 누적 → G1 로그(PR2 계승).
- **R11 dry-run 미러**. dryRun에서 댓글/worklog 존재 행은 UPDATE 권한을 project 스코프로 미리 확인, 없으면 경고(실행 NoPermission 미러). timeSpent≤0도 미리 경고 가능. informational "생성했습니다" 경고는 실행 전용(PR2 CONCERN-B 원칙). 실제 insert 0.
...
- **E6**. CSV 셀이 3파트(댓글)/4파트(worklog) 규약 미준수 → best-effort 폴백(전체=본문 등)+경고.
...
- 어댑터 import 통합테스트(댓글/worklog 생성·author 보존·best-effort 3축·dry-run·행 원자성) green.
