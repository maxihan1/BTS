...
이슈를 종료(StateCategory.DONE) 상태로 전환할 때 Resolution(해결 결과: Fixed / Won't Fix / Duplicate 등)을 필수로 선택하게 한다. Resolution 미설정 시 종료 전환을 거부한다. 강제는 기존 project-workflow `RequiredFieldValidator`를 재사용한다(Jira 방식, ADR 채택).
...
- **When** 상태 셀렉터에서 종료(DONE 카테고리) 전환("완료"/"Done")를 선택한다.
- **Then** Resolution 선택 모달이 뜬다. 표준 Resolution 목록(Fixed/Won't Fix/Duplicate/Cannot Reproduce/Done)이 드롭다운에 표시된다.
- **And** Resolution을 고르고 확인하면 전환이 성공하고, 이슈에 선택한 Resolution이 표시된다.

### S2. Resolution 미선택 시 종료 거부 (guard)
- **Given** 종료 모달이 떠 있다.
- **When** Resolution을 고르지 않고 확인하려 한다.
- **Then** 확인 버튼이 비활성(클라이언트 1차 방어)이거나, 우회 시 서버가 전환을 거부한다(409 TRANSITION_NOT_ALLOWED, RequiredFieldValidator Fail → resolution 누락. 코드 확인: IssueExceptionHandler.kt:37/224).
- **And** 이슈 상태는 변하지 않는다.

### S3. 비종료 전환은 Resolution 불필요
- **Given** "할 일" 이슈를 보고 있다.
- **When** "진행 중"(IN_PROGRESS 카테고리) 전환을 선택한다.
- **Then** 모달 없이 즉시 전환된다. Resolution은 묻지 않는다.

### S4. 재오픈 시 Resolution clear (edge)
- **Given** 종료된(DONE) 이슈에 Resolution=Fixed가 설정돼 있다.
- **When** 비종료 상태(TODO/IN_PROGRESS)로 재전환(재오픈)한다.
...
- **FR3** 종료(DONE) 전환 가드: project-workflow DONE 전환에 `RequiredField` validator(config `{ "field": "resolution" }`) 설정. issue-tracking은 전환 시 `issueFields`에 resolution 전달.
- **FR4** 전환 요청 DTO에 `resolutionId` 추가. transport `TransitionIssueRequest`(toStatusKey/expectedVersion → + resolutionId nullable), application `TransitionIssueRequest`(toStateKey/expectedVersion → + resolutionId nullable).
- **FR5** 전환 성공 시 issue.resolution_id 영속. DONE→비DONE 전환 시 resolution_id = null(clear). **영속은 기존 전환 영속 경로(raw jOOQ `IssueRepository.applyTransition`, 낙관락 UPDATE)를 확장**(resolutionId 파라미터 추가) — 전환 영속이 raw jOOQ인 게 이 BC의 정본 설계다. resolution 필수 불변식은 Issue Aggregate가 아니라 **워크플로우 RequiredField validator가 `plan()`(EXECUTION phase)에서 강제**하므로 patch-merge-domain-bypass(도메인 검증 우회)에 해당하지 않는다. (plan B6 갱신과 일치 — 2026-06-03 재개 시 정정.)
- **FR6** `GET /api/v1/resolutions` — 활성 Resolution 목록(id/key/name/description/displayOrder/isStandard). 종료 모달 드롭다운 소스.
- **FR7** 가용 전환 응답에 목표 카테고리 노출(아래 §결정 1). 프론트가 어떤 전환이 종료인지 판별해 모달 트리거.
- **FR8** 종료 모달(프론트): DONE 카테고리 전환 선택 시 Resolution 드롭다운 표시. 미선택 시 확인 비활성. IssueResponse에 resolution 표시.

## 결정 (스펙 단계 확정 필요 — gate 1)

### 결정 1. 종료 모달 트리거 — 가용 전환에 `toCategory` 노출 (추천)
`AvailableTransitionView`(shared-kernel published language)와 REST `TransitionItem`에 `toCategory: StateCategory`(또는 string "DONE"/"IN_PROGRESS"/"TODO") **additive 추가**. 프론트는 `toCategory === 'DONE'`인 전환 선택 시 모달을 띄운다.
- 장점: "서버가 정답지" 원칙 일관(메모리/ADR bulk-available-transitions-server-side), 1회 호출로 판별, UX 매�끄러움.
- 단점: 가용 전환 published DTO 변경(bulk 교집합 응답도 동반). 단 additive라 회귀 위험 낮음.
- **대안(기각 후보)**: 프론트가 모달 없이 전환 시도 → 서버 409(resolution 누락) → 그때 모달 표시 후 재시도. 2회 왕복·UX 저하. 기각 권장.
...
- `POST /api/v1/issues/{key}/transition` body: `{ toStatusKey, expectedVersion, resolutionId? }`. DONE 전환인데 resolutionId 누락 → **409 TRANSITION_NOT_ALLOWED**(전환 거부 — 기존 전환거부 계약 재사용, IssueExceptionHandler.kt:37/224). 비DONE 전환에 resolutionId 무시(또는 검증 없음).
- `GET /api/v1/issues/{key}/transitions` 응답 TransitionItem에 `toCategory` 추가(결정 1).

### 결정 3. [BLOCKER — Brainstorming 발견] availableTransitions가 RequiredField로 DONE 전환을 숨기는 문제

`WorkflowEngine.availableTransitions`(line 303~314)는 validator가 Fail이면 해당 전환을 목록에서 **제거**한다. DONE 전환에 `RequiredField(resolution)`를 설정하면, 가용 전환 목록 조회 시점(사용자가 아직 resolution 미선택)엔 resolution이 null → **DONE 전환이 목록에서 사라져 사용자가 이슈를 영영 닫지 못한다.** Jira는 전환을 노출하고 transition screen(=종료 모달)에서 필수를 강제한다. 즉 RequiredField는 "노출(availability)"이 아니라 "실행(execution)" 시점 검증이어야 한다.
...
모든 전환에서 `issue.resolution_id = request.resolutionId`로 영속한다. 비DONE 전환은 종료 모달이 없어 resolutionId=null → resolution_id 자동 clear. issue-tracking이 목표 카테고리를 알 필요 없다(S4/FR5 충족). 영속 경로는 FR5 참조(raw jOOQ applyTransition 확장, 도메인 우회 아님 — 불변식은 validator가 강제).
...
- E4. 동일 전환을 두 사용자가 동시에(낙관락) → 기존 IssueVersionConflictException 경로 유지(resolution 추가가 락 시맨틱 변경 안 함).
- E5. 여러 DONE 상태가 있는 워크플로우 → 모든 DONE 전환에 validator 설정돼야 함(seed/설정 책임, plan에서 다룸).
...
- resolution 영속은 기존 전환 영속 경로(raw jOOQ applyTransition) 확장. 필수 불변식은 워크플로우 validator가 plan()에서 강제하므로 patch-merge-domain-bypass 비해당(FR5 참조).
- 프로파일 한정 빈 부팅(메모리 profile-scoped-bean-boot-failure): 새 컨트롤러/빈 추가 시 모듈 전체 test로 부팅 확인.

## 측정 가능한 완료 기준

- [ ] V010 마이그레이션 + init_codegen 미러 → jOOQ 컴파일 통과.
- [ ] DONE 전환 시 resolution 누락 → 422 거부(Testcontainers 통합 테스트로 표면화 — validator/락 경로).
- [ ] DONE 전환 시 resolution 설정 → 성공 + 영속 확인.
