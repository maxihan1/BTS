...
- **E13. 런타임 키 집합 ≠ 정적 분석 결과.** 위 59는 `router.ts`를 정적 분석해 센 수다. 런타임
  `router.routesById`에는 **루트 라우트 항목**(`__root__` 등 `createRootRoute` 유래)이 추가로 들어 있을
  수 있고, 그러면 R5(미분류 = 실패)가 거기서 걸린다. 구현 단계는 **실제 키 집합을 1회 출력해 확인한
  뒤** 서명 맵을 작성한다 — 추측 금지. 키 형식의 기존 증거는 `router.admin-guards.test.tsx:43~47`의
  `/_shell/admin/workflow-schemes` 계열이며, `_shell` pathless 재부모화가 접두사로 붙는다.
  R6의 하한(59)도 실제 키 개수 확인 후 확정한다.
...
- 1회 — 전제 정정 3건(B1 누락 확정 · B2 범위 gap 5건 → Maxi D5=B로 행렬 확대 · B3 상수 이름-거동 불일치)
  + 자체 탐지식 오류 1건(E12) 발견. 스펙 개정.
- 2회 — 개정안 자체 점검에서 런타임 키 집합 미확인 리스크(E13) 발견. "구현 단계에서 실제 키 1회 출력 후
  확정" 으로 명시하고 종료. 남은 미해결 gap 없음.
