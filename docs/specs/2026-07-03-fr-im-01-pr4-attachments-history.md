...
- **R13. shared VO 확장.** `IssueImportCommand`(shared-kernel)에 `sourceKey: String?`·`attachments: List<ImportAttachment>`·`changelog: List<ImportChangeGroup>` additive. `ImportAttachment(filename, authorEmail?, createdAt: Instant?, mimeType?, sizeBytes: Long?)`, `ImportChangeGroup(authorEmail?, occurredAt: Instant?, items: List<ImportChangeItem>)`, `ImportChangeItem(field, fromValue?, toValue?)`. 기존 필드 불변.
- **R14. 어댑터 위임.** `IssueImportAdapter.executeImport`에 createIssue 후 (기존 priority/labels/assignee/version/status/comments/worklogs 뒤) **attachments → changelog** 단계 추가. UPDATE 권한 사전체크(comments/worklogs와 공유). 이메일 해석은 `resolveByEmails` 단일 배치에 첨부 author·이력 author 이메일 합류(`FieldResolution` 확장).
- **R15. tx 오염 사전체크.** 잔여 throw 강등 — 첨부(권한·zip 부재·MIME 사전판정; 스캔 예외는 upload가 repo.insert 전 throw라 catch 안전) / 이력(author 미해석→null·시각 실패→스킵·미매핑→스킵으로 사전 정제, recordImported는 append-only nullable insert라 throw 위험 낮음). **첨부 upload가 행 tx를 오염시키지 않음(내부 MinIO/ClamAV I/O tx 밖·scan은 insert 전)을 통합테스트로 실증**.
