...
- `AutomationRuleRepository.findByProject(projectKey)`로 프로젝트의 **삭제되지 않은 모든 규칙**을
  로드하고, 액션·조건을 **배치 조회**(`findByRuleIds`)로 hydrate한다 — 쿼리 총 3회(규칙·액션·조건).
- 사이클·필드·우선순위 분석의 그래프 노드는 **enabled 규칙만** (disabled는 발화하지 않으므로 제외).
- 권한 분석은 **enabled 규칙만** 대상 (disabled는 실행 안 됨).
- 성능(NFR): 프로젝트당 배치 로드 1회. **규칙별 N+1 조회는 금지한다** — `IN` 절 배치로 묶는다.

  > **★2026-09-09 정정.** 원문은 「규칙별 액션 조회 N+1은 허용하되 100규칙 1s 임계 내
  > (필요 시 IN 절 배치 조회로 최적화 — plan에서 판단)」이었다. 그 판단이 실측으로 내려졌다.
  >
  > 맥에서는 성립했다(`#268` 실측 0.876s). 그러나 운영과 같은 **2코어 리눅스**에서 재자
  > 조건이 깨졌다 — 젠킨스 빌드 #18 에서 `12,583ms`.
  >
  > | n | findByProject | hydrate(N+1) | analyze(CPU) |
  > |---|---|---|---|
  > | 100 | 19ms | **2,331ms** | 7ms |
  > | 200 | 17ms | **4,152ms** | 16ms |
  >
  > 왕복 1회당 약 11.6ms · **전체의 98.9%**. 같은 100건을 `findByProject` 는 한 번에 19ms 에
  > 가져오므로 쿼리가 느린 게 아니라 **왕복이 많다**. 배치 2회로 바꿔 `1,363ms` (9.2배).
  >
  > 함께 배운 것 — `RuleConflictAnalyzer` 는 O(n²)(`buildTriggerGraph` n×n +
  > `coFiringPairs` n(n-1)/2)인데 200개에서 **16ms** 다. 코드만 보면 그쪽이 범인처럼 보이지만
  > 실측이 0.3% 로 만들었다. **재보지 않고 고쳤으면 절반이 헛수고였다.**
  >
  > 「임계 내면 허용」이라는 조건부 허용은 **어느 기계에서의 임계인지**를 함께 적지 않으면
  > 환경이 바뀌는 순간 의미가 증발한다.
...
- **대량 규칙(100+)**. NFR 1s 임계. 배치 로드로 해소(2026-09-09 · 2코어에서 1.4초).
  그래프 크기 제한은 **불필요**하다 — CPU 분석은 200개에서 16ms 로 병목이 아니다.
