...
Then 번다운의 **세로축**이 그 방식을 따른다 — `잔여 추정 + 소요 시간` 이면 **시간(초)**,
`없음` 이면 **이슈 개수**다(첫날 3개 → 하나 완료 후 2개). **칸반 보드에서는 이 탭이 잠긴다**(J37).
★**task-35 이전에는 이 문장이 사실이 아니었다.** 값은 저장·조회됐지만 `SprintBurndownService` 와
`BurndownCalculator` 어디에도 `timeTracking` 을 읽는 줄이 없어 차트가 한 픽셀도 바뀌지 않았고,
그 상태를 리뷰가 머지 BLOCKER 로 잡았다. 지금은 응답이 `unit`(`SECONDS`/`ISSUE_COUNT`)을 함께 싣고
`BurndownIssueCountAxisTest` 의 대조군이 두 축을 한 쌍으로 잰다.
★**기본값 `NONE` 은 「현행 유지」가 아니다.** `boards.time_tracking` 이 `NOT NULL DEFAULT 'NONE'`
(V509)이라 기존 보드 전량이 개수 축으로 바뀐다 — **배포 시점의 동작 변경이며 선언된 변경**이다.
근거 넷은 `SprintBurndownService.resolveUnit` KDoc 이 정본이고, 시간 축을 유지하려면 배포와 함께
`UPDATE boards SET time_tracking = 'REMAINING_AND_SPENT'` 를 돌리면 된다.
...
- **R4b (갭 C · task-35)** 그 값이 **번다운 세로축의 단위**를 정한다 — `REMAINING_AND_SPENT` 는
  시간(초), `NONE` 은 **이슈 개수**다(S2). 개수 축은 「가시 이슈 수」와 「완료 시각」을 필요로 하므로
  `SprintBurndownLookupPort` 가 `visibleIssueCount` · `issueCompletions` 를 함께 나른다(X9 의 자리).
  **완료 = 지금 DONE 카테고리이고 시각은 마지막 DONE 전환**이다(전환 이력이 정본 — 되돌린 이슈는
  완료가 아니다). 개수는 **가시 이슈 수**여서 기밀 이슈가 개수 차로 새지 않는다.
...
★**반대로 `time_tracking` 은 그 관용구를 쓸 수 없다**(task-35). `NOT NULL DEFAULT 'NONE'` + CHECK 2값
이라 「미설정」을 표현할 자리가 없고, 칸만 NULL 로 바꾸면 화면(라디오 2개)은 「없음」이라 말하는데
차트는 시간으로 도는 **새 거짓말**이 된다. 그래서 R4b 는 기본값 `NONE` 을 그대로 개수 축으로 읽고,
그 대신 **배포 시점에 기존 보드의 차트가 바뀐다는 사실을 S2 에 명시**한다. V509 주석의
「기본값이 현행 동작이라 백필이 무변경이다」는 그 칸을 **읽는 코드가 없던 시점의 사실**이었고,
`TimeTracking` enum KDoc 이 그 문장을 정정해 두었다.

## 엣지 케이스

- **E1** 근무일을 **0개**로 저장한다 → 400. 허용하면 ideal 선이 0 나눗셈이다
  (`computeIdeal` 이 `totalDays == 0` 을 이미 특수 처리하지만 그것은 1일 스프린트용 경로다 —
  task-35 에서 두 축이 같은 계산을 공유하며 `computeIdealSeconds` 에서 이름이 바뀌었다).
...
4b. ★**추정 탭이 번다운에 닿는다**(task-35 · S2). 같은 원천 데이터에 설정만 바꿔 재는 대조군으로,
   `REMAINING_AND_SPENT` 는 시간 축(24h → 16h)이고 `NONE` 은 개수 축(3개 → 2개)이다.
   한쪽만 두면 「항상 시간 축」/「항상 개수 축」 중 하나가 그대로 통과한다 —
   게이팅을 끊는 뮤테이션이 죽이는 판정을 `BurndownIssueCountAxisTest` KDoc 이 실측으로 적는다.
