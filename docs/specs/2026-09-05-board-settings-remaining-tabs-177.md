...
| **J45** | B | **뷰별 구성** — *"The fields can be different for the backlog and Active sprints, if you are using a Scrum board."* | [Customize cards](https://support.atlassian.com/jira-software-cloud/docs/customize-cards/) · Cloud · 2026-09-05 |
...
- **R2 (갭 B)** 카드에 표시할 **추가 필드를 최대 3개** 고른다(J31). 후보는 **커스텀 필드**(J32 의 2층)와
  `BoardCardResponse` 가 이미 나르는 표준 필드 둘 다이다. 요약(`summary`)은 J32 의 1층이라
  **항상 최상단이고 토글 대상이 아니다.**
...
- **R3 (갭 B)** ★**구성은 뷰마다 따로다.** 스크럼 보드는 **백로그**와 **활성 스프린트**가 서로 다른
  필드 집합을 가질 수 있다(J45 원문). 칸반은 보드 뷰 하나뿐이다.
  **초안의 「보드와 백로그에 함께 적용」은 오독이었다** — J32 의 *"on the board and backlog"* 는
...
- 400 — 한 뷰의 `cardLayout` 이 4개 이상(J31) · 미지원 필드 키 · `timezone` 이 IANA 가 아님 ·
...
-- 갭 B. ★뷰마다 따로다(J45) — 배열 한 칸으로는 못 담는다.
CREATE TABLE board_card_layout_fields (
  board_id   UUID    NOT NULL REFERENCES boards (id) ON DELETE CASCADE,
  view_scope VARCHAR(16) NOT NULL,   -- 'BOARD' | 'BACKLOG'
  position   SMALLINT NOT NULL,      -- 0..2 — 상한 3(J31)을 DB 가 지킨다
...
3. ★**뷰별 분리 판정.** 스크럼 보드에서 `BOARD` 구성과 `BACKLOG` 구성을 **다르게** 저장하면
   보드 카드와 백로그 카드가 서로 다른 필드를 그린다(J45).
...
*"The fields can be different for the backlog and Active sprints, if you are using a Scrum board."*(J45) 다.
J32 의 *"on the board and backlog"* 를 「구성 공유」로 읽은 오독이었다 — 그 문장은 **요약이 항상 맨 위**라는 서술이다.
