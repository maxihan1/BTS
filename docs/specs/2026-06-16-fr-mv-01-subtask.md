...
| FR-S-3 | `POST /{key}/move` 요청에 선택적 `subtasks: [{issueId, expectedVersion, targetStateKey, targetStateIsDone, componentMapping, affectsVersionMapping, fixVersionMapping, customFieldValues}]` 추가. **하위 호환** — 비어있으면 단건 경로 |
...
| EC19 (신규) | 자식 노드 상태 비호환인데 `targetStateKey` 미지정/대상에 없음 | 422 `INVALID_TARGET_STATE` |
...
- move 의 `subtasks` 식별은 `issueId`(UUID). 루트는 path `{key}`
