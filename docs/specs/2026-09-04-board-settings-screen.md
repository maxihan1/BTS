...
When 화면이 열린다
Then **기본 보드를 스스로 고르지 않고** 「보드를 지목해 주세요」 안내와 보드 화면 링크를 보인다. 「기본 보드」 규칙을 **4번째로 늘리지 않는다**.

> **초안 정정 (구현 중 · 리뷰 CONCERNS C8).** 초안은 「보드 화면으로 **되돌린다**」였다 — 자동
> 리다이렉트다. 구현은 **안내한다**로 갔고, 그 판단이 맞다. 자동으로 튕기면 사용자는 URL 을 직접
> 친 이유(북마크·공유 링크)를 잃고 「왜 튕겼지」만 남는다. 되돌릴 목적지도 애매하다 — 어느 보드로
> 갈지가 곧 「기본 보드」 규칙이고, 이 시나리오가 막으려던 것이 정확히 그 4번째 규칙이다.
> 안내 + 링크는 **선택을 사용자에게 남기면서** 규칙을 하나도 늘리지 않는다.
...
- **E8 한 사람의 연속 드롭 (eng 리뷰 CONCERN-3).** E6 은 **두 사람**만 봤다. 실제로 더 자주 밟는 것은
  **한 사람이 빠르게 두 번 드래그**하는 경우다. R5 가 **집합 전체**를 보내므로 두 요청이 같은
  ...
  아무도 오류를 못 본다.
  **처방(구현이 확정) — in-flight 동안 드래그 자체를 잠근다.**
  초안은 「뮤테이션을 직렬화하고 서버가 돌려준 집합에서 파생한다」였다. 코드리뷰 BLOCKER-B2 가
  **그 처방이 구현되지 않은 채 이름만 남았음**을 실증했고(테스트 describe 제목에 `E8` 이 있으나
  단언은 한 계획 *내부* 순서만 쟀다), 고치면서 기제를 바꿨다.

  큐에 쌓는 대신 **두 번째 드롭을 만들지 못하게** 한다 — `useColumnSettingsDrag` 가 `isMutating` 을
  돌려주고 화면이 그것으로 `draggable` 을 끈다. 큐는 「무엇이 반영됐나」를 더 어렵게 만들고,
  무효화 재조회가 끝나면 잠금이 저절로 풀리므로 체감 지연이 한 왕복뿐이다.
  판정은 `use-column-settings-drag.test.tsx` 의 **T-DR-5·6·7** 이 진다(두 축을 따로 잰다).
...
2. `?board=` 없이 들어오면 **자동 이동 없이** 보드 지목 안내와 보드 화면 링크를 보인다 (S8 · 기본 보드 규칙 4번째 미생성).
