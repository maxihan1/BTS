# FR-WF-08 프로젝트 소유 워크플로우 — PROJECT_ADMIN 이 자기 프로젝트의 워크플로우·스킴을 만들고 고친다

**신설 FR.** 정본 카운트 145 → 146. 선행 FR-WF-02(스킴) · FR-WF-04(워크플로우 편집) · FR-PM-04(권한)를 **전제로 삼되 그 FR 들을 다시 열지 않는다**(Maxi 2026-09-08 확정). Jira Cloud team-managed 프로젝트 모델 채택.
...
## 7. 확정 결정 (Maxi 2026-09-08)

### D7. FR 은 신설한다 — `FR-WF-08`, 정본 145 → 146

선행 3 FR(WF-02 · WF-04 · PM-04)의 범위 확장으로 처리하지 **않는다.**

- 완료 선언한 FR 3개를 다시 미완료로 되돌리지 않는다 — 「145/145 D 완료」가 거짓이 되고 진척 대시보드가 퇴행한다.
- 「이 변경이 어느 FR 의 일인가」가 세 곳으로 흩어지지 않는다.
- 대가는 카운트 리터럴 전수 동기화다. `verify-master-plan.sh`(EXIT 4)가 기계로 강제하므로 누락은 막힌다. 대상 정본은 `docs/rules/fr-sync-checklist.md` 전 항목이다.

★**카운트 리터럴을 손으로 세지 않는다.** 이 문서에도 개수를 새기지 않는다 — 「145」·「146」은 이 절과 마스터플랜에만 있고, 나머지는 생성기·검증기가 판정한다.

### D8. 스코프 타입은 shared-kernel 공용 `WorkflowScope` 로 신설한다

`WorkflowSchemeScope` 를 일반화해 재사용하지 **않는다.**

`WorkflowDefinitionPermission` 의 KDoc 이 이미 이 함정을 이름으로 경고하고 있다.

> 그쪽은 **스킴**(이슈 타입별 워크플로우 매핑 묶음)의 관리·배정 권한이다. 이름이 스킴을 뜻하는데 워크플로우 정의 편집을 담게 되면 다음 사람이 두 개념을 같은 것으로 읽는다. 도메인당 enum 1개 + resolver 1개가 이 저장소의 관례다.

이 저장소는 **같은 이유로 enum 을 이미 둘로 갈랐다.** 스코프 타입만 이름을 섞으면 그 결정을 되돌리는 것이다.

동시에 **정의 resolver 에 전용 스코프 타입을 따로 두지도 않는다** — 두 스코프 타입은 서로를 검사하지 않는 두 목록이 되어, 한쪽에만 새 스코프가 추가돼도 조용히 통과한다.

⇒ 중립 이름의 **공용 `WorkflowScope`(shared-kernel)** 하나를 두고 스킴·정의 resolver 가 함께 쓴다. 마이그레이션은 `WorkflowSchemeScope` 사용처 치환 1회.

## 8. 티어

**T3.** 마이그레이션(2테이블) + shared-kernel(공용 `WorkflowScope` 신설) + 보안 경로(권한 판정기 2종). ADR + plan + TDD + 마이그레이션 검증 + 리뷰 2종 + ceo 필요.
