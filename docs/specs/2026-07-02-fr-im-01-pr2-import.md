# FR-IM-01 PR2 — Import 컴포넌트/버전 자동생성 + 소스 상태 전환 — 스펙
...
3. **소스 상태 전환** — Jira 이슈 상태를 대상 워크플로우 상태로 직접 set(FSM 우회, IssueMoveService 선례).
...
- **S6 (소스 상태 전환)**. Given 실행자 TRANSITION 보유 + 대상 워크플로우에 "In Progress" 상태 존재, When 행의 status="In Progress", Then 이슈를 "In Progress"로 직접 이동(생성 직후 시작 상태 → 목표 상태).
- **S7 (상태 미매칭/권한없음 → best-effort)**. Given status="Frozen"이 대상 워크플로우에 없음(또는 TRANSITION 없음), Then 경고 + 이슈는 **시작 상태 유지**.
- **S8 (dry-run 미러)**. When dryRun=true, Then 실제 생성/전환 없이 컴포넌트/버전 생성 권한·TRANSITION 권한·상태 name 매칭 여부를 미리 검증하고 **유효성-예측 경고**(권한 부재·상태 미매칭)를 리포트. 단 "자동 생성했습니다" 같은 **생성-완료 informational 경고는 실행 전용** — dry-run 은 실제 생성이 없어 그대로 옮기면 거짓이므로 미러하지 않는다(codereview CONCERN-B 확정).
...
- **R7 dry-run 실경로 미러**. dryRun에서 PR1의 CREATE/UPDATE 확인에 더해 — 버전 링크 유발 행(fix/affects 존재)은 UPDATE(EDIT_ISSUE)로 `rowTriggersUpdate` 확장, 컴포넌트 생성 필요 행은 MANAGE_COMPONENTS, 버전 생성 필요 행은 MANAGE_VERSIONS, 상태 전환 유발 행은 TRANSITION을 project 스코프로 미리 확인하고 **유효성-예측 경고를 산출한다**(PR1 CONCERN C1 원칙 계승 — dry-run 유효성 판정 == 실행 판정). admin 축 권한 부재는 dry-run에서도 경고(행 유효), basic UPDATE 부재는 dry-run에서도 FORBIDDEN(행 실패 예측). 상태 name 미매칭(TRANSITION 권한 有)도 dry-run에서 `IssueImportStatusService.statusNameMatches`로 실행과 동일 기준 미리 검사해 NoMatch 경고를 산출한다(codereview CONCERN-A). 단 컴포넌트/버전 "자동 생성했습니다" 같은 **생성-완료 informational 경고는 실행 전용**(dry-run은 생성이 없어 미러 시 거짓 — codereview CONCERN-B).
...
- **전환 축** — status=`IssuePermission.TRANSITION`. D-B에 따라 **권한 부재·상태 미매칭 모두 경고+시작 상태 유지**. status-set(별도 `IssueImportStatusService`)이 `hasPermission` **사전 체크**로 판정해 예외 없이 경고화. 마이그레이션 특성상 "시도"(경고)로 처리(D-B 확정).
...
- **권한 위임 불변**. 컴포넌트/버전 생성·상태 전환 모두 actor=requester로 기존 게이트 재사용. search BC는 권한을 알지 못하고 우회 불가(PR1 핵심 불변 계승).
...
- **E4**. status == 시작 상태 → 전환 no-op, 경고 없음.
...
- 통합 테스트(Testcontainers 실 DB). 새 컴포넌트/버전 생성 확인 · affects/fix 링크 테이블 행 확인 · current_state_key 전환 확인 · 권한 없을 때 경고+이슈 생성 확인 · dry-run 무생성 확인.
...
- **G2**. 권한 3축(생성=admin 경고 / 편집=basic 행실패 / 전환=경고) 명시화 + status는 `hasPermission` 사전체크로 경고화.
