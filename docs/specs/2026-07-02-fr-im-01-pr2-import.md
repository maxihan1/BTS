...
- **S8 (dry-run 미러)**. When dryRun=true, Then 실제 생성/전이 없이 컴포넌트/버전 생성 권한·TRANSITION 권한·상태 name 매칭 여부를 미리 검증하고 **유효성-예측 경고**(권한 부재·상태 미매칭)를 리포트. 단 "자동 생성했습니다" 같은 **생성-완료 informational 경고는 실행 전용** — dry-run 은 실제 생성이 없어 그대로 옮기면 거짓이므로 미러하지 않는다(codereview CONCERN-B 확정).
