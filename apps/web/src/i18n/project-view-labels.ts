 ...
 * ④ `리포트` 조회는 **역할(role)까지 지정**해야 한다. 아래 실측대로 두 레지스트리 «안»에서는
 *    substring 관계가 0건이지만, 탭바 «밖»에 같은 낱말이 한 화면에 둘 더 있다(2026-09-08 실측).
 *
 *    | 공존 문구 | 관계 | 어디서 만나나 | role |
 *    |---|---|---|---|
 *    | `리포트 전환` (`ProjectReportsNav` 의 `aria-label`) | `리포트` ⊂ 값 | 리포트 4화면 | `navigation` |
 *    | `리포트` (리포트 착지 화면 제목) | **값이 완전히 같다** | `/projects/$key/reports` | `heading` |
 *
 *    둘 다 탭 링크와 **role 이 달라** `getByRole('link', { name: '리포트', exact: true })` 로는
 *    안 잡힌다. 그래서 규약 ①(nav 스코프) + role 지정이면 충분하고 면제표에 줄이 늘지 않는다.
 *    🛑 `getByText('리포트')` 처럼 role 을 안 쓰는 조회는 착지 화면에서 **2건**을 잡는다.
 */
export const projectViewLabels = {
  ...
  /**
   * 리포트 탭 — 착지는 개별 차트가 아니라 4종 카드 목록이다 (Jira 패리티 JR-2).
   *
   * ⚠️ 두 레지스트리 «안»에서는 substring 관계 0건이지만(실측), 탭바 밖의 `리포트 전환`·
   * 착지 화면 제목 `리포트` 와 한 화면에 공존한다. 조회 규약 ④ 참조.
   */
  reports: '리포트',
