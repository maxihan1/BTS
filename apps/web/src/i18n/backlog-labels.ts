...
export const backlogLabels = {
  ...
  startDialog: {
    ...
    /** ② `PATCH` 409(E9) — 남이 먼저 고쳤다. 최신 값을 받아 기준값을 교체한 뒤 재확인을 요청한다 */
    patchConflict: '다른 사람이 먼저 수정했습니다. 최신 값을 불러왔으니 확인 후 다시 시도해 주세요.',
  ...
  completeDialog: {
    ...
    /** E14 — 워크플로우 조회 실패. 전건 미완료로 보되 다이얼로그를 막지 않는다 (FR-7 fail-safe) */
    workflowLoadFailed: '상태 분류를 불러오지 못해 모든 이슈를 미완료로 봅니다.',
