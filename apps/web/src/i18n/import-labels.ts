...
export const importLabels = {
   ...
   *
   * ★**실패 가능성은 반드시 남긴다 — 단, 「거부」라고 단정하지 않는다** (게이트 2 재리뷰 D7).
   *   두 모드의 실패 방식이 다르기 때문이다.
   *   · 기본(바로 가져오기) — 서버가 mutation **이전에** 확실히 거절한다
   *     (`ImportJobService.kt:380-394` 가 `storage.put`·insert·enqueue 보다 앞선다).
   *   · 마법사(매핑) — confirm 은 CREATE 를 재검사하지 않아(`ImportMappingService.kt:439-450` —
   *     소유권·상태만) **접수된 뒤 뒤늦게 실패**한다. 서버 몫은 부채 매핑 `35` 로 등재했다.
   *   「**실패할 수 있습니다**」는 두 경로 모두에 참인 유일한 표현이다. 초판은 「제출이 거부될 수
   *   있습니다」라 마법사 경로에서 거짓이었고, 그 다음 판은 그 문장을 통째로 빼서 **반드시 실패할
   *   작업을 두고 아무 경고도 안 하게** 됐다. 이 판이 그 둘 사이다.
   *
   * ★이 문구에 「실패」가 남아 있는지는 `settings.import.test.tsx` 가 단언으로 고정한다 —
   *   무해한 소음으로 보여 조용히 깎이는 것을 막는다.
   */
  gateChangedWhileBusy:
    '이 프로젝트의 상태가 바뀐 것 같습니다. 진행 중인 작업은 그대로 두었지만, 이 작업은 실패할 수 있습니다.',
  /**
