...
export const profileLabels = {
   ...
   * 문구는 provider-중립("디렉터리") — 현재 LDAP만 결선이나 SAML/OIDC 확장에도 그대로 통용.
   */
  ldapSource: {
    /** 동기화 상태 배지(source=LDAP) */
    syncedBadge: '디렉터리에서 동기화됨',
    /** 동기화 상태 힌트 — 직접 수정 시 동기화가 끊김을 안내 */
    syncedHint: '직접 수정하면 디렉터리 동기화가 중단됩니다.',
    /** 직접 편집 상태 배지(source=USER) */
    overriddenBadge: '직접 편집됨',
    /** 직접 편집 상태 힌트 — 재설정의 지연 동기화 semantics 안내 */
    overriddenHint: '재설정하면 다음 로그인 시 디렉터리 값으로 동기화됩니다.',
    /** 디렉터리 값으로 재설정 버튼 텍스트(평상시) */
    resyncButton: '디렉터리 값으로 재설정',
    /** 디렉터리 값으로 재설정 버튼 텍스트(제출 중, isPending) */
...
export function mapProfileError(error: ApiError): string {
  ...
  switch (extractProfileErrorCode(error.body)) {
    ...
    case 'DISPLAY_NAME_NOT_LDAP_LINKED':
      return '연결된 디렉터리 계정이 없어 재설정할 수 없습니다.'
    default:
