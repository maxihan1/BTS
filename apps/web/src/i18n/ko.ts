...
export const issueDetailStrings = {
   ...
   *
   * ★「화면에 보이지 않는」을 문구에 넣는 이유. 백엔드는 **활성 정의 전량**을 기준으로
   * 병합·검증하는데 클라이언트는 열람 권한이 없는 필드를 **렌더도 검증도 하지 않는다**.
   * 그 필드가 required 이고 비어 있으면 사용자는 **화면에 없는 필드 때문에** 저장에 실패한다.
   * 이전에는 이 경우가 「잠시 후 다시 시도해 주세요」로 떨어져 **재시도해도 안 되는데
   * 재시도를 권했다.** 원인을 짐작할 단서를 준다.
   */
...
export const issueCreateStrings = {
  ...
  /**
   * 선택된 프로젝트에 이슈 생성 권한이 없을 때 (2026-08-09 봉합).
   *
   * 이전에는 이 경우도 `errorDefault` 로 떨어져 **재시도해도 안 되는데 재시도를 권하는**
...
export const issueMoveStrings = {
   ...
   * 다만 실행은 preview 를 통과한 뒤라 대상 키는 **이미 서버가 받아들인 값**이므로,
   * preview 문구의 「키부터 확인」 도입부는 뺀다.
   *
   * **★2026-08-12 정정 (PR #367).** 이전 서술은 [errorPreviewForbidden] 과 **같은 어휘**
   * (「이 이슈나 대상 프로젝트」)를 쓴다고 적었는데, 같은 PR 에서 preview 문구가 매핑 `18`
   * 재판정으로 「권한이 없는 것이니 …」로 압축되면서 **그 서술이 거짓이 됐다.**
   * 지금은 **일부러 다르다.**
   *
   * | | preview 403 | 실행 403 (여기) |
   * |---|---|---|
   * | 대상 키 | 아직 서버가 안 받아들였다 → 「키 확인」을 앞세운다 | 이미 통과했다 → 언급하지 않는다 |
   * | 권한 두 갈래 | 글자 예산상 「권한이 없는 것이니」로 압축 | **명시**(「이 이슈나 대상 프로젝트」) |
   * | 다음 행동 | 「다른 프로젝트로 시도」 | 없음 — 여기까지 온 사용자에겐 재시도가 답이 아니다 |
   *
   * 두 문구가 갈린 것은 **단계마다 사용자가 아는 것이 다르기 때문**이지 관리 소홀이 아니다.
   * 어느 쪽도 원인을 **원본 이슈 쪽으로 단정하지 않는다**(2026-08-10 재리뷰 요구).
   */
   ...
   * 다시 붙여 놓았고**, 무엇보다 이 경로의 원인과 무관하다.
   */
  errorPreview: '이슈 이동 정보를 불러오지 못했습니다. 페이지를 새로고침한 뒤 다시 시도해 주세요.',
  /**
   * preview 실패 — **5xx 또는 응답 자체를 받지 못한 경우**(네트워크 단절 · 타임아웃 ·
   * 응답 해석 실패).
   *
   * 이 셋은 **대상 키와 아무 상관이 없다.** 옛 문구는 여기까지 「키를 확인해 주세요」로
   * 받아 멀쩡한 키를 몇 번씩 고쳐 쓰다 포기하게 만들었다 — 이 부채의 본체가 그것이다.
   *
   * ⚠️ **응답 해석 실패(Zod 계약 파손)도 여기 들어온다.** 계약 파손에 「일시적」은
   * 정확하지 않지만 **키를 의심하게 만들지 않는다**는 요구는 충족한다. 계약 파손을 따로
   * 가르는 것은 이 문구의 문제가 아니라 오류 분류 체계의 문제라 여기서 다루지 않는다.
   */
  errorPreviewTemporary:
   ...
   *
   * ─── 2026-08-12 재판정 (PR #367 · Maxi 확정) ─────────────────────────────
   *
   * **① 이 세 원인은 형식 게이트를 넣어도 셋 그대로다.** `isValidProjectKey` 는 **형식만**
   * 보므로 `NOPE`(형식 OK · 미존재)는 그대로 여기까지 온다. 「게이트 도입 후 원인이 3 → 2 로
   * 줄어 「키 확인」 앞부분을 뺄 글자 예산이 생긴다」는 재판정 지시의 전제는 **거짓이었다**
   * (`MoveIssueDialog.test.tsx` T4-8 「게이트는 존재를 모른다」가 실측).
   *
   * **② 그래서 앞부분은 유지하고, 뒤에 다음 행동을 되살렸다.** 이전 문구(53자)는 「키는 맞는데
   * 대상 프로젝트 CREATE 권한만 없는 사용자」에게 **다음에 할 일이 없었다.**
   *
   * **③ 되살리는 대가는 실측으로 0 이다.** 실제 브라우저에서 잰 줄수
   * (`issue-move.spec.ts` E2E-6).
   *
   * | 안 | 자수 | 데스크톱 464px | 390px | 320px |
   * |---|---|---|---|---|
   * | 이전(행동 안내 없음) | 53 | 2줄 | 2줄 | 2줄 |
   * | 원안 복원 | 74 | 2줄 | **3줄** | **3줄** |
   * | **현행(압축 복원)** | **55** | **2줄** | **2줄** | **2줄** |
   *
   * 원안(74자)을 그대로 되살렸다면 좁은 폭에서 3줄이 됐다 — 「70자를 넘기면 두세 줄이 된다」는
   * 원래 반대 근거는 **데스크톱에서만 거짓**이었다. 데스크톱 한 폭만 쟀으면 반대로 결론 냈다.
   *
   * ⚠️ **「관리자에게 문의」는 되살리지 않았다** — 재리뷰가 지적한 막다른 길이다.
   * ⚠️ 두 갈래 명시(「이 이슈나 대상 프로젝트」)를 「권한이 없는 것이니」로 압축한 것이 대가다.
   *    어느 쪽인지 **단정하지 않으므로** 재리뷰 요구(원본 이슈 쪽으로 단정 금지)는 충족한다.
   *
   * 근거 전문. `TODOS.md` 「이동·임포트 진입점」 ② · 매핑 `18` 해소 블록.
   */
...
export const commentStrings = {
  ...
  // ── FR-UX-03 인박스 딥링크 ────────────────────────────────────────────
  /**
   * 딥링크가 지목한 댓글 행에 붙는 sr-only 안내.
   *
   * 강조는 링·배경색으로만 표현되므로 시각 밖에서는 존재하지 않는다. 포커스 이동이
   * 댓글 내용은 읽어 주지만 "이게 알림이 가리킨 그 댓글" 이라는 의미까지는 옮기지 못한다.
   */
  commentFocusTargetScreenReader: '알림에서 지목한 댓글',
} as const
