...
export const issueMoveStrings = {
  ...
  /**
   * preview 403 — 권한 부족 전용 문구.
   *
   * ★`errorForbidden`(「이슈를 이동할 권한이 없습니다.」)을 재사용하지 않는 이유.
   * 서버는 **원본 UPDATE 없음**과 **대상 CREATE 없음** 둘 다에서 같은 403 을 준다
   * (`MovePreviewService.kt:185-190` 이 두 권한을 연달아 assert 하고, 응답 errorCode 는
   * `ACCESS_DENIED` 하나뿐이라 클라이언트가 둘을 구분할 수 없다).
   * 「이슈를 이동할 권한이 없습니다」는 원인을 **이슈 쪽으로 단정**해, 원본 권한은 있고
   * 대상 프로젝트만 막힌 사용자에게 「이 이슈는 영영 못 옮긴다」로 읽힌다 — 다른 대상을
   * 고르면 되는데도. 그래서 두 가능성을 **함께** 말하고 다음 행동을 제시한다.
   *
   * `errorPreview`(키 오타 안내)와 반드시 갈라야 한다 — 권한 문제를 오타처럼 보이게 하면
   * 사용자는 맞는 키를 계속 다시 친다.
   */
