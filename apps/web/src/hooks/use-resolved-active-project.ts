...
export function useResolvedActiveProject(
  ...
  const resolved = useMemo<ResolvedActiveProject>(() => {
    ...
    if (isPending || projects === undefined) return { status: 'loading' }
    ...
    if (key === null) return { status: 'empty' }
    return { status: 'ready', projectKey: key, source }
  }, [isError, isPending, projects, urlProjectKey, activeProjectKey])
