...
import { ruleExecutionDetailSchema, ruleExecutionSummarySchema } from '@/api/automation-executions.types'
...
describe('GET /automation/executions/:id (단건 trace)', () => {
  it('존재하면 outcomes/triggerEvent를 포함한 상세를 200으로 반환한다', async () => {
    ...
    const parsed = ruleExecutionDetailSchema.parse(body)
    expect(parsed.id).toBe(SEED_EXECUTION_IDS.scheduledSuccess)
    expect(parsed.outcomes.length).toBeGreaterThan(0)
    expect(parsed.projectKey).toBe(DEFAULT_AUTOMATION_PROJECT_KEY)
  })
...
describe('POST /automation/executions/:id/replay', () => {
  it('원본을 복제해 새 실행 이력을 생성하고 후속 목록 GET에 즉시 반영한다(stateful)', async () => {
    ...
    const parsed = ruleExecutionDetailSchema.parse(body)
    expect(parsed.id).not.toBe(sourceId)
    expect(parsed.replayedFrom).toBe(sourceId)
    expect(parsed.ruleId).toBe(SEEDED_EXECUTION_RULE_IDS.scheduled)
    expect(parsed.issueKey).toBe('ATLAS-101')
