...
describe('passwordHandlers — POST /api/v1/users/me/password (EC-7 분기순서)', () => {
  ...
  it('EC-7-1: 새 비번 12자 미만("short") → 400 POLICY_VIOLATION + violations[MIN_LENGTH]', async () => {
    ...
    const body = await res.json() as { code: string; message: string; violations: string[] }
  ...
  // 분기순서 보장: SAME_AS_CURRENT는 CURRENT_PASSWORD_MISMATCH 전에 판정됨
  // 새 비번 == 현재 비번이고 currentPassword가 틀린 경우 → POLICY 판정 후 SAME_AS_CURRENT(분기 3)이어야 함
  // (이 케이스는 currentPassword 틀린 경우지만 new==seed가 아니라서 mismatch 도달)
  it('EC-7-order: POLICY_VIOLATION → SAME_AS_CURRENT → MISMATCH 순서 보장', async () => {
    // 정책 위반이 SAME_AS_CURRENT보다 먼저 판정되는지 확인
    // 새 비번이 seed와 같지만 정책도 위반하는 경우 → POLICY_VIOLATION이 먼저
    const res = await postPassword('CurrentPass123!', 'short')
    const body = await res.json() as { code: string }
  ...
  it('EC-7-violations: POLICY_VIOLATION 응답에 violations 배열 포함', async () => {
    ...
    const body = await res.json() as { code: string; violations: unknown }
  ...
  it('EC-7-message: 각 에러 응답에 message 필드 포함', async () => {
    ...
    const policyBody = await policyRes.json() as { message: string }
