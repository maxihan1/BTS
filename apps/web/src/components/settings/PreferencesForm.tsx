// 사용자 환경설정(테마/언어/날짜포맷/시작 페이지) 편집 폼 — 선택 즉시 PATCH 저장 + 날짜포맷 라이브 프리뷰 (FR-PF-01 Task 7, FR-PF-02 Task 6)
...
import { START_PAGE_KEYS, START_PAGE_LABELS, isStartPage } from '@/lib/start-page'
...
/** 시작 페이지 셀렉터 하단 안내 문구 — 즉시 적용이 아니라 다음 로그인 시 반영됨을 명시(게이트1 확정) */
const START_PAGE_NOTICE = '다음 로그인부터 적용됩니다.'
...
/** whoami의 startPage 필드에서 유효한 StartPage를 읽는다 — 부재/알 수 없는 값이면 'dashboards' */
function resolveInitialStartPage(value: string | undefined): StartPage {
  return value !== undefined && isStartPage(value) ? value : 'dashboards'
}
 ...
 * 성공 응답으로 네 필드를 모두 재동기화한다. 저장 성공 후 whoami를 재조회해 authStore를
...
export function PreferencesForm(): JSX.Element {
  ...
  const [startPage, setStartPage] = useState<StartPage>(() => resolveInitialStartPage(user?.startPage))

  function syncFromResponse(updated: PreferencesResponse): void {
    setTheme(updated.theme)
    setLocale(updated.locale)
    setDateFormat(updated.dateFormat)
    setStartPage(updated.startPage)
  }
  ...
  function handleStartPageChange(value: string): void {
    const next = value as StartPage
    setStartPage(next)
    mutation.mutate({ startPage: next }, { onSuccess: syncFromResponse })
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-1.5">
        <Label htmlFor="preferences-start-page">시작 페이지</Label>
        <Select value={startPage} onValueChange={handleStartPageChange} disabled={mutation.isPending}>
          <SelectTrigger id="preferences-start-page" aria-label="시작 페이지" className="w-48">
            <SelectValue />
          </SelectTrigger>
          <SelectContent>
        ...
        <p className="text-xs text-muted-foreground">{START_PAGE_NOTICE}</p>
      </div>
    </div>
  )
}
