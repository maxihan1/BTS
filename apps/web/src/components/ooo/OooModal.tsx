 ...
 *
 * 열릴 때마다 현재 설정으로 폼을 초기화한다 — 재열림 시 이전 세션 입력이 stale하게 남지 않도록
 * 한다(Radix controlled Dialog 토글닫기 stale 함정 방어, memory: react-usestate-stale-key-prop).
 *
 * 대리자는 id만 상태로 보관하고 표시 이름은 {@link useUsersByIds}로 파생한다(ComponentFormDialog/
 * ProjectLeadSelect 선례 — 검색으로 새로 고른 경우와 기존 설정을 단일 출처로 통일).
 *
 * 기간 검증(종료<=시작, 종료<=현재)은 클라이언트에서 즉시 alert로 안내한다(백엔드 400 응답도
 * mutation onError에서 동일하게 alert 처리).
 */
export function OooModal({ open, onOpenChange }: OooModalProps): JSX.Element {
  ...
  }, [open, current?.startsAt, current?.endsAt, current?.delegateUserId, current?.message])
  ...
  const selectedDelegateLabel = selectedDelegate !== undefined
    ...
    : (current?.delegateUserId === delegateUserId ? current?.delegateName ?? null : null)
  ...
  function handleSave(): void {
    ...
    if (startInstant === null || endInstant === null) {
      window.alert(oooLabels.errorMessage)
      return
    }
    ...
    updateMutation.mutate(
      ...
      { onSuccess: handleSaved, onError: () => { window.alert(oooLabels.errorMessage) } },
    )
  }

  function handleClear(): void {
    clearMutation.mutate(undefined, { onSuccess: handleSaved })
  }
