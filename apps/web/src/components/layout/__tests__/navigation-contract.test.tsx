...
describe('navigation-contract (aria-label 4종 회귀 가드)', () => {
  ...
  //
  // ★ 종전 주석은 "일반"(SETTINGS_LINKS 신규 항목, FE-4)이 「프로젝트 행의 설정 서브그룹을
  //   펼쳐야만 노출된다」고 적고 있었다. **그 서브그룹이 사라져 문장이 사실이 아니게 됐다** —
  //   설정 목록은 이제 사이드바 트리가 아니라 설정 서브앱(`ProjectSettingsNav`)이 그리고,
  //   트리에서 그리로 가는 길은 스페이스 `⋯` 의 「프로젝트 설정」 항목 하나다(Jira JS-1).
  //   이 파일의 단언은 애초에 `SETTINGS_LINKS` 에 의존하지 않으므로 코드는 그대로 두고
  //   설명만 현재 사실로 갱신한다. 트리 하위 목록의 전수 검증은 ProjectTree.test.tsx 가,
  //   16경로 도달성은 `scripts/workflow/project-nav-reachability.test.ts` 가 전담한다.
  //   여기서는 신규 "모든 프로젝트" 라벨이 기존 4종 계약(메인 메뉴·관리 메뉴·프로젝트 뷰
  //   전환·검색 단일)을 깨지 않는지만 확인한다.
  // ───────────────────────────────────────────────────────────────────────────
