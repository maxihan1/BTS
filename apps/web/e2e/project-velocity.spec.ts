...
//
// 시나리오.
//   S1. 백로그 페이지 "프로젝트 뷰 전환" nav의 "벨로시티" 링크 클릭 → 라우트 이동(URL) + 차트 컨테이너 표시.
...
test.describe('FR-RP-02 D6/D7 프로젝트 벨로시티 차트', () => {
  ...
  // When   /projects/ATLAS/backlog 진입 → "프로젝트 뷰 전환" nav의 "벨로시티" 링크 클릭
  // Then   URL이 /projects/ATLAS/reports/velocity 로 이동
  //        벨로시티 페이지 헤더(h1) + 차트 컨테이너(role="img") 표시
  // ───────────────────────────────────────────────────────────────────────────
  test('S1 백로그 "프로젝트 뷰 전환" nav "벨로시티" 클릭 → 라우트 이동 + 차트 컨테이너 표시', async ({ page }) => {
