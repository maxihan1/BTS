...
**진척** — 143 FR 중 139 의 D 단계 완료 · FR-WF-04 는 백엔드(D1~D5) 완료·UI 미착수 · FR-WF-05~07 미착수. 정본 `docs/plan/README.md`.
...
정본은 전역 `~/.claude/CLAUDE.md` §Explanation Style. 여기선 **적용 범위만**.

- T0·T1 축약형 — `✅` 1줄 + `💡` 1줄 + `🔧` 에 바꾼 파일·확인 방법 각 1줄.
- T2·T3 — `🔧` 하위 4구성(무엇을 왜 · 확인 방법 · 남은 위험 · 다음 할 일).
- 서브에이전트 결과는 그대로 붙이지 않는다 — **재정리 + 확신도**.
- 새로 물린 함정은 말미에 `learnings: <slug>` 1줄.
