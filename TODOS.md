...
>
> **동작 불변 확인.** apps/web 유닛 582 파일 / 9537 테스트 전량 통과(직전 세션과 같은 수치).
> 폼 밖 제출 경로의 계약 2종(진행 신호 · E7 재클릭 차단)은 분할 전에도 무검증이었음을 실측으로
> 확인하고 같은 PR 에서 못 박았다. 새로 생긴 함수 중 200줄을 넘는 것은 없다(최대 97).
>
> ⚠️ **남은 것 (이 항목 밖).** 제출 함수가 34줄로 `DEVELOPMENT.md §2.2` 의 「함수 30줄 이내」를
> 넘는다. 분할 전 `handleSubmit` 과 **글자가 같은** 선재이고, 기계 강제는 200줄 래칫뿐이라
> 이 항목의 상환 대상이 아니었다. 아래 본문이 이미 34줄이라 적어 둔 그 함수다.
...
> 4건(20%)이고 래칫 도입(#364) 이후는 2건인데, **그 2건 모두 이미 베이스라인을 갱신했다**
> (#367 은 자발적으로 326→311 로 낮췄고, #384 는 288→289 로 단조 판정에 강제됐다).
> 즉 엄격 규칙이 있었어도 **추가 작업은 0** 이었다. 아래 결정이 가장 두려워한
...
## ⬜ apps/web — `CreateIssueDialog` 에 테스트 파일이 없다 (신규 · 미착수 · T1)

**무엇.** `apps/web/src/components/issue/CreateIssueDialog.tsx` 는 테스트 파일이 **한 개도 없다**
(`ls src/components/issue/ | grep -i createissuedialog` → 소스 1건뿐). 그래서 이 다이얼로그가
쥔 제출 상태 사슬이 통째로 무검증이다.

```
IssueCreateForm onPendingChange → setSubmitting → 푸터 <Button disabled={submitting}>
                                                 → 문구 「이슈 생성」↔「생성 중…」 전환
```

**실측으로 확인했다.** 폼 쪽 진행 신호 배선(`onPendingChange?.(mutation.isPending)`)을 지워도
분할 전 기준 60/60 초록이었다. 지금은 매핑 `8` 이 폼 쪽에 계약 2종을 못 박아 그 절반이 닫혔다 —
**남은 것은 다이얼로그 쪽 소비**다. 푸터 버튼이 비활성이 되는지, 문구가 바뀌는지는 여전히
아무도 안 잰다.

**왜 지금 안 고쳤나.** 매핑 `8`·`22` PR 의 범위 밖이다. 그 PR 은 `IssueCreateForm` 의 줄수와
래칫 판정을 다뤘고, 다이얼로그 테스트 신설은 별개 표면이다. 대신 폼 쪽에서 잴 수 있는 절반
(신호가 나가는가 · 제출 중 재클릭이 막히는가)을 같은 PR 에서 닫았다.

**착수 시.** `CreateIssueDialog.test.tsx` 신설. 최소 3종 —
① 제출 중 푸터 버튼이 비활성이 된다 ② 문구가 「생성 중…」으로 바뀐다
③ 완료되면 둘 다 되돌아온다. 응답을 손으로 푸는 MSW 핸들러가 필요하다
(선례 — `IssueCreateForm.test.tsx` 의 `holdCreateIssue`).

**발견 경위.** #385 매핑 `8` 구현 중 뮤테이션 실측. 계획에 없던 자리다.
...
빈도는 낮다 — 최근 20 PR 중 두 파일을 고친 PR 은 #364·#385 둘뿐이다.
