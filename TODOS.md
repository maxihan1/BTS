...
| 실측 | 값 |
|---|---|
| `PROJECTS.KEY.eq(projectKey)` grep 히트 | **8** |
...
> | 항 | 단언 |
> |---|---|
> | I-1 | SDD 의 `파일 300줄` 은 **전체 1행 + `Kotlin:` 태그 1행** |
> | I-2 | SDD 에 `TypeScript: 컴포넌트 200줄 이내` **1행** (반대 방향) |
> | I-3 | `DEVELOPMENT.md` 쪽 사본 각 **1행** (사본이 늘면 정본이 모호해진다) |
>
> **★행 부재도 FAIL 이다.** 「없으면 검사 안 함」으로 짜면 행을 지우는 것만으로 가드가 죽는다
> (`[[seal-blinds-existing-guard]]` 양식). 그래서 **전체 건수와 태그 건수를 함께** 잰다.
>
> **4) 뮤테이션 3종 전부 기대대로.** ①`Kotlin:` 태그 제거 → **EXIT 4** ②`파일 300줄` 행 자체 삭제
> → **EXIT 4** ③무관한 줄 수정 → **EXIT 0**(과잉결합 아님). 기준선은 EXIT 0 이었고
> 룰 I 추가 직후 EXIT 4(RED) → SDD 정정 후 EXIT 0(GREEN) 순서로 확인했다.
>
