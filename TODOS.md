...
**무엇.** `i18n/workflow-editor-labels.ts:1` 이 「E2E 셀렉터 정본 — 단일 진입점」이라 선언하고
`nav.label` 에 「이 값은 '워크플로우 관리' 다」를 못박지만, 실제 리터럴은 `Sidebar.tsx` ·
`admin.index.tsx` · `Sidebar.test.tsx` · `e2e/workflow-editor.spec.ts` 에 **따로 5벌** 있다.
`nav.label` 의 **소비처는 0** 이다 — 정본을 '워크플로우' 로 줄여도 화면이 안 바뀐다.
