...
**★닫힌 범위는 「진입점 2곳의 UI 처방」뿐이다.** 같은 뿌리에서 나온 아래 6건은
**알면서 남겼고** 전부 별도 항목으로 승격했다. 이 헤딩만 보고 「이 부류가 끝났다」로 읽지 말 것.
① 이동 대상 프로젝트 키 대소문자 정규화 부재 ② `errorProjectNotFound` prod 도달 불가
③ 백엔드 테스트가 도달 불가 조합을 스텁 ④ 임포트 CREATE 게이트 E2E 부재
⑤ 임포트 로딩 프레임 계약 테스트 공백 ⑥ 권한 재조회로 임포트 화면이 사라질 수 있음.
...
`MoveIssueDialog.tsx:255`(label) / `:260-276`(`<Input>`) 의 대상 프로젝트는 드롭다운이 아니다.
...
| 실측 | 근거 |
|---|---|
| 권한 assert 가 존재 확인보다 **먼저** | `MovePreviewService.kt:185-186` vs `:190-192` |
...
## ⬜ apps/web — 임포트 페이지 **로딩 프레임 계약**이 테스트로 고정돼 있지 않다 (신규 · 미착수)

**무엇.** `projects.$projectKey.settings.import.test.tsx` 의 CREATE 게이트 3 테스트는 전부
쿼리가 **정착한 뒤**만 단언한다. 그래서 「로딩 중에는 폼을 보여 준다」(`use-issue-create-permission-gate.ts`
KDoc 이 못박은 계약)를 아무도 지키지 않는다 — 로딩 프레임에 거부 카드를 띄웠다가 정착 후
폼으로 바꾸는 구현도 세 테스트를 **전부 통과**한다.

**처방.** 지연 핸들러(`await delay()`)로 로딩 프레임을 만들고 그 프레임에서 폼이 보이는지
단언. **지연 핸들러는 pending mutation 누수 항목과 같은 부류이므로** 그 항목의 규율
(「지연 MSW 핸들러 34개 파일」)을 먼저 볼 것.

**같은 공백이 프로젝트 존재 분기에도 있다** — 404 정착 전 프레임에서 폼이 보이는지 미고정.
