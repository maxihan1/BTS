...
**왜 선재 봉합이 이것을 못 덮나.** 2026-08-12 PR #367 이 닫은 것은 「**시작 시점에 이미** 꺼져 있는
상태」다(항목 `30`). 이번은 「**시작 후** 죽는 상태」로 **다른 실패 모드**다. `runner-health.yml` 의
Docker 점검 주석도 「기동 중」 시나리오만 다룬다 — 시작 후 사망은 설계에 없다.
...
**처방 후보.** ① Jetsam 로그로 범인 규명이 **선행 필수** ② 재부팅 시 Docker Desktop 자동 시작
(로그인 항목 등록) — 항목 `57` 의 오진 실패를 구조적으로 없앤다 ③ 러너 자원 가드에 「패닉·Jetsam
이력」 판정 추가 — 지금 가드는 load·swap 두 축만 본다.
