...
**무엇.** 로드맵 PR 7 은 이관 실행 경로(`IssueStatusMigrationPort` 와 그 어댑터·워커)를 만들되 **호출자는 만들지 않는** 분할이었다. `WorkflowPublishService` 는 지금도 남은 이슈를 세어 409 로 막고 상태별 건수만 응답에 싣는다. 그 결선이 로드맵 **PR 7b** 이고, 이 항목이 그것의 장부 등재다.
