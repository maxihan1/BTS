...
(`i++ / total // …` 에서 `++` 뒤 `/` 를 정규식 시작으로 오판해 주석 여는 슬래시를 종결자로 먹는다).
지금은 위치가 아니라 **파생 술어를 만족하는가**로 판정한다. 문자열·정규식 안의 `//`(URL·경로 글롭)까지
물지 않으면서 속일 수 있는 형태만 잡는 자리다. 그래도 「모든 살아남은 주석 0」은 **아니다** — 판정을
못 속이는 주석은 여전히 안 잡힌다. 코드에 없는 보장을 문서가 선언하면 다음 사람이 틀린 근거로 사각을
재확인한다(#383 C4 의 교훈).
...
**처방 후보.** ① 지금 상태를 유지하고 위 세 축의 실측 짝으로 계속 지킨다 — **오늘 저장소 안에서 이 오판을
부르는 소스는 없다**(「피해 0」이 아니라 그것이 사실이다. 후위 연산자 형태는 실제로 뚫렸고, 트랩으로
동결했다). ② 토크나이저로 옮긴다 — 의존성이 늘고 러너 기동이 느려지는 대가가 있다.
...
**지금 막아 둔 것.** 정적 층위가 아니라 **실측 층위**다. git 을 부르는 파일과 그것에 닿는 파일 전량에서
러너가 단독으로 돌릴 수 있는 것을 파생해 `GIT_DIR` 를 건 자식으로 돌리고, victim 저장소가 바뀌는지를
값으로 잰다(`★★원본을 그 파일만 단독 실행해도 victim 이 그대로다`). 우회한 파일은 여기서 victim 을
바꾸므로 red 다. 자식이 실제로 판정을 세웠는지도 계수와 이름으로 확인한다 — 종료 코드 0 이
「아무것도 안 돌았다」를 함께 뜻하기 때문이다.

**그래서 남은 사각은 하나다.** 러너 글롭에 안 걸리고 어느 자식도 임포트하지 않는 새 파일이 생기면
두 층위 모두 못 본다. 그 자리는 `★★git 을 부르는 전량이 자식으로 돌거나 자식이 임포트한다` 가 red 로 막는다.
