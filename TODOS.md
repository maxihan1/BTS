...
## 번다운 — 스프린트 마지막 점의 「오늘」 경계가 아직 UTC 다 (부채 177 에서 발견)
...
## 프론트 — 다크 모드에서 네이티브 날짜 위젯이 라이트로 그려진다 (부채 177 에서 발견)
...
## 백로그 — 카드 레이아웃의 커스텀 필드가 백로그에서만 안 뜬다 (부채 177 에서 발견)
...
**등재 서식 주의.** 이 PR 이 앞서 등재한 「부채 177 에서 발견」 8건과 **같은 모양**(상태 마커 없음)
으로 적었다. `## ⬜ ` 로 적으면 `debt-ledger-mapping.test.ts` 가 마스터 계획
(`docs/plans/2026-08-12-debt24-master.md`)에 대응 행을 요구해 **초록이던 판별식이 red 가 된다** —
그 행은 「어느 PR 이 닫는가」를 정하는 자리라 이 task 가 지어낼 수 없다. 대신 마커 없는 섹션은
