...
> 「파일 320 → 300」은 갚을 대상이 아니다.** 담당 PR #371 은 이 범위로 착수할 것.
...
**★Maxi 확정 (2026-08-12).** **② 백엔드가 마스킹된 required 필드를 검증 대상에서 제외한다.** 담당 PR #373.
...
바꿀지 판단(**존재 은닉 정책과 충돌하므로 Maxi 확인 필수**) ③컨트롤러 KDoc 정정.
**②를 고르면 FR-PM-05 존재 은닉 일관성이 깨진다** — 기본은 ①+③.
...
PR #365 는 `_index.md:59` 만 정정하고, `README.md:92` 는 **스크립트가 실제로 생기는 PR #374 에서**
「향후 추가 예정」 절 밖으로 옮기면 된다. **이 문단을 근거로 README 를 지금 고치지 말 것.**
...
**★Maxi 확정 (2026-08-12).** ① **`sync-obsidian.ts` 를 실제로 만든다.** 담당 PR #374.
정정만 하고 넘어가지 않는다 — `CLAUDE.md:63` 이 「Phase 1 에 자동화」라 예고했고 지금이 Phase 1 이다.
