...
**`ui` 행은 1종으로 남겼다 (착수 때 2종으로 썼다가 리뷰가 뒤집었다).** 등재문은 「`/bts` 티어 표가
T2 에 2종을 요구하는데 `ui` 행은 1종뿐」이라 적었고 #384 도 그 근거로 `/plan-eng-review` 를 더했다.
그런데 **그 요구 자체가 오독이었다**(아래 곁가지 정정). 근거가 사라졌으므로 2종을 유지할 이유가
없고, 대신 `backend` 행의 기존 서식을 대칭으로 써 `/plan-design-review` (로직·계약 포함 시
`/plan-eng-review` 추가)로 적었다. **원칙 없는 비대칭을 만들지 않는 쪽을 골랐다.**
