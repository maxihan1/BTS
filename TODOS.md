...
**방치하면.** 조용히 통과한다. 슬랙 열쇠 공식이 바뀌면 배포 순간 pgmq 에 떠 있던 이벤트가 재전달될 때 옛 키 행이 있어도 새 키로 삽입돼 **같은 슬랙 메시지가 두 번** 나간다. 그런데 그 변경을 막는 판별식이 없어 전 테스트가 초록이다. 인앱 쪽은 golden hex 가 있어 같은 실수가 즉시 red 다 — **같은 결함을 두 벌 고쳤는데 가드는 한 벌뿐**이라는 비대칭 그 자체가 이 항목이다.

**무엇.** `SlackChannelBroadcasterTest` 에 `Notification.computeDedupKey` 의 golden 단언(`4f83935a…`)과 짝이 되는 단언을 추가한다 — 「commentId 없는 이벤트의 dedupKey 가 **변경 이전 공식의 산출물과 바이트 단위로 같다**」. 값은 `SHA-256("<projectKey>|<eventType.wireValue>|<issueKey>|<occurredAt>")` 로 계산해 상수로 박는다.

**실측(2026-09-05 · PR #461).** 현재 `BC-7` 은 「두 무댓글 이벤트의 dedupKey 가 **서로** 같다」만 단언한다. 이것은 결정성만 재고 **옛 키와의 동일성은 재지 않는다** — 공식이 통째로 바뀌어도 두 호출은 여전히 서로 같으므로 초록이다.
