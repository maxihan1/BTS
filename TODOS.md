...
## ⬜ apps/web — 보드 상세 보기 구성이 첫 보드 것만 반영된다 (부채 177 에서 발견)

**쉬운 말.** 「이슈 상세에 어떤 항목을 보여줄지」를 보드마다 따로 저장할 수 있는데, 정작 화면은 **그 프로젝트의 첫 보드 설정만** 읽는다. 둘째 보드에서 저장하면 저장은 성공하고 화면은 그대로다.

**방치하면.** 관리자가 둘째 보드의 상세 보기 탭을 고치고 저장 성공을 본 뒤, 이슈를 열어 보면
아무것도 안 바뀌어 있다. 오류도 안 뜬다 — **성공했다고 말하고 아무 일도 안 하는** 화면이다.
「저장이 안 됐나」로 읽고 같은 조작을 반복하게 되며, 그동안 원인은 어디에도 안 보인다.

**무엇.** 저장 축과 소비 축이 어긋나 있다.

- **저장은 보드 단위다.** `PATCH /api/v1/boards/{boardId}/detail-view-fields` 가 보드마다 따로
  받고 `board_detail_view_fields` 도 보드 단위로 쌓는다(스펙 J46 · 편차 X8·X10).
- **소비는 보드 하나로 고정이다.** `apps/web/src/components/issue/IssueMetaPanel.tsx` 의
  `useIssueDetailViewFields` 가 `useBoards(projectKey).data?.[0]?.boardId` 로 **첫 보드**를 집는다.

즉 한 프로젝트에 보드가 둘 이상이면 **둘째 보드부터의 구성은 어디에도 반영되지 않는다.**

**왜 그렇게 됐나 (당시 판단은 근거가 있다).** 이슈는 보드가 아니라 **프로젝트**에 속하는데
구성은 보드 단위다. 스펙(R7·R7c)도 계획(Task 21)도 「보드가 여럿일 때 어느 것인가」에 답이 없었다.
그래서 가장 단순한 해석을 택했다 — 보드 화면이 `?board=` 없이 열렸을 때 고르는 것과 **같은 보드**
(`projects.$projectKey.board.tsx` 의 `boards[0]?.boardId`). 새 「기본 보드」 규칙을 하나 더 만들지
않으려는 선택이었고, 그 규칙이 이미 세 곳에서 갈렸다는 것이 **부채 164** 다.

**★고칠 자리는 한 곳이다.** 진단은 `useIssueDetailViewFields` 의 KDoc 에 이미 적혀 있다 —
보드 스코프를 상세까지 흘리기로 결정하면 **고칠 자리는 그 훅 하나**다. 다만 그 결정이
싼 것은 아니다. 상세는 진입점이 13곳(보드·백로그·목록·인박스·검색·대시보드…)이라 「열린 맥락의
보드」를 쓰려면 그 13곳이 전부 보드 id 를 실어 와야 하고, 실어 오지 않는 곳에서는 **같은 이슈가
어디서 열었느냐에 따라 다르게 보인다** — 스펙 R7c 가 막으려는 증상과 같은 종류다.

**처방 후보.** ① 프로젝트에 「기본 보드」를 명시적으로 두고 상세는 그것을 읽는다 — 부채 164
(「기본 보드」 규칙이 세 곳에서 다르다)와 **같은 자리에서 함께 닫는 편이 싸다**.
② 진입점이 보드 맥락을 실어 오고 없으면 기본 보드로 떨어진다 — 지라에 가깝지만 13곳을 다 고쳐야 하고
R7c 를 새로 지켜야 한다. ③ 구성을 프로젝트 단위로 되돌린다 — 저장 축을 바꾸는 것이라 마이그레이션이 따른다.

**착수 시 주의.** 지금 상태가 「조용히」인 이유는 화면이 **실패로 읽지 않기 때문**이다
(보드가 없는 프로젝트도 정상 부재로 다룬다 — T21-7). 어느 안을 고르든 **둘째 보드에 구성을 넣고
이슈를 여는 판정**을 함께 세워야 한다. 지금은 그 회귀를 잡는 테스트가 없다.

## ⬜ apps/web — 다크 모드에서 네이티브 날짜 위젯이 라이트로 그려진다 (부채 177 에서 발견)

**쉬운 말.** 다크 테마인데 날짜 입력칸의 달력 아이콘과 브라우저 달력 팝업만 흰 배경으로 뜬다. 화면 하나에서 두 테마가 섞여 보인다.

**방치하면.** 다크를 쓰는 사용자가 날짜를 고를 때마다 밝은 팝업이 튀어나온다. 그리고 빈 날짜칸의
안내 문구가 `mm/dd/yyyy` 로 보인다 — 앱은 전부 한국어인데 그 자리만 브라우저 기본 로케일이다.

**무엇.** CSS 에 `color-scheme` 선언이 **한 곳도 없다**(`apps/web/src` 전수 — 매치되는 것은
`prefers-color-scheme` 미디어 쿼리뿐이고 성격이 다르다). 브라우저는 그 선언이 없으면 네이티브
위젯(날짜 피커·스크롤바·체크박스 기본형)을 **라이트 스킴으로** 그린다.

`<input type="date">` 를 쓰는 화면이 **9곳**이다 — `SprintForm` · `AuditLogFilters` ·
`InboxFilters` · `WorklogAggregateReport` · `VersionFormDialog` · `IssueScheduleFields` ·
`CustomFieldInput` · `WorkingDaysPanel`(부채 177 신규) 등.

**왜 지금인가.** 부채 177 Task 18 이 작업일 탭에 날짜 입력을 넣으며 눈확인에서 관측했다.
**이 PR 이 만든 것이 아니라 선재 조건**이고, 9곳이 같은 증상이라 한 자리에서 닫는 편이 싸다.

**처방 후보.** ① 루트에 `:root { color-scheme: light dark }` 를 선언하고 테마 토글이
`document.documentElement.style.colorScheme` 을 함께 바꾼다 — 한 줄에 가깝고 9곳이 동시에 낫는다.
② 날짜 입력을 커스텀 컴포넌트로 바꾼다 — 로케일 문구까지 통제되지만 9곳 교체 비용이 크고
접근성을 새로 져야 한다.

①이 먼저다. ②는 `mm/dd/yyyy` 문구까지 고치고 싶을 때의 후속이다.

**착수 시 주의.** `color-scheme` 은 **스크롤바와 기본 폼 위젯 전반**에 영향을 준다 — 날짜칸만
보고 끝내지 말고 라이트/다크 양쪽에서 목록·모달·셀렉트를 함께 눈확인하라.
★그리고 **테마 전환 직후 프레임을 캡처하지 마라.** `transition-*` 이 도는 동안 색이 전 상태로
남아 보여 결함으로 오판한다 — 부채 177 에서 T16·T17·T18 이 각각 한 번씩 밟았다.
정착 후 재촬영하거나 계산 스타일을 직접 떠서 확인하라.

## ⬜ apps/web — MSW 목의 오류 봉투가 실제 백엔드와 다르다 — 코드값 4종은 백엔드에 아예 없다 (부채 177 에서 발견)

**쉬운 말.** 테스트용 가짜 서버가 내놓는 오류 응답의 모양이 진짜 서버와 다르다. 가짜 쪽에만 있고 진짜에는 없는 오류 코드가 네 개다.

**방치하면.** 화면이 오류 본문을 읽기 시작하는 순간, 테스트에서는 통과하던 파싱이 실제 서버 응답에서 깨진다. 테스트는 초록인 채로 실제 화면만 고장난다.
...
**등재 서식 (정정 2026-09-06).** 사유는 「보드 오류 봉투 advice」 항목의 「등재 서식」과 같다 —
마커 없이 적은 것이 회피였고, 지금은 `⬜` 마커와 마스터 §전수 매핑의 `**미배정**` 행으로 고쳤다.
...
## ⬜ apps/web — 키보드로 카드·상태를 옮기면 화살표를 20번 넘게 눌러야 한다 (신규 · 미착수 · T1)

**쉬운 말.** 마우스 없이 키보드로 카드를 옮길 수 있다. 그런데 옆 칸으로 한 칸 옮기는 데 화살표를 스무 번 넘게 눌러야 한다. 되기는 되지만 아무도 그렇게 쓰지 않는다.

**방치하면.** 「접근성 있음」으로 집계되지만 실제로는 쓸 수 없다. 기능이 없는 것과 있는 것 사이의 회색지대로 남고, 접근성 점검에서 「동작한다」로 통과해 아무도 다시 안 본다.

**무엇.** `@dnd-kit` 의 기본 `KeyboardSensor` 는 화살표 한 번에 **25px** 을 옮긴다(라이브러리 기본 `defaultCoordinatesGetter`). 보드 컬럼은 288px(`w-72`), 보드 설정의 미매핑 패널은 256px(`w-64`)이라 옆 대상에 닿으려면 12~25회를 눌러야 한다. `apps/web/src/components/board/board-drag-sensors.ts` 가 센서를 거는 유일한 자리이고 `KanbanBoard`·보드 설정 두 화면이 그것을 공유한다.

**실측 (2026-09-04 · 부채 177 E2E).** `board-settings.spec.ts` 의 키보드 매핑 시나리오가 `ArrowLeft` **1회로는 실패**했고 25회로 통과했다. 그 수가 이 항목의 근거다 — 추정이 아니라 실행 결과다.

**할 일.** `useSensor(KeyboardSensor, { coordinateGetter })` 에 **드롭 대상 단위로 건너뛰는** getter 를 준다. `@dnd-kit/sortable` 의 `sortableKeyboardCoordinates` 가 정렬 목록에는 그 일을 하지만, 이 두 화면은 정렬이 아니라 컨테이너 간 이동이라 그대로 쓸 수 없다 — 컬럼·패널 목록을 알고 그 사이를 오가는 getter 를 직접 쓴다. `board-drag-sensors.ts` 한 곳만 고치면 두 화면이 함께 좋아진다(그 파일을 공유 자리로 만든 이유가 이것이다).

**판별식.** E2E 에서 화살표 **1회**로 옆 대상에 닿는지 잰다. 지금 spec 의 25회 루프가 그 자리의 표식이다 — 고치면 그 루프를 1회로 줄이는 것이 곧 검증이다.
...
## ⬜ apps/web — 프로젝트 캘린더·대시보드 탭이 프로젝트로 좁혀지지 않는다 (편차 X-J5-13 · 신규 · T2)

**쉬운 말.** 프로젝트 안에서 「캘린더」와 「대시보드」 탭을 누르면 화면은 열리지만, 내용은 그 프로젝트 것이 아니라 **내 전체 일정·전체 대시보드**다. 주소창에는 프로젝트 이름이 들어 있어서 좁혀진 것처럼 보인다.

**방치하면.** 사용자가 「이 프로젝트의 이번 달 일정」을 본다고 믿고 다른 프로젝트 일정까지 함께 읽는다. 오늘 실제 피해는 잘못된 일정 판단 하나뿐이고 데이터가 새지는 않는다 — 두 화면 모두 원래 **본인에게 보이는 것만** 보여 준다.

**무엇.** Jira 패리티 J5-12(2026-09-07)가 편차 X9 를 폐기하면서 9탭 전부에 프로젝트 스코프 라우트를 줬다(`/projects/$projectKey/calendar` · `/dashboards`). 탭바가 유지되는 **탐색 패리티**는 얻었지만 내용 필터는 없다. 백킹 계약에 프로젝트 축이 아예 없기 때문이다.

- 캘린더 — `GET /api/v1/users/me/calendar?from=&to=` 는 identity-access BC 의 **개인** 캘린더다(FR-CA-01). 프로젝트 파라미터가 없다.
- 대시보드 — 대시보드 레코드에 프로젝트 연관 필드 자체가 없다(FR-DB-01).

이슈 탭은 `projectKey` 필터가 이미 있어 **실제로 좁힌다** — 셋 중 이 둘만 남은 것이다.

🛑 **클라이언트에서 이슈 키 접두(`ATLAS-`)로 거르지 않는다.** 캘린더 응답에 `truncated` 가 있어 **잘린 뒤 거르면 조용히 빈 화면**이 된다. 「좁힌 척」이 「안 좁힘」보다 나쁘다.

**처방.** 백엔드 계약을 먼저 넓힌다 — 캘린더에 `projectKey` 질의 파라미터(identity-access BC), 대시보드에 프로젝트 연관(notification BC). **BC 가 둘이라 한 PR = 한 BC 규칙상 최소 2 PR 이다.** 프론트는 그 뒤 `useParams().projectKey` 를 실어 보내면 된다.

**근거.** `docs/plans/2026-09-07-jira-parity-project-header-and-scoped-tabs.md` §의도적 편차 · `apps/web/src/router.ts` 의 `projectCalendarRoute` KDoc.

---

# 기능 동작

## ⬜ issue-tracking — 보드 카드 경로가 코어 필드를 마스킹하지 않는다 (FR-PM-07 두 번째 표면 · 부채 177 에서 발견)
...
## ⬜ agile-planning — `agile-planning` 이 선언한 detekt baseline 파일이 존재하지 않는다 (부채 177 에서 발견)
...
## ⬜ agile-planning — 스프린트 마지막 점의 「오늘」 경계가 아직 UTC 다 (부채 177 에서 발견)

**쉬운 말.** 번다운 그래프에서 worklog 는 보드 타임존 기준으로 날짜 칸이 정해지는데, 「오늘까지만 그린다」의 그 **오늘**은 아직 UTC 다. 서울 보드에서는 하루의 마지막 아홉 시간 동안 그래프가 한 칸 덜 그려진다.

**방치하면.** 서울 팀이 오전에 번다운을 보면 **어제까지만** 그려져 있다. 오늘 적은 worklog 가
반영된 점이 아직 없다. 반대로 UTC− 보드는 아직 오지 않은 날의 점이 먼저 생긴다.
데이터가 틀린 것이 아니라 **잘림 위치가 어긋난다** — 그래서 「버그인지 아직 안 들어온 건지」를
사용자가 못 가른다.

**무엇.** 부채 177 Task 12 가 worklog 일 귀속을 보드 타임존으로 옮겼지만(`aggregateByBoardDate`),
`BurndownCalculator.kt:71` 의 잘림 기준은 그대로다.

```kotlin
val today = LocalDate.now(clock)          // ← UTC
val asOf  = minOf(end, today)
```

`clock` 이 UTC 라 `today` 도 UTC 다. 보드가 `Asia/Seoul`(UTC+9)이면 한국 시각 00:00~09:00 사이
`today` 가 아직 전날이고, 그 창 동안 마지막 점이 안 채워진다. `America/New_York`(UTC−4)은 반대다.

**왜 그때 안 고쳤나 — 범위 밖 판단이었고 독립 검증도 동의했다.** 스펙 R10 은 **worklog 일 귀속**만
요구한다. `asOf` 를 옮기면 `SprintBurndownIntegrationTest` 가 기대는 **결정성 계약**(고정 clock 으로
점 개수를 단언)도 함께 바뀐다. Task 12 는 손대지 않고 서비스 KDoc(`:57-59`)에 남겼다.

**처방 후보.** ① `today` 를 보드 타임존으로 계산한다 — `LocalDate.now(clock.withZone(boardZone))`.
한 줄이지만 **결정성 계약이 바뀌므로** 기존 번다운 테스트의 고정 clock 기대값을 함께 봐야 한다.
② `asOf` 를 계산기 인자로 올려 **호출부가 정한다** — 서비스가 보드 타임존을 이미 알고 있으니
책임이 그쪽에 맞고, 계산기는 순수 함수로 남는다(NFR N2).

②가 구조적으로 맞다. 계산기가 `clock` 을 직접 읽는 지금 형태는 「도메인 순수 함수」와 어긋난다.

**착수 시 주의.** ★**대조군을 한 쌍으로 둬라** — 「서울 보드에서 마지막 점이 오늘까지 온다」만
두면 「무조건 +1 하는」 구현이 통과한다. **UTC 보드는 그대로**와 **UTC− 보드는 반대 방향**을 함께 재라.
부채 177 Task 12 가 같은 자리에서 `America/New_York` arm 으로 부호 뒤집기를 잡았다.
★그리고 **시각이 경계를 넘는 고정 clock** 을 써라. 낮 시각으로 고정하면 두 타임존에서 같은 날이라
**판정을 지워도 통과한다**(스펙 완료 기준 11 과 같은 함정).

## ⬜ agile-planning — 카드 레이아웃의 커스텀 필드가 백로그에서만 안 뜬다 (부채 177 에서 발견)

**쉬운 말.** 보드 설정에서 「백로그 카드에 이 커스텀 필드를 보여 줘」를 고를 수는 있는데, 백로그로 가면 안 보인다. 표준 필드(에픽·우선순위·담당자 등)는 잘 뜬다.

**방치하면.** 사용자가 설정을 고르고 저장까지 성공했는데 화면이 안 바뀐다. 그리고 **그 이유를
화면에서 알 수 없다** — 카드 레이아웃은 「그 이슈에 값이 없으면 생략」(스펙 E4)이 정상 동작이라,
「값이 없어서 생략된 것」과 「응답에 아예 안 실려서 못 그리는 것」이 **화면상 구분되지 않는다.**

**무엇.** 백로그 응답에 `customFields` 가 **아예 없다** — 프론트 `api/backlog.ts` 의
`backlogIssueSchema` 와 백엔드 `BacklogResponses.kt` 둘 다 0건이다.

부채 177 Task 26 은 `BoardCardResponse` 만 넓혔다. 보드 카드는 그래서 뜨고, 백로그 행은 못 뜬다.
Task 32 가 프론트 배선을 닫았지만 **원천이 없는 것은 배선으로 못 고친다.**

**왜 그때 안 했나.** T26 은 「Task 5 구현자가 넘긴 미러 결손」을 닫으려고 신설된 task 였고
`BoardCardResponse` 한 곳만 대상이었다. 백로그 응답은 그 시야에 없었다 — 계획 32개 어디에도
백로그 커스텀 필드를 소유한 task 가 없다.

**처방 후보.** ① `BacklogIssueResponse` 에 `customFields` 를 더하고 프론트 스키마도 넓힌다 —
T26 이 보드에서 한 것과 **같은 모양**이라 본이 있다. ★**마스킹 주체를 다시 만들지 마라** —
어댑터가 이미 걸렀다(T25 · 스펙 C-6). 백로그 조회가 같은 포트를 지나는지 먼저 확인하고,
아니면 그 경로에도 같은 게이트가 필요한지부터 판정하라.
② 설정 UI 에서 **백로그 뷰의 커스텀 필드 후보를 막는다** — 스펙 후퇴다. 지라는 양쪽 다 준다.

①이 맞다. ②는 「고를 수 있는데 안 뜬다」를 「고를 수 없다」로 바꿀 뿐 사용자가 원한 것을 못 준다.

**착수 시 주의.** ★**보드 쪽과 값이 같은지 재라.** 두 응답이 각자 커스텀 필드를 만들면
「두 목록이 서로를 검사하지 않는다」가 된다 — 같은 이슈를 보드와 백로그에서 열어 **같은 값**이
나오는지가 판정축이다. ★그리고 **마스킹이 양쪽에 다 걸리는지** 반드시 재라. 한쪽만 걸리면
권한 없는 사용자가 백로그로 우회해 값을 본다 — 부채 177 T25 가 닫은 것이 정확히 그 종류의 구멍이다.

## ⬜ agile-planning — 보드 오류 봉투 advice 가 대상 컨트롤러를 손으로 열거한다 (부채 177 에서 발견)

**쉬운 말.** 보드 화면의 오류 응답을 한 모양으로 맞춰 주는 장치가 있는데, 어느 화면에 적용할지를
**컨트롤러 이름을 하나씩 적어 두는 방식**으로 정한다. 새 탭을 만든 사람이 그 목록에 자기 이름을
적는 것을 잊으면, 그 탭만 오류 응답의 모양이 달라진다.

**방치하면.** 다섯 번째 설정 탭이 생길 때 같은 사고가 그대로 반복된다. 이미 한 번 일어났다 —
부채 177 의 탭 컨트롤러 넷(T8·T9·T10·T13)이 자기가 목록에 없는 줄 모르고 **각자 다른 우회**를
만들었고, 되돌리는 데 계획에 없던 Task 29 가 통째로 들었다.

**무엇.** `BoardExceptionHandler` 의 `@RestControllerAdvice(assignableTypes = [...])` 가 컨트롤러
6개를 **열거**한다. 목록에 없는 컨트롤러는 조용히 안 덮이고, **안 덮인다는 사실은 오류를 실제로
내 보기 전에는 드러나지 않는다** — 컴파일도 통과하고 정상 경로 테스트도 전부 초록이다.
Task 29 가 실측한 성질이 이것을 더 나쁘게 만든다. 넷 중 셋의 오류 경로가
`ResponseStatusException` 계열을 지나므로 **advice 를 통째로 지워도 상태 코드는 그대로 나온다** —
「404 인가」만 재는 테스트는 덮였는지 아닌지를 전혀 재지 못한다(`BoardSettingsTabErrorEnvelopeTest`
KDoc 의 뮤테이션 X2 가 그 실측이다).

**처방 후보.** ① 「`com.bts.agileplanning.web` 의 `@RestController` 중 어느 advice 의
`assignableTypes` 에도 안 잡히는 것이 있으면 red」인 판별식을 둔다 — 열거는 유지하되 **잊으면
드러나게** 한다. ② 공통 마커 인터페이스를 두고 `assignableTypes` 에 그것을 지정한다 — 새 컨트롤러가
인터페이스를 구현하지 않아도 컴파일은 통과하므로 여전히 잊을 수 있다. ③ 패키지 스캔으로 바꾼다 —
스코프가 넓어져 `SprintController` 등 형제 advice 관할까지 삼킬 위험을 새로 만든다
(memory: domain-exception-http-handler-basepackage-scope).

①이 이 저장소의 관용구에 가장 가깝다 — 두 목록의 차집합을 재는 판별식이다. ②③은 위험을 새로 만든다.

**왜 지금 안 했나.** 부채 177 Task 29 의 범위는 「탭 컨트롤러 넷을 한 봉투로 모은다」였고,
열거 방식 자체를 바꾸는 것은 **그 PR 범위 밖**이라고 계획이 명시했다.

**등재 서식 (정정 2026-09-06).** 이 항목을 포함해 「부채 177 에서 발견」 10건은 처음에 **상태 마커
없이** 등재됐다. 사유는 「마커를 붙이면 `debt-ledger-mapping.test.ts` 가 마스터 계획에 대응 행을
요구해 초록이던 판별식이 red 가 된다」였는데 **그 전제가 틀렸다** — 마스터 §전수 매핑의 PR 열은
`**미배정**` 을 허용한다(실측). 두 판별식이 서로 반대를 요구한 적은 없다. 「등재하려면 담당까지
적어라, 미정이면 미정이라고 적어라」는 **하나의 요구**였고, 마커를 뺀 것은 그 요구의 **회피**였다.
지금은 10건 전부 `⬜` 마커 + 마스터 `**미배정**` 행으로 고쳤다.
...
**★사각은 셋이다(2026-09-06 Task 4 실측 보강).** ①미러에 없는 테이블은 비교 대상에서 통째로
빠진다 ②`ADD CONSTRAINT` 는 화이트리스트로 흘러 제약 누락을 안 잡는다 ③**인덱스는 아예 안 본다.**
즉 **미러를 일부러 비워도 판별식은 초록이 될 수 있다.**

③의 실물 사례가 이미 있다 — `idx_board_column_states_column ON board_column_states (column_id)`
(V508)가 미러에 **선재 누락**인데 판별식은 초록이다. Task 4 가 미러 SQL 만 적용한 DB 와
V500~V509 전체를 적용한 DB 를 각각 띄워 스키마 차집합을 내서 찾았다.

**처방 후보.** ① 테이블 집합도 차집합 양방향으로 대조한다(마이그레이션에만 있는 테이블 = red).
② `ADD CONSTRAINT` 를 화이트리스트에서 빼고 제약 이름 집합을 대조한다.
③ **판별 방식 자체를 바꾼다** — 컬럼 집합 파싱 대신 **두 DB 를 띄워 스키마 스냅숏 차집합**을 낸다.
Task 4 가 이번에 손으로 한 절차이고, 컬럼·제약·인덱스를 한 번에 덮는다. Testcontainers 가
이미 있으니 비용은 실행 시간뿐이다.

③이 근본이다. 파싱을 고치는 ①② 는 다음 사각(트리거·시퀀스·뷰)을 또 부른다.
...
## ⬜ 워크플로우 — 마이그레이션 테스트의 「N개 컬럼」이 실제 리스트와 어긋난다 (한 패키지에서 4회 반복 · 부채 177 에서 발견)

**쉬운 말.** 테스트 이름과 주석이 「7개 컬럼」이라 말하는데 실제로 재는 목록은 8개다. 마이그레이션이 칸을 더할 때 목록만 고치고 숫자는 안 고친다. 실패 리포트에서 가장 먼저 읽히는 줄이 거짓말을 한다.

**방치하면.** 다음 사람이 리포트 헤드라인을 믿고 「7칸짜리 테스트」로 읽는다. 그리고 이 결함은
**아무도 안 잡는다** — 숫자는 사람만 읽는 산문이라 어떤 판정에도 안 걸린다.

**무엇 — 같은 패키지에서 네 번이다.**
`backend/modules/agile-planning/src/test/kotlin/com/bts/agileplanning/migration/`

| 자리 | 남긴 마이그레이션 | 상태 |
|---|---|---|
| `BoardSchemaMigrationTest` boards | V505 `board_type` · V509 3칸 | ✅ 부채 177 Task 28 이 닫음 |
| `BoardSchemaMigrationTest` board_columns | V501 `wip_limit` | ✅ Task 28 이 닫음 |
| `BoardSchemaMigrationTest` 파일 헤더 · 24행 「검증 범위」 | 범위가 V509 인데 라벨이 V500/V501 | 열림(숫자 아님) |
| **`SprintSchemaMigrationTest` sprints** | **V506 `board_id`** | **열림** |

`SprintSchemaMigrationTest.kt` 실측 — `SPRINTS_COLUMNS` 는 **12개**인데
`:67` 상수 주석이 「V503 에서 보유해야 하는 **11개**」, `:25-26` 클래스 KDoc 이 「**11개** 컬럼」이고
괄호 열거에서 `board_id` 가 빠져 있다. `:352` 테스트 이름 `V503+V506 sprints 12개 컬럼 존재` 만 맞다.
**boards/V505 와 완전히 같은 양식이다** — 리스트와 이름은 갱신, 주석 2자리는 방치.

**왜 지금인가.** 네 번 반복은 개인의 부주의가 아니라 구조다. 이 저장소가 이름 붙인
「두 목록이 서로를 검사하지 않는다」의 변종이고, 여기서는 **한쪽이 기계가 읽는 리스트, 다른 쪽이
사람만 읽는 숫자**라 갈라져도 아무 신호가 없다.

**처방 후보.** ① **판별식** — 마이그레이션 테스트에서 `N개 컬럼` 패턴을 뽑아 같은 파일의
대응 리스트 길이와 대조한다. 이 저장소의 `scripts/workflow/*.test.ts` 관용구에 맞는다.
② 숫자를 **문구에서 없앤다** — 이름·주석에서 개수를 빼고 리스트가 유일한 진실이 되게 한다.
갱신할 자리가 하나로 줄어 애초에 갈릴 수 없다. ③ 어긋난 자리만 손으로 고친다 — 다섯 번째를 부른다.

②가 근본이고 ①이 그것을 강제한다. ③ 단독은 권하지 않는다.
...
## ⬜ 워크플로우 — 진입점 이름 판별식이 「보드 설정」 화면을 안 덮는다 (부채 177 에서 발견)

**쉬운 말.** 화면마다 버튼 이름이 서로 헷갈리지 않는지 검사하는 장치가 있는데, 그 목록에 보드 설정 화면이 빠져 있다. 그 화면에 이름이 겹치는 버튼을 넣어도 아무도 못 잡는다.

**방치하면.** E2E 가 `getByRole('button', { name: … })` 로 버튼을 찾을 때 **두 개가 잡혀 strict mode
violation** 으로 죽는다. 실제로 부채 177 이 그 실패를 이미 밟고 있다 —
`e2e/board-settings.spec.ts` 가 `#454` 의 사이드바 `⋯` 와 접근성 이름이 겹쳐 red 다.

**무엇.** `apps/web/src/i18n/__tests__/create-entry-point-names.test.ts` 의 `SCREENS` 가
**`{백로그, 보드}` 둘뿐**이다. 보드 **설정** 화면은 등록돼 있지 않다.

부채 177 Task 16 이 그 화면 최초의 버튼 2종(`다시 시도` · `커스텀 필드 다시 불러오기`)을 넣었고,
독립 검증이 **손으로 전수 대조**해 기존 버튼들과 substring 관계가 없음을 확인했다 — 즉 지금은
결함이 없지만 **기계가 그것을 지키고 있지 않다.** T17·T18·T19 가 버튼을 더 넣을 예정이다.

**왜 지금인가.** 「판별식이 표의 일부 열만 읽으면 안 읽는 열은 조용히 썩는다」의 변종이다.
여기서는 **행(화면)이 빠졌다.** 화면이 늘 때 `SCREENS` 를 갱신하라는 강제가 없다.

**처방 후보.** ① `SCREENS` 에 보드 설정 화면을 더한다 — 한 줄이지만 다음 화면에서 또 밟는다.
② **화면 목록을 라우트에서 파생**시킨다(`routes/` 스캔). 갱신을 잊을 자리가 사라진다.
③ 「등록되지 않은 화면이 있으면 red」인 차집합 판정을 더한다 — ②가 어려우면 이쪽.

②가 근본이고 ③이 그것을 강제한다. ① 단독은 다음 화면을 부른다.

**착수 시 주의.** 판별식을 고치면 **일부러 끊어 red 를 1회 봐라.** 그리고 뮤테이션 방향을 따져라 —
「이름이 겹치는 버튼을 넣으면 red」는 당연하고, 물어야 할 것은 **「화면을 새로 만들고 등록을
안 했을 때 red 인가」**다. 판별식이 실제로 막아야 하는 것은 그쪽이다.

## ⬜ 워크플로우 — 테스트 범위 계산기가 한 번 푸시한 브랜치에서 「변경 없음」을 낸다 (신규 · 미착수 · T2)

**쉬운 말.** 어떤 테스트를 돌려야 하는지 계산해 주는 도구가 있다. 그런데 작업 내용을 서버에 한 번 올리고 나면, 그 도구가 「바뀐 게 하나도 없다」고 답한다. 그래서 **테스트를 하나도 안 돌리고 초록불**이 뜬다.

**방치하면.** 조용히 통과한다. 도구가 「생략」이라고 적어 주므로 화면상으로는 정상으로 보이고, 사람은 좁혀진 범위를 믿는다. 검증되지 않은 코드가 초록으로 머지되는 경로이고, 그 도구를 도입한 PR 본문이 **바로 그 방향을 「치명적」이라 못박았다** — *"넓힘의 방향은 한쪽뿐이다 … 좁게 고르는 실수만이 치명적이다(검증 안 된 코드가 초록으로 머지된다)"*.

**무엇.** `scripts/workflow/select-test-scope.ts:110` 이 비교 기준을 **브랜치 자신의 upstream** 으로 잡는다.

> `const base = git(['rev-parse', '--abbrev-ref', '--symbolic-full-name', '@{u}']) ?? FALLBACK_BASE`

기능 브랜치의 `@{u}` 는 `origin/<그 브랜치>` 다. 푸시 직후에는 그것이 `HEAD` 와 같으므로 `merge-base` 도 `HEAD` 가 되고 `git diff` 가 빈 목록을 낸다. `FALLBACK_BASE = 'origin/main'`(`:96`)은 **upstream 이 아예 없을 때만** 쓰이므로, 첫 푸시 이후에는 기준이 조용히 뒤바뀐다. `--base` 같은 우회 인자도 없다(`main()` 은 `--plan` 하나만 읽는다 · `:279-285`).

**실측 (2026-09-04 · `ui/board-settings-screen`).** 백엔드 8파일을 바꾼 브랜치에서

| 기준 | `git diff --name-only` 결과 | 계산기 판정 |
|---|---|---|
| 브랜치 upstream (계산기가 쓰는 것) | **0건** | `# 백엔드 — 생략 (변경 없음)` |
| `origin/main` (의도된 것) | **8건** (`agile-planning` main 5 + test 3) | `:modules:agile-planning:test` |

**★도입 PR 이 스스로 예고한 양식이다.** #451 은 「좁힘 장치가 세 겹 있었는데 이 자리가 셋 다 안 불렀다」를 고쳤다. 이번 것은 그 계산기가 **자기 기준을 잘못 읽어** 결과적으로 같은 자리로 돌아간 경우다. 「기준을 못 읽으면 전량으로 넓힌다」는 fail-safe 는 `changedFiles()` 가 `null` 을 낼 때만 작동하는데, 여기서는 `[]`(없다)를 내므로 **fail-safe 가 발동하지 않는다** — 그 함수 KDoc 이 `null`(모른다)과 `[]`(없다)를 구분해야 한다고 적어 둔 바로 그 구분이 호출부에서 지켜졌으나, 기준 자체가 틀려 「없다」가 거짓이 됐다.

**할 일.** ①기준을 `origin/<기본 브랜치>` 와의 merge-base 로 바꾼다(현재 `FALLBACK_BASE` 를 기본 경로로 승격). 브랜치 upstream 은 「내가 뭘 아직 안 올렸나」를 재는 값이지 「이 브랜치가 main 대비 뭘 바꿨나」가 아니다. ②`--base` 인자를 열어 두면 릴리스 브랜치 등에서 기준을 지정할 수 있다. ③**판별식**이 이 축을 직접 잰다 — 픽스처 저장소에서 브랜치를 만들고 **푸시된 상태를 흉내 낸 뒤**(upstream ref 를 HEAD 로 맞춘 뒤) `changedFiles()` 가 여전히 비지 않는지 본다. 문자열 계약으로는 못 잡는 축이라 실제 ref 조작이 필요하다.
...
- **미는 방향은 세로다.** 처음에는 각 간선의 법선으로 40px 밀었는데 **E2E 가 red 였다** —
  `Submit for Review` 는 폭이 100px 를 넘어 가로 40px 분리로는 여전히 포개진다. 라벨은
  가로로 길고 세로로 짧아 세로가 같은 간격으로 훨씬 확실하게 떨어진다.
...
**★리뷰가 뒤집은 것 — 판정 좌표계가 그리는 좌표계와 달랐다.** 위 처방으로 픽스처는 통과했지만
코드 리뷰가 그 셈의 전제를 깼다. 라벨 자리를 **노드 top-left** 로 셈했는데 실제로는 **핸들
좌표**(오른쪽·왼쪽 중앙)에 그려지고, 노드 폭은 `min-w-36 max-w-52` 로 **가변**이라
「모든 노드가 같은 크기」라 적은 KDoc 자체가 사실이 아니었다. 격자 칸으로 묶는 방식도
경계를 사이에 둔 2px 차이를 다른 칸으로 봤다.

- 노드 기하를 lib 정본으로 올리고 앵커를 핸들 좌표 + `offset` 법선까지 반영해 다시 셈한다.
- 겹침을 격자가 아니라 **실제 사각형 교차**로 판정한다.
- self-loop 과 시작 노드 간선도 해소에 넣는다 — 빼면 그 라벨들은 **영영 안 떨어진다.**
- ★ 첫 판정은 **공허했다.** 자리를 못 정한 라벨을 0×0 사각형으로 두었더니 폭이 0 이라
  어떤 것과도 안 겹쳐, self-loop 을 해소에서 빼는 뮤테이션이 초록이었다. `null` 로 바꾸고
  「전부 자리를 받았는가」를 겹침보다 **먼저** 재게 고쳤다.
...
**★리뷰가 잡은 것 — 링이 편집을 시작하면 사라졌다.** 초안이 바뀌면 캔버스가 노드 배열을 새로
만들어 `setLiveNodes` 로 갈아끼우는데 그 배열에는 `selected` 가 없다. 드래그는
`onNodeDragStop → onMoveState` 로 초안을 바꾸므로 **놓는 순간 링이 사라졌다** — 이 항목이
적은 목적(「여러 상태를 오가며 편집할 때 지금 무엇을 고른 상태인지 알려 준다」)이 편집을
시작하는 그 순간 무너진 것이다. 실측으로 확인하고(`ring-2 = false`) 재-seed 때 선택을
살리도록 고쳤다. 선택은 xyflow 가 들고 있는 화면 상태이지 초안의 일부가 아니다.
단위 테스트는 `selected` 를 직접 넘겨 재므로 못 잡는다 — E2E `D8-6` 이 그 자리를 맡는다.
...
**★리뷰가 잡은 것 — 상한이 겹침을 되살렸다.** 첫 계수(기본 20 · 증가 12 · 상한 44)는
`20, 32, 44, 44, 44` 를 내어 **셋째 self-loop 부터 반지름이 같았다.** 고리 셋이 byte 단위로
같은 경로를 그려 화면에는 하나로 보인다 — 부채 170 을 고치며 **부채 168 을 되살린** 셈이다.
그리고 그때 세운 판정(`0 < offset <= 44`)은 상한만 재고 서로 다름을 안 재서 **그 겹침을
고정시켰다.** 계수를 14/6 으로 낮춰 여섯째까지 구분되게 하고, 「다섯 개의 반지름이 서로
다르다」를 판정으로 세웠다.

**★가로 관통도 함께 막았다.** 세로 상한만 보면 이름이 긴 노드(`max-w-52` = 208px)의 고리가
`COLUMN_GAP_PX`(260)에서 남는 52px 를 넘어 **옆 열 노드를 뚫는다** — 이 항목이 적은 증상이
90도 돌아간 것이다. 반지름이 가로 여유에도 걸리게 했다.

**★라벨 상자가 고리를 덮던 것도 고쳤다.** 좌표가 상자의 **중심**이라 여백 10px 로는 폭의
절반이 고리 안으로 들어갔고, 판정은 중심만 재서 그것을 못 봤다. 고리 라벨만 **왼쪽 정렬**로
바꿔 폭을 몰라도 상자가 고리 바깥에서 시작하게 했다.
