...
## ✅ apps/web — 이동·임포트 진입점에 CREATE 게이트가 없다 (해소 2026-08-10 · PR #361 · **이동 처방 정정**)
...
`MoveIssueDialog.tsx:254-265` 의 대상 프로젝트는 드롭다운이 아니라 `<Input>` 이다.
...
`MovePreviewService.kt:185-190` 이 preview 단계에서 원본 UPDATE + 대상 CREATE 를 assert 한다.
「다음」을 누르는 순간 서버가 확정 판정을 준다. 클라이언트 추정 게이트가 더할 것이 없다.

**그래서 진짜 결함은 게이트 부재가 아니라 문구다.** 그 403 을 `MoveIssueDialog.tsx:114-116` 이
`errorPreview`(「대상 프로젝트 키를 확인해 주세요」)로 뭉개 **권한 문제를 키 오타처럼** 보이게
했다. PR #361 은 `handleNext` catch 에서 `ApiError.status === 403` 만 분기해 권한 전용 문구로
매핑하고, 403 이외(404·409·500)는 기존 문구를 유지한다.

**★403 이 두 원인을 구분하지 못하는 문제.** 서버 403 은 **원본 UPDATE 없음**과
**대상 CREATE 없음** 둘 다에서 나오고 `errorCode` 는 `ACCESS_DENIED` 하나뿐이라
(`IssueExceptionHandler.kt:259-269`) 클라이언트가 구분할 수 없다. 기존 `errorForbidden`
(「이슈를 이동할 권한이 없습니다.」)을 재사용하면 원인을 **이슈 쪽으로 단정**해, 원본 권한은
있고 대상만 막힌 사용자에게 「이 이슈는 영영 못 옮긴다」로 읽힌다. 그래서 두 가능성을 함께
말하는 전용 문구(`errorPreviewForbidden`)를 신설했다.
