 ...
 *
 * ## 2026-07-29 갱신 — 49 → 57 (분모 증가에 따른 재동결, 키워드 퇴행 아님)
 * PR #320 이 FR-UX-07 을 기능 단위 8개로 분할해 FR 총수가 **131 → 139(+8)** 가 됐고,
 * 새로 등록된 제목 8건이 전부 불일치로 잡혀 **49/131 → 57/139** 가 됐다.
 * 비율로는 37.4% → 41.0% 다. **키워드가 나빠진 것이 아니라 오라클 모집단이 늘어난 것**이다.
 *
 * ★이 회귀가 #320 시점에 안 잡힌 이유. `backend-ci.yml` 의 트리거 경로는
 * `backend/**` · `.github/workflows/backend-ci.yml` · `scripts/workflow/**` · `package.json` 인데
 * #320 은 `docs/**` 와 `apps/web/**` 만 건드려 **워크플로우가 아예 트리거되지 않았다.**
 * FR 문서만 바꾸는 PR 은 이 판별식의 입력(FR 제목)을 바꾸면서도 가드를 건너뛴다 —
 * 트리거 경로에 FR 계획 문서를 넣을지는 별건으로 판단할 사안이라 여기서는 재동결만 한다.
 *
 * 갱신 주체. PR #321(조립 앱 비-prod 부팅 봉합) — 그 PR 이 `scripts/workflow/**` 를 건드려
 * backend-ci 를 켜면서 이 선재 실패가 처음 드러났다. 그 PR 은 FR 문서·키워드를 0줄 바꿨고
 * 불일치 수는 main 과 동일한 57 이다(양 트리 실측 대조).
 */
const MAX_MISMATCHES = 57;
