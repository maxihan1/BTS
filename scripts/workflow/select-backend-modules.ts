 ...
 * `WIDEN_PREFIXES` 는 `moduleOf()` 가 null 을 준 파일에만 걸리므로 **모듈 안 마이그레이션에는
 * 영원히 발화하지 않는다.** 2026-08-14 이전 상태가 정확히 그것이었고, 그 사실을
 * `backend/db/migration/V999__x.sql`(이 저장소에 존재하지 않는 형태)을 쓰는 테스트가 가리고 있었다.
 *
 * ★★뮤테이션으로 확인한 **정확한** 요건 — 「씨앗 루프보다 앞」이 아니라 **「모듈에 귀속된
 * 파일에도 검사가 닿을 것」**이다. 씨앗 루프는 좁히지 않고 모으기만 하므로 이 판정을 그 뒤로
 * 옮겨도 동작이 같다(등가 뮤턴트, 2026-08-14 실측 — 처음엔 「순서가 전부」라 적었다가 뮤테이션이
 * 그 서술을 반증했다). 실제로 깨지는 배치는 `moduleOf() === null` 가지 안으로 넣는 것 하나이고,
 * 그것은 판별식 2건이 RED 로 잡는다.
 *
