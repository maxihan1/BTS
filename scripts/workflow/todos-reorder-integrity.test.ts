...
describe('TODOS.md — 대량 이동 무손실 대조', () => {
  ...
  test('양성 ⑩ — 해소 전환은 순수 이동이 아니다 (**줄 축**이 판정한다)', () => {
    // 해소는 정당한 편집이지 이동이 아니다. 개명·편집과 같은 자리에 둔다.
    //
    // ★이름에 「줄 축」을 박아 둔다. 이 red 를 세우는 것은 `items.resolved.length === 0` 이
    //   아니라 줄 축(마커 줄이 바뀌므로)이다. 그 조건을 지워도 전량이 초록이다 — 같은 자리의
    //   기존 조건 2건(`renamed`·`edited`)도 마찬가지다. 부채 `98`.
    //   이름을 그냥 두면 다음 사람이 「항목 축 조건은 판정되고 있다」고 읽는다.
