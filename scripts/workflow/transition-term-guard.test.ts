 ...
 * ★ 네 열을 **전부** 선언한다.
 *
 * 종전에는 `총건수`·`파일별` 둘만 있었다. 그러면 `파일수`·`낱말별` 은 파일에 남아 있는데
 * 아무도 읽지 않아, 틀려도 영원히 초록이다 — 저장소가 「판별식이 표의 일부 열만 읽으면 안 읽는
 * 열은 조용히 썩는다」로 이름 붙인 그 양식이고, 실제로 PR #411 이 앞의 둘만 갱신하며 뒤의 둘을
 * 어긋나게 만들었다(파일수 154 vs 실제 156 · 낱말별 합 375 vs 총건수 378).
 */
interface Baseline {
  총건수: number
  파일수: number
  낱말별: Record<string, number>
...
describe('구 표기 「전이」 잔존 동결 (양방향)', () => {
  ...
  test('동결 파일의 네 열이 서로 맞는다 (안 읽는 열이 썩지 않게)', () => {
    const b = readBaseline()
    const 파일별합 = Object.values(b.파일별).reduce((a, c) => a + c, 0)
    const 낱말별합 = Object.values(b.낱말별).reduce((a, c) => a + c, 0)

    assert.equal(
      b.파일수,
      Object.keys(b.파일별).length,
      `파일수(${b.파일수})가 파일별 항목 수(${Object.keys(b.파일별).length})와 다르다. ` +
        '항목을 더하거나 뺐으면 이 숫자도 함께 고쳐라.',
    )
    assert.equal(b.총건수, 파일별합, `총건수(${b.총건수})가 파일별 합(${파일별합})과 다르다.`)
    assert.equal(
      b.총건수,
      낱말별합,
      `총건수(${b.총건수})가 낱말별 합(${낱말별합})과 다르다. ` +
        '새로 동결한 자리의 낱말을 낱말별에도 더해야 한다 — 안 그러면 「어느 표기가 남았나」가 거짓이 된다.',
    )
  })
