...
# ★둘 다 「비율」이다. 절대값을 쓰면 러너를 다른 머신으로 바꾸는 순간 같은 값이 다른 의미가
#   되는데 아무도 눈치채지 못한다. load 만 비율로 하고 swap 을 절대 MB 로 두면 논리를 절반만
#   적용한 것이고, 실제로 이 러너의 4,348MB(16GB 대비 26.5%)가 오탐으로 걸렸다.
